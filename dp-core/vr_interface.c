...
static uint8_t vif_fat_flow_port_get(struct vr_interface *, uint8_t, uint16_t);
...
extern struct vr_interface *vif_bridge_get_sub_interface(vr_htable_t, unsigned short,
                                                         unsigned char *);
extern int vif_bridge_get_index(struct vr_interface *, struct vr_interface *, uint8_t *);
extern int vif_bridge_init(struct vr_interface *);
extern void vif_bridge_deinit(struct vr_interface *);
extern int vif_bridge_delete(struct vr_interface *, struct vr_interface *, uint8_t *);
extern int vif_bridge_add(struct vr_interface *, struct vr_interface *, uint8_t *);
...
static inline uint64_t vr_htonll(uint64_t n) {
  uint64_t t = 1;
  uint8_t *p, out[8];
...
static inline void fat_flow_ipv6_plen_to_mask(uint16_t plen, uint64_t *high_mask,
                                              uint64_t *low_mask) {
  if (plen > 64) {
...
struct vr_interface_stats *vif_get_stats(struct vr_interface *vif, unsigned short cpu) {
  return &vif->vif_stats[cpu & VR_CPU_MASK];
}

static int vif_discard_tx(struct vr_interface *vif, struct vr_packet *pkt,
...
static int vif_discard_rx(struct vr_interface *vif, struct vr_packet *pkt,
...
void vif_drop_pkt(struct vr_interface *vif, struct vr_packet *pkt, bool input) {
 ...
 */
static int vif_cmn_rewrite(struct vr_interface *vif, struct vr_packet **pkt,
...
static int vr_interface_service_enable(struct vr_interface *vif) {
  int i;
  ...
  if (!vif->vif_vrf_table) {
    vif->vif_vrf_table = vr_malloc(sizeof(struct vr_vrf_assign) * VIF_VRF_TABLE_ENTRIES,
...
static void vr_interface_service_disable(struct vr_interface *vif) {
 ...
 */
void vif_set_xconnect(struct vr_interface *vif) {
  if (!vif)
    return;
...
void vif_remove_xconnect(struct vr_interface *vif) {
  if (!vif)
    return;
...
int vif_xconnect(struct vr_interface *vif, struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
...
static inline void vif_mirror(struct vr_interface *vif, struct vr_packet *pkt,
...
/* agent driver */
static int agent_set_rewrite(struct vr_interface *vif, struct vr_packet **pkt,
                             struct vr_forwarding_md *fmd, unsigned char *rewrite,
                             unsigned short len) {
  unsigned char *head;
...
static int agent_rx(struct vr_interface *vif, struct vr_packet *pkt,
                    unsigned short vlan_id __attribute__unused__) {
  unsigned short cmd;
  ...
  case AGENT_CMD_ROUTE:
     ...
     */
    agent_vif = __vrouter_get_interface(vrouter_get(0), ntohs(hdr->hdr_ifindex));
    if (!agent_vif) {
      agent_vif = vif;
    }
    pkt->vp_if = agent_vif;
    ...
    vr_virtual_input(ntohs(hdr->hdr_vrf), agent_vif, pkt, &fmd, VLAN_ID_INVALID);
  ...
  case AGENT_CMD_SWITCH:
    ...
    pkt_set_inner_network_header(pkt, pkt->vp_data + sizeof(struct vr_eth));
...
static int agent_tx(struct vr_interface *vif, struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
  int ret;
  struct vr_interface_stats *stats = vif_get_stats(vif, pkt->vp_cpu);
 ...
 */
static int agent_trap_may_truncate(int trap_reason) {
  switch (trap_reason) {
...
static int agent_send(struct vr_interface *vif, struct vr_packet *pkt, void *ifspecific) {
  ...
  struct agent_send_params *params = (struct agent_send_params *)ifspecific;
  ...
  case AGENT_TRAP_FLOW_ACTION_HOLD:
    if (params->trap_param) {
      ...
      hdr->hdr_cmd_param_4 =
          htonl((fta->vfta_stats.flow_bytes_oflow | (fta->vfta_stats.flow_packets_oflow << 16)));
...
static int agent_drv_del(struct vr_interface *vif) {
...
static int agent_drv_add(struct vr_interface *vif, vr_interface_req *vifr __attribute__unused__) {
  int ret;
...
/* vhost driver */
static mac_response_t vhost_mac_request(struct vr_interface *vif, struct vr_packet *pkt,
  ...
  if (pkt->vp_type == VP_TYPE_ARP) {
    ...
    if (IS_LINK_LOCAL_IP(sarp->arp_dpa) || (vif->vif_type == VIF_TYPE_GATEWAY)) {
...
static int vhost_rx(struct vr_interface *vif, struct vr_packet *pkt,
                    unsigned short vlan_id __attribute__unused__) {
  struct vr_forwarding_md fmd;
  struct vr_interface_stats *stats = vif_get_stats(vif, pkt->vp_cpu);
...
static int vhost_tx(struct vr_interface *vif, struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
  int ret;
  struct vr_interface_stats *stats = vif_get_stats(vif, pkt->vp_cpu);
  ...
  if (vif->vif_type == VIF_TYPE_XEN_LL_HOST)
    ...
    in_vif = pkt->vp_if;
    if (!hif_ops->hif_get_encap || (hif_ops->hif_get_encap(in_vif) == VIF_ENCAP_TYPE_ETHER)) {
      /* Untag any tagged packets */
      eth = pkt_data(pkt);
      eth_proto = ntohs(*(unsigned short *)(eth + VR_ETHER_PROTO_OFF));
      while (eth_proto == VR_ETH_PROTO_VLAN) {
        vlan = (struct vr_vlan_hdr *)(pkt_data(pkt) + pull_len + VR_ETHER_HLEN);
...
static int vhost_drv_del(struct vr_interface *vif) {
...
static int vhost_drv_add(struct vr_interface *vif, vr_interface_req *vifr __attribute__unused__) {
  int ret = 0;
...
/* vlan driver */
static int vlan_rx(struct vr_interface *vif, struct vr_packet *pkt,
                   unsigned short vlan_id __attribute__unused__) {
  int8_t tos;
  struct vr_interface_stats *stats = vif_get_stats(vif, pkt->vp_cpu);
  struct vr_forwarding_md fmd;
...
static int vlan_tx(struct vr_interface *vif, struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
...
static int vlan_sub_interface_manage(struct vr_interface *pvif, struct vr_interface *vif,
                                     int src_mac_num, uint8_t *src_macs) {
  int i, j, ret;
...
static int vlan_drv_del(struct vr_interface *vif) {
  int ret = 0;
  struct vr_interface *pvif;
  ...
  if (vif->vif_src_mac) {
    ...
    if (pvif->vif_sub_interfaces && (pvif->vif_sub_interfaces[vif->vif_vlan_id] == vif)) {
...
static int vlan_drv_add(struct vr_interface *vif, vr_interface_req *vifr) {
  int ret = 0;
  ...
  if (vifr->vifr_src_mac_size && vifr->vifr_src_mac) {
    ...
    if (!pvif->vif_sub_interfaces) {
      pvif->vif_sub_interfaces =
          vr_zalloc(VLAN_ID_MAX * sizeof(struct vr_interface *), VR_INTERFACE_OBJECT);
      if (!pvif->vif_sub_interfaces)
        return -ENOMEM;
    }
  }
  ...
  if (!vif->vif_src_mac) {
    ...
    return vlan_sub_interface_manage(pvif, vif, (vifr->vifr_src_mac_size / VR_ETHER_ALEN),
                                     vifr->vifr_src_mac);
  }
...
/* eth driver */
static int vm_srx(struct vr_interface *vif, struct vr_packet *pkt, unsigned short vlan_id) {
  unsigned short vrf;
  struct vr_interface_stats *stats = vif_get_stats(vif, pkt->vp_cpu);
  struct vr_forwarding_md fmd;
...
static mac_response_t vm_mac_request(struct vr_interface *vif, struct vr_packet *pkt,
                                     struct vr_forwarding_md *fmd, unsigned char *dmac) {
  if (pkt->vp_type == VP_TYPE_ARP) {
...
static int vm_rx(struct vr_interface *vif, struct vr_packet *pkt, unsigned short vlan_id) {
  struct vr_forwarding_md fmd;
  ...
  if (vlan_id != VLAN_ID_INVALID && vlan_id < VLAN_ID_MAX) {
    if (vif->vif_btable) {
      sub_vif = vif_bridge_get_sub_interface(vif->vif_btable, vlan_id, eth->eth_smac);
    } else {
...
static int tun_rx(struct vr_interface *vif, struct vr_packet *pkt, unsigned short vlan_id) {
 ...
 *
 */
static int ipsec_rx(struct vr_interface *vif, struct vr_packet *pkt, unsigned short vlan_id) {
  struct vrouter *router = vrouter_get(0);
  pkt->vp_if = router->vr_eth_if;
  return eth_rx(vif, pkt, vlan_id);
}

static int eth_set_rewrite(struct vr_interface *vif, struct vr_packet **pkt,
                           struct vr_forwarding_md *fmd, unsigned char *rewrite,
                           unsigned short len) {
  if (!len)
    return 0;
   ...
   * rewrite data
   */
  if (((*pkt)->vp_if->vif_type == VIF_TYPE_HOST) && (!((*pkt)->vp_flags & VP_FLAG_FROM_DP)) &&
...
static mac_response_t eth_mac_request(struct vr_interface *vif, struct vr_packet *pkt,
                                      struct vr_forwarding_md *fmd, unsigned char *dmac) {
...
static int eth_rx(struct vr_interface *vif, struct vr_packet *pkt, unsigned short vlan_id) {
  struct vr_forwarding_md fmd;
  struct vr_interface *sub_vif = NULL;
  struct vr_interface_stats *stats = vif_get_stats(vif, pkt->vp_cpu);
  struct vr_eth *eth = (struct vr_eth *)pkt_data(pkt);
  ...
  if ((vlan_id == VLAN_ID_INVALID) && (vif->vif_flags & VIF_FLAG_NATIVE_VLAN_TAG))
    vlan_id = 0;
...
static int eth_tx(struct vr_interface *vif, struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
  ...
  if (vif_is_virtual(vif)) {
    ...
    if ((pkt->vp_nh && (pkt->vp_nh->nh_dev != vif)) ||
        ((pkt->vp_if->vif_type == VIF_TYPE_AGENT) && (fmd->fmd_vlan != VLAN_ID_INVALID))) {
      from_subvif = true;
      stats_count = false;
    }
...
static int eth_drv_del(struct vr_interface *vif) {
  int ret;
...
static int eth_drv_del_sub_interface(struct vr_interface *pvif, struct vr_interface *vif) {
...
static int eth_drv_add_sub_interface(struct vr_interface *pvif, struct vr_interface *vif) {
  return hif_ops->hif_add(vif);
}

static int eth_drv_add(struct vr_interface *vif, vr_interface_req *vifr __attribute__unused__) {
  int ret = 0;
  ...
  if ((vif->vif_type == VIF_TYPE_PHYSICAL) && (hif_ops->hif_get_encap(vif) == VIF_ENCAP_TYPE_L3)) {
    vif->vif_rx = tun_rx;
   ...
   */
  if ((!(vif->vif_flags & VIF_FLAG_VHOST_PHYS)) || (vif->vif_bridge) ||
      (hif_ops->hif_get_encap(vif) == VIF_ENCAP_TYPE_L3_DECRYPT)) {
    ret = hif_ops->hif_add_tap(vif);
    if (ret)
      hif_ops->hif_del(vif);
...
unsigned int vif_get_mtu(struct vr_interface *vif) { return hif_ops->hif_get_mtu(vif); }

static void vif_free(struct vr_interface *vif) {
  unsigned int i;
  ...
  for (i = 0; i < vr_num_cpus; i++) {
    if (vif->vif_stats[i].vis_queue_ierrors_to_lcore) {
      vr_free(vif->vif_stats[i].vis_queue_ierrors_to_lcore, VR_INTERFACE_TO_LCORE_ERRORS_OBJECT);
    }
  }
  ...
  if (vif->vif_bridge_table_lock) {
    vr_free(vif->vif_bridge_table_lock, VR_INTERFACE_BRIDGE_LOCK_OBJECT);
...
void vrouter_put_interface(struct vr_interface *vif) {
...
struct vr_interface *__vrouter_get_interface(struct vrouter *router, unsigned int idx) {
...
struct vr_interface *__vrouter_get_interface_os(struct vrouter *router, unsigned int os_idx) {
  unsigned int i;
...
struct vr_interface *vrouter_get_interface(unsigned int rid, unsigned int idx) {
  struct vr_interface *vif;
...
static void vrouter_del_interface(struct vr_interface *vif) {
...
static void vrouter_setup_vif(struct vr_interface *vif) {
  switch (vif->vif_type) {
  case VIF_TYPE_AGENT:
    agent_alive = true;
 ...
 */
static void vrouter_set_rewrite(struct vr_interface *vif) {
  unsigned char *ptr;
 ...
 * reference count
 */
static int vrouter_add_interface(struct vr_interface *vif, vr_interface_req *vifr) {
...
void vif_attach(struct vr_interface *vif) {
...
void vif_detach(struct vr_interface *vif) {
...
static void vif_drv_delete(struct vr_interface *vif) {
  if (hif_ops->hif_lock)
    hif_ops->hif_lock();
...
int vif_delete(struct vr_interface *vif) {
  vif_drv_delete(vif);
  vrouter_del_interface(vif);
  return 0;
}

struct vr_interface *vif_find(struct vrouter *router, char *name) {
  int i;
  struct vr_interface *vif;
...
static int vif_drv_add(struct vr_interface *vif, vr_interface_req *req) {
  int ret = 0;
...
static int vr_interface_delete(vr_interface_req *req, bool need_response) {
  int ret = 0;
  struct vr_interface *vif;
   ...
   */
  if (req->vifr_name) {
    if (2 == sscanf(req->vifr_name, "vif%u/%u", &req->vifr_rid, &req->vifr_idx))
      vif = __vrouter_get_interface(vrouter_get(req->vifr_rid), req->vifr_idx);
    else
...
static void vif_free_bridge_table_lock(struct vrouter *router, void *data) {
...
static int vif_set_flags(struct vr_interface *vif, vr_interface_req *req) {
  void *mem;
  ...
  if ((req->vifr_flags & VIF_FLAG_MAC_LEARN) || (vif_is_fabric(vif))) {
    if (!vif->vif_bridge_table_lock) {
      vif->vif_bridge_table_lock =
          vr_zalloc(vr_num_cpus * sizeof(uint8_t), VR_INTERFACE_BRIDGE_LOCK_OBJECT);
      if (!vif->vif_bridge_table_lock) {
        return -ENOMEM;
      }
    }
    ...
    if (vif->vif_bridge_table_lock) {
      ...
      if (vdd) {
        ...
        vif->vif_bridge_table_lock = NULL;
        vdd->vdd_data = mem;
        vr_defer(vif->vif_router, vif_free_bridge_table_lock, (void *)vdd);
      }
    }
  }

  vif->vif_flags = (vif->vif_flags & VIF_VR_CAP_MASK) | (req->vifr_flags & ~VIF_VR_CAP_MASK);
...
static int vr_interface_mirror_md_set(struct vr_interface *vif, vr_interface_req *req) {
  /*
  ...
  if (req->vifr_in_mirror_md_size) {
    if (!vif->vif_in_mirror_md) {
      ...
      vif->vif_in_mirror_md =
          vr_zalloc(req->vifr_in_mirror_md_size, VR_INTERFACE_MIRROR_META_OBJECT);
      if (!vif->vif_in_mirror_md)
        return -ENOMEM;
    ...
    memcpy(vif->vif_in_mirror_md, req->vifr_in_mirror_md, req->vifr_in_mirror_md_size);
  ...
  if (req->vifr_out_mirror_md_size) {
    if (!vif->vif_out_mirror_md) {
      ...
      vif->vif_out_mirror_md =
          vr_zalloc(req->vifr_out_mirror_md_size, VR_INTERFACE_MIRROR_META_OBJECT);
      if (!vif->vif_out_mirror_md)
        return -ENOMEM;
    ...
    memcpy(vif->vif_out_mirror_md, req->vifr_out_mirror_md, req->vifr_out_mirror_md_size);
...
static int vr_interface_change(struct vr_interface *vif, vr_interface_req *req) {
  int ret = 0;
  uint64_t *ip6;

  if (req->vifr_flags & VIF_FLAG_SERVICE_IF && !(vif->vif_flags & VIF_FLAG_SERVICE_IF)) {
    ret = vr_interface_service_enable(vif);
    if (ret)
      return ret;
  } else if ((vif->vif_flags & VIF_FLAG_SERVICE_IF) && !(req->vifr_flags & VIF_FLAG_SERVICE_IF)) {
    vr_interface_service_disable(vif);
  }
  ...
  if (!(vif->vif_flags & VIF_FLAG_MIRROR_RX) && !(vif->vif_flags & VIF_FLAG_MIRROR_TX)) {
  ...
  *(ip6 + 1) = req->vifr_ip6_l;
  ...
  return vlan_sub_interface_manage(vif->vif_parent, vif, req->vifr_src_mac_size / VR_ETHER_ALEN,
                                   req->vifr_src_mac);
}

static bool vif_transport_valid(vr_interface_req *req) {
...
int vr_interface_add(vr_interface_req *req, bool need_response) {
  int i, ret = 0;
  uint64_t *ip6;
  ...
  vif->vif_stats =
      vr_zalloc(vr_num_cpus * sizeof(struct vr_interface_stats), VR_INTERFACE_STATS_OBJECT);
  if (!vif->vif_stats) {
    ret = -ENOMEM;
    goto error;
  }
  ...
  for (i = 0; i < vr_num_cpus; i++) {
    vif->vif_stats[i].vis_queue_ierrors_to_lcore =
        vr_zalloc(vr_num_cpus * sizeof(uint64_t), VR_INTERFACE_TO_LCORE_ERRORS_OBJECT);
    if (!vif->vif_stats[i].vis_queue_ierrors_to_lcore) {
      ret = -ENOMEM;
      goto error;
    }
   ...
   */
  vif->vif_drop_stats = vr_zalloc((VP_DROP_MAX * sizeof(uint64_t)), VR_DROP_STATS_OBJECT);
  if (!vif->vif_drop_stats) {
    ret = -ENOMEM;
    goto error;
  }
  ...
  *(ip6 + 1) = req->vifr_ip6_l;
  ...
  if (req->vifr_hw_queues_size) {
    vif->vif_hw_queues =
        vr_malloc(sizeof(uint16_t) * req->vifr_hw_queues_size, VR_INTERFACE_QUEUE_OBJECT);
    if (!vif->vif_hw_queues) {
      ret = -ENOMEM;
      goto error;
    }
    ...
    memcpy(vif->vif_hw_queues, req->vifr_hw_queues, req->vifr_hw_queues_size * sizeof(uint16_t));
  }
...
static void vr_interface_add_response(vr_interface_req *req, struct vr_interface_stats *stats) {
  int i;
...
static uint64_t vr_interface_get_drops(struct vr_interface *vif) {
  uint8_t *count, cpu;
  int stats_index;
  ...
  for (stats_index = 0; stats_index < VP_DROP_MAX; stats_index++) {
    ...
    if (vif->vif_pcpu_drop_stats) {
      for (cpu = 0; cpu < vr_num_cpus; cpu++) {
        count = vr_btable_get(vif->vif_pcpu_drop_stats, ((cpu * VP_DROP_MAX) + stats_index));
        total_drops += *count;
      }
    }
  }
...
static int vr_interface_copy_bond_info(vr_interface_req *req,
  ...
  req->vifr_fab_name =
      vr_zalloc(req->vifr_fab_name_size * sizeof(uint8_t), VR_INTERFACE_BOND_OBJECT);
  ...
  req->vifr_fab_drv_name =
      vr_zalloc(req->vifr_fab_drv_name_size * sizeof(uint8_t), VR_INTERFACE_BOND_OBJECT);
  ...
  if (bond_info->vif_num_slave) {
    for (i = 0; i < bond_info->vif_num_slave; i++) {
      ...
      sl_name_iter +=
          snprintf((buffer + sl_name_iter), (VR_INTERFACE_STR_BUF_LEN - sl_name_iter - 1),
    ...
    req->vifr_bond_slave_name =
        vr_zalloc(req->vifr_bond_slave_name_size * sizeof(uint8_t), VR_INTERFACE_BOND_OBJECT);
    if (!req->vifr_bond_slave_name)
      return -ENOMEM;
    req->vifr_bond_slave_drv_name =
        vr_zalloc(req->vifr_bond_slave_drv_name_size * sizeof(uint8_t), VR_INTERFACE_BOND_OBJECT);
    if (!req->vifr_bond_slave_drv_name)
      return -ENOMEM;

    memcpy(req->vifr_bond_slave_name, buffer, req->vifr_bond_slave_name_size);
    memcpy(req->vifr_bond_slave_drv_name, buffer_drv, req->vifr_bond_slave_drv_name_size);
  }
  return 0;
}

static int __vr_interface_make_req(vr_interface_req *req, struct vr_interface *intf,
                                   unsigned int core) {
  unsigned int cpu, i;
  uint64_t *ip6;
  int ret;
  ...
  if (req->vifr_mac_size && req->vifr_mac)
    memcpy(req->vifr_mac, intf->vif_mac, MINIMUM(req->vifr_mac_size, sizeof(intf->vif_mac)));
  ...
  if (intf->vif_src_mac) {
    ...
    for (i = 0; i < VIF_SRC_MACS; i++) {
      ...
      req->vifr_bridge_idx[i] =
          vif_bridge_get_index(intf->vif_parent, intf, intf->vif_src_mac + (i * VR_ETHER_ALEN));
  ...
  if (intf->vif_in_mirror_md_len) {
    memcpy(req->vifr_in_mirror_md, intf->vif_in_mirror_md, intf->vif_in_mirror_md_len);
    req->vifr_in_mirror_md_size = intf->vif_in_mirror_md_len;
  }

  req->vifr_out_mirror_md_size = 0;
  if (intf->vif_out_mirror_md_len) {
    memcpy(req->vifr_out_mirror_md, intf->vif_out_mirror_md, intf->vif_out_mirror_md_len);
  ...
  if (intf->vif_type == VIF_TYPE_PHYSICAL) {
    ...
    if (hif_ops->hif_get_vlan_info) {
      ...
      if (!hif_ops->hif_get_vlan_info(intf, &vlan_info)) {
        ...
        req->vifr_vlan_name =
            vr_zalloc(req->vifr_vlan_name_size * sizeof(uint8_t), VR_INTERFACE_OBJECT);
        if (!req->vifr_vlan_name)
          return -ENOMEM;
        snprintf(req->vifr_vlan_name, req->vifr_vlan_name_size, vlan_info.vlan_name);
      }
    }
  }

  for (i = 0; i < intf->fat_flow_cfg_size; i++) {
    req->vifr_fat_flow_protocol_port[i] = (intf->fat_flow_cfg[i].port) |
    ...
    req->vifr_fat_flow_src_prefix_mask[i] = intf->fat_flow_cfg[i].src_prefix_mask;
    req->vifr_fat_flow_src_aggregate_plen[i] = intf->fat_flow_cfg[i].src_aggregate_plen;
    req->vifr_fat_flow_dst_prefix_h[i] = intf->fat_flow_cfg[i].dst_prefix_h;
    req->vifr_fat_flow_dst_prefix_l[i] = intf->fat_flow_cfg[i].dst_prefix_l;
    req->vifr_fat_flow_dst_prefix_mask[i] = intf->fat_flow_cfg[i].dst_prefix_mask;
    req->vifr_fat_flow_dst_aggregate_plen[i] = intf->fat_flow_cfg[i].dst_aggregate_plen;
  }
...
static int vr_interface_make_req(vr_interface_req *req, struct vr_interface *vif,
                                 unsigned int core) {
  ...
  if (fat_flow_config_size) {
    req->vifr_fat_flow_protocol_port =
        vr_zalloc(fat_flow_config_size * sizeof(uint32_t), VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    ...
    req->vifr_fat_flow_src_prefix_h =
        vr_zalloc(fat_flow_config_size * sizeof(uint64_t), VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    ...
    req->vifr_fat_flow_src_prefix_mask =
        vr_zalloc(fat_flow_config_size * sizeof(uint8_t), VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
  ...
  if (vif->vif_fat_flow_ipv4_exclude_list_size) {
    req->vifr_fat_flow_exclude_ip_list =
        vr_zalloc(vif->vif_fat_flow_ipv4_exclude_list_size * sizeof(uint64_t),
  ...
  if (vif->vif_fat_flow_ipv6_exclude_list_size) {
    req->vifr_fat_flow_exclude_ip6_u_list =
        vr_zalloc(vif->vif_fat_flow_ipv6_exclude_list_size * sizeof(uint64_t),
...
unsigned int vr_interface_req_get_size(void *req_p) {
  unsigned int size;
...
static vr_interface_req *vr_interface_req_get(void) {
  ...
  req->vifr_bridge_idx =
      vr_zalloc(sizeof(uint32_t) * VIF_SRC_MACS, VR_INTERFACE_REQ_BRIDGE_ID_OBJECT);
  if (req->vifr_bridge_idx)
    req->vifr_bridge_idx_size = 0;

  req->vifr_name = vr_zalloc(VR_INTERFACE_NAME_LEN, VR_INTERFACE_REQ_NAME_OBJECT);

  req->vifr_queue_ierrors_to_lcore =
      vr_zalloc(vr_num_cpus * sizeof(uint64_t), VR_INTERFACE_REQ_TO_LCORE_ERRORS_OBJECT);
  if (req->vifr_queue_ierrors_to_lcore)
    req->vifr_queue_ierrors_to_lcore_size = 0;

  req->vifr_in_mirror_md_size = 0;
  req->vifr_in_mirror_md = vr_zalloc(VIF_MAX_MIRROR_MD_SIZE, VR_INTERFACE_REQ_MIRROR_META_OBJECT);

  req->vifr_out_mirror_md_size = 0;
  req->vifr_out_mirror_md = vr_zalloc(VIF_MAX_MIRROR_MD_SIZE, VR_INTERFACE_REQ_MIRROR_META_OBJECT);
  req->vifr_pbb_mac = vr_zalloc(VR_ETHER_ALEN, VR_INTERFACE_REQ_PBB_MAC_OBJECT);

  return req;
}

static void vr_interface_req_free_fat_flow_config(vr_interface_req *req) {
  if (req->vifr_fat_flow_protocol_port) {
    vr_free(req->vifr_fat_flow_protocol_port, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    req->vifr_fat_flow_protocol_port = NULL;
    req->vifr_fat_flow_protocol_port_size = 0;
  }

  if (req->vifr_fat_flow_src_prefix_h) {
    vr_free(req->vifr_fat_flow_src_prefix_h, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    req->vifr_fat_flow_src_prefix_h = NULL;
    req->vifr_fat_flow_src_prefix_h_size = 0;
  }

  if (req->vifr_fat_flow_src_prefix_l) {
    vr_free(req->vifr_fat_flow_src_prefix_l, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    req->vifr_fat_flow_src_prefix_l = NULL;
    req->vifr_fat_flow_src_prefix_l_size = 0;
  }

  if (req->vifr_fat_flow_src_prefix_mask) {
    vr_free(req->vifr_fat_flow_src_prefix_mask, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    req->vifr_fat_flow_src_prefix_mask = NULL;
    req->vifr_fat_flow_src_prefix_mask_size = 0;
  }

  if (req->vifr_fat_flow_src_aggregate_plen) {
    vr_free(req->vifr_fat_flow_src_aggregate_plen, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    req->vifr_fat_flow_src_aggregate_plen = NULL;
    req->vifr_fat_flow_src_aggregate_plen_size = 0;
  }

  if (req->vifr_fat_flow_dst_prefix_h) {
    vr_free(req->vifr_fat_flow_dst_prefix_h, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    req->vifr_fat_flow_dst_prefix_h = NULL;
    req->vifr_fat_flow_dst_prefix_h_size = 0;
  }

  if (req->vifr_fat_flow_dst_prefix_l) {
    vr_free(req->vifr_fat_flow_dst_prefix_l, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    req->vifr_fat_flow_dst_prefix_l = NULL;
    req->vifr_fat_flow_dst_prefix_l_size = 0;
  }

  if (req->vifr_fat_flow_dst_prefix_mask) {
    vr_free(req->vifr_fat_flow_dst_prefix_mask, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    req->vifr_fat_flow_dst_prefix_mask = NULL;
    req->vifr_fat_flow_dst_prefix_mask_size = 0;
  }

  if (req->vifr_fat_flow_dst_aggregate_plen) {
    vr_free(req->vifr_fat_flow_dst_aggregate_plen, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
...
static void vr_interface_req_destroy(vr_interface_req *req) {
  if (!req)
    return;
  ...
  if (req->vifr_queue_ierrors_to_lcore) {
    vr_free(req->vifr_queue_ierrors_to_lcore, VR_INTERFACE_REQ_TO_LCORE_ERRORS_OBJECT);
    req->vifr_queue_ierrors_to_lcore_size = 0;
  }

  if (req->vifr_in_mirror_md) {
    vr_free(req->vifr_in_mirror_md, VR_INTERFACE_REQ_MIRROR_META_OBJECT);
    req->vifr_in_mirror_md_size = 0;
    req->vifr_in_mirror_md = NULL;
  }

  if (req->vifr_out_mirror_md) {
    vr_free(req->vifr_out_mirror_md, VR_INTERFACE_REQ_MIRROR_META_OBJECT);
    req->vifr_out_mirror_md_size = 0;
    req->vifr_out_mirror_md = NULL;
  }

  if (req->vifr_pbb_mac) {
    vr_free(req->vifr_pbb_mac, VR_INTERFACE_REQ_PBB_MAC_OBJECT);
  ...
  if (req->vifr_vlan_name) {
    vr_free(req->vifr_vlan_name, VR_INTERFACE_OBJECT);
...
static int vr_interface_get(vr_interface_req *req) {
  int ret = 0, obj_cnt = 0;
...
static int vr_interface_dump(vr_interface_req *r) {
  int ret = 0;
  unsigned int i;
  ...
  for (i = (unsigned int)(r->vifr_marker + 1); i < router->vr_max_interfaces; i++) {
    vif = router->vr_interfaces[i];
    if (vif) {
      /* zero vifr_core means to sum up all the per-core stats */
...
void vr_interface_req_process(void *s_req) {
  int ret;
...
unsigned int vif_vrf_table_get_nh(struct vr_interface *vif, unsigned short vlan) {
...
int vif_vrf_table_get(struct vr_interface *vif, vr_vrf_assign_req *req) {
 ...
 */
int vif_vrf_table_set(struct vr_interface *vif, unsigned int vlan, int vrf, unsigned int nh_id) {
  int ret = 0;
   ...
   * the table
   */
  if (!(vif->vif_flags & VIF_FLAG_SERVICE_IF) && !vif->vif_vrf_table_users) {
    vr_free(vif->vif_vrf_table, VR_INTERFACE_VRF_TABLE_OBJECT);
    vif->vif_vrf_table = NULL;
  }
...
unsigned int vif_fat_flow_get_proto_index(uint8_t proto) {
...
static void __vif_fat_flow_free_defer_cb(struct vrouter *router, void *data) {
  struct vr_defer_data *vdd = (struct vr_defer_data *)data;
...
void __vif_fat_flow_try_free(struct vr_interface *vif, unsigned int proto_index,
  ...
  if (!memcmp(mem_column, vif_fat_flow_mem_zero, sizeof(vif_fat_flow_mem_zero))) {
...
static void vif_fat_flow_free(uint8_t **mem) {
  int i;
...
int __vif_fat_flow_delete(struct vr_interface *vif, unsigned int proto_index, uint16_t port) {
...
int vif_fat_flow_delete(struct vr_interface *vif, uint8_t proto, uint16_t port) {
...
static int __vif_fat_flow_add_no_prefix_rule(uint8_t **no_prefix_rules[VIF_FAT_FLOW_MAXPROTO_INDEX],
  ...
  if (!no_prefix_rules[proto_index]) {
    no_prefix_rules[proto_index] = vr_zalloc(VIF_FAT_FLOW_NUM_BITMAPS * sizeof(unsigned int *),
  ...
  if (!no_prefix_rules[proto_index][port_row]) {
    no_prefix_rules[proto_index][port_row] =
        vr_zalloc(VIF_FAT_FLOW_BITMAP_BYTES, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
    if (!no_prefix_rules[proto_index][port_row]) {
      if (alloced) {
        vr_free(no_prefix_rules[proto_index], VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
  ...
  if ((port_data == VIF_FAT_FLOW_PORT_SIP_IGNORE) || (port_data == VIF_FAT_FLOW_PORT_DIP_IGNORE))
    port_val = port_data;

  /* First reset the old value */
  no_prefix_rules[proto_index][port_row][port_word] &= ~(VIF_FAT_FLOW_DATA_MASK << port_bit);

  /* Set the new val*/
  no_prefix_rules[proto_index][port_row][port_word] |= (port_val << port_bit);

  return 0;
}

static int __vif_is_fat_flow_src_prefix_rule(vr_fat_flow_cfg_t *cfg) {
...
static int __vif_is_fat_flow_dst_prefix_rule(vr_fat_flow_cfg_t *cfg) {
  switch (VIF_FAT_FLOW_CFG_PREFIX_AGGR_DATA(cfg->port_aggr_info)) {
...
static int __vif_is_fat_flow_src_dst_prefix_rule(vr_fat_flow_cfg_t *cfg) {
  switch (VIF_FAT_FLOW_CFG_PREFIX_AGGR_DATA(cfg->port_aggr_info)) {
...
static int __vif_is_fat_flow_v4_prefix_rule(vr_fat_flow_cfg_t *cfg) {
  switch (VIF_FAT_FLOW_CFG_PREFIX_AGGR_DATA(cfg->port_aggr_info)) {
  case VR_AGGREGATE_SRC_IPV4:
  case VR_AGGREGATE_DST_IPV4:
  case VR_AGGREGATE_SRC_DST_IPV4:
...
static int vif_fat_flow_cfg_is_changed(struct vr_interface *vif, vr_interface_req *req) {
  int i;
  ...
  for (i = 0; i < vif->fat_flow_cfg_size; i++) {
    new_cfg.protocol = VIF_FAT_FLOW_PROTOCOL(req->vifr_fat_flow_protocol_port[i]);
    new_cfg.port = VIF_FAT_FLOW_PORT(req->vifr_fat_flow_protocol_port[i]);
    new_cfg.port_aggr_info = VIF_FAT_FLOW_PORT_AGGR_INFO(req->vifr_fat_flow_protocol_port[i]);
...
static int vif_fat_flow_cfg_build(vr_interface_req *req, vr_fat_flow_cfg_t **new_fat_flow_cfg,
  ...
  *new_fat_flow_cfg = vr_zalloc(sizeof(vr_fat_flow_cfg_t) * req->vifr_fat_flow_protocol_port_size,
                                VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
  if (!(*new_fat_flow_cfg)) {
    return -ENOMEM;
  }
  *new_fat_flow_cfg_size = req->vifr_fat_flow_protocol_port_size;
  for (i = 0; i < req->vifr_fat_flow_protocol_port_size; i++) {
    cfg.protocol = VIF_FAT_FLOW_PROTOCOL(req->vifr_fat_flow_protocol_port[i]);
    cfg.port = VIF_FAT_FLOW_PORT(req->vifr_fat_flow_protocol_port[i]);
    cfg.port_aggr_info = VIF_FAT_FLOW_PORT_AGGR_INFO(req->vifr_fat_flow_protocol_port[i]);
...
static void vif_fat_flow_cfg_swap(struct vr_interface *vif, vr_fat_flow_cfg_t *new_fat_flow_cfg,
                                  uint16_t new_fat_flow_cfg_size, uint16_t *new_fat_flow_num_rules,
  ...
  *old_fat_flow_cfg = vif->fat_flow_cfg;
  ...
  memcpy(vif->fat_flow_num_rules, new_fat_flow_num_rules, sizeof(vif->fat_flow_num_rules));
}

static void vif_fat_flow_cfg_free(vr_fat_flow_cfg_t *old_fat_flow_cfg) {
  if (old_fat_flow_cfg) {
...
__vif_fat_flow_alloc_prefix_rule_data(vr_fat_flow_cfg_t *cfg) {
  ...
  rdata = vr_zalloc(sizeof(vr_fat_flow_prefix_rule_data_t), VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
  if (!rdata) {
    goto err;
  }
  rdata->proto_info =
      vr_zalloc(sizeof(vr_fat_flow_prefix_rule_proto_info_t), VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
  if (!rdata->proto_info) {
    goto err;
  }
  for (i = 0; i < VIF_FAT_FLOW_MAXPROTO_INDEX; i++) {
  ...
  if (__vif_is_fat_flow_src_dst_prefix_rule(cfg)) {
    ...
    if (VIF_FAT_FLOW_CFG_PORT_DATA(cfg->port_aggr_info) == VIF_FAT_FLOW_PORT_SIP_IGNORE) {
      port_data->rule_type |= PREFIX_RULE_HAS_IGNORE_SRC;
    } else if (VIF_FAT_FLOW_CFG_PORT_DATA(cfg->port_aggr_info) == VIF_FAT_FLOW_PORT_DIP_IGNORE) {
...
static int __vif_fat_flow_update_prefix_rule_data(vr_fat_flow_prefix_rule_data_t *rdata,
                                                  vr_fat_flow_cfg_t *cfg) {
  int proto_index;
  vr_itable_t ptable;
...
static int __vif_fat_flow_add_prefix_rule(struct ip_mtrie *rules, vr_fat_flow_cfg_t *cfg,
  ...
  if (__vif_is_fat_flow_src_prefix_rule(cfg) || __vif_is_fat_flow_src_dst_prefix_rule(cfg)) {
    if (__vif_is_fat_flow_v4_prefix_rule(cfg)) {
...
static void __vif_flat_flow_free_no_prefix_rules_cb(struct vrouter *router, void *data) {
  struct vr_defer_data *vdd = (struct vr_defer_data *)data;
...
static void
__vif_fat_flow_delete_all_no_prefix_rules(uint8_t **no_prefix_rules[VIF_FAT_FLOW_MAXPROTO_INDEX]) {
...
static void __vif_flat_flow_free_prefix_rules_cb(struct vrouter *router, void *data) {
...
static void __vif_fat_flow_delete_all_prefix_rules(struct ip_mtrie *prefix_rules) {
...
static void __vif_fat_flow_free_port_data_cb(unsigned int index, void *data) {
  vr_fat_flow_prefix_rule_port_data_t *port_data;
  if (!data) {
    return;
  }
  ...
  while (head) {
    ...
    for (i = 0; i < VIF_FAT_FLOW_MAXPROTO_INDEX; i++) {
      if (!head->proto_info->proto[i])
        continue;
      vr_itable_delete(head->proto_info->proto[i], __vif_fat_flow_free_port_data_cb);
    }
...
static void __vif_fat_flow_free_rule_data_defer_cb(struct vrouter *router, void *data) {
  struct vr_defer_data *vdd = (struct vr_defer_data *)data;
  ...
  for (i = 0; i < VIF_FAT_FLOW_MAXPROTO_INDEX; i++) {
    if (!rdata->proto_info->proto[i])
      continue;
    vr_itable_delete(rdata->proto_info->proto[i], __vif_fat_flow_free_port_data_cb);
  }
  vr_free(rdata->proto_info, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
  vr_free(rdata, VR_INTERFACE_FAT_FLOW_CONFIG_OBJECT);
  return;
...
static void __vif_fat_flow_delete_all_rule_data_list(vr_fat_flow_prefix_rule_data_t *head) {
  vr_fat_flow_prefix_rule_data_t *nextent;
...
static int vif_fat_flow_rules_build(vr_fat_flow_cfg_t *new_cfg, uint16_t new_cfg_size,
                                    struct vr_interface *vif) {
  uint8_t **no_prefix_rules[VIF_FAT_FLOW_MAXPROTO_INDEX] = {NULL},
          **old_no_prefix_rules[VIF_FAT_FLOW_MAXPROTO_INDEX] = {NULL};
  struct ip_mtrie *v4_src_prefix_rules = NULL, *old_v4_src_prefix_rules = NULL;
  struct ip_mtrie *v4_dst_prefix_rules = NULL, *old_v4_dst_prefix_rules = NULL;
  struct ip_mtrie *v6_src_prefix_rules = NULL, *old_v6_src_prefix_rules = NULL;
  struct ip_mtrie *v6_dst_prefix_rules = NULL, *old_v6_dst_prefix_rules = NULL;
  struct ip_mtrie **prefix_rules = NULL;
  vr_fat_flow_prefix_rule_data_t *rule_data_list = NULL, *old_rule_data_list = NULL;
  uint8_t prefix_aggr;
  ...
  for (i = 0; i < new_cfg_size; i++) {
    ...
    case VR_AGGREGATE_NONE:
      ...
      if (proto_index == VIF_FAT_FLOW_NOPROTO_INDEX)
        port = proto;
      ret = __vif_fat_flow_add_no_prefix_rule(no_prefix_rules, proto, port, port_data);
      if (ret) {
        goto err;
      }
      break;
    ...
    case VR_AGGREGATE_SRC_DST_IPV4:
    case VR_AGGREGATE_SRC_DST_IPV6:
      if ((prefix_aggr == VR_AGGREGATE_SRC_IPV4) || (prefix_aggr == VR_AGGREGATE_SRC_DST_IPV4)) {
      ...
      ret = __vif_fat_flow_add_prefix_rule((*prefix_rules), &new_cfg[i], &rule_data_list);
      if (ret != 0) {
        goto err;
      }
      break;
...
static int vif_fat_flow_add(struct vr_interface *vif, vr_interface_req *req) {
  int rc;
  ...
  if (!req->vifr_fat_flow_exclude_ip6_l_list_size) {
    ...
    if ((req->vifr_fat_flow_exclude_ip6_l_list_size !=
         req->vifr_fat_flow_exclude_ip6_u_list_size) ||
        (req->vifr_fat_flow_exclude_ip6_l_list_size !=
         req->vifr_fat_flow_exclude_ip6_plen_list_size) ||
  ...
  if (vif_fat_flow_cfg_is_changed(vif, req)) {
    ...
    vif_fat_flow_cfg_swap(vif, new_fat_flow_cfg, new_fat_flow_cfg_size, new_fat_flow_num_rules,
                          &old_fat_flow_cfg);
    /* Free the old cfg */
...
static uint8_t vif_fat_flow_port_get(struct vr_interface *vif, uint8_t proto_index, uint16_t port) {
  ...
  if (vif->vif_fat_flow_no_prefix_rules[row]) {
    if (vif->vif_fat_flow_no_prefix_rules[row][column]) {
      ...
      return (vif->vif_fat_flow_no_prefix_rules[row][column][byte] >> bit) & VIF_FAT_FLOW_DATA_MASK;
    }
  }
...
static uint8_t vif_fat_flow_exclude_list_lookup(struct vr_interface *vif, unsigned int *saddr,
                                                unsigned int *daddr, unsigned char *ip6_src,
                                                unsigned char *ip6_dst) {
  int i;
  ...
  if (saddr) {
    ...
    for (i = 0; i < vif->vif_fat_flow_ipv6_exclude_list_size; i++) {
      fat_flow_ipv6_plen_to_mask(vif->vif_fat_flow_ipv6_exclude_plen_list[i], &v6_mask_h,
                                 &v6_mask_l);
      /* compare src ip */
...
static int __vif_fat_flow_ip_prefix_lookup(struct ip_mtrie *mtrie, unsigned int *ip,
...
static int vif_fat_flow_prefix_proto_port_match(uint8_t rule_type, int incoming_vif,
                                                vr_fat_flow_prefix_rule_data_t *rdata,
  ...
  int proto_index;
  vr_fat_flow_prefix_rule_port_data_t *src_port_info = NULL, *dst_port_info = NULL,
                                      *zero_port_info = NULL;
  vr_itable_t ptable = NULL;
  ...
  if (ptable) {
      ...
      if (src_port_info) {
        if (rule_type == PREFIX_RULE_TYPE_SINGLE_PREFIX) {
          ...
          *fat_flow_mask = VR_FAT_FLOW_DST_PORT_MASK;
          ...
          if (__vif_fat_flow_ip_prefix_lookup(src_port_info->second_prefix, dip, dip6,
                                              aggr_mask1)) {
            *aggr_mask = src_port_info->aggr_plen;
            *fat_flow_mask = VR_FAT_FLOW_DST_PORT_MASK;
            return 1;
      ...
      if (dst_port_info) {
        if (rule_type == PREFIX_RULE_TYPE_SINGLE_PREFIX) {
          ...
          *fat_flow_mask = VR_FAT_FLOW_SRC_PORT_MASK;
          ...
          if (__vif_fat_flow_ip_prefix_lookup(dst_port_info->second_prefix, dip, dip6,
                                              aggr_mask1)) {
            *aggr_mask = dst_port_info->aggr_plen;
            *fat_flow_mask = VR_FAT_FLOW_SRC_PORT_MASK;
            return 1;
    ...
    if (zero_port_info) {
      if (rule_type == PREFIX_RULE_TYPE_SINGLE_PREFIX) {
        ...
        *fat_flow_mask = (VR_FAT_FLOW_DST_PORT_MASK | VR_FAT_FLOW_SRC_PORT_MASK);
        ...
        if (__vif_fat_flow_ip_prefix_lookup(zero_port_info->second_prefix, dip, dip6, aggr_mask1)) {
          *aggr_mask = zero_port_info->aggr_plen;
          *fat_flow_mask = (VR_FAT_FLOW_DST_PORT_MASK | VR_FAT_FLOW_SRC_PORT_MASK);
          return 1;
        }
...
static uint16_t vif_fat_flow_prefix_rule_match(int incoming_vif, struct vr_interface *vif,
                                               uint8_t proto, uint16_t sport, uint16_t dport,
  ...
  uint64_t prefix[2];
  vr_fat_flow_prefix_rule_data_t *rdata_src = NULL, *rdata_dst = NULL;
  ...
  if (rdata_src && (vr_req.rtr_req.rtr_prefix_len != 0)) {
    ...
    if (vif_fat_flow_prefix_proto_port_match(PREFIX_RULE_TYPE_SINGLE_PREFIX, incoming_vif,
                                             rdata_src, NULL, NULL, proto, *sport_flow, *dport_flow,
                                             &aggr_mask, NULL, &fat_flow_mask)) {
      if (saddr) {
  ...
  if (rdata_dst && (vr_req.rtr_req.rtr_prefix_len != 0)) {
    if (vif_fat_flow_prefix_proto_port_match(PREFIX_RULE_TYPE_SINGLE_PREFIX, incoming_vif,
                                             rdata_dst, NULL, NULL, proto, *sport_flow, *dport_flow,
                                             &aggr_mask, NULL, &fat_flow_mask)) {
      if (daddr) {
  ...
  if (rdata_src) {
    if (vif_fat_flow_prefix_proto_port_match(PREFIX_RULE_TYPE_DUAL_PREFIX, incoming_vif, rdata_src,
                                             dip_flow, dip6_flow, proto, *sport_flow, *dport_flow,
                                             &aggr_mask, &aggr_mask1, &fat_flow_mask)) {
      if (saddr) {
        *sip_flow = (*sip_flow) & FAT_FLOW_IPV4_PLEN_TO_MASK(aggr_mask);
      } else {
        fat_flow_ipv6_plen_to_mask(aggr_mask, &ip6_mask_h, &ip6_mask_l);
...
uint16_t vif_fat_flow_lookup(int incoming_vif, struct vr_interface *vif, uint8_t proto,
                             uint16_t sport, uint16_t dport, unsigned int *saddr,
                             unsigned int *daddr, unsigned char *ip6_src, unsigned char *ip6_dst) {
  ...
  return vif_fat_flow_prefix_rule_match(incoming_vif, vif, proto, h_sport, h_dport, saddr, daddr,
                                        ip6_src, ip6_dst);
}

int vr_gro_vif_add(struct vrouter *router, unsigned int os_idx, char *name, unsigned short idx) {
  int ret = 0;
 ...
 */
void vr_interface_shut(struct vrouter *router) {
  unsigned int i;
  struct vr_interface *vif;
...
void vr_interface_exit(struct vrouter *router, bool soft_reset) {
  unsigned int i;
  struct vr_interface *vif;
...
int vr_interface_init(struct vrouter *router) {
  int ret = 0;
  unsigned int table_memory = 0;

  if (!router->vr_interfaces) {
    router->vr_max_interfaces = vr_interfaces;
    table_memory = router->vr_max_interfaces * sizeof(struct vr_interface *);
    router->vr_interfaces = vr_zalloc(table_memory, VR_INTERFACE_TABLE_OBJECT);
    if (!router->vr_interfaces && (ret = -ENOMEM))
      return vr_module_error(ret, __FUNCTION__, __LINE__, table_memory);
  }
