...
int vr_offloads_interface_add(struct vr_interface *vif) {
  if (!vif)
    return -EINVAL;
...
int vr_offloads_interface_del(struct vr_interface *vif) {
  if (!vif)
    return -EINVAL;
  if (vif->vif_type == VIF_TYPE_PHYSICAL) {
    if (vif != pvif)
...
static bool vr_offloads_is_offloaded_nexthop(struct vr_nexthop *nh) {
  return ((nh->nh_flags & NH_FLAG_VALID) && !(nh->nh_flags & NH_FLAG_MCAST) &&
          (nh->nh_flags & NH_FLAG_POLICY_ENABLED) && (nh->nh_type == NH_ENCAP) && nh->nh_dev);
}

int vr_offloads_mpls_add(struct vr_nexthop *nh, int label) {
...
int vr_offloads_vxlan_add(struct vr_nexthop *nh, int vnid) {
  struct vr_offload_tag *otag;
...
int vr_offloads_mpls_del(int label) {
  struct vr_offload_tag *otag;
...
int vr_offloads_vxlan_del(int vnid) {
  struct vr_offload_tag *otag;
  struct vrouter *router = vrouter_get(0);
...
int vr_offloads_flow_del(struct vr_flow_entry *fe) {
  ...
  oflow = (struct vr_offload_flow *)vr_btable_get(offload_flows, fe->fe_hentry.hentry_index);
  if (!oflow) {
...
int vr_offloads_flow_set(struct vr_flow_entry *fe, unsigned int fe_index,
  ...
  if (snh->nh_flags & NH_FLAG_TUNNEL_GRE) {
    if (otag[VR_OFFLOADS_TAG_TYPE_MPLS_L2].nh && (nh->nh_dev->vif_flags & VIF_FLAG_L2_ENABLED)) {
    ...
    } else if (otag[VR_OFFLOADS_TAG_TYPE_MPLS_L3].nh &&
               (nh->nh_dev->vif_flags & VIF_FLAG_L3_ENABLED)) {
      oflow->tunnel_type = NH_FLAG_TUNNEL_GRE;
...
int vr_offloads_init(struct vrouter *router) {
  ...
  if (!vr_offload_flow_destroy || !vr_offload_flow_create || !vr_offload_prepare) {
  ...
  if (!offload_flows) {
    /* Round up to the next divisor */
    for (entry_size = sizeof(struct vr_offload_flow); VR_SINGLE_ALLOC_LIMIT % entry_size;
         entry_size++)
      ;
...
static void _vr_offloads_exit(struct vrouter *router, bool soft_reset) {
  struct vr_offload_flow *oflow;
  struct vr_offload_tag *otag;
  unsigned int i;
...
inline struct vr_offload_flow *vr_offloads_flow_get(unsigned int index) {
  struct vr_offload_flow *oflow = (struct vr_offload_flow *)vr_btable_get(offload_flows, index);
 ...
 */
static void _vr_offload_unregister(void) {
