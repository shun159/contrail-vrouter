...
static void mtrie_ip_bkt_info_init(struct mtrie_bkt_info *ip_bkt_info, int pfx_len) {
  int level;
  ...
  for (level = 1; level < (pfx_len / IPBUCKET_LEVEL_BITS); level++) {
    ...
    ip_bkt_info[level].bi_pfx_len = ip_bkt_info[level - 1].bi_pfx_len + IPBUCKET_LEVEL_BITS;
 ...
 */
static inline struct ip_mtrie *vrfid_to_mtrie(unsigned int vrf_id, unsigned int family) {
  int index = 0;
...
static inline unsigned int ip_bkt_get_max_level(int family) {
  if (family == AF_INET6)
...
static struct mtrie_bkt_info *ip_bkt_info_get(unsigned int family) {
  if (family == AF_INET6)
    return ip6_bkt_info;
  else
    return ip4_bkt_info;
 ...
 */
static inline unsigned char rt_to_index(struct vr_route_req *rt, unsigned int level) {
  return PREFIX_TO_INDEX(rt->rtr_req.rtr_prefix, level);
}

static inline struct ip_bucket_entry *index_to_entry(struct ip_bucket *bkt, int index) {
  return &bkt->bkt_data[index];
}

static void set_entry_to_bucket(struct ip_bucket_entry *ent, struct ip_bucket *bkt) {
 ...
 */
static void set_entry_to_nh(struct ip_bucket_entry *entry, struct vr_nexthop *nh) {
...
static void set_entry_to_vdata(struct ip_bucket_entry *entry, void *data) {
  /* update the entry */
...
static inline struct ip_bucket *entry_to_bucket(struct ip_bucket_entry *ent) {
...
static void mtrie_free_entry(struct ip_bucket_entry *entry, unsigned int level) {
  unsigned int i;
...
static void mtrie_free_bkt(struct ip_bucket *bkt) {
  unsigned int i;
...
static void mtrie_free_bkt_cb(struct vrouter *router, void *data) {
...
static int mtrie_free_bkt_defer(struct vrouter *router, struct ip_bucket *bkt) {
...
static void mtrie_delete_bkt(struct ip_bucket_entry *ent, struct vr_route_req *rt, int defer_delete,
                             int data_is_nh) {
  struct ip_bucket *bkt;
 ...
 */
static struct ip_bucket *mtrie_alloc_bucket(struct mtrie_bkt_info *ip_bkt_info, unsigned char level,
  ...
  bkt = vr_zalloc(sizeof(struct ip_bucket) + sizeof(struct ip_bucket_entry) * bkt_size,
...
static void add_to_tree(struct ip_bucket_entry *ent, int level, struct vr_route_req *rt) {
  unsigned int i;
  struct ip_bucket *bkt;
...
static void mtrie_reset_entry(struct ip_bucket_entry *ent, int level, void *data, int data_is_nh) {
 ...
 * void *vdata
 */
static int __mtrie_add(struct ip_mtrie *mtrie, struct vr_route_req *rt, int data_is_nh) {
  ...
  for (level = 0; level < ip_bkt_get_max_level(rt->rtr_req.rtr_family); level++) {
    ...
    if (rt->rtr_req.rtr_prefix_len > ip_bkt_info[level].bi_pfx_len) {
      ...
      i = index;
      for (; ((i <= (ip_bkt_info[level].bi_size - 1)) && fin); i++, fin--) {
        ent = index_to_entry(bkt, i);
        add_to_tree(ent, level, rt);
      }
 ...
 * or void * data nodes;
 */
static int __mtrie_delete(struct vr_route_req *rt, struct ip_bucket_entry *ent, unsigned char level,
                          int defer_delete, int data_is_nh) {
  ...
  for (i = 1; i < ip_bkt_info[level].bi_size; i++) {
    if (memcmp(bkt->bkt_data + i, bkt->bkt_data, sizeof(struct ip_bucket_entry)))
      return 0;
  }
...
static int mtrie_dumper_route_encode(struct vr_message_dumper *dumper, vr_route_req *resp) {
  int len;
...
static void mtrie_dumper_make_response(struct vr_message_dumper *dumper, vr_route_req *resp,
                                       struct ip_bucket_entry *ent, int8_t *prefix,
...
static int mtrie_dump_entry(struct vr_message_dumper *dumper, struct ip_bucket_entry *orig_ent,
  ...
  if (ENTRY_IS_BUCKET(orig_ent)) {
    ...
    if (!dumper->dump_been_to_marker) {
      i = ip_bkt_info[level].bi_mask & (PREFIX_TO_INDEX(req->rtr_marker, level));
      ent = index_to_entry(bkt, i);
      prefix[level] = i;
...
static int mtrie_walk(struct vr_message_dumper *dumper, unsigned int family) {
  vr_route_req *req;
...
static int mtrie_dump(struct vr_rtable *__unsued, struct vr_route_req *rt) {
  int ret = 0;
 ...
 * concurrency.
 */
static int mtrie_delete(struct vr_rtable *_unused, struct vr_route_req *rt) {
  ...
  if ((rt->rtr_req.rtr_mac_size == VR_ETHER_ALEN) && (!IS_MAC_ZERO(rt->rtr_req.rtr_mac))) {
...
static inline struct vr_vrf_stats *mtrie_stats(int vrf, unsigned int cpu) {
...
static int mtrie_stats_get(vr_vrf_stats_req *req, vr_vrf_stats_req *response) {
  unsigned int i;
  ...
  for (i = 0; i < vr_num_cpus; i++) {
    ...
    if (stats) {
      ...
      response->vsr_udp_mpls_over_mpls_tunnels += stats->vrf_udp_mpls_over_mpls_tunnels;
    }
  }
...
static bool mtrie_stats_empty(vr_vrf_stats_req *r) {
  if (r->vsr_discards || r->vsr_resolves || r->vsr_receives || r->vsr_ecmp_composites ||
      r->vsr_l2_mcast_composites || r->vsr_fabric_composites || r->vsr_udp_tunnels ||
      r->vsr_udp_mpls_tunnels || r->vsr_gre_mpls_tunnels || r->vsr_l2_encaps || r->vsr_encaps ||
      r->vsr_gros || r->vsr_diags || r->vsr_encap_composites || r->vsr_evpn_composites ||
      r->vsr_vrf_translates || r->vsr_vxlan_tunnels || r->vsr_arp_virtual_proxy ||
      r->vsr_arp_virtual_stitch || r->vsr_arp_virtual_flood || r->vsr_arp_physical_stitch ||
      r->vsr_arp_tor_proxy || r->vsr_arp_physical_flood || r->vsr_l2_receives ||
      r->vsr_uuc_floods || r->vsr_pbb_tunnels || r->vsr_udp_mpls_over_mpls_tunnels)
    return false;

  return true;
}

static int mtrie_stats_dump(struct vr_rtable *rtable, vr_vrf_stats_req *req) {
  int ret = 0, len;
  unsigned int i;
  struct vr_message_dumper *dumper;
  ...
  for (i = req->vsr_marker + 1; i < rtable->algo_max_vrfs; i++) {
    ...
    if (mtrie_stats_empty(&response))
      continue;
    len = vr_message_dump_object(dumper, VR_VRF_STATS_OBJECT_ID, &response);
    if (len <= 0)
      goto generate_response;
  }
 ...
 */
void *__mtrie_lookup(struct vr_route_req *rt, struct ip_bucket *bkt, unsigned int level) {
  ...
  if (rt->rtr_req.rtr_prefix_len > ip_bkt_info[level].bi_pfx_len) {
    ...
    limit = (1 << (ip_bkt_info[level].bi_pfx_len - rt->rtr_req.rtr_prefix_len));
  }
 ...
 */
static struct vr_nexthop *mtrie_lookup(unsigned int vrf_id, struct vr_route_req *rt) {
 ...
 */
static int mtrie_add(struct vr_rtable *_unused, struct vr_route_req *rt) {
 ...
 */
static int mtrie_get(unsigned int vrf_id, struct vr_route_req *rt) {
  struct vr_nexthop *nh;
...
static struct ip_mtrie *mtrie_alloc_vrf(unsigned int vrf_id, unsigned int family) {
  struct ip_mtrie *mtrie;
  struct ip_mtrie **mtrie_table;
  int index = 0;
...
static void mtrie_free_vrf(struct vr_rtable *rtable, unsigned int vrf_id) {
  struct ip_mtrie *mtrie;
...
static void mtrie_stats_cleanup(struct vr_rtable *rtable, bool soft_reset) {
...
void mtrie_algo_deinit(struct vr_rtable *rtable, struct rtable_fspec *fs, bool soft_reset) {
  unsigned int i;
...
static int mtrie_stats_init(struct vr_rtable *rtable) {
  int ret = 0, i = 0;
  ...
  if (!mtrie_vrf_stats) {
    ...
    if (!mtrie_vrf_stats)
      return vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, stats_memory);
    for (i = 0; i < rtable->algo_max_vrfs; i++) {
      stats_memory = sizeof(struct vr_vrf_stats) * vr_num_cpus;
      mtrie_vrf_stats[i] = vr_zalloc(stats_memory, VR_MTRIE_STATS_OBJECT);
  ...
  if (!invalid_vrf_stats) {
    invalid_vrf_stats = vr_zalloc(sizeof(struct vr_vrf_stats) * vr_num_cpus, VR_MTRIE_STATS_OBJECT);
...
struct vr_nexthop *vr_inet_route_lookup(unsigned int vrf_id, struct vr_route_req *rt) {
  if (!vn_rtable[0] || !vn_rtable[1])
    return NULL;
  return mtrie_lookup(vrf_id, rt);
}

int mtrie_algo_init(struct vr_rtable *rtable, struct rtable_fspec *fs) {
  int ret = 0;
  ...
  /* ipv6 table */
  vn_rtable[1] = (struct ip_mtrie **)((unsigned char **)rtable->algo_data + fs->rtb_max_vrfs);
