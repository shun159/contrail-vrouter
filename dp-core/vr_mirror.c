...
static void vr_mirror_defer_delete(struct vrouter *router, void *arg) {
...
int __vr_mirror_del(struct vrouter *router, unsigned int index) {
...
int vr_mirror_del(vr_mirror_req *req) {
  int ret = -EINVAL;
...
int vr_mirror_add(vr_mirror_req *req) {
  int ret = 0;
  struct vrouter *router;
...
static void vr_mirror_make_req(vr_mirror_req *req, struct vr_mirror_entry *mirror,
...
static int vr_mirror_dump(vr_mirror_req *r) {
  int ret = 0;
  unsigned int i;
  ...
  for (i = (unsigned int)(r->mirr_marker + 1); i < router->vr_max_mirror_indices; i++) {
...
static int vr_mirror_get(vr_mirror_req *req) {
  int ret = 0;
  struct vrouter *router;
  struct vr_mirror_entry *mirror = NULL;

  router = vrouter_get(req->mirr_rid);
  if (!router || (unsigned int)req->mirr_index >= router->vr_max_mirror_indices) {
    ret = -ENODEV;
  } else {
    mirror = router->vr_mirrors[req->mirr_index];
    if (!mirror)
...
static void vr_mirror_meta_destroy(struct vr_mirror_meta_entry *me) {
  if (!me)
    return;
...
static void vr_mirror_meta_destructor(struct vrouter *router, void *arg) {
  struct vr_defer_data *defer = (struct vr_defer_data *)arg;
...
static void vr_mirror_meta_entry_destroy(struct vrouter *router, struct vr_mirror_meta_entry *me) {
  struct vr_defer_data *defer;
...
struct vr_mirror_meta_entry *vr_mirror_meta_entry_set(struct vrouter *router, unsigned int index,
                                                      unsigned int mir_sip,
                                                      unsigned short mir_sport, void *meta_data,
...
void vr_mirror_meta_entry_del(struct vrouter *router, struct vr_mirror_meta_entry *me) {
  if (me)
...
static struct vr_mirror_meta_entry *vr_mirror_meta_entry_get(struct vrouter *router,
...
int vr_mirror(struct vrouter *router, uint8_t mirror_id, struct vr_packet *pkt,
  ...
  if (pkt->vp_if && (pkt->vp_if->vif_type == VIF_TYPE_PHYSICAL)) {
    ...
    if (mtype == MIRROR_TYPE_ACL) {
      ...
      pkt_nh = pkt->vp_nh;
      if (pkt_nh && (pkt_nh->nh_flags & NH_FLAG_VALID) && (pkt_nh->nh_type == NH_ENCAP) &&
        ...
        if (pkt_nh->nh_dev->vif_set_rewrite(pkt_nh->nh_dev, &pkt, fmd, pkt_nh->nh_data,
        ...
        if (pkt->vp_type == VP_TYPE_IP6) {
          proto_p = (unsigned short *)(pkt_data(pkt) + pkt_nh->nh_encap_len - VR_ETHER_PROTO_LEN);
...
void vr_mirror_exit(struct vrouter *router, bool soft_reset) {
  unsigned int i;
...
int vr_mirror_init(struct vrouter *router) {
  unsigned int size;
