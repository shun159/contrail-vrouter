...
static char *vr_message_default_malloc(unsigned int size) {
  return vr_malloc(size, VR_MESSAGE_OBJECT);
}

static void vr_message_default_free(char *buf) {
  if (buf)
...
void *vr_mtrans_alloc(unsigned int size) {
...
int vr_message_request(struct vr_message *message) {
  int ret;
  ...
  ret = message_h.vm_proto->mproto_decode(message->vr_message_buf, message->vr_message_len, NULL,
                                          NULL);
  if (ret < 0)
    return ret;
...
static int vr_message_queue_response(char *buf, int len, bool broadcast) {
  ...
  vr_queue_enqueue(&message_h.vm_response_queue, &response->vr_message_queue);
...
bool vr_response_queue_empty(void) { return vr_queue_empty(&message_h.vm_response_queue); }
...
int vr_message_make_request(unsigned int object_type, void *object) {
  char *buf = NULL;
  int ret;
  unsigned int len;
  ...
  ret = proto->mproto_encode(buf, len, object_type, object, VR_MESSAGE_TYPE_REQUEST);
  if (ret < 0)
    goto request_fail;
...
int vr_message_process_response(int (*cb)(void *, unsigned int, void *), void *cb_arg) {
  struct vr_message *response;
  struct vr_mproto *proto;
  struct vr_mtransport *trans;
  ...
  while ((response = vr_message_dequeue_response())) {
    proto->mproto_decode(response->vr_message_buf, response->vr_message_len, cb, cb_arg);
...
int vr_message_multi_response(struct vr_message_multi *objects) {
  char *buf = NULL;
  int ret = 0;
  ...
  if ((!objects) || (objects->vr_mm_object_count >= VR_MESSAGE_MULTI_MAX_OBJECTS))
    goto response_fail;
  ...
  for (i = 0; i < objects->vr_mm_object_count; i++) {
    buf_len += proto->mproto_buf_len(objects->vr_mm_object_type[i], objects->vr_mm_object[i]);
  }
...
int vr_message_response(unsigned int object_type, void *object, int ret, bool broadcast) {
  char *buf = NULL;
  unsigned int len = 0;
  struct vr_mproto *proto;
  struct vr_mtransport *trans;
  ...
  ret = proto->mproto_encode_response(buf, len, object_type, object, ret);
  if (ret < 0)
    goto response_fail;
...
int vr_send_response(int code) { return vr_message_response(VR_NULL_OBJECT_ID, NULL, code, false); }

int vr_send_broadcast(unsigned int object_type, void *object, unsigned int sandesh_op, int code) {
...
int vr_message_dump_object(void *arg, unsigned int object_type, void *object) {
  int ret;
  struct vr_mproto *proto;
  struct vr_mtransport *trans;
  ...
  ret = proto->mproto_encode(dumper->dump_buffer + dumper->dump_offset,
                             dumper->dump_buf_len - dumper->dump_offset, object_type, object,
                             VR_MESSAGE_TYPE_RESPONSE);
  if (ret < 0) {
...
void vr_message_dump_exit(void *context, int ret) {
  ...
  if (dumper) {
    if (!dumper->dump_offset) {
      ...
      vr_message_queue_response(dumper->dump_buffer, dumper->dump_offset, false);
...
struct vr_message_dumper *vr_message_dump_init(void *req) {
  char *buf;
...
void vr_message_transport_unregister(struct vr_mtransport *trans) {
...
int vr_message_transport_register(struct vr_mtransport *trans) {
  message_h.vm_trans = trans;
  return 0;
}

void vr_message_proto_unregister(struct vr_mproto *proto) {
...
int vr_message_proto_register(struct vr_mproto *proto) {
  if (message_h.vm_proto)
    return -EEXIST;
