...
unsigned int vr_pkt_drop_log_req_get_size(void *object) {
  ...
  return size = 4 * sizeof(*req) + (vr_pkt_droplog_bufsz * sizeof(vr_pkt_drop_log_t));
}

/* Function to repond packet drop log buffer for requested core */
static void vr_pkt_drop_log_get(unsigned int rid, short core, int index) {
  ...
  if (vr_pkt_droplog_sysctl_en == 1) {
    ...
    if (vr_pkt_droplog_buf_en == 1) {
       ...
       * sandesh*/
      response->vdl_pkt_droplog_arr_size = pkt_buffer_size * sizeof(vr_pkt_drop_log_t);

      response->vdl_pkt_droplog_arr =
          (char *)vr_zalloc(response->vdl_pkt_droplog_arr_size, VR_PKT_DROP_LOG_REQ_OBJECT);
      ...
      memcpy(response->vdl_pkt_droplog_arr, router->vr_pkt_drop->vr_pkt_drop_log[core] + (index),
...
int vr_pkt_drop_log_init(struct vrouter *router) {
  ...
  if (vr_pkt_droplog_buf_en == 1) {
    ...
    router->vr_pkt_drop = vr_zalloc(sizeof(struct vr_pkt_drop_st), VR_PKT_DROP_LOG_OBJECT);
    ...
    for (i = 0; i < vr_num_cpus; i++) {
      ...
      vr_pkt_drop->vr_pkt_drop_log[i] = vr_zalloc(size, VR_PKT_DROP_LOG_OBJECT);
    ...
    vr_pkt_drop->vr_pkt_drop_log_buffer_index = vr_zalloc(size, VR_PKT_DROP_LOG_OBJECT);
