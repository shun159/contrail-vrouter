...
static int vr_vrf_assign_dump(vr_vrf_assign_req *req) {
  int ret = 0, i;
...
static int vr_vrf_assign_get(vr_vrf_assign_req *req) {
  int ret;
  vr_vrf_assign_req resp;
...
static int vr_vrf_assign_set(vr_vrf_assign_req *req) {
  int ret;
  struct vr_interface *vif;
  ...
  ret = vif_vrf_table_set(vif, req->var_vlan_id, req->var_vif_vrf, req->var_nh_id);
  if (ret == 0) {
