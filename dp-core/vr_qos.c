...
static void vr_qos_message_error(int error) {
  vr_send_response(error);
  return;
}

unsigned int vr_qos_map_req_get_size(void *object) {
  unsigned int size;
...
static int vr_qos_map_request_validate(vr_qos_map_req *req) {
...
static struct vr_forwarding_class *vr_qos_map_get_fc(struct vrouter *router, unsigned int id) {
...
static int vr_qos_map_set_fc(struct vrouter *router, unsigned int id,
...
static void vr_qos_map_req_destroy(vr_qos_map_req *req) {
  if (!req)
    return;
...
static vr_qos_map_req *vr_qos_map_req_get(void) {
  vr_qos_map_req *req;
  ...
  req->qmr_dscp = vr_zalloc(sizeof(uint8_t) * VR_DSCP_QOS_ENTRIES, VR_QOS_MAP_OBJECT);
  if (!req->qmr_dscp) {
    goto alloc_failure;
  }
  req->qmr_dscp_size = VR_DSCP_QOS_ENTRIES;

  req->qmr_dscp_fc_id = vr_zalloc(sizeof(uint8_t) * VR_DSCP_QOS_ENTRIES, VR_QOS_MAP_OBJECT);
  if (!req->qmr_dscp_fc_id) {
    goto alloc_failure;
  }
  req->qmr_dscp_fc_id_size = VR_DSCP_QOS_ENTRIES;

  req->qmr_mpls_qos = vr_zalloc(sizeof(uint8_t) * VR_MPLS_QOS_ENTRIES, VR_QOS_MAP_OBJECT);
  if (!req->qmr_mpls_qos) {
    goto alloc_failure;
  }
  req->qmr_mpls_qos_size = VR_MPLS_QOS_ENTRIES;

  req->qmr_mpls_qos_fc_id = vr_zalloc(sizeof(uint8_t) * VR_MPLS_QOS_ENTRIES, VR_QOS_MAP_OBJECT);
  if (!req->qmr_mpls_qos_fc_id) {
    goto alloc_failure;
  }
  req->qmr_mpls_qos_fc_id_size = VR_MPLS_QOS_ENTRIES;

  req->qmr_dotonep = vr_zalloc(sizeof(uint8_t) * VR_DOTONEP_QOS_ENTRIES, VR_QOS_MAP_OBJECT);
  if (!req->qmr_dotonep) {
    goto alloc_failure;
  }
  req->qmr_dotonep_size = VR_DOTONEP_QOS_ENTRIES;

  req->qmr_dotonep_fc_id = vr_zalloc(sizeof(uint8_t) * VR_DOTONEP_QOS_ENTRIES, VR_QOS_MAP_OBJECT);
  if (!req->qmr_dotonep_fc_id) {
    goto alloc_failure;
  }
...
static void vr_qos_map_make_req(unsigned int qmrid, vr_qos_map_req *resp,
                                struct vr_forwarding_class *fc_p) {
  unsigned int i;
  ...
  for (i = 0; i < VR_DOTONEP_QOS_ENTRIES; i++) {
    ...
    resp->qmr_dotonep[i] = fc_p[VR_DSCP_QOS_ENTRIES + VR_MPLS_QOS_ENTRIES + i].vfc_dotonep_qos;
    resp->qmr_dotonep_fc_id[i] = fc_p[VR_DSCP_QOS_ENTRIES + VR_MPLS_QOS_ENTRIES + i].vfc_id;
  }

  return;
}

static void vr_qos_map_free_fc_cb(struct vrouter *router, void *data) {
...
static int vr_qos_map_free_fc_defer(struct vrouter *router, struct vr_forwarding_class *fc_p) {
...
static void vr_qos_map_delete(vr_qos_map_req *req) {
  int ret = 0;
...
static void vr_qos_map_dump(vr_qos_map_req *req) {
  int ret = 0;
  unsigned int i;
...
static void vr_qos_map_get(vr_qos_map_req *req) {
  int ret = 0;
...
static void vr_qos_map_add(vr_qos_map_req *req) {
  int ret = 0;
  bool need_set = false;
  unsigned int size, i;
  ...
  for (i = 0; i < req->qmr_dotonep_size; i++) {
    ...
    fc_e = &fc_p[VR_DSCP_QOS_ENTRIES + VR_MPLS_QOS_ENTRIES + req->qmr_dotonep[i]];
...
struct vr_forwarding_class *vr_fc_map_get_fc(struct vrouter *router, unsigned int i) {
...
static void vr_fc_map_req_destroy(vr_fc_map_req *req) {
  if (!req)
    return;
...
static vr_fc_map_req *vr_fc_map_req_get(unsigned int entries) {
  vr_fc_map_req *req;
...
static void vr_fc_map_delete(vr_fc_map_req *req) {
  int ret = 0;
...
static void vr_fc_map_dump(vr_fc_map_req *req) {
  int ret = 0;
  unsigned int i;
...
static void vr_fc_map_get(vr_fc_map_req *req) {
  int ret = 0;
...
static void vr_fc_map_add(vr_fc_map_req *req) {
  int ret = 0;
  unsigned int i;
  ...
  if (!req->fmr_id || !req->fmr_id_size || !req->fmr_dscp || !req->fmr_dscp_size ||
      !req->fmr_mpls_qos || !req->fmr_mpls_qos_size || !req->fmr_dotonep ||
      !req->fmr_dotonep_size || !req->fmr_queue_id || !req->fmr_queue_id_size) {
    ret = -EINVAL;
    goto generate_response;
  }
...
struct vr_forwarding_class_qos *vr_qos_get_forwarding_class(struct vrouter *router,
