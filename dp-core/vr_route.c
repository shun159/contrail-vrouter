...
static void inet_rtb_family_deinit(struct rtable_fspec *fs, struct vrouter *router,
                                   bool soft_reset);
static int bridge_rtb_family_init(struct rtable_fspec *fs, struct vrouter *router);
static void bridge_rtb_family_deinit(struct rtable_fspec *fs, struct vrouter *router,
                                     bool soft_reset);
...
static struct rtable_fspec *vr_get_family(unsigned int family) {
  switch (family) {
  case AF_INET:
...
int vr_route_delete(vr_route_req *req) {
  ...
  if (!fs)
    ...
    if (req->rtr_family == AF_BRIDGE && (!req->rtr_mac_size || !req->rtr_mac)) {
      ret = -EINVAL;
      goto error;
    }
...
int vr_route_add(vr_route_req *req) {
  struct rtable_fspec *fs;
  struct vr_route_req vr_req;
  int ret;
  uint32_t rt_prefix[4];
...
int vr_route_get(vr_route_req *req) {
  int ret = 0;
  uint32_t rt_prefix[4];
...
int vr_route_dump(vr_route_req *req) {
  struct vr_route_req vr_req;
  struct vrouter *router;
  struct vr_rtable *rtable = NULL;
  int ret;
...
static void vr_inet_vrf_stats_dump(struct vrouter *router, vr_vrf_stats_req *req) {
  int ret = 0;
...
static void vr_inet_vrf_stats_get(struct vrouter *router, vr_vrf_stats_req *req) {
  int ret = 0;
  struct vr_rtable *rtable;
  ...
  if (req->vsr_vrf >= 0 && (unsigned int)req->vsr_vrf >= rtable->algo_max_vrfs) {
    ret = -EINVAL;
    goto generate_error;
  }
...
static void vr_inet_vrf_stats_op(struct vrouter *router, vr_vrf_stats_req *req) {
...
static void vr_vrf_stats_op(vr_vrf_stats_req *req) {
  int ret = 0;
  struct vrouter *router;
...
int inet_route_add(struct rtable_fspec *fs, struct vr_route_req *req) {
  ...
  /* V4 and V6 only */
  if (req->rtr_req.rtr_family != AF_INET && req->rtr_req.rtr_family != AF_INET6)
    return -EINVAL;
  ...
  rtable = router->vr_inet_rtable;
  if (!rtable || ((unsigned int)req->rtr_req.rtr_vrf_id >= fs->rtb_max_vrfs) ||
  ...
  if (req->rtr_req.rtr_prefix) {
    ...
    if (pmask) {
      pmask = ~((1 << (8 - pmask)) - 1);
      req->rtr_req.rtr_prefix[pmask_byte] = req->rtr_req.rtr_prefix[pmask_byte] & pmask;
      pmask_byte++;
    }
    for (; pmask_byte < RT_IP_ADDR_SIZE(req->rtr_req.rtr_family); pmask_byte++) {
...
int inet_route_del(struct rtable_fspec *fs, struct vr_route_req *req) {
  struct vr_rtable *rtable;
  struct vrouter *router;
...
static int inet_rtb_family_init(struct rtable_fspec *fs, struct vrouter *router) {
  ...
  if (!router->vr_inet_rtable) {
    router->vr_inet_rtable = vr_zalloc(sizeof(struct vr_rtable), VR_ROUTE_TABLE_OBJECT);
    if (!router->vr_inet_rtable)
...
int bridge_entry_add(struct rtable_fspec *fs, struct vr_route_req *req) {
  struct vrouter *router;
  ...
  if (!router->vr_bridge_rtable || ((unsigned int)req->rtr_req.rtr_vrf_id >= fs->rtb_max_vrfs) ||
...
int bridge_entry_del(struct rtable_fspec *fs, struct vr_route_req *req) {
  struct vrouter *router;
...
int vr_fib_mem(struct vrouter *router) {
  int i = 0, ret, size;
  struct rtable_fspec *fs;
...
int vr_fib_init(struct vrouter *router) { return 0; }
