...
unsigned int vr_bitmap_used_bits(vr_bmap_t b) {
...
int vr_bitmap_alloc_bit(vr_bmap_t b) {
  int i, j, stride_num;
  ...
  for (i = 0; i < bmap->bmap_size; i++, stride_num++) {
    ...
    for (j = 0; ((bmap->bmap_data[stride_num] != 0xFFFFFFFF) && (j < VR_BITMAP_STRIDE_LEN)); j++) {
      ...
      free_bit -= 1;
      if (vr_sync_bool_compare_and_swap_32u(&bmap->bmap_data[stride_num], data,
                                            (data | (1 << free_bit)))) {
...
bool vr_bitmap_is_set_bit(vr_bmap_t b, unsigned int bit) {
...
bool vr_bitmap_clear_bit(vr_bmap_t b, unsigned int bit) {
  struct vr_bitmap *bmap = (struct vr_bitmap *)b;
...
void vr_bitmap_delete(vr_bmap_t b) {
  struct vr_bitmap *bmap = (struct vr_bitmap *)b;
...
vr_bmap_t vr_bitmap_create(unsigned int nbits) {
