...
static struct vr_nexthop *vrouter_get_label(unsigned int rid, unsigned int label) {
  struct vrouter *router = vrouter_get(rid);

  return __vrouter_get_label(router, label);
}

static int __vrouter_set_label(struct vrouter *router, unsigned int label, struct vr_nexthop *nh) {
...
int __vr_mpls_del(struct vrouter *router, unsigned int label) {
  struct vr_nexthop *nh;

  /* hardware packet filtering (Flow Director) support */
  nh = __vrouter_get_label(router, label);
  if (nh) {
    if (vrouter_host->hos_del_mpls && nh->nh_type == NH_ENCAP && !(nh->nh_flags & NH_FLAG_MCAST))
...
int vr_mpls_del(vr_mpls_req *req) {
  int ret = 0;
...
int vr_mpls_add(vr_mpls_req *req) {
  struct vrouter *router;
  struct vr_nexthop *nh;
  int ret = 0;
  ...
  /* hardware packet filtering (Flow Director) support */
  if (vrouter_host->hos_add_mpls && nh->nh_type == NH_ENCAP && !(nh->nh_flags & NH_FLAG_MCAST))
...
static void vr_mpls_make_req(vr_mpls_req *req, struct vr_nexthop *nh, unsigned int label) {
  req->mr_rid = 0;
...
int vr_mpls_dump(vr_mpls_req *r) {
  int ret = 0;
  unsigned int i;
  struct vr_nexthop *nh;
  ...
  for (i = (unsigned int)(r->mr_marker + 1); i < router->vr_max_labels; i++) {
...
int vr_mpls_get(vr_mpls_req *req) {
  int ret = 0;
...
int vr_mpls_tunnel_type(unsigned int label, unsigned int control_data, unsigned short *reason) {
  struct vr_nexthop *nh;
  ...
  if (!router) {
    res = VP_DROP_MISC;
    PKT_LOG(res, 0, 0, VR_MPLS_C, __LINE__);
    goto fail;
  }
...
int vr_mpls_input(struct vrouter *router, struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
...
int vr_mpls_init(struct vrouter *router) {
  ...
  if (!router->vr_ilm) {
    ...
    router->vr_ilm = vr_btable_alloc(router->vr_max_labels, sizeof(struct vr_nexthop *));
    if (!router->vr_ilm)
      return vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, ilm_memory);
  }
