...
struct vr_vrf_table_entry *vrouter_get_vrf_table(struct vrouter *router, unsigned int index) {
...
static void vr_vrf_table_defer_delete(struct vrouter *router, void *arg) {
...
int __vr_vrf_table_entry_del(struct vrouter *router, unsigned int index) {
  ...
  if (vrf_table_entry->vrf_flags & VRF_FLAG_HBF_L_VALID) {
    hbs_l = vrf_table_entry->hbs_l_vif;
    vrouter_put_interface(hbs_l);
    vrf_table_entry->hbs_l_vif = NULL;
  }

  if (vrf_table_entry->vrf_flags & VRF_FLAG_HBF_R_VALID) {
    hbs_r = vrf_table_entry->hbs_r_vif;
    vrouter_put_interface(hbs_r);
    vrf_table_entry->hbs_r_vif = NULL;
  }
...
int vr_vrf_table_entry_del(vr_vrf_req *req) {
  int ret = -EINVAL;
...
int vr_vrf_table_entry_add(vr_vrf_req *req) {
  int ret = 0;
  struct vrouter *router;
  struct vr_interface *hbs_l = NULL, *hbs_r = NULL;
  struct vr_interface *old_hbs_l = NULL, *old_hbs_r = NULL;
  struct vr_vrf_table_entry *vrf_table_entry;
  ...
  if (!vrf_table_entry) {
    ...
    /* save old values for future use (if any) */
    if (vrf_table_entry->vrf_flags & VRF_FLAG_HBF_L_VALID)
      old_hbs_l = vrf_table_entry->hbs_l_vif;
    if (vrf_table_entry->vrf_flags & VRF_FLAG_HBF_R_VALID)
      old_hbs_r = vrf_table_entry->hbs_r_vif;
  }

  if (req->vrf_flags & VRF_FLAG_HBF_L_VALID) {
    if (req->vrf_hbfl_vif_idx == -1) {
      /* Request to clear the field */
      vrf_table_entry->vrf_flags &= ~VRF_FLAG_HBF_L_VALID;
      if (old_hbs_l) {
        vrouter_put_interface(old_hbs_l);
        vrf_table_entry->hbs_l_vif = NULL;
      }
    } else {
      /* Add/update the field */
      hbs_l = vrouter_get_interface(req->vrf_rid, req->vrf_hbfl_vif_idx);
      if (!hbs_l) {
        ret = -EINVAL;
        goto generate_resp;
      }
      if (old_hbs_l)
        vrouter_put_interface(old_hbs_l);
      vrf_table_entry->hbs_l_vif = hbs_l;
      vrf_table_entry->vrf_flags |= VRF_FLAG_HBF_L_VALID;
    }
  }

  if (req->vrf_flags & VRF_FLAG_HBF_R_VALID) {
    if (req->vrf_hbfr_vif_idx == -1) {
      /* Request to clear the field */
      vrf_table_entry->vrf_flags &= ~VRF_FLAG_HBF_R_VALID;
      if (old_hbs_r) {
        vrouter_put_interface(old_hbs_r);
        vrf_table_entry->hbs_r_vif = NULL;
      }
    } else {
      /* Add/update the field */
      hbs_r = vrouter_get_interface(req->vrf_rid, req->vrf_hbfr_vif_idx);
      if (!hbs_r) {
        ret = -EINVAL;
        goto generate_resp;
      }
      if (old_hbs_r)
        vrouter_put_interface(old_hbs_r);
      vrf_table_entry->hbs_r_vif = hbs_r;
      vrf_table_entry->vrf_flags |= VRF_FLAG_HBF_R_VALID;
    }
  }

  vrf_table_entry->rid = req->vrf_rid;
  vrf_table_entry->vrf_flags |= VRF_FLAG_VALID;
...
static void vr_vrf_table_make_req(vr_vrf_req *req, struct vr_vrf_table_entry *vrf_table_entry,
                                  unsigned short index) {
  req->vrf_idx = index;
  if (vrf_table_entry->hbs_l_vif)
    req->vrf_hbfl_vif_idx = vrf_table_entry->hbs_l_vif->vif_idx;
  if (vrf_table_entry->hbs_r_vif)
    req->vrf_hbfr_vif_idx = vrf_table_entry->hbs_r_vif->vif_idx;
...
static int vr_vrf_table_dump(vr_vrf_req *r) {
  int ret = 0;
  unsigned int i;
  ...
  for (i = (unsigned int)(r->vrf_marker + 1); i < router->vr_max_vrfs; i++) {
...
static int vr_vrf_table_entry_get(vr_vrf_req *req) {
  int ret = 0;
  struct vrouter *router;
  struct vr_vrf_table_entry *vrf_table_entry = NULL;

  router = vrouter_get(req->vrf_rid);
  if (!router || (unsigned int)req->vrf_idx >= router->vr_max_vrfs) {
    ret = -ENODEV;
  } else {
    vrf_table_entry = router->vr_vrf_table[req->vrf_idx];
    if (!vrf_table_entry)
...
void vr_vrf_table_exit(struct vrouter *router, bool soft_reset) {
  unsigned int i;
...
int vr_vrf_table_init(struct vrouter *router) {
  unsigned int size;
