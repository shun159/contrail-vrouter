...
static inline void __fragment_key(struct vr_fragment_key *key, unsigned short vrf, uint64_t sip_u,
                                  uint64_t sip_l, uint64_t dip_u, uint64_t dip_l, uint32_t id) {
  key->fk_sip_u = sip_u;
  key->fk_sip_l = sip_l;
  key->fk_dip_u = dip_u;
  key->fk_dip_l = dip_l;
...
#define VR_FRAGMENT_FROM_HENTRY(entry)                                                             \
  (struct vr_fragment *)((entry) ? CONTAINER_OF(f_hentry, struct vr_fragment, entry) : NULL)

#define VR_HENTRY_FROM_FRAGMENT(fe) (vr_hentry_t *)((fe) ? &fe->f_hentry : NULL)

static vr_hentry_key vr_fragment_get_entry_key(vr_htable_t table, vr_hentry_t *entry,
                                               unsigned int *key_len) {
  struct vr_fragment *fe = (struct vr_fragment *)CONTAINER_OF(f_hentry, struct vr_fragment, entry);
...
static inline void fragment_entry_set(struct vr_fragment *fe, struct vr_fragment_key *key,
...
static void vr_fragment_queue_element_free(struct vr_fragment_queue_element *vfqe,
...
static void fragment_free_frag(struct vr_fragment *frag) {
...
static void fragment_unlink_frag(struct vr_fragment **prev, struct vr_fragment *frag) {
  *prev = frag->f_next;
  return;
}

unsigned int vr_assembler_table_scan(struct vr_fragment **head) {
...
int vr_assembler_table_scan_init(void (*scanner)(void *)) {
  ...
  vtimer->vt_msecs = (VR_ASSEMBLER_TIMEOUT_TIME * 1000) / VR_ASSEMBLER_BUCKET_COUNT;
...
static void vr_fragment_flush_queue_element(struct vr_fragment_queue_element *vfqe) {
...
int vr_fragment_assemble(struct vr_fragment **head_p, struct vr_fragment_queue_element *vfqe) {
  int ret = 0;
  uint64_t sec, nsec;
  ...
  if (vr_ip_is_ip6(ip)) {
    ...
    __fragment_key(&vfk, pnode->pl_vrf, *v6_addr, *(v6_addr + 1), *(v6_addr + 2), *(v6_addr + 3),
                   v6_frag->ip6_frag_id);
  } else {
    __fragment_key(&vfk, pnode->pl_vrf, 0, pnode->pl_inner_src_ip, 0, pnode->pl_inner_dst_ip,
                   ip->ip_id);
  }
...
void vr_fragment_assemble_queue(struct vr_fragment_queue *vfq) {
  struct vr_packet_node *pnode;
...
uint32_t __vr_fragment_get_hash(unsigned int vrf, uint64_t sip_u, uint64_t sip_l, uint64_t dip_u,
                                uint64_t dip_l, uint32_t id) {
  struct vr_fragment_key vfk;
...
uint32_t vr_fragment_get_hash(struct vr_packet_node *pnode) {
  uint64_t *v6_addr;
  struct vr_ip *ip;
  struct vr_ip6 *ip6;
  struct vr_ip6_frag *v6_frag;
  ...
  if (vr_ip_is_ip6(ip)) {
    ...
    return __vr_fragment_get_hash(pnode->pl_vrf, *v6_addr, *(v6_addr + 1), *(v6_addr + 2),
                                  *(v6_addr + 3), v6_frag->ip6_frag_id);
  } else if (vr_ip_is_ip4(ip)) {
    return __vr_fragment_get_hash(pnode->pl_vrf, 0, pnode->pl_inner_src_ip, 0,
...
int vr_fragment_enqueue(struct vrouter *router, struct vr_fragment_queue *vfq,
  ...
  if (vrouter_host->hos_is_frag_limit_exceeded && vrouter_host->hos_is_frag_limit_exceeded()) {
    goto fail;
  }
...
void vr_fragment_del(vr_htable_t table, struct vr_fragment *fe) {
...
static int vr_fragment_add(struct vrouter *router, struct vr_fragment_key *key,
...
int vr_v4_fragment_add(struct vrouter *router, unsigned short vrf, struct vr_ip *iph,
                       unsigned short sport, unsigned short dport) {
  struct vr_fragment_key key;

  __fragment_key(&key, vrf, 0, iph->ip_saddr, 0, iph->ip_daddr, iph->ip_id);

  return vr_fragment_add(router, &key, sport, dport, (ntohs(iph->ip_len) - iph->ip_hl * 4));
}

int vr_v6_fragment_add(struct vrouter *router, unsigned short vrf, struct vr_ip6 *ip6,
                       unsigned short sport, unsigned short dport) {
  uint64_t *v6_addr;
  struct vr_fragment_key key;
  struct vr_ip6_frag *v6_frag;
  ...
  v6_addr = (uint64_t *)(ip6->ip6_src);
  v6_frag = (struct vr_ip6_frag *)(ip6 + 1);
  __fragment_key(&key, vrf, *v6_addr, *(v6_addr + 1), *(v6_addr + 2), *(v6_addr + 3),
                 v6_frag->ip6_frag_id);
...
struct vr_fragment *vr_fragment_get(struct vrouter *router, unsigned short vrf, struct vr_ip *ip) {
  uint64_t sec, nsec;
  uint64_t *v6_addr;
  struct vr_ip6 *ip6;
  struct vr_ip6_frag *v6_frag;
...
static void __fragment_reap(vr_htable_t table, vr_hentry_t *ent, unsigned int index, void *data) {
  uint64_t sec, nsec;
  struct vr_fragment *fe;
...
static int fragment_reap(vr_htable_t htable, int start) {
  return vr_htable_trav_range(htable, start, ENTRIES_PER_SCAN, __fragment_reap, NULL);
}

static void fragment_table_scanner(void *arg) {
  int ret;
...
static struct vr_timer *fragment_table_scanner_init(struct vrouter *router) {
  struct vr_timer *vtimer;
...
static void vr_fragment_table_scanner_exit(struct vrouter *router) {
  if (router->vr_fragment_table_scanner) {
    vr_delete_timer(router->vr_fragment_table_scanner);
    vr_free(router->vr_fragment_table_scanner->vt_vr_arg, VR_FRAGMENT_SCANNER_OBJECT);
...
static int vr_fragment_table_scanner_init(struct vrouter *router) {
  if (!router->vr_fragment_table_scanner) {
    router->vr_fragment_table_scanner = fragment_table_scanner_init(router);
    if (!router->vr_fragment_table_scanner) {
      return -ENOMEM;
    }
  }
...
int vr_fragment_table_init(struct vrouter *router) {
  ...
  if (!router->vr_fragment_table) {
    router->vr_fragment_table = vr_htable_create(
        router, FRAG_TABLE_ENTRIES, FRAG_OTABLE_ENTRIES, sizeof(struct vr_fragment),
        sizeof(struct vr_fragment_key), FRAG_TABLE_BUCKETS, vr_fragment_get_entry_key);
    if (!router->vr_fragment_table) {
