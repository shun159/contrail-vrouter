...
extern bool vr_has_to_fragment(struct vr_interface *, struct vr_packet *, unsigned int);
...
extern struct vr_nexthop *vr_inet_src_lookup(unsigned short, struct vr_packet *);
...
struct vr_nexthop *__vrouter_get_nexthop(struct vrouter *router, unsigned int index) {
...
struct vr_nexthop *vrouter_get_nexthop(unsigned int rid, unsigned int index) {
...
static int __vrouter_set_nexthop(struct vrouter *router, unsigned int index,
                                 struct vr_nexthop *nh) {
...
static bool vr_l2_control_data_add(struct vr_packet **pkt) {

  unsigned int *data;
  struct vr_packet *tmp_pkt;

  if (pkt_head_space(*pkt) < VR_L2_CTRL_DATA_LEN) {
    tmp_pkt = vr_pexpand_head(*pkt, VR_L2_CTRL_DATA_LEN - pkt_head_space(*pkt));
    if (!tmp_pkt)
      return false;
    *pkt = tmp_pkt;
  }
...
static void vrouter_free_nexthop(struct vr_nexthop *nh) {
  ...
  } else if ((nh->nh_type == NH_TUNNEL) && (nh->nh_flags & NH_FLAG_TUNNEL_UDP) &&
...
static void vrouter_free_nexthop_cb(struct vrouter *router, void *data) {
...
static int vrouter_free_nexthop_defer(struct vr_nexthop *nh) {
...
void vrouter_put_nexthop(struct vr_nexthop *nh) {
  int i, component_cnt;
...
static int vrouter_add_nexthop(struct vr_nexthop *nh) {
...
static void nh_del(struct vr_nexthop *nh) {
  struct vrouter *router = vrouter_get(nh->nh_rid);
...
bool vr_gateway_nexthop(struct vr_nexthop *nh) {
  if (nh) {
    if (!nh->nh_dev)
      return false;

    if ((nh->nh_type == NH_ENCAP) && (nh->nh_dev->vif_type == VIF_TYPE_AGENT))
      return true;
  }
...
bool vr_hosted_nexthop(struct vr_nexthop *nh) {
  if (nh) {
    if (!nh->nh_dev)
      return false;
...
static int nh_tunnel_loop_detect_handle(struct vr_packet *pkt, struct vr_nexthop *nh,
...
static nh_processing_t nh_resolve(struct vr_packet *pkt, struct vr_nexthop *nh,
...
static nh_processing_t nh_vrf_translate(struct vr_packet *pkt, struct vr_nexthop *nh,
                                        struct vr_forwarding_md *fmd) {
  struct vr_vrf_stats *stats;
...
static nh_processing_t nh_l2_rcv(struct vr_packet *pkt, struct vr_nexthop *nh,
                                 struct vr_forwarding_md *fmd) {
...
static nh_processing_t nh_l3_rcv(struct vr_packet *pkt, struct vr_nexthop *nh,
                                 struct vr_forwarding_md *fmd) {
  struct vr_vrf_stats *stats;
...
static int nh_push_mpls_header(struct vr_packet *pkt, unsigned int label,
  ...
  *lbl = htonl((label << VR_MPLS_LABEL_SHIFT) | exp_qos | (label_pos << 8) | ttl);
 ...
 */
static bool nh_udp_tunnel_helper(struct vr_packet *pkt, unsigned short sport, unsigned short dport,
...
static bool nh_udp_tunnel6_helper(struct vr_packet *pkt, struct vr_nexthop *nh, uint8_t *sip,
...
static bool nh_pbb_tunnel_helper(struct vrouter *router, struct vr_packet **pkt,
...
static bool nh_vxlan_tunnel_helper(struct vrouter *router, struct vr_packet **pkt,
  ...
  return nh_udp_tunnel_helper(*pkt, htons(udp_src_port), htons(VR_VXLAN_UDP_DST_PORT), sip, dip,
                              qos);
}

static struct vr_packet *nh_mcast_clone(struct vr_packet *pkt, unsigned short head_room) {
...
static int nh_composite_ecmp_validate_src(struct vr_packet *pkt, struct vr_nexthop *nh,
  ...
  if ((fmd->fmd_ecmp_src_nh_index >= 0) && (fmd->fmd_ecmp_src_nh_index < nh->nh_component_cnt)) {
  ...
  if (!cnh || (!cnh->nh_validate_src) ||
    ...
    for (i = 0; i < nh->nh_component_cnt; i++) {
      ...
      if (!cnh || !(cnh->nh_flags & NH_FLAG_VALID) || !cnh->nh_validate_src)
        continue;
 ...
 */
static void nh_ecmp_store_ecmp_config_hash(vr_nexthop_req *req, struct vr_nexthop *nh) {
  unsigned char hash;
  unsigned short flow_hash;
  int i;

  if (!nh || (nh->nh_type != NH_COMPOSITE) || (!(nh->nh_flags & NH_FLAG_COMPOSITE_ECMP))) {
    return;
  }
 ...
 */
static int nh_composite_ecmp_select_by_rflow(struct vr_packet *pkt, struct vr_nexthop *nh,
                                             struct vr_forwarding_md *fmd,
...
static int nh_composite_ecmp_select_nh(struct vr_packet *pkt, struct vr_nexthop *nh,
                                       struct vr_forwarding_md *fmd) {
    ...
    if (fe) {
      ...
      if (fmd->fmd_ecmp_nh_index == -1) {
        ...
        if (rflow_src_info != (unsigned int)-1) {
          ecmp_index = nh_composite_ecmp_select_by_rflow(pkt, nh, fmd, rflow_src_info);
        }
      }
    }
  }
  ...
  if (!fe) {
    ...
    if (pkt->vp_type == VP_TYPE_IP) {
      ...
      ret = vr_inet6_get_flow_key(nh->nh_router, fmd->fmd_dvrf, pkt, fmd->fmd_vlan, flowp, hash);
      if (ret < 0)
        return ret;
    } else {
      /*
...
static nh_processing_t nh_composite_ecmp(struct vr_packet *pkt, struct vr_nexthop *nh,
                                         struct vr_forwarding_md *fmd) {
  ...
  if ((fmd->fmd_ecmp_nh_index >= 0) && (fmd->fmd_ecmp_nh_index < nh->nh_component_cnt)) {
...
static int nh_composite_mcast_validate_src(struct vr_packet *pkt, struct vr_nexthop *nh,
  ...
  for (j = 0; j < nh->nh_component_cnt; j++) {
    ...
    if (fabric_nh->nh_type != NH_COMPOSITE ||
        !(fabric_nh->nh_flags &
          (NH_FLAG_COMPOSITE_FABRIC | NH_FLAG_COMPOSITE_EVPN | NH_FLAG_COMPOSITE_TOR)))
      continue;
    ...
    for (i = 0; i < fabric_nh->nh_component_cnt; i++) {
      ...
      if (tun_dip && fmd->fmd_outer_src_ip && fmd->fmd_outer_src_ip == tun_dip) {
...
static int nh_handle_unknown_unicast(struct vr_packet *pkt, struct vr_eth *eth,
...
static int nh_handle_mcast_control_pkt(struct vr_packet *pkt, struct vr_eth *eth,
                                       struct vr_forwarding_md *fmd, unsigned int pkt_src,
                                       bool *flood_to_vms) {
  int handled = 1;
  ...
  if (pkt->vp_type == VP_TYPE_ARP) {
    ...
    if ((pkt_src == PKT_SRC_INGRESS_REPL_TREE) && (rt_flags & VR_BE_EVPN_CONTROL_PROCESSING_FLAG)) {
  ...
  } else if (pkt_src == (PKT_SRC_TOR_REPL_TREE) || (pkt_src == PKT_SRC_INGRESS_REPL_TREE)) {
   ...
   */
  if ((!pkt_src) || (pkt_src == PKT_SRC_TOR_REPL_TREE) ||
      ((pkt_src == PKT_SRC_INGRESS_REPL_TREE) && (rt_flags & VR_BE_EVPN_CONTROL_PROCESSING_FLAG))) {
    ...
    if (l4_type != L4_TYPE_UNKNOWN) {
      ...
      if (trap) {
        if (flood) {
          vr_trap(pkt_c, fmd->fmd_dvrf, AGENT_TRAP_L3_PROTOCOLS, NULL);
          goto unhandled;
        } else {
...
static nh_processing_t nh_composite_mcast(struct vr_packet *pkt, struct vr_nexthop *nh,
                                          struct vr_forwarding_md *fmd) {

  int i, clone_size;
  bool flood_to_vms = true, l2_control_data = false;
  unsigned short drop_reason, label, pkt_vrf, pull_len = 0, pbb_pull_len = 0;
  ...
  if (nh->nh_family == AF_BRIDGE) {
    eth = (struct vr_eth *)pkt_data_at_offset(pkt, pkt->vp_data + pull_len);
  ...
  for (i = 0; i < nh->nh_component_cnt; i++) {
    ...
    /* If direct nexthop is not valid, dont process it */
    if ((!dir_nh) || !(dir_nh->nh_flags & NH_FLAG_VALID) || (dir_nh->nh_type != NH_COMPOSITE))
      continue;
    ...
    if (dir_nh->nh_flags & NH_FLAG_COMPOSITE_ENCAP) {
      ...
      if (nh->nh_family == AF_BRIDGE && (pkt_src == PKT_SRC_EDGE_REPL_TREE)) {
        if (pull_header) {
          pull_len += VR_VXLAN_HDR_LEN;
        }
      }
...
static nh_processing_t nh_composite_encap(struct vr_packet *pkt, struct vr_nexthop *nh,
                                          struct vr_forwarding_md *fmd) {
  int i;
  struct vr_vrf_stats *stats;
  struct vr_nexthop *dir_nh;
...
static nh_processing_t nh_composite_tor(struct vr_packet *pkt, struct vr_nexthop *nh,
                                        struct vr_forwarding_md *fmd) {
  int i;
  struct vr_vrf_stats *stats;
  struct vr_nexthop *dir_nh;
  ...
  for (i = 0; i < nh->nh_component_cnt; i++) {
    ...
    if (fmd->fmd_outer_src_ip && fmd->fmd_outer_src_ip == dir_nh->nh_udp_tun_dip)
      continue;
    ...
    vr_fmd_set_label(fmd, nh->nh_component_nh[i].cnh_label, VR_LABEL_TYPE_UNKNOWN);
    fmd->fmd_dvrf = dir_nh->nh_dev->vif_vrf;
    nh_output(new_pkt, dir_nh, fmd);
  }
...
static nh_processing_t nh_composite_evpn(struct vr_packet *pkt, struct vr_nexthop *nh,
                                         struct vr_forwarding_md *fmd) {
  int i;
  ...
  for (i = 0; i < nh->nh_component_cnt; i++) {
    ...
    if (nh->nh_flags & NH_FLAG_TUNNEL_PBB) {
      if (!nh_pbb_tunnel_helper(nh->nh_router, &new_pkt, fmd, eth_mac, pkt->vp_if->vif_pbb_mac,
                                pkt->vp_if->vif_isid)) {
...
static nh_processing_t nh_composite_fabric(struct vr_packet *pkt, struct vr_nexthop *nh,
                                           struct vr_forwarding_md *fmd) {
  int i;
  int32_t label;
  ...
  for (i = 0; i < nh->nh_component_cnt; i++) {
    ...
    if (vif_is_vlan(pkt->vp_if)) {
      if (vif_is_vlan(dir_nh->nh_dev) && pkt->vp_if->vif_parent == dir_nh->nh_dev->vif_parent)
        continue;
    ...
    if (vif_is_virtual(new_pkt->vp_if) || (fmd->fmd_src == TOR_SOURCE)) {
      /*
      ...
      if (nh->nh_family == AF_BRIDGE) {
        ...
        vr_fmd_set_label(fmd, label, VR_LABEL_TYPE_UNKNOWN);
        fmd->fmd_dvrf = dir_nh->nh_dev->vif_vrf;
        if (nh_vxlan_tunnel_helper(nh->nh_router, &new_pkt, fmd, sip, sip) == false) {
          PKT_LOG(VP_DROP_PUSH, pkt, 0, VR_NEXTHOP_C, __LINE__);
          vr_pfree(new_pkt, VP_DROP_PUSH);
          break;
        }
...
static nh_processing_t nh_discard(struct vr_packet *pkt, struct vr_nexthop *nh,
                                  struct vr_forwarding_md *fmd) {
  struct vr_vrf_stats *stats;
...
static uint8_t *nh_generate_mirroring_sip(struct vr_nexthop *nh, struct vr_packet *pkt,
                                          struct vr_forwarding_md *fmd) {
  uint16_t intf_id;
  mirror_type_t mtype;
...
static nh_processing_t nh_udp_tunnel(struct vr_packet *pkt, struct vr_nexthop *nh,
                                     struct vr_forwarding_md *fmd) {
  int ret = -1;
  uint8_t *vif_ip = NULL;
  uint16_t sport = 0;
  ...
  if (pkt->vp_type == VP_TYPE_IP) {
    ret = vr_inet_get_flow_key(nh->nh_router, pkt, fmd, flowp, VR_FLOW_KEY_ALL);
  } else if (pkt->vp_type == VP_TYPE_IP6) {
    ret = vr_inet6_get_flow_key(nh->nh_router, fmd->fmd_dvrf, pkt, fmd->fmd_vlan, flowp,
                                VR_FLOW_KEY_ALL);
  }
  ...
  if (!ret) {
    ...
    sport = (uint16_t)(((uint64_t)hash * port_range) >> 32);
  ...
  if (nh->nh_family == AF_INET) {
    ...
    if (nh_udp_tunnel_helper(pkt, htons(sport), nh->nh_udp_tun_dport, sip, nh->nh_udp_tun_dip,
                             qos) == false) {
      goto send_fail;
    }
    ...
    if (nh_udp_tunnel6_helper(pkt, nh, vif_ip, htons(sport), nh->nh_udp_tun6_dport) == false) {
      goto send_fail;
    }
 ...
 */
static nh_processing_t nh_vxlan_tunnel(struct vr_packet *pkt, struct vr_nexthop *nh,
                                       struct vr_forwarding_md *fmd) {
  ...
  if ((pkt->vp_type == VP_TYPE_IP) || (pkt->vp_type == VP_TYPE_IP6)) {
    if (vr_has_to_fragment(nh->nh_dev, pkt, overhead_len) && vr_ip_dont_fragment_set(pkt)) {
      ...
      trap_arg.df_mtu =
          vif_get_mtu(nh->nh_dev) - (overhead_len + pkt_get_network_header_off(pkt) - pkt->vp_data);
  ...
  if (vif->vif_set_rewrite(vif, &pkt, fmd, nh->nh_data, nh->nh_udp_tun_encap_len) < 0) {
    goto send_fail;
  }
...
static nh_processing_t nh_pbb_tunnel(struct vr_packet *pkt, struct vr_nexthop *nh,
                                     struct vr_forwarding_md *fmd) {
  struct vr_vrf_stats *stats;

  if (vr_fmd_etree_is_enabled(fmd)) {
    if ((!vr_fmd_etree_is_root(fmd)) && (!(nh->nh_flags & NH_FLAG_ETREE_ROOT))) {
  ...
  if (!nh_pbb_tunnel_helper(nh->nh_router, &pkt, fmd, nh->nh_pbb_mac, pkt->vp_if->vif_pbb_mac,
                            pkt->vp_if->vif_isid)) {
    PKT_LOG(VP_DROP_PUSH, pkt, 0, VR_NEXTHOP_C, __LINE__);
    vr_pfree(pkt, VP_DROP_PUSH);
    return NH_PROCESSING_COMPLETE;
...
static int nh_pbb_tunnel_validate_src(struct vr_packet *pkt, struct vr_nexthop *nh,
                                      struct vr_forwarding_md *fmd, void *ret_data) {
  bool root = false;
...
static int nh_mpls_udp_tunnel_validate_src(struct vr_packet *pkt, struct vr_nexthop *nh,
                                           struct vr_forwarding_md *fmd, void *ret_data) {
...
static int nh_vxlan_tunnel_validate_src(struct vr_packet *pkt, struct vr_nexthop *nh,
                                        struct vr_forwarding_md *fmd, void *ret_data) {
  if (fmd->fmd_outer_src_ip == nh->nh_udp_tun_dip)
    return NH_SOURCE_VALID;
 ...
 */
static nh_processing_t nh_mpls_udp_tunnel(struct vr_packet *pkt, struct vr_nexthop *nh,
                                          struct vr_forwarding_md *fmd) {
  ...
  mudp_head_space = (label_count * VR_MPLS_HDR_LEN) + sizeof(struct vr_ip) + sizeof(struct vr_udp);
  if (vr_fmd_l2_control_data_is_enabled(fmd))
  ...
  if (nh_udp_tunnel_helper(pkt, htons(udp_src_port), htons(VR_MPLS_OVER_UDP_DST_PORT), tun_sip,
                           tun_dip, qos) == false) {
    goto send_fail;
  }
  ...
  tun_encap_rewrite = vif->vif_set_rewrite(vif, &pkt, fmd, nh->nh_data, tun_encap_len);
...
static int nh_gre_tunnel_validate_src(struct vr_packet *pkt, struct vr_nexthop *nh,
                                      struct vr_forwarding_md *fmd, void *ret_data) {
...
static nh_processing_t nh_gre_tunnel(struct vr_packet *pkt, struct vr_nexthop *nh,
                                     struct vr_forwarding_md *fmd) {
  ...
  gre_head_space = (label_count * VR_MPLS_HDR_LEN) + sizeof(struct vr_ip) + sizeof(struct vr_gre);
  if (vr_fmd_l2_control_data_is_enabled(fmd))
  ...
  tun_encap_rewrite = vif->vif_set_rewrite(vif, &pkt, fmd, nh->nh_data, nh->nh_gre_tun_encap_len);
  if (tun_encap_rewrite < 0) {
    drop_reason = VP_DROP_PUSH;
    PKT_LOG(drop_reason, pkt, 0, VR_NEXTHOP_C, __LINE__);
    goto send_fail;
 ...
 */
int nh_output(struct vr_packet *pkt, struct vr_nexthop *nh, struct vr_forwarding_md *fmd) {
  ...
  if ((pkt->vp_type == VP_TYPE_IP) || (pkt->vp_type == VP_TYPE_IP6)) {
    ...
    if (!(pkt->vp_flags & VP_FLAG_FLOW_SET)) {
      if (nh->nh_flags & (NH_FLAG_POLICY_ENABLED | NH_FLAG_FLOW_LOOKUP)) {
...
static nh_processing_t nh_encap_l2(struct vr_packet *pkt, struct vr_nexthop *nh,
                                   struct vr_forwarding_md *fmd) {
  ...
  if ((pkt->vp_flags & VP_FLAG_MULTICAST) || (fmd->fmd_vlan != VLAN_ID_INVALID)) {
  ...
  if (!IS_MAC_ZERO(fmd->fmd_dmac) && !IS_MAC_ZERO(vif->vif_pbb_mac)) {
  ...
  if (vif_is_fabric(vif)) {
    if (!(pkt->vp_flags & VP_FLAG_GROED)) {
      ...
      if (qos) {
        if (pkt->vp_type == VP_TYPE_IP) {
          vr_inet_set_tos((struct vr_ip *)pkt_network_header(pkt), VR_IP_DSCP(qos->vfcq_dscp));
        } else if (pkt->vp_type == VP_TYPE_IP6) {
          vr_inet6_set_tos((struct vr_ip6 *)pkt_network_header(pkt), qos->vfcq_dscp);
        }
        pkt->vp_queue = qos->vfcq_queue_id;
        pkt->vp_priority = qos->vfcq_dotonep_qos;
      }
  ...
  if (vr_pkt_is_gro(pkt) && vif_is_virtual(vif) && (!(vif->vif_flags & VIF_FLAG_MIRROR_TX))) {
...
static int nh_encap_validate_src(struct vr_packet *pkt, struct vr_nexthop *nh,
                                 struct vr_forwarding_md *fmd, void *ret_data) {
  bool root = false;
...
static nh_processing_t nh_encap_l3_mcast(struct vr_packet *pkt, struct vr_nexthop *nh,
                                         struct vr_forwarding_md *fmd) {
  uint8_t *ptr;
...
static nh_processing_t nh_encap_l3(struct vr_packet *pkt, struct vr_nexthop *nh,
                                   struct vr_forwarding_md *fmd) {
  int rewrite_len;
...
static int vr_nexthop_delete(vr_nexthop_req *req) {
  struct vr_nexthop *nh;
  int ret = 0;
...
static int nh_resolve_add(struct vr_nexthop *nh, vr_nexthop_req *req) {
  nh->nh_reach_nh = nh_resolve;
  return 0;
}

static int nh_l2_rcv_add(struct vr_nexthop *nh, vr_nexthop_req *req) {
  nh->nh_reach_nh = nh_l2_rcv;
  return 0;
}

static int nh_rcv_add(struct vr_nexthop *nh, vr_nexthop_req *req) {
  int ret = 0;
...
static int nh_vrf_translate_add(struct vr_nexthop *nh, vr_nexthop_req *req) {
  nh->nh_reach_nh = nh_vrf_translate;
  return 0;
}

static int nh_composite_mcast_validate(struct vr_component_nh *component_nh, vr_nexthop_req *req) {
  unsigned int i;
  ...
  if (req->nhr_flags & NH_FLAG_MCAST) {
    ...
    for (i = 0; i < req->nhr_nh_list_size; i++) {
      ...
      if (!(tmp_nh->nh_flags & (NH_FLAG_COMPOSITE_FABRIC | NH_FLAG_COMPOSITE_EVPN |
...
static int nh_composite_add(struct vr_nexthop *nh, vr_nexthop_req *req) {
  int ret = 0;
  ...
  if (req->nhr_nh_list_size) {
    component_nh = vr_zalloc(req->nhr_nh_list_size * sizeof(struct vr_component_nh),
    ...
    for (i = 0; i < req->nhr_nh_list_size; i++) {
      component_nh[i].cnh = vrouter_get_nexthop(req->nhr_rid, req->nhr_nh_list[i]);
    ...
    if (req->nhr_flags & NH_FLAG_COMPOSITE_ECMP) {
      if (active) {
        component_ecmp =
            vr_zalloc(active * sizeof(struct vr_component_nh), VR_NEXTHOP_COMPONENT_OBJECT);
        if (!component_ecmp) {
          ret = -ENOMEM;
          goto exit_add;
        }
      ...
      for (i = 0; i < req->nhr_nh_list_size; i++) {
        if (component_nh[i].cnh) {
          memcpy(&component_ecmp[j++], &component_nh[i], sizeof(struct vr_component_nh));
...
static inline void nh_tunnel_set_reach_nh(struct vr_nexthop *nh) {
  bool dev = false;
...
static int nh_tunnel_add(struct vr_nexthop *nh, vr_nexthop_req *req) {
  int ret = 0;
  ...
  if (nh->nh_flags & NH_FLAG_TUNNEL_GRE) {
    ...
    if (req->nhr_family == AF_INET) {
      ...
      if (!nh->nh_udp_tun6_sip) {
        nh->nh_udp_tun6_sip = vr_malloc(VR_IP6_ADDRESS_LEN, VR_NETWORK_ADDRESS_OBJECT);
        if (!nh->nh_udp_tun6_sip) {
          ret = -ENOMEM;
          goto exit_error;
        }
      ...
      if (!nh->nh_udp_tun6_dip) {
        nh->nh_udp_tun6_dip = vr_malloc(VR_IP6_ADDRESS_LEN, VR_NETWORK_ADDRESS_OBJECT);
        if (!nh->nh_udp_tun6_dip) {
          ret = -ENOMEM;
          goto exit_error;
        }
    ...
    nh->nh_dev = vif;
    if (nh->nh_flags & NH_FLAG_L3_VXLAN) {
      if ((req->nhr_rw_dst_mac_size != VR_ETHER_ALEN) || (IS_MAC_ZERO(req->nhr_rw_dst_mac))) {
        ret = -EINVAL;
        goto exit_add;
      }
    ...
    if ((req->nhr_pbb_mac_size != VR_ETHER_ALEN) || (IS_MAC_ZERO(req->nhr_pbb_mac))) {
      ret = -EINVAL;
      goto exit_add;
    }
...
static int nh_indirect_add(struct vr_nexthop *nh, vr_nexthop_req *req) {
   ...
   * indirect nexthop
   */
  if ((nh->nh_type != NH_TUNNEL) || (!(nh->nh_flags & NH_FLAG_TUNNEL_PBB)))
    return -EINVAL;
...
static int nh_encap_add(struct vr_nexthop *nh, vr_nexthop_req *req) {
  int ret = 0;
  struct vr_interface *vif, *old_vif;
...
static int nh_discard_add(struct vr_nexthop *nh, vr_nexthop_req *req) {
...
static bool vr_nexthop_valid_request(vr_nexthop_req *req) {
...
static int vr_nexthop_size(vr_nexthop_req *req) {
...
static bool vr_nexthop_valid_change(vr_nexthop_req *req, struct vr_nexthop *nh) {
  ...
  if (req->nhr_encap_size && req->nhr_encap_size != nh->nh_data_size)
    return false;

  /* Indirect to non-indirect and other way is not allowed */
  if ((req->nhr_flags & NH_FLAG_INDIRECT) ^ (nh->nh_flags & NH_FLAG_INDIRECT))
    return false;

  return true;
}

int vr_nexthop_add(vr_nexthop_req *req) {
  int ret = 0, len = 0;
  ...
  if (!nh) {
    ...
    if ((req->nhr_flags & NH_FLAG_VALID) && !(nh->nh_flags & NH_FLAG_VALID))
...
unsigned int vr_nexthop_req_get_size(void *req_p) {
  ...
  if ((req->nhr_type == NH_TUNNEL) && (req->nhr_flags & NH_FLAG_TUNNEL_UDP) &&
...
static int vr_nexthop_make_req(vr_nexthop_req *req, struct vr_nexthop *nh) {
  unsigned int i;
  ...
  if ((nh->nh_flags & NH_FLAG_INDIRECT) && (cnh = nh->nh_direct_nh)) {
    ...
    req->nhr_nh_list =
        vr_zalloc(req->nhr_nh_list_size * sizeof(unsigned int), VR_NEXTHOP_REQ_LIST_OBJECT);
    if (!req->nhr_nh_list)
      return -ENOMEM;
  ...
  case NH_COMPOSITE:
    ...
    req->nhr_ecmp_config_hash = nh->nh_ecmp_config_hash & NH_ECMP_CONFIG_HASH_MASK;
    ...
    if (nh->nh_component_cnt) {
      ...
      req->nhr_label_list =
          vr_zalloc(req->nhr_label_list_size * sizeof(unsigned int), VR_NEXTHOP_REQ_LIST_OBJECT);
    ...
    if (nh->nh_flags & NH_FLAG_TUNNEL_GRE) {
      ...
      if (nh->nh_family == AF_INET) {
        ...
        if (req->nhr_tun_sip6_size && req->nhr_tun_sip6)
          memcpy(req->nhr_tun_sip6, nh->nh_udp_tun6_sip, VR_IP6_ADDRESS_LEN);
        if (req->nhr_tun_dip6_size && req->nhr_tun_dip6)
          memcpy(req->nhr_tun_dip6, nh->nh_udp_tun6_dip, VR_IP6_ADDRESS_LEN);
      ...
      if (nh->nh_flags & NH_FLAG_L3_VXLAN) {
        ...
        req->nhr_rw_dst_mac = vr_zalloc(req->nhr_rw_dst_mac_size, VR_NEXTHOP_REQ_BMAC_OBJECT);
      ...
      req->nhr_pbb_mac = vr_zalloc(req->nhr_pbb_mac_size, VR_NEXTHOP_REQ_BMAC_OBJECT);
      if (!req->nhr_pbb_mac)
        return -ENOMEM;
  ...
  if (req->nhr_encap_size) {
    req->nhr_encap = vr_zalloc(req->nhr_encap_size, VR_NEXTHOP_REQ_ENCAP_OBJECT);
    if (req->nhr_encap) {
      memcpy(req->nhr_encap, encap, req->nhr_encap_size);
    } else {
...
static vr_nexthop_req *vr_nexthop_req_get(struct vr_nexthop *nh) {
  vr_nexthop_req *nhr;
  ...
  if ((nh->nh_type == NH_TUNNEL) && (nh->nh_flags & NH_FLAG_TUNNEL_UDP) &&
      (nh->nh_family == AF_INET6)) {
    nhr->nhr_tun_sip6 = vr_malloc(VR_IP6_ADDRESS_LEN, VR_NETWORK_ADDRESS_OBJECT);
    if (!nhr->nhr_tun_sip6)
      goto fail;
    nhr->nhr_tun_sip6_size = VR_IP6_ADDRESS_LEN;

    nhr->nhr_tun_dip6 = vr_malloc(VR_IP6_ADDRESS_LEN, VR_NETWORK_ADDRESS_OBJECT);
    if (!nhr->nhr_tun_dip6)
      goto fail;
...
static void vr_nexthop_req_destroy(vr_nexthop_req *req) {
  if (!req)
    return;
...
int vr_nexthop_get(vr_nexthop_req *req) {
  int ret = 0;
...
int vr_nexthop_dump(vr_nexthop_req *r) {
  int ret = 0;
  unsigned int i;
  vr_nexthop_req *resp = NULL;
  ...
  for (i = (unsigned int)(r->nhr_marker + 1); i < router->vr_max_nexthops; i++) {
    ...
    if (nh) {
      ...
      if ((ret < 0) || ((ret = vr_message_dump_object(dumper, VR_NEXTHOP_OBJECT_ID, resp)) <= 0)) {
        vr_nexthop_req_destroy(resp);
        break;
      }
 ...
 */
void vr_nexthop_req_process(void *s_req) {
  int ret;
...
int vr_is_local_ecmp_nh(struct vr_nexthop *nh) {
  int i;
...
struct vr_interface *vr_get_ecmp_first_member_dev(struct vr_nexthop *nh) {
  if (!nh || (nh->nh_type != NH_COMPOSITE) || (!(nh->nh_flags & NH_FLAG_COMPOSITE_ECMP))) {
    return NULL;
  }
...
static void nh_table_exit(struct vrouter *router, bool soft_reset) {
  unsigned int i;
  struct vr_nexthop *nh;
...
static int nh_allocate_discard(void) {
...
static int nh_table_init(struct vrouter *router) {
  int ret;
  ...
  if (!router->vr_max_nexthops) {
    ...
    router->vr_nexthops = vr_btable_alloc(router->vr_max_nexthops, sizeof(struct vr_nexthop *));
    if (!router->vr_nexthops)
...
void vr_nexthop_exit(struct vrouter *router, bool soft_reset) {
  nh_table_exit(router, soft_reset);
  return;
}

int vr_nexthop_init(struct vrouter *router) { return nh_table_init(router); }

/*
 * Called by offload module to update vrfstats with packets which have been
 * offloaded. Expect counters in host byte order.
 */
int vr_nexthop_update_offload_vrfstats(uint32_t vrfid, uint32_t num_cntrs, uint64_t *cntrs) {
  uint64_t *dst_cntr;
  struct vr_vrf_stats *stats;
