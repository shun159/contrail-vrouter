...
static bool vr_ip6_fragment_tail(struct vr_ip6 *ip6) {
 ...
 */
static bool vr_icmp6_pkt_dad(struct vr_packet *pkt) {
  struct vr_ip6 *ip6;
...
static inline uint16_t vr_sum(unsigned char *buf, unsigned int length) {
  int num_words;
  uint32_t total;
  uint16_t *ptr;
...
static inline uint16_t vr_ip6_pseudo_header_sum(struct vr_ip6 *ip6) {
...
uint16_t vr_icmp6_checksum(struct vr_ip6 *ip6, struct vr_icmp *icmph) {
  uint16_t sum[2];
...
static bool vr_icmp6_input(struct vrouter *router, struct vr_packet *pkt,
...
void vr_inet6_fill_flow_from_req(struct vr_flow *flow_p, vr_flow_req *req) {
  uint64_t *dst;

  vr_fill_flow_common(flow_p, req->fr_flow_nh_id, req->fr_flow_proto, req->fr_flow_sport,
                      req->fr_flow_dport, AF_INET6, VR_FLOW_KEY_ALL);
...
void vr_inet6_fill_rflow_from_req(struct vr_flow *flow_p, vr_flow_req *req) {
  uint64_t *dst;

  vr_fill_flow_common(flow_p, req->fr_rflow_nh_id, req->fr_flow_proto, req->fr_rflow_sport,
                      req->fr_rflow_dport, AF_INET6, VR_FLOW_KEY_ALL);
...
void vr_inet6_fill_flow(struct vr_flow *flow_p, unsigned int nh_id, unsigned char *sip,
                        unsigned char *dip, uint8_t proto, uint16_t sport, uint16_t dport,
                        uint8_t valid_fkey_params) {
  memset(flow_p, 0, VR_FLOW_IPV6_HASH_SIZE);
  valid_fkey_params &= VR_FLOW_KEY_ALL;

  vr_fill_flow_common(flow_p, nh_id, proto, sport, dport, AF_INET6, valid_fkey_params);
...
bool vr_inet6_flow_is_fat_flow(struct vrouter *router, struct vr_packet *pkt,
...
static int vr_inet6_proto_flow(struct vrouter *router, unsigned short vrf, struct vr_packet *pkt,
                               uint16_t vlan, struct vr_ip6 *ip6, struct vr_flow *flow_p,
                               uint8_t valid_fkey_params, bool frag_calc, bool forward) {
  int i, ret = 0;
  uint8_t ip6_nxt;
  unsigned char *ip6_src, *ip6_dst, *ip6_addr;
  unsigned char ip6_src_flow[16], ip6_dst_flow[16];
  unsigned short *t_hdr, sport, dport, port, fat_flow_mask = VR_FAT_FLOW_NO_MASK;
  unsigned int nh_id;
  struct vr_ip6_frag *v6_frag;
  struct vr_icmp *icmph;
  ...
  if (ip6_nxt == VR_IP_PROTO_ICMP6) {
    ...
    if (vr_icmp6_error(icmph)) {
      if ((unsigned char *)ip6 == pkt_network_header(pkt)) {
        ret = vr_inet6_form_flow(router, vrf, pkt, vlan, (struct vr_ip6 *)(icmph + 1), flow_p,
  ...
  } else if ((ip6_nxt == VR_IP_PROTO_TCP) || (ip6_nxt == VR_IP_PROTO_UDP) ||
  ...
  fat_flow_mask = vr_flow_fat_flow_lookup(router, pkt, ip6_nxt, sport, dport, NULL, NULL,
                                          ip6_src_flow, ip6_dst_flow);
  ...
  vr_inet6_fill_flow(flow_p, nh_id, ip6_src_flow, ip6_dst_flow, ip6_nxt, sport, dport,
                     valid_fkey_params);

  return 0;
}

static int vr_inet6_fragment_flow(struct vrouter *router, unsigned short vrf, struct vr_packet *pkt,
                                  uint16_t vlan, struct vr_ip6 *ip6, struct vr_flow *flow_p,
                                  uint8_t valid_fkey_params, bool frag_calc, bool forward) {
  ...
  frag = vr_fragment_get(router, vrf, (struct vr_ip *)(pkt_network_header(pkt)));
  if (!frag) {
    return -1;
  }
  ...
  if (frag_calc) {
    ...
    if (vr_ip6_fragment_tail(ip6)) {
      frag->f_expected = (((ntohs(v6_frag->ip6_frag_offset)) >> VR_IP6_FRAG_OFFSET_BITS) * 8) +
  ...
  nh_id = vr_inet_flow_nexthop(pkt, vlan);
  vr_inet6_fill_flow(flow_p, nh_id, ip6_src, ip6_dst, v6_frag->ip6_frag_nxt, sport, dport,
                     valid_fkey_params);

  return 0;
}

int vr_inet6_form_flow(struct vrouter *router, unsigned short vrf, struct vr_packet *pkt,
                       uint16_t vlan, struct vr_ip6 *ip6, struct vr_flow *flow_p,
                       uint8_t valid_fkey_params, bool frag_calc, bool forward) {
  int ret = 0;

  if (vr_ip6_transport_header_valid(ip6)) {
    ret = vr_inet6_proto_flow(router, vrf, pkt, vlan, ip6, flow_p, valid_fkey_params, frag_calc,
                              forward);
  } else {
    ret = vr_inet6_fragment_flow(router, vrf, pkt, vlan, ip6, flow_p, valid_fkey_params, frag_calc,
                                 forward);
  }
...
int vr_inet6_get_flow_key(struct vrouter *router, unsigned short vrf, struct vr_packet *pkt,
                          uint16_t vlan, struct vr_flow *flow_p, uint8_t valid_fkey_params) {
  int ret;
  struct vr_ip6 *ip6;

  ip6 = (struct vr_ip6 *)pkt_network_header(pkt);
  ret = vr_inet6_form_flow(router, vrf, pkt, vlan, ip6, flow_p, valid_fkey_params, false, true);
  if (ret < 0)
    return ret;

  if (vr_ip6_fragment_head(ip6)) {
    vr_v6_fragment_add(router, vrf, ip6, flow_p->flow6_sport, flow_p->flow6_dport);
  }

  return ret;
}

flow_result_t vr_inet6_flow_lookup(struct vrouter *router, struct vr_packet *pkt,
                                   struct vr_forwarding_md *fmd) {
  int ret;
  bool lookup = false;
   ...
   */
  if ((pkt->vp_if->vif_flags & VIF_FLAG_POLICY_ENABLED) || (pkt->vp_flags & VP_FLAG_FLOW_GET)) {
    lookup = true;
  }
...
void vr_neighbor_proxy(struct vr_packet *pkt, struct vr_forwarding_md *fmd, unsigned char *dmac) {
  ...
  icmph = (struct vr_icmp *)pkt_data(pkt);
  nopt =
      (struct vr_neighbor_option *)((unsigned char *)icmph + sizeof(*icmph) + VR_IP6_ADDRESS_LEN);
...
mac_response_t vm_neighbor_request(struct vr_interface *vif, struct vr_packet *pkt,
...
int vr_neighbor_input(struct vr_packet *pkt, struct vr_forwarding_md *fmd,
...
bool vr_ip6_dhcp_packet(struct vr_packet *pkt) {
  unsigned char *data = pkt_data(pkt);
  struct vr_ip6 *ip6;
  struct vr_udp *udph = NULL;

  if ((pkt->vp_type != VP_TYPE_IP6) || (!(pkt->vp_flags & VP_FLAG_MULTICAST)))
    return false;
