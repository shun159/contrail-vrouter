...
static void vr_module_debug_dump(void) {
  struct vr_module *module = module_under_init;
  ...
  vr_printf("vrouter (%s): Init failed at %s:%d with return %d (data %d)\n", module->mod_name,
            module->error_func, module->error_line, module->error, module->error_data);
...
struct vrouter *vrouter_get(unsigned int vr_id) {
  return &router;
}

unsigned int vrouter_generation_num_get(struct vrouter *router) {
  return ++router->vr_generation_num;
}

static void vrouter_ops_destroy(vrouter_ops *req) {
  if (!req)
    return;
...
static vrouter_ops *vrouter_ops_get(void) {
  vrouter_ops *req;
  ...
  req->vo_build_info = vr_zalloc(strlen(ContrailBuildInfo) + 1, VR_BUILD_INFO_OBJECT);
  if (!req->vo_build_info) {
    vr_free(req, VR_VROUTER_REQ_OBJECT);
    return NULL;
  }
...
void vrouter_ops_get_process(void *s_req) {
  int ret = 0;
  ...
  /* Build info */
  strncpy(resp->vo_build_info, ContrailBuildInfo, strlen(ContrailBuildInfo) + 1);

  /* Logging entries */
  resp->vo_log_level = vr_get_log_level();
  resp->vo_log_type_enable = vr_get_enabled_log_types(&resp->vo_log_type_enable_size);

  /* Used entries */
  resp->vo_flow_used_entries = vr_flow_table_used_total_entries(router);
  resp->vo_flow_used_oentries = vr_flow_table_used_oflow_entries(router);

  resp->vo_bridge_used_entries = vr_bridge_table_used_total_entries(router);
  resp->vo_bridge_used_oentries = vr_bridge_table_used_oflow_entries(router);

  vr_flow_get_burst_params(router, &resp->vo_burst_tokens, &resp->vo_burst_interval,
                           &resp->vo_burst_step);
 ...
 */
void vrouter_ops_add_process(void *s_req) {
  int i;
  ...
  if (req->vo_mudp != -1)
    vr_mudp = req->vo_mudp;
  vr_flow_set_burst_params(vrouter_get(req->vo_rid), req->vo_burst_tokens, req->vo_burst_interval,
                           req->vo_burst_step);
...
static int vrouter_soft_reset(void) {
  int ret = 0;
  vrouter_exit(true);
  ret = vrouter_init();
  if (!ret)
...
void vr_hugepage_config_process(void *s_req) {
  ...
  if (vr_huge_page_config) {
    ret = vr_huge_page_config(req->vhp_mem, req->vhp_mem_size, req->vhp_msize, req->vhp_msize_size);
  }
  ...
  vr_message_response(VR_HPAGE_CFG_OBJECT_ID, &hcfg_resp, ret, false);
