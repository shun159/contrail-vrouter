...
mac_response_t vr_get_proxy_mac(struct vr_packet *pkt, struct vr_forwarding_md *fmd,
  ...
  if (vif_is_vhost(vif) ||
      (from_fabric && (fmd->fmd_label == -1) && (fmd->fmd_dvrf == vif->vif_vrf)))
    over_lay = false;
  ...
  if (from_fabric) {
     ...
     */
    if (to_vcp || to_gateway || hosted_vm || (fmd->fmd_src == TOR_SOURCE) ||
...
static void vr_arp_proxy(struct vr_arp *sarp, struct vr_packet *pkt, struct vr_forwarding_md *fmd,
                         unsigned char *dmac) {
  struct vr_eth *eth;
  struct vr_arp *arp;
...
static int vr_handle_arp_request(struct vr_arp *sarp, struct vr_packet *pkt,
 ...
 *
 */
static int vr_handle_arp_reply(struct vr_arp *sarp, struct vr_packet *pkt,
 ...
 */
int vif_plug_mac_request(struct vr_interface *vif, struct vr_packet *pkt,
                         struct vr_forwarding_md *fmd) {
  ...
  if (pkt->vp_type == VP_TYPE_ARP) {
    ...
    if (pkt->vp_len < (nheader + sizeof(struct vr_ip6) + sizeof(struct vr_icmp) +
                       VR_IP6_ADDRESS_LEN + sizeof(struct vr_neighbor_option) + VR_ETHER_ALEN))
      goto unhandled;
...
void vr_mac_reply_send(struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
  bool vif_tx = false;
...
int vr_arp_input(struct vr_packet *pkt, struct vr_forwarding_md *fmd, unsigned char *eth_dmac) {
  ...
  if (((htons(sarp.arp_hw) == VR_ARP_HW_TYPE_ETHER) && (sarp.arp_hwlen != VR_ARP_HW_LEN)) ||
...
int vr_trap(struct vr_packet *pkt, unsigned short trap_vrf, unsigned short trap_reason,
            void *trap_param) {
  struct vr_interface *vif = pkt->vp_if;
  ...
  if (router->vr_agent_if && router->vr_agent_if->vif_send) {
    ...
    return router->vr_agent_if->vif_send(router->vr_agent_if, pkt, &params);
  } else {
...
unsigned int vr_reinject_packet(struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
  ...
  if (vif_is_vhost(vif) || (vif_is_fabric(vif) && (fmd->fmd_label < 0))) {
 ...
 */
unsigned int vr_virtual_input(unsigned short vrf, struct vr_interface *vif, struct vr_packet *pkt,
   ...
   * happen)
   */
  if ((pkt->vp_flags & VP_FLAG_MULTICAST) && (vif_is_service(vif)) &&
...
unsigned int vr_fabric_input(struct vr_interface *vif, struct vr_packet *pkt,
                             struct vr_forwarding_md *fmd, unsigned short vlan_id) {
  int handled = 0;
   ...
   */
  if ((pkt->vp_type != VP_TYPE_ARP) && (pkt->vp_flags & VP_FLAG_MULTICAST)) {
    return vif_xconnect(vif, pkt, fmd);
  }
 ...
 * modify the data pointer of skb
 */
int vr_tag_pkt(struct vr_packet **pkt, unsigned short vlan_id, bool force_tag) {
  uint8_t priority = 0;
...
int __vr_pbb_decode(struct vr_eth *eth, int len, struct vr_forwarding_md *fmd) {
...
int vr_pbb_decode(struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
  int pbb_size, decode_error = 1;

  pbb_size = __vr_pbb_decode((struct vr_eth *)pkt_data(pkt), pkt_head_len(pkt), fmd);
  if (pbb_size <= 0) {
