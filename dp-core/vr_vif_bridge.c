...
__attribute__packed__open__ struct vif_bridge_key {
  unsigned short vbk_vlan;
  unsigned char vbk_mac[VR_ETHER_ALEN];
} __attribute__packed__close__;

__attribute__packed__open__ struct vif_bridge_entry {
  vr_hentry_t vif_bridge_hentry;
  struct vif_bridge_key vbe_key;
  struct vr_interface *vbe_vif;
} __attribute__packed__close__;

static struct vif_bridge_entry *vif_bridge_get(vr_htable_t htable, unsigned short vlan,
                                               unsigned char *mac) {
...
struct vr_interface *vif_bridge_get_sub_interface(vr_htable_t htable, unsigned short vlan,
                                                  unsigned char *mac) {
...
int vif_bridge_get_index(struct vr_interface *pvif, struct vr_interface *vif, uint8_t *mac) {
...
static void vif_bridge_free(vr_htable_t htable, vr_hentry_t *hentry,
...
struct vif_bridge_entry *vif_bridge_alloc(vr_htable_t htable, unsigned short vlan,
                                          unsigned char *mac) {
  struct vif_bridge_key key;
  struct vif_bridge_entry *vbe;
  ...
  vbe = (struct vif_bridge_entry *)vr_htable_find_free_hentry(htable, &key, 0);
  if (!vbe)
    return NULL;
...
int vif_bridge_delete(struct vr_interface *pvif, struct vr_interface *vif, uint8_t *src_mac) {
  struct vif_bridge_entry *be;
...
int vif_bridge_add(struct vr_interface *pvif, struct vr_interface *vif, uint8_t *src_mac) {
  struct vif_bridge_entry *be;
...
vr_hentry_key vif_bridge_get_key(vr_htable_t table, vr_hentry_t *entry, unsigned int *key_size) {
  struct vif_bridge_entry *vif_be = CONTAINER_OF(vif_bridge_hentry, struct vif_bridge_entry, entry);
...
int vif_bridge_init(struct vr_interface *vif) {
  ...
  vif->vif_btable = vr_htable_create(vrouter_get(0), vif_bridge_entries, vif_bridge_oentries,
                                     sizeof(struct vif_bridge_entry), sizeof(struct vif_bridge_key),
                                     0, vif_bridge_get_key);
