...
static int vr_stride_empty(void **ptr, unsigned int cnt) {
  unsigned int i;
  for (i = 0; i < cnt; i++) {
    if (ptr[i] != NULL) {
      return 0;
...
static int print_ind(unsigned int index, void *data, void *udata) {
  vr_printf("Index %d Data 0x%p\n", index, data);
  return 1;
}

#if defined(__KERNEL__)
static unsigned int get_page_shift(void) { return PAGE_SHIFT; }
#else
static unsigned int get_page_shift(void) { return ffs(getpagesize()); }
#endif

static int __vr_itable_del(struct vr_itbl *table, unsigned int index, void **ptr, unsigned int cnt,
                           void **old) {
  unsigned int id;
  ...
  if ((cnt < (table->stride_cnt - 1)) && ptr) {
    id = (index >> table->stride_shift[cnt]) & (table->stride_len[cnt] - 1);
    if (ptr[id] && __vr_itable_del(table, index, (void **)(ptr[id]), (cnt + 1), old) == 1) {
...
static int __vr_itable_dump(struct vr_itbl *table, vr_itable_trav_cb_t func, void **ptr,
                            unsigned int cnt, unsigned int index, unsigned int marker,
                            void *udata) {
  unsigned int i, j;
  int res = 1;
...
static void __vr_itable_exit(struct vr_itbl *table, vr_itable_del_cb_t func, void **ptr,
...
void vr_print_table_struct(vr_itable_t t) {
...
void *vr_itable_set(vr_itable_t t, unsigned int index, void *data) {
  ...
  if (index & (~(((0x1 << (table->index_len - 1)) - 1) | (0x1 << (table->index_len - 1))))) {
    vr_printf("Index %x has more bits than %d Ignoring MSB\n", index, table->index_len);
  }

  if (!table->data) {
    table->data = vr_zalloc(table->stride_len[0] * sizeof(void *), VR_ITABLE_OBJECT);
    if (!table->data) {
      return VR_ITABLE_ERR_PTR;
    }
  }
  ...
  for (i = 0; i < table->stride_cnt - 1; i++) {
    ...
    if (!ptr[id]) {
      ptr[id] = vr_zalloc(table->stride_len[i + 1] * sizeof(void *), VR_ITABLE_OBJECT);
...
vr_itable_t vr_itable_create(unsigned int index_len, unsigned int stride_cnt, ...) {
  ...
  table->stride_shift = vr_zalloc(table->stride_cnt * sizeof(unsigned int), VR_ITABLE_OBJECT);
  if (!table->stride_shift) {
    goto fail;
  }

  table->stride_len = vr_zalloc(table->stride_cnt * sizeof(unsigned int), VR_ITABLE_OBJECT);
  if (!table->stride_len) {
    goto fail;
  }
