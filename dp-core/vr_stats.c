...
static void vr_drop_stats_make_response(vr_drop_stats_req *response, uint64_t *stats) {
  if (!response || !stats)
    return;
  ...
  response->vds_flow_queue_limit_exceeded += stats[VP_DROP_FLOW_QUEUE_LIMIT_EXCEEDED];
  response->vds_flow_no_memory += stats[VP_DROP_FLOW_NO_MEMORY];
  response->vds_flow_invalid_protocol += stats[VP_DROP_FLOW_INVALID_PROTOCOL];
  ...
  response->vds_fragment_queue_fail += stats[VP_DROP_FRAGMENT_QUEUE_FAIL];
...
void vr_drop_stats_get_vif_stats(vr_drop_stats_req *response, struct vr_interface *vif) {
  uint8_t *count;
  int stats_index, cpu;
...
static void vr_drop_stats_get(unsigned int rid, short core) {
  int ret = 0;
  unsigned int cpu;
...
static void vr_drop_stats_clear(vr_drop_stats_req *req) {
...
void vr_drop_stats_req_process(void *s_req) {
  int ret = 0;
...
static void vr_mem_stats_get(void) {
  int ret = 0;
  unsigned int cpu, i;
  ...
  for (cpu = 0; cpu < vr_num_cpus; cpu++) {
    ...
    response->vms_bridge_mac_object +=
        (stats_block[VR_BRIDGE_MAC_OBJECT].ms_alloc - stats_block[VR_BRIDGE_MAC_OBJECT].ms_free);
    response->vms_btable_object +=
        (stats_block[VR_BTABLE_OBJECT].ms_alloc - stats_block[VR_BTABLE_OBJECT].ms_free);
    response->vms_build_info_object +=
        (stats_block[VR_BUILD_INFO_OBJECT].ms_alloc - stats_block[VR_BUILD_INFO_OBJECT].ms_free);
    response->vms_defer_object +=
        (stats_block[VR_DEFER_OBJECT].ms_alloc - stats_block[VR_DEFER_OBJECT].ms_free);
    response->vms_drop_stats_object +=
        (stats_block[VR_DROP_STATS_OBJECT].ms_alloc - stats_block[VR_DROP_STATS_OBJECT].ms_free);
    ...
    response->vms_flow_queue_object +=
        (stats_block[VR_FLOW_QUEUE_OBJECT].ms_alloc - stats_block[VR_FLOW_QUEUE_OBJECT].ms_free);
    response->vms_flow_req_object +=
        (stats_block[VR_FLOW_REQ_OBJECT].ms_alloc - stats_block[VR_FLOW_REQ_OBJECT].ms_free);
    ...
    response->vms_fragment_object +=
        (stats_block[VR_FRAGMENT_OBJECT].ms_alloc - stats_block[VR_FRAGMENT_OBJECT].ms_free);
    ...
    response->vms_fragment_queue_element_object +=
        (stats_block[VR_FRAGMENT_QUEUE_ELEMENT_OBJECT].ms_alloc -
    ...
    response->vms_htable_object +=
        (stats_block[VR_HTABLE_OBJECT].ms_alloc - stats_block[VR_HTABLE_OBJECT].ms_free);
    response->vms_interface_object +=
        (stats_block[VR_INTERFACE_OBJECT].ms_alloc - stats_block[VR_INTERFACE_OBJECT].ms_free);
    ...
    response->vms_interface_vrf_table_object +=
        (stats_block[VR_INTERFACE_VRF_TABLE_OBJECT].ms_alloc -
         stats_block[VR_INTERFACE_VRF_TABLE_OBJECT].ms_free);
    response->vms_itable_object +=
        (stats_block[VR_ITABLE_OBJECT].ms_alloc - stats_block[VR_ITABLE_OBJECT].ms_free);
    response->vms_malloc_object +=
        (stats_block[VR_MALLOC_OBJECT].ms_alloc - stats_block[VR_MALLOC_OBJECT].ms_free);
    response->vms_message_object +=
        (stats_block[VR_MESSAGE_OBJECT].ms_alloc - stats_block[VR_MESSAGE_OBJECT].ms_free);
    ...
    response->vms_mirror_object +=
        (stats_block[VR_MIRROR_OBJECT].ms_alloc - stats_block[VR_MIRROR_OBJECT].ms_free);
    response->vms_mirror_table_object += (stats_block[VR_MIRROR_TABLE_OBJECT].ms_alloc -
                                          stats_block[VR_MIRROR_TABLE_OBJECT].ms_free);
    response->vms_mirror_meta_object +=
        (stats_block[VR_MIRROR_META_OBJECT].ms_alloc - stats_block[VR_MIRROR_META_OBJECT].ms_free);
    response->vms_mtrie_object +=
        (stats_block[VR_MTRIE_OBJECT].ms_alloc - stats_block[VR_MTRIE_OBJECT].ms_free);
    response->vms_mtrie_bucket_object += (stats_block[VR_MTRIE_BUCKET_OBJECT].ms_alloc -
                                          stats_block[VR_MTRIE_BUCKET_OBJECT].ms_free);
    response->vms_mtrie_stats_object +=
        (stats_block[VR_MTRIE_STATS_OBJECT].ms_alloc - stats_block[VR_MTRIE_STATS_OBJECT].ms_free);
    response->vms_mtrie_table_object +=
        (stats_block[VR_MTRIE_TABLE_OBJECT].ms_alloc - stats_block[VR_MTRIE_TABLE_OBJECT].ms_free);
    ...
    response->vms_nexthop_object +=
        (stats_block[VR_NEXTHOP_OBJECT].ms_alloc - stats_block[VR_NEXTHOP_OBJECT].ms_free);
    ...
    response->vms_nexthop_req_bmac_object += (stats_block[VR_NEXTHOP_REQ_BMAC_OBJECT].ms_alloc -
    ...
    response->vms_nexthop_req_object +=
        (stats_block[VR_NEXTHOP_REQ_OBJECT].ms_alloc - stats_block[VR_NEXTHOP_REQ_OBJECT].ms_free);
    response->vms_route_table_object +=
        (stats_block[VR_ROUTE_TABLE_OBJECT].ms_alloc - stats_block[VR_ROUTE_TABLE_OBJECT].ms_free);
    ...
    response->vms_timer_object +=
        (stats_block[VR_TIMER_OBJECT].ms_alloc - stats_block[VR_TIMER_OBJECT].ms_free);
    response->vms_usock_object +=
        (stats_block[VR_USOCK_OBJECT].ms_alloc - stats_block[VR_USOCK_OBJECT].ms_free);
    response->vms_usock_poll_object +=
        (stats_block[VR_USOCK_POLL_OBJECT].ms_alloc - stats_block[VR_USOCK_POLL_OBJECT].ms_free);
    response->vms_usock_buf_object +=
        (stats_block[VR_USOCK_BUF_OBJECT].ms_alloc - stats_block[VR_USOCK_BUF_OBJECT].ms_free);
    response->vms_usock_iovec_object +=
        (stats_block[VR_USOCK_IOVEC_OBJECT].ms_alloc - stats_block[VR_USOCK_IOVEC_OBJECT].ms_free);
    response->vms_vrouter_req_object +=
        (stats_block[VR_VROUTER_REQ_OBJECT].ms_alloc - stats_block[VR_VROUTER_REQ_OBJECT].ms_free);
    response->vms_qos_map_object +=
        (stats_block[VR_QOS_MAP_OBJECT].ms_alloc - stats_block[VR_QOS_MAP_OBJECT].ms_free);
    response->vms_fc_object +=
        (stats_block[VR_FC_OBJECT].ms_alloc - stats_block[VR_FC_OBJECT].ms_free);
...
void vr_mem_stats_req_process(void *s_req) {
  int ret;
...
void vr_free_stats(unsigned int object) {
  struct vrouter *router = vrouter_get(0);
  unsigned int cpu;
...
void vr_malloc_stats(unsigned int size, unsigned int object) {
  struct vrouter *router = vrouter_get(0);
  unsigned int cpu;
...
static void vr_malloc_stats_exit(struct vrouter *router) {
  unsigned int i;
...
static int vr_malloc_stats_init(struct vrouter *router) {
...
static void vr_pkt_drop_stats_exit(struct vrouter *router) {
  unsigned int i;
...
static int vr_pkt_drop_stats_init(struct vrouter *router) {
  unsigned int i = 0;
  unsigned int size = 0;
  ...
  if (!router->vr_pdrop_stats) {
    vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, size);
    goto cleanup;
  }
...
static void vr_pkt_drop_stats_reset(struct vrouter *router) {
  unsigned int i;
