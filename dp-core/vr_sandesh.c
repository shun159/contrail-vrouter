...
struct sandesh_object_md sandesh_md[] = {
    ...
    [VR_MPLS_OBJECT_ID] = {.obj_len = 4 * sizeof(vr_mpls_req), .obj_type_string = "vr_mpls_req"},
    [VR_MIRROR_OBJECT_ID] =
        {
    ...
    [VR_PKT_DROP_LOG_OBJECT_ID] =
        {
            .obj_len = 4 * sizeof(vr_pkt_drop_log_req),
            .obj_get_size = vr_pkt_drop_log_req_get_size,
            .obj_type_string = "vr_pkt_drop_log_req",
        },
    ...
    [VR_FLOW_TABLE_DATA_OBJECT_ID] =
        {
            .obj_len =
                ((4 * sizeof(vr_flow_table_data)) + (VR_FLOW_MAX_CPUS * sizeof(unsigned int))),
...
static unsigned int sandesh_proto_buf_len(unsigned int object_type, void *object) {
...
static int sandesh_proto_encode(char *buf, unsigned int len, unsigned int object_type, void *object,
                                unsigned int message_type) {
  int off = 0, error = 0;

  if (object)
    off = sandesh_encode(object, sandesh_md[object_type].obj_type_string, vr_find_sandesh_info,
...
static int sandesh_proto_encode_response(char *buf, unsigned int len, unsigned int object_type,
                                         void *object, int ret) {
  int off, error;
  vr_response resp;
  ...
  off =
      sandesh_encode(&resp, "vr_response", vr_find_sandesh_info, (unsigned char *)buf, len, &error);
  if (off < 0)
    return off;

  error = sandesh_proto_encode(buf + off, len - off, object_type, object, VR_MESSAGE_TYPE_RESPONSE);
  if (error < 0)
    return error;
...
static int sandesh_proto_decode(char *buf, unsigned int len,
                                int (*cb)(void *, unsigned int, void *), void *cb_arg) {
  int ret = 0;

  return sandesh_decode((unsigned char *)buf, len, vr_find_sandesh_info, &ret);
}
