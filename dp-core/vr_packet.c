...
struct vr_packet *pkt_copy(struct vr_packet *pkt, unsigned short off, unsigned short len) {
...
struct vr_packet *pkt_cow(struct vr_packet *pkt, unsigned short head_room) {
...
bool vr_ip_proto_pull(struct vr_ip *iph) {
  ...
  if ((proto == VR_IP_PROTO_TCP) || (proto == VR_IP_PROTO_UDP) || (proto == VR_IP_PROTO_ICMP) ||
...
bool vr_ip6_proto_pull(struct vr_ip6 *ip6h) {
  unsigned char proto = ip6h->ip6_nxt;

  if ((proto == VR_IP_PROTO_TCP) || (proto == VR_IP_PROTO_UDP) || (proto == VR_IP_PROTO_ICMP6) ||
      (proto == VR_IP_PROTO_SCTP) || (proto == VR_IP6_PROTO_FRAG)) {
    return true;
  }
...
int vr_ip_transport_parse(struct vr_ip *iph, struct vr_ip6 *ip6h, void **thp,
  ...
  if (iph) {
    ...
    if (thdr_valid) {
      ...
      } else if ((ip_proto == VR_IP_PROTO_ICMP) || (ip_proto == VR_IP_PROTO_ICMP6)) {
      ...
      if (ip_proto == VR_IP_PROTO_TCP) {
         ...
         * to the slow path.
         */
        if ((ntohs(tcph->tcp_offset_r_flags) & VR_TCP_FLAG_SYN) && vr_to_vm_mss_adj) {
          if (do_tcp_mss_adj) {
            /* Kernel will never get here, it will return slow path */
            do_tcp_mss_adj((struct tcphdr *)tcph, VROUTER_L2_OVERLAY_LEN, hlen);
          } else {
            return PKT_RET_SLOW_PATH;
          }
        }
        ...
        if (vr_icmp_error(icmph)) {
          ...
          if (vr_ip_proto_pull(icmp_pl_iph)) {
            ...
            if (icmp_pl_iph->ip_proto == VR_IP_PROTO_TCP) {
              ...
              th_csum = ((struct vr_icmp *)((unsigned char *)icmp_pl_iph + icmp_pl_iph->ip_hl * 4))
                            ->icmp_csum;
            }
          }
        }
      } else if (iph->ip_proto == VR_IP_PROTO_UDP) {
        th_csum = ((struct vr_udp *)((unsigned char *)iph + hlen))->udp_csum;
        ...
        if (icmph->icmp_type == VR_ICMP6_TYPE_NEIGH_SOL) {
           ...
           * take care of it
           */
          pull_len += sizeof(struct vr_neighbor_option) + VR_IP6_ADDRESS_LEN + VR_ETHER_ALEN;
          ...
          if (vr_ip6_proto_pull(icmp_pl_ip6h)) {
            ...
            if (icmp_pl_frag_hdr) {
              v6_frag =
                  (struct vr_ip6_frag *)((unsigned char *)icmp_pl_ip6h + sizeof(struct vr_ip6));
              ip_proto = v6_frag->ip6_frag_nxt;
              thdr = (unsigned char *)v6_frag + sizeof(struct vr_ip6_frag);
            } else {
...
int vr_inner_pkt_parse(unsigned char *va,
  ...
  if ((ip_proto == VR_IP_PROTO_GRE && gre_udp_encap == VR_GRE_PROTO_MPLS_NO) ||
    ...
    *encap_type = PKT_ENCAP_MPLS;
    ...
    if (frag_size < (pull_len + VR_MPLS_HDR_LEN + VR_L2_CTRL_DATA_LEN)) {
      return PKT_RET_SLOW_PATH;
    }
    ...
    if (pkt_type == PKT_MPLS_TUNNEL_L3) {
      ...
      eth = (struct vr_eth *)(va + pull_len + mpls_label_len + VR_L2_CTRL_DATA_LEN +
                              VR_VXLAN_HDR_LEN);
      pull_len += mpls_label_len + VR_L2_CTRL_DATA_LEN + VR_VXLAN_HDR_LEN + sizeof(struct vr_eth);
    } else if (pkt_type == PKT_MPLS_TUNNEL_L2_CONTROL_DATA) {
      /* L2 packet with control information */
      eth = (struct vr_eth *)(va + pull_len + mpls_label_len + VR_L2_CTRL_DATA_LEN);
      pull_len += mpls_label_len + VR_L2_CTRL_DATA_LEN + sizeof(struct vr_eth);
  ...
  } else if (ip_proto == VR_IP_PROTO_UDP && ntohs(gre_udp_encap) == VR_VXLAN_UDP_DST_PORT) {
