...
struct vr_nexthop *vr_inet_src_lookup(unsigned short vrf, struct vr_packet *pkt) {
  struct vr_ip *ip;
  struct vr_ip6 *ip6;
...
static inline unsigned char vr_ip_decrement_ttl(struct vr_ip *ip) {
  unsigned int diff = 0;
...
void vr_ip_update_csum(struct vr_packet *pkt, unsigned int ip_inc, unsigned int inc) {
  struct vr_ip *ip;
  struct vr_tcp *tcp;
  struct vr_udp *udp;
  unsigned int csum;
...
int vr_forward(struct vrouter *router, struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
  ...
  if (rt.rtr_req.rtr_label_flags & VR_RT_LABEL_VALID_FLAG) {
    ...
    vr_fmd_set_label(fmd, rt.rtr_req.rtr_label, VR_LABEL_TYPE_UNKNOWN);
  }
...
unsigned int vr_icmp_input(struct vrouter *router, struct vr_packet *pkt,
  ...
  if (vr_icmp_error(icmph)) {
    ...
    if ((iph->ip_proto != VR_IP_PROTO_GRE) && (iph->ip_proto != VR_IP_PROTO_UDP))
      return unhandled;
 ...
 */
unsigned int vr_udp_input(struct vrouter *router, struct vr_packet *pkt,
                          struct vr_forwarding_md *fmd) {
  ...
  if (vr_perfp && vr_pull_inner_headers_fast) {
    handled =
        vr_pull_inner_headers_fast(pkt, VR_IP_PROTO_UDP, vr_mpls_tunnel_type, &ret, &encap_type);
    if (!handled) {
      return 1;
    }
  ...
  udph = (struct vr_udp *)vr_pheader_pointer(pkt, sizeof(struct vr_udp), &udp);
  if (udph == NULL) {
  ...
  if (vr_pull_inner_headers) {
    if (!vr_pull_inner_headers(pkt, VR_IP_PROTO_UDP, &reason, vr_mpls_tunnel_type)) {
      vr_pfree(pkt, reason);
      return 0;
    }
  }
...
unsigned int vr_gre_input(struct vrouter *router, struct vr_packet *pkt,
                          struct vr_forwarding_md *fmd) {
  ...
  if (vr_perfp && vr_pull_inner_headers_fast) {
    handled =
        vr_pull_inner_headers_fast(pkt, VR_IP_PROTO_GRE, vr_mpls_tunnel_type, &ret, &encap_type);
    if (!handled) {
      goto unhandled;
    }
   ...
   */
  if (vr_pull_inner_headers) {
    if (!vr_pull_inner_headers(pkt, VR_IP_PROTO_GRE, &reason, vr_mpls_tunnel_type)) {
      vr_pfree(pkt, reason);
      return 0;
    }
  }
...
int vr_ip_rcv(struct vrouter *router, struct vr_packet *pkt, struct vr_forwarding_md *fmd) {
  ...
  if (unhandled) {
    ...
    if (pkt->vp_nh) {
      ...
      if (!(pkt->vp_flags & VP_FLAG_FLOW_SET) &&
        ...
        if (vif_is_vhost(vif) && (vif->vif_flags & VIF_FLAG_POLICY_ENABLED)) {
          ...
          if ((ip->ip_proto == VR_IP_PROTO_UDP) || (ip->ip_proto == VR_IP_PROTO_TCP) ||
            ...
            if (l4_port &&
                vr_valid_link_local_port(router, AF_INET, ip->ip_proto, ntohs(l4_port))) {
              flow_processing = true;
    ...
    if ((pkt->vp_flags & VP_FLAG_FROM_DP) || !vr_phead_len(pkt)) {
...
flow_result_t vr_inet_flow_nat(struct vr_flow_entry *fe, struct vr_packet *pkt,
                               struct vr_forwarding_md *fmd) {
  ...
  if (ip->ip_proto == VR_IP_PROTO_ICMP) {
    ...
    if (vr_icmp_error(icmph)) {
      ...
      t_sport = (unsigned short *)((unsigned char *)icmp_pl_ip + (icmp_pl_ip->ip_hl * 4));
      t_dport = t_sport + 1;
  ...
  if ((fe->fe_flags & VR_FLOW_FLAG_SNAT) && (ip->ip_saddr == fe->fe_key.flow4_sip)) {
  ...
  if (vr_ip_transport_header_valid(ip)) {
    t_sport = (unsigned short *)((unsigned char *)ip + (ip->ip_hl * 4));
    t_dport = t_sport + 1;
  ...
  if ((fe->fe_flags & VR_FLOW_FLAG_VRFT) && pkt->vp_nh &&
      ((pkt->vp_nh->nh_vrf != fmd->fmd_dvrf) || (pkt->vp_nh->nh_flags & NH_FLAG_ROUTE_LOOKUP))) {
...
unsigned int vr_inet_flow_nexthop(struct vr_packet *pkt, unsigned short vlan) {
  unsigned int nh_id;

  if (vif_is_fabric(pkt->vp_if) && pkt->vp_nh) {
    /* this is more a requirement from agent */
    if ((pkt->vp_nh->nh_type == NH_ENCAP) || (pkt->vp_nh->nh_type == NH_RCV)) {
...
void vr_inet_fill_flow(struct vr_flow *flow_p, unsigned int nh_id, uint32_t sip, uint32_t dip,
                       uint8_t proto, uint16_t sport, uint16_t dport, uint8_t valid_fkey_params) {
  valid_fkey_params &= VR_FLOW_KEY_ALL;
  memset(flow_p, 0, VR_FLOW_IPV4_HASH_SIZE);

  vr_fill_flow_common(flow_p, nh_id, proto, sport, dport, AF_INET, valid_fkey_params);
...
static int vr_inet_fragment_flow(struct vrouter *router, unsigned short vrf, struct vr_packet *pkt,
                                 uint16_t vlan, struct vr_flow *flow_p, uint8_t valid_fkey_params,
                                 bool frag_calc) {
  ...
  if (frag_calc) {
    ...
    if (vr_ip_fragment_tail(ip)) {
      frag->f_expected =
          ((ntohs(ip->ip_frag_off) & 0x1FFF) * 8) + ntohs(ip->ip_len) - (ip->ip_hl * 4);
    }
  ...
  vr_inet_fill_flow(flow_p, nh_id, ip->ip_saddr, ip->ip_daddr, ip->ip_proto, sport, dport,
                    valid_fkey_params);
  return 0;
}
...
bool vr_inet_flow_is_fat_flow(struct vrouter *router, struct vr_packet *pkt,
...
static int vr_inet_proto_flow(struct vrouter *router, unsigned short vrf, struct vr_packet *pkt,
                              uint16_t vlan, struct vr_ip *ip, struct vr_flow *flow_p,
  ...
  if (proto == VR_IP_PROTO_ICMP) {
    ...
    if (vr_icmp_error(icmph)) {
      ...
      if (!icmp_error_payload) {
        ret = vr_inet_proto_flow(router, vrf, pkt, vlan, (struct vr_ip *)(icmph + 1), flow_p,
                                 valid_fkey_params);
        if (ret)
          return ret;
  ...
  } else if ((proto == VR_IP_PROTO_TCP) || (proto == VR_IP_PROTO_UDP) ||
  ...
  if (!icmp_error_payload) {
    fat_flow_mask =
        vr_flow_fat_flow_lookup(router, pkt, proto, sport, dport, &saddr, &daddr, NULL, NULL);
  ...
  nh_id = vr_inet_flow_nexthop(pkt, vlan);
  vr_inet_fill_flow(flow_p, nh_id, saddr, daddr, proto, sport, dport, valid_fkey_params);

  return 0;
}

int vr_inet_form_flow(struct vrouter *router, unsigned short vrf, struct vr_packet *pkt,
                      uint16_t vlan, struct vr_flow *flow_p, uint8_t valid_fkey_params,
                      bool frag_calc) {
  int ret;
  struct vr_ip *ip = (struct vr_ip *)pkt_network_header(pkt);
  ...
  if (vr_ip_transport_header_valid(ip)) {
    ...
    ret = vr_inet_fragment_flow(router, vrf, pkt, vlan, flow_p, valid_fkey_params, frag_calc);
  }

  return ret;
}

static bool vr_inet_should_trap(struct vr_packet *pkt, struct vr_flow *flow_p) {
  uint32_t proto_port;
  ...
  if (vif_is_virtual(pkt->vp_if) && vif_dhcp_enabled(pkt->vp_if)) {
    proto_port = (flow_p->flow4_proto << VR_FLOW_PROTO_SHIFT) | flow_p->flow4_sport;
...
int vr_inet_get_flow_key(struct vrouter *router, struct vr_packet *pkt,
  ...
  ret =
      vr_inet_form_flow(router, fmd->fmd_dvrf, pkt, fmd->fmd_vlan, flow, valid_fkey_params, false);
  if (ret < 0)
    return ret;

  ip = (struct vr_ip *)pkt_network_header(pkt);
  if (vr_ip_fragment_head(ip)) {
    vr_v4_fragment_add(router, fmd->fmd_dvrf, ip, flow->flow4_sport, flow->flow4_dport);
  }

  return 0;
}

flow_result_t vr_inet_flow_lookup(struct vrouter *router, struct vr_packet *pkt,
                                  struct vr_forwarding_md *fmd) {
  int ret;
  bool lookup = false;
  ...
  if (fmd->fmd_fe)
    ...
    ret =
        vr_inet_form_flow(router, fmd->fmd_dvrf, pkt, fmd->fmd_vlan, flow_p, VR_FLOW_KEY_ALL, true);
    if (ret < 0) {
  ...
  if (vr_ip_fragment_head(ip)) {
    vr_v4_fragment_add(router, fmd->fmd_dvrf, ip, flow_p->flow4_sport, flow_p->flow4_dport);
...
mac_response_t vm_arp_request(struct vr_interface *vif, struct vr_packet *pkt,
  ...
  *(uint32_t *)rt.rtr_req.rtr_prefix = (sarp->arp_dpa);
   ...
   */
  if (vif_is_vhost(vif) ||
      (vif_is_fabric(vif) && (fmd->fmd_label == -1) && (fmd->fmd_dvrf == vif->vif_vrf)))
    return MR_XCONNECT;
  ...
  if (pkt_is_gso(pkt)) {
    ...
    if (pkt->vp_type == VP_TYPE_IP) {
      ...
      if (ip6->ip6_nxt == VR_IP_PROTO_TCP)
        tcp = (struct vr_tcp *)((unsigned char *)ip6 + sizeof(struct vr_ip6));
    } else {
      return false;
    }
...
int vr_myip(struct vr_interface *vif, unsigned int ip) {
  struct vr_route_req rt;
  struct vr_nexthop *nh;
  uint32_t rt_prefix;
...
unsigned int vr_inet_route_flags(unsigned int vrf, unsigned int ip) {
  struct vr_route_req rt;
  uint32_t rt_prefix;
...
l4_pkt_type_t vr_ip_well_known_packet(struct vr_packet *pkt) {
  ...
  if ((iph->ip_proto == VR_IP_PROTO_UDP) && vr_ip_transport_header_valid(iph)) {
