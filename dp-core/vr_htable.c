...
#define VR_HENTRY_FLAG_UNDER_DELETION                                                              \
  (VR_HENTRY_FLAG_DELETE_MARKED | VR_HENTRY_FLAG_DELETE_PROCESSED)
...
int vr_htable_trav_range(vr_htable_t htable, unsigned int start, unsigned int range,
...
void vr_htable_trav(vr_htable_t htable, unsigned int marker, htable_trav_cb cb, void *data) {
  unsigned int range;
  struct vr_htable *table = (struct vr_htable *)htable;
...
static vr_hentry_t *vr_htable_get_free_oentry(struct vr_htable *table) {
  vr_hentry_t *ent;
  ...
  do {
    ...
    if (vr_sync_bool_compare_and_swap_p(&table->ht_free_oentry_head, ent, ent->hentry_next)) {
...
static void vr_htable_put_free_oentry(struct vr_htable *table, vr_hentry_t *ent) {
  ...
  do {
    ...
    if (vr_sync_bool_compare_and_swap_p(&table->ht_free_oentry_head, tmp, ent)) {
 ...
 */
vr_hentry_t *__vr_htable_get_hentry_by_index(vr_htable_t htable, unsigned int index) {
  struct vr_htable *table = (struct vr_htable *)htable;
...
static void vr_htable_oentry_invalidate(struct vr_htable *table, vr_hentry_t *ent) {
  if (!table || !ent || (ent->hentry_index < table->ht_hentries))
    return;
...
static void vr_htable_hentry_defer_delete(struct vrouter *router, void *arg) {
  vr_hentry_t *ent;
  ...
  ent = __vr_htable_get_hentry_by_index((vr_htable_t)table, defer_data->hd_index);
...
void vr_htable_hentry_scheduled_delete(void *arg) {
  ...
  head_ent = __vr_htable_get_hentry_by_index((vr_htable_t)(table), delete_data->hd_index);
  ...
  while (count && ent) {
    ...
    if (ent->hentry_flags & VR_HENTRY_FLAG_DELETE_MARKED) {
       ...
       */
      if (prev == head_ent) {
        if (!vr_sync_bool_compare_and_swap_p(&prev->hentry_next, ent, ent->hentry_next)) {
          prev = head_ent;
          ent = head_ent->hentry_next;
          continue;
        }
    ...
    if (ent->hentry_flags & VR_HENTRY_FLAG_DELETE_PROCESSED) {
      ...
      if (!vr_not_ready) {
        ...
        if (defer_data) {
          ...
          vr_defer(delete_data->hd_table->ht_router, vr_htable_hentry_defer_delete,
                   (void *)defer_data);
        }
      } else {
        vr_htable_oentry_invalidate(table, ent);
...
void vr_htable_release_hentry(vr_htable_t htable, vr_hentry_t *ent) {
  ...
  if (vr_sync_bool_compare_and_swap_16u(&delete_data->hd_scheduled, 0, 1)) {
    ...
    if (vr_schedule_work(cpu_num, vr_htable_hentry_scheduled_delete, (void *)delete_data)) {
      /*
...
vr_hentry_t *vr_htable_find_free_hentry(vr_htable_t htable, void *key, unsigned int key_size) {
  ...
  if (table->ht_oentries) {
    ...
    do {
      ...
      if (vr_sync_bool_compare_and_swap_p(&ent->hentry_next, o_ent->hentry_next, o_ent)) {
...
int vr_htable_find_duplicate_hentry_index(vr_htable_t htable, vr_hentry_t *hentry) {
...
vr_htable_t __vr_htable_create(struct vrouter *router, unsigned int entries, void *htable,
                               unsigned int oentries, void *otable, unsigned int entry_size,
                               unsigned int key_size, unsigned int bucket_size,
                               get_hentry_key get_entry_key) {
  int i;
  struct vr_htable *table;
  ...
  if (!table) {
    vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, sizeof(struct vr_htable));
    goto exit;
  }
  ...
  if (oentries) {
    ...
    table->ht_dtable = vr_btable_alloc(i, sizeof(struct vr_hentry_delete_data));
    if (!table->ht_dtable) {
...
vr_htable_t vr_htable_attach(struct vrouter *router, unsigned int entries, void *htable,
                             unsigned int oentries, void *otable, unsigned int entry_size,
                             unsigned int key_size, unsigned int bucket_size,
                             get_hentry_key get_entry_key) {
  ...
  return __vr_htable_create(router, entries, htable, oentries, otable, entry_size, key_size,
                            bucket_size, get_entry_key);
}
...
vr_htable_t vr_htable_create(struct vrouter *router, unsigned int entries, unsigned int oentries,
                             unsigned int entry_size, unsigned int key_size,
                             unsigned int bucket_size, get_hentry_key get_entry_key) {
  return __vr_htable_create(router, entries, NULL, oentries, NULL, entry_size, key_size,
                            bucket_size, get_entry_key);
}
