...
int vr_bridge_get_mac_type(char *mac) {
  int flags = 0;
  if (IS_MAC_BCAST(mac)) {
    flags |= MAC_BC_BIT_SET;
  } else if (IS_MAC_BMCAST(mac)) {
    flags |= MAC_MC_BIT_SET;
  } else {
    flags |= MAC_UC_BIT_SET;
  }

  return flags;
}

void *vr_bridge_get_va(struct vrouter *router, uint64_t offset) {
  return vr_htable_get_address(vn_rtable, offset);
}

unsigned int vr_bridge_table_size(struct vrouter *router) { return vr_htable_size(vn_rtable); }

bool vr_unknown_uc_flood(struct vr_interface *ingress_vif, struct vr_nexthop *ingress_nh) {
  if (!ingress_vif)
    return false;
...
struct vr_bridge_entry *vr_find_bridge_entry(struct vr_bridge_entry_key *key) {
  if (!vn_rtable || !key)
    return NULL;
...
struct vr_bridge_entry *vr_find_free_bridge_entry(unsigned int vrf_id, char *mac) {
  ...
  be = (struct vr_bridge_entry *)vr_htable_find_free_hentry(vn_rtable, &key, 0);
  return be;
}

static struct vr_bridge_entry *bridge_add(unsigned int router_id, unsigned int vrf, uint8_t *mac,
                                          int nh_id) {
  struct vr_bridge_entry *be;
  struct vr_bridge_entry_key key;
...
static int __bridge_table_add(struct vr_route_req *rt) {
  int ret;
  unsigned short flags, i;

  struct vr_bridge_entry *be;

  be = bridge_add(rt->rtr_req.rtr_rid, rt->rtr_req.rtr_vrf_id, rt->rtr_req.rtr_mac,
                  rt->rtr_req.rtr_nh_id);
  if (!be)
    return -ENOMEM;

  rt->rtr_req.rtr_label_flags = VR_BRIDGE_FLAG_MASK(rt->rtr_req.rtr_label_flags);
...
static int bridge_table_add(struct vr_rtable *_unused, struct vr_route_req *rt) {
  int ret;
...
static void bridge_table_entry_free(vr_htable_t table, vr_hentry_t *hentry, unsigned int index,
                                    void *data) {
...
static int bridge_table_delete(struct vr_rtable *_unused, struct vr_route_req *rt) {
  struct vr_bridge_entry_key key;
  struct vr_bridge_entry *be;
...
static void bridge_update_route_req(struct vr_bridge_entry *be, struct vr_route_req *rt) {
...
static struct vr_bridge_entry *__bridge_lookup(unsigned int vrf_id, struct vr_route_req *rt) {
  struct vr_bridge_entry *be;
  struct vr_bridge_entry_key key;
  ...
  if (rt->rtr_req.rtr_index != VR_BE_INVALID_INDEX) {
    be = (struct vr_bridge_entry *)vr_htable_get_hentry_by_index(vn_rtable, rt->rtr_req.rtr_index);
  } else {
    VR_MAC_COPY(key.be_mac, rt->rtr_req.rtr_mac);
    key.be_vrf_id = rt->rtr_req.rtr_vrf_id;
    be = vr_find_bridge_entry(&key);
...
static struct vr_nexthop *bridge_table_lookup(unsigned int vrf_id, struct vr_route_req *rt) {
  struct vr_bridge_entry *be;
...
struct vr_nexthop *vr_bridge_lookup(unsigned int vrf_id, struct vr_route_req *rt) {
  if (!vn_rtable)
    return NULL;
...
static struct vr_bridge_entry *bridge_lookup(uint8_t *mac, struct vr_forwarding_md *fmd) {
...
struct vr_nexthop *__vrouter_bridge_lookup(unsigned int vrf_id, unsigned char *mac) {
  struct vr_route_req rt;
...
int vr_bridge_set_route_flags(struct vr_bridge_entry *be, unsigned short flags) {
  ...
  if (be) {
    ...
    if (be_flags & VR_BE_VALID_FLAG) {
      ...
      be_flags = vr_sync_val_compare_and_swap_16u(&be->be_flags, be_flags, flags | be_flags);
...
static int bridge_table_get(unsigned int vrf_id, struct vr_route_req *rt) {
  struct vr_nexthop *nh;
...
static int bridge_entry_make_req(struct vr_route_req *resp, struct vr_bridge_entry *ent) {
...
static void bridge_entry_req_destroy(struct vr_route_req *resp) {
  if (resp->rtr_req.rtr_mac)
    vr_free(resp->rtr_req.rtr_mac, VR_ROUTE_REQ_MAC_OBJECT);
}

static int __bridge_table_dump(struct vr_message_dumper *dumper) {
  ...
  for (i = 0; i < (vr_bridge_entries + vr_bridge_oentries); i++) {
    be = (struct vr_bridge_entry *)vr_htable_get_hentry_by_index(vn_rtable, i);
    if (!be)
      continue;
...
static int bridge_table_dump(struct vr_rtable *__unsued, struct vr_route_req *rt) {
  int ret = 0;
...
vr_hentry_key bridge_entry_key(vr_htable_t table, vr_hentry_t *entry, unsigned int *key_len) {
  struct vr_bridge_entry *be = CONTAINER_OF(be_hentry, struct vr_bridge_entry, entry);
...
static void vr_bridge_table_data_destroy(vr_bridge_table_data *data) {
  if (data) {
...
static vr_bridge_table_data *vr_bridge_table_data_get(void) {
  vr_bridge_table_data *data = vr_zalloc(sizeof(vr_bridge_table_data), VR_BRIDGE_TABLE_DATA_OBJECT);
  if (!data)
    return NULL;

  if (vr_bridge_table_path) {
    data->btable_file_path = vr_zalloc(VR_UNIX_PATH_MAX, VR_BRIDGE_TABLE_DATA_OBJECT);
    if (!data->btable_file_path) {
      goto exit_func;
    }
  }
...
void vr_bridge_table_data_process(void *s_req) {
  int ret = 0;
...
#endif
    if (vr_bridge_table_path) {
      strncpy(resp->btable_file_path, vr_bridge_table_path, VR_UNIX_PATH_MAX - 1);
    }
    break;
...
int bridge_table_init(struct vr_rtable *rtable, struct rtable_fspec *fs) {
  ...
  if (!vr_bridge_table && vr_huge_page_mem_get) {
    vr_bridge_table = vr_huge_page_mem_get(VR_BRIDGE_TABLE_SIZE + VR_BRIDGE_OFLOW_TABLE_SIZE);
    if (vr_bridge_table)
      vr_bridge_otable = (unsigned char *)vr_bridge_table + VR_BRIDGE_TABLE_SIZE;
  }

  rtable->algo_data = vr_htable_attach(
      vrouter_get(0), vr_bridge_entries, vr_bridge_table, vr_bridge_oentries, vr_bridge_otable,
      sizeof(struct vr_bridge_entry), sizeof(struct vr_bridge_entry_key), 0, bridge_entry_key);

  if (!rtable->algo_data)
    return vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, vr_bridge_entries);
...
void bridge_table_deinit(struct vr_rtable *rtable, struct rtable_fspec *fs, bool soft_reset) {
  if (!vn_rtable)
    return;
...
static void bridge_table_unlock(struct vr_interface *vif, uint8_t *mac, int cpu) {
...
static int bridge_table_lock(struct vr_interface *vif, uint8_t *mac) {
...
mac_learn_t vr_bridge_learn(struct vrouter *router, struct vr_packet *pkt, struct vr_eth *eth,
  ...
  if (trap) {
    ...
    vr_trap(pkt_c, fmd->fmd_dvrf, trap_reason, (void *)&be->be_hentry.hentry_index);
  }

  return ml_res;
}

unsigned int vr_bridge_input(struct vrouter *router, struct vr_packet *pkt,
                             struct vr_forwarding_md *fmd) {
  int reason, handled;
  ...
  if ((pkt->vp_type == VP_TYPE_IP) || (pkt->vp_type == VP_TYPE_IP6)) {
    if (fmd->fmd_dscp < 0) {
      if (pkt->vp_type == VP_TYPE_IP) {
        fmd->fmd_dscp = vr_inet_get_tos((struct vr_ip *)pkt_network_header(pkt));
      } else if (pkt->vp_type == VP_TYPE_IP6) {
        fmd->fmd_dscp = vr_inet6_get_tos((struct vr_ip6 *)pkt_network_header(pkt));
      }
    }
  } else {
  ...
  if (!fmd->fmd_to_me) {
    if ((pkt->vp_if->vif_flags & VIF_FLAG_MAC_LEARN) ||
      ...
      ml_res = vr_bridge_learn(router, pkt, (struct vr_eth *)pkt_data(pkt), fmd);
  ...
  if (!fmd->fmd_to_me) {
    ...
    if (vif_is_virtual(pkt->vp_if)) {
       ...
       */
      if (!(mac_flags & MAC_BMC_BIT_SET) || (pkt->vp_if->vif_flags & VIF_FLAG_MAC_PROXY)) {
        handled = 0;
    ...
    if (!nh || nh->nh_type == NH_DISCARD) {
      ...
      if (!vr_unknown_uc_flood(pkt->vp_if, pkt->vp_nh) || (mac_flags & MAC_BMC_BIT_SET)) {
    ...
    if (nh->nh_flags & NH_FLAG_TUNNEL_PBB) {
      overlay_len += VR_ETHER_HLEN + sizeof(struct vr_vlan_hdr) + sizeof(struct vr_pbb_itag);
    }
  }
  ...
  if ((pkt->vp_type == VP_TYPE_IP) || (pkt->vp_type == VP_TYPE_IP6)) {
    ...
    if (vif_is_virtual(pkt->vp_if) && vr_from_vm_mss_adj && vr_pkt_from_vm_tcp_mss_adj) {
