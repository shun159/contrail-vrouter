...
static int vr_lib_create_timer(struct vr_timer *vtimer) {
...
static void vr_lib_delete_timer(struct vr_timer *vtimer) {
  vr_free(vtimer->vt_os_arg, VR_TIMER_OBJECT);
}

static void *vr_lib_page_alloc(unsigned int size) {
  int pages;
...
static void vr_lib_page_free(void *address, unsigned int size) {
  if (address)
    free(address);
}

static int vr_lib_printf(const char *format, ...) {
  int printed;
  va_list args;
...
static void *vr_lib_malloc(unsigned int size, unsigned int object) { return malloc(size); }

static void *vr_lib_zalloc(unsigned int size, unsigned int object) { return calloc(size, 1); }

static void vr_lib_free(void *mem, unsigned int object) {
  if (mem)
    free(mem);
  return;
}

static struct vr_packet *vr_lib_get_packet(struct vr_hpacket *hpkt, struct vr_interface *vif) {
...
static struct vr_packet *vr_lib_palloc(unsigned int size) {
...
static struct vr_packet *vr_lib_palloc_head(struct vr_packet *pkt, unsigned int size) {
...
static struct vr_packet *vr_lib_pclone(struct vr_packet *pkt) {
...
static void vr_lib_preset(struct vr_packet *pkt) {
  struct vr_hpacket *hpkt;
...
static void vr_lib_pfree(struct vr_packet *pkt, unsigned short reason) {
  struct vr_hpacket *hpkt;
...
static int vr_lib_pcopy(unsigned char *dst, struct vr_packet *p_src, unsigned int offset,
                        unsigned int len) {
...
static unsigned short vr_lib_pfrag_len(struct vr_packet *pkt) {
  struct vr_hpacket *hpkt;
...
static void vr_lib_get_time(uint64_t *sec, uint64_t *usec) {
  struct timeval tv;
...
static unsigned int vr_lib_get_cpu(void) { return 0; }

static int vr_lib_schedule_work(unsigned int cpu, void (*fn)(void *), void *arg) {
  return -EOPNOTSUPP;
}

static void vr_lib_delay_op(void) { return; }
...
static int vr_message_init(unsigned int message_proto) {
  int ret = 0;
...
int vrouter_host_init(unsigned int message_proto) {
  int ret;
