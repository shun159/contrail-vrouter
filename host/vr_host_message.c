...
struct diet_object_md {
  ...
  int (*obj_response)(struct diet_message *, void *, int (*)(void *, unsigned int, void *), void *);
};
...
int diet_object_response(struct diet_message *, void *, int (*)(void *, unsigned int, void *),
                         void *);
unsigned int diet_object_buf_len(unsigned int, void *);

struct diet_object_md diet_md[] = {[VR_NULL_OBJECT_ID] =
                                       {
                                           .obj_len = 0,
                                       },
                                   [VR_INTERFACE_OBJECT_ID] =
                                       {
                                           .obj_len = sizeof(vr_interface_req) + VR_ETHER_ALEN,
                                   ...
                                   [VR_NEXTHOP_OBJECT_ID] =
                                       {
                                           .obj_len = sizeof(vr_nexthop_req) + VR_ETHER_HLEN,
...
int diet_dropstats_object_copy(char *dst, unsigned int buf_len, void *object) {
...
int diet_route_object_copy(char *dst, unsigned int buf_len, void *object) {
...
int diet_mpls_object_copy(char *dst, unsigned int buf_len, void *object) {
...
int diet_nexthop_object_copy(char *dst, unsigned int buf_len, void *object) {
  ...
  if (src->nhr_encap_size) {
    ...
    memcpy(tmp->nhr_encap, src->nhr_encap, src->nhr_encap_size);
  }

  return total_len;
}

int diet_interface_object_copy(char *dst, unsigned int buf_len, void *object) {
  ...
  if (src->vifr_mac_size) {
    ...
    memcpy(tmp->vifr_mac, src->vifr_mac, src->vifr_mac_size);
  }

  return total_len;
}

int diet_response_object_copy(char *dst, unsigned int len, void *object) {
...
int diet_object_response(struct diet_message *hdr, void *object,
  ...
  ret = diet_md[hdr->m_oid].obj_copy(dst_buf, diet_md[hdr->m_oid].obj_len, object);
...
static int diet_object_copy(char *dst, unsigned int len, unsigned int object_type, void *src) {
  return diet_md[object_type].obj_copy(dst, len, src);
}

static int diet_encode(char *buf, unsigned int len, unsigned int object_type, void *object,
  ...
  encode_length = diet_object_copy((char *)(hdr + 1), len - sizeof(hdr), object_type, object);
  encode_length += sizeof(*hdr);

  return encode_length;
}

static int diet_encode_response(char *buf, unsigned int len, unsigned int object_type, void *object,
                                int ret) {
  ...
  ret = diet_encode(buf, len, VR_RESPONSE_OBJECT_ID, &resp, VR_MESSAGE_TYPE_RESPONSE);
  if (ret < 0)
    return ret;
  ...
  if (object) {
    ret = diet_encode(buf + ret, len - ret, object_type, object, VR_MESSAGE_TYPE_RESPONSE);
    if (ret < 0)
      return ret;
...
static int diet_object_decode(struct diet_message *hdr, int (*cb)(void *, unsigned int, void *),
                              void *cb_arg) {
  int ret = 0;
...
static int diet_decode(char *buf, unsigned int len, int (*cb)(void *, unsigned int, void *),
                       void *cb_arg) {
  int ret, processed;
  struct diet_message *hdr;
 ...
 */
unsigned int diet_object_buf_len(unsigned int obj_type, void *object) {
  unsigned int len;
...
void vr_diet_message_proto_exit(void) {
  vr_message_proto_unregister(&diet_message_proto);
  return;
}

int vr_diet_message_proto_init(void) {
