...
static void vr_netif_rx(struct vr_hinterface *hif, struct vr_hpacket *hpkt) {
...
static int hif_udp_rx(void *arg) {
  int ret = 0;
...
static unsigned int hif_udp_tx(struct vr_hinterface *hif, struct vr_hpacket *hpkt) {
  int i = 0;
  struct msghdr msg;
...
int vr_hif_udp_create(struct vr_hinterface *hif, unsigned int vif_type) {
  int sock = -1, port, ret;
  ...
  memset(&sock_addr, 0, sizeof(sock_addr));
  sock_addr.sin_family = AF_INET;
  sock_addr.sin_port = htons(port - HIF_SOURCE_UDP_PORT_START + HIF_DESTINATION_UDP_PORT_START);
...
static void vr_hif_udp_destroy(struct vr_hinterface *hif) {
  struct hif_interface_md *hif_info;
...
struct vr_hinterface *vr_hinterface_create(unsigned int index, unsigned int hif_type,
...
void vr_hinterface_destroy(struct vr_hinterface *hif) {
  switch (hif->hif_type) {
  case HIF_TYPE_UDP:
...
struct vr_hinterface *vr_hinterface_get(unsigned int index) {
  struct vr_hinterface *hif;
...
void vr_hinterface_put(struct vr_hinterface *hif) {
  /*
...
void vr_hinterface_delete(struct vr_hinterface *hif) {
...
static int vr_lib_interface_rx(struct vr_interface *vif, struct vr_packet *pkt) { return 0; }

static int vr_lib_interface_tx(struct vr_interface *vif, struct vr_packet *pkt) {
...
static int vr_lib_interface_del_tap(struct vr_interface *vif) { return 0; }

static int vr_lib_interface_add_tap(struct vr_interface *vif) { return 0; }

static int vr_lib_interface_del(struct vr_interface *vif) {
  struct vr_hinterface *hif = (struct vr_hinterface *)vif->vif_os;
...
static int vr_lib_interface_add(struct vr_interface *vif) {
  struct vr_hinterface *hif;
