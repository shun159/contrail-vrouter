...
void vr_free_req(void *req) {
  vr_mtrans_free(req);
  return;
}

int vr_process_response(void *arg, unsigned int obj_type, void *object) {
  ...
  vr_queue_enqueue(&request_i->req_responses, &response_i->resp_queue_elem);
 ...
 * the operation
 */
int vr_send(unsigned int obj_type, void *object, unsigned int len) {
