...
enable_tap:
  ...
  if (vif_is_vhost(vif) && vr_dpdk.tap_nl_fd <= 0) {
    ...
    if (bind(vr_dpdk.tap_nl_fd, (struct sockaddr *)&tap_nl_addr, sizeof(tap_nl_addr)) < 0) {
...
int vr_dpdk_tapdev_release(struct vr_interface *vif) {
  ...
  if (tapdev->tapdev_rx_ring) {
    while (rte_ring_sc_dequeue(tapdev->tapdev_rx_ring, (void **)&mbuf) == 0) {
  ...
  RTE_LCORE_FOREACH(lcore_id) {
    if (tapdev->tapdev_tx_rings[lcore_id]) {
      while (rte_ring_sc_dequeue(tapdev->tapdev_tx_rings[lcore_id], (void **)&mbuf) == 0) {
        rte_pktmbuf_free(mbuf);
      }
    }
  }
 ...
 */
static void dpdk_tapdev_rx_queue_release(unsigned lcore_id,
...
struct vr_dpdk_queue *vr_dpdk_tapdev_rx_queue_init(unsigned lcore_id, struct vr_interface *vif,
  ...
  if (tapdev->tapdev_rx_ring == NULL) {
    tapdev->tapdev_rx_ring = vr_dpdk_ring_allocate(lcore_id, "tapdev_rx_ring", VR_DPDK_RX_RING_SZ,
...
error:
  RTE_LOG(ERR, VROUTER, "    error initializing tapdev %s RX queue\n", vif->vif_name);
  return NULL;
}
 ...
 */
static void dpdk_tapdev_tx_queue_release(unsigned lcore_id, unsigned queue_index,
                                         struct vr_interface *vif) {
  struct vr_dpdk_lcore *lcore = vr_dpdk.lcores[lcore_id];
...
struct vr_dpdk_queue *vr_dpdk_tapdev_tx_queue_init(unsigned lcore_id, struct vr_interface *vif,
  ...
  if (tapdev->tapdev_tx_rings[lcore_id] == NULL) {
    tapdev->tapdev_tx_rings[lcore_id] = vr_dpdk_ring_allocate(
        lcore_id, "tapdev_tx_ring", VR_DPDK_TX_RING_SZ, RING_F_SP_ENQ | RING_F_SC_DEQ);
    if (tapdev->tapdev_tx_rings[lcore_id] == NULL)
      goto error;
  }
...
error:
  RTE_LOG(ERR, VROUTER, "    error initializing tapdev %s TX queue\n", vif->vif_name);
  return NULL;
}
 ...
 */
unsigned vr_dpdk_tapdev_rx_burst(struct vr_dpdk_tapdev *tapdev, struct rte_mbuf **mbufs,
  ...
  for (i = 0; i < num; i++) {
    ...
    len = read(fd, rte_pktmbuf_mtod(mbuf, void *), (mbuf->buf_len - rte_pktmbuf_headroom(mbuf)));
    if (unlikely(len <= 0)) {
 ...
 */
unsigned vr_dpdk_tapdev_dequeue_burst(struct vr_dpdk_tapdev *tapdev, struct rte_mbuf **mbufs,
                                      unsigned num) {
  int fd;
  ...
  if (likely(tapdev->tapdev_rx_ring != NULL)) {
    ...
    return rte_ring_sc_dequeue_burst(tapdev->tapdev_rx_ring, (void **)mbufs, num, NULL);
#else
    return rte_ring_sc_dequeue_burst(tapdev->tapdev_rx_ring, (void **)mbufs, num);
#endif
  }
  return 0;
}
 ...
 */
unsigned vr_dpdk_tapdev_tx_burst(struct vr_dpdk_tapdev *tapdev, struct rte_mbuf **mbufs,
                                 unsigned num, int datapath) {
  int i, fd;
  unsigned ret = 0;
  struct rte_mbuf *mbuf;
  ...
  for (i = 0; i < num; i++) {
    ...
    len = write(fd, rte_pktmbuf_mtod(mbufs[i], void *), rte_pktmbuf_data_len(mbuf));
 ...
 */
unsigned vr_dpdk_tapdev_enqueue_burst(struct vr_dpdk_tapdev *tapdev, struct rte_mbuf **mbufs,
                                      unsigned num) {
  int fd;
  unsigned lcore_id = rte_lcore_id();
...
#if (RTE_VERSION >= RTE_VERSION_NUM(17, 11, 0, 0))
    return rte_ring_sp_enqueue_burst(tapdev->tapdev_tx_rings[lcore_id], (void **)mbufs, num, NULL);
#else
    return rte_ring_sp_enqueue_burst(tapdev->tapdev_tx_rings[lcore_id], (void **)mbufs, num);
#endif
  }
...
uint64_t vr_dpdk_tapdev_rxtx(void) {
  ...
  for (i = 0; i < VR_DPDK_MAX_TAP_INTERFACES; i++) {
    ...
    if (fd > 0) {
      ...
      /* Try to RX from the TAP. */
      if (likely(tapdev->tapdev_rx_ring != NULL)) {
        nb_pkts = vr_dpdk_tapdev_rx_burst(tapdev, &mbuf, 1, VR_DPDK_MGMTPATH);

        if (likely(nb_pkts > 0)) {
          total_pkts++;
          if (unlikely(rte_ring_sp_enqueue(tapdev->tapdev_rx_ring, mbuf) != 0)) {
            rte_pktmbuf_free(mbuf);
      ...
      RTE_LCORE_FOREACH(lcore_id) {
        if (likely(tapdev->tapdev_tx_rings[lcore_id] != NULL)) {
          if (likely(rte_ring_sc_dequeue(tapdev->tapdev_tx_rings[lcore_id], (void **)&mbuf) == 0)) {
            total_pkts++;
            nb_pkts = vr_dpdk_tapdev_tx_burst(tapdev, &mbuf, 1, VR_DPDK_MGMTPATH);
  ...
  if (vhost_mtu != mtu) {
    ...
    if (ethdev->ethdev_nb_slaves > 0) {
      ...
      if (ret < 0) {
         ...
         * set to support jumbo frames, so continue further here to set vif_mtu.
         */
        RTE_LOG(DEBUG, VROUTER, "Error changing eth device MTU: %s (%d)\n", rte_strerror(-ret),
                -ret);
      }
    }
  ...
  for (h = (struct nlmsghdr *)buf; NLMSG_OK(h, (unsigned int)status); h = NLMSG_NEXT(h, status)) {
    ...
    if (h->nlmsg_type == RTM_NEWLINK) {
      ...
      if (ifname && (strncmp(ifname, VHOST_IFNAME, (strlen(VHOST_IFNAME) + 1)) == 0)) {
        ...
        vr_dpdk_handle_vhost0_notification(mtu, (ifi->ifi_flags & IFF_UP));
      }
    }
  }
}
