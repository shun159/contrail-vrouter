...
static int vr_hugepage_info_init(void) {
  ...
  while (fgets(line, MAX_LINE_SIZE, fp)) {
    ...
    if ((str = strstr(line, "hugetlbfs"))) {
      ...
      if (strstr(options, "pagesize=1G") || strstr(options, "pagesize=1024M")) {
        ...
        sys_hp_file = "/sys/kernel/mm/hugepages/hugepages-1048576kB/nr_hugepages";
      } else {
        hp->page_size = (2 * 1024 * 1024);
        sys_hp_file = "/sys/kernel/mm/hugepages/hugepages-2048kB/nr_hugepages";
      }
...
int vr_dpdk_table_mem_init(unsigned int table, unsigned int entries, unsigned long size,
                           unsigned int oentries, unsigned long osize) {
  int ret, i, fd;
  ...
  if (no_huge_set) {
    ...
    ret = snprintf(shm_file, VR_UNIX_PATH_MAX, "%s/%s", vr_socket_dir, shmem_name);
  ...
  if (touse_file_name) {
    ...
    *table_p = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
