 ...
 * @{
 */
static void dpdk_fragment_assemble_queue(void *arg) {
...
static void dpdk_assembler_table_exit(void) {
  int i;
...
static int dpdk_assembler_table_init(void) {
  unsigned int size;
  int i;
...
static int dpdk_fragment_queue_init(void) {
  unsigned int size;
...
static void dpdk_fragment_queue_exit(void) {
  int i;
...
int dpdk_fragment_assembler_enqueue(struct vrouter *router, struct vr_packet *pkt,
  ...
  if (cpu >= vr_num_cpus || cpu < VR_DPDK_FWD_LCORE_ID) {
    RTE_LOG(ERR, VROUTER,
            "%s:%d Enqueue to the assembler can only be "
  ...
  ret = vr_fragment_enqueue(router, &per_cpu_queues[cpu - VR_DPDK_FWD_LCORE_ID].queue, pkt, fmd);

  if (!ret) {
    lcore = vr_dpdk.lcores[cpu];
    vr_dpdk_lcore_schedule_assembler_work(lcore, dpdk_fragment_assemble_queue,
 ...
 */
void dpdk_fragment_assembler_table_scan(void *arg) {
