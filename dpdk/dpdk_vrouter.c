...
static char *dpdk_argv[] = {
    "dpdk", "-n", VR_DPDK_MAX_MEMCHANNELS, "-m", VR_DPDK_DEF_MEM,
...
void vr_dpdk_pktmbuf_pool_init(struct rte_mempool *mp, void *opaque_arg) {
  ...
  priv.mbuf_data_room_size = mp->elt_size - sizeof(struct rte_mbuf) - sizeof(struct vr_packet);
  priv.mbuf_priv_size = sizeof(struct vr_packet);

  rte_pktmbuf_pool_init(mp, &priv);
#else
  rte_pktmbuf_pool_init(
      mp, (void *)(mp->elt_size - sizeof(struct rte_mbuf) - sizeof(struct vr_packet)));
#endif
}
...
/* Create memory pools */
static int dpdk_mempools_create(void) {
  ...
  vr_dpdk.rss_mempool = rte_mempool_create(
      "rss_mempool", rss_mempool_sz, VR_DPDK_MBUF_HDR_SZ + vr_packet_sz,
      VR_DPDK_RSS_MEMPOOL_CACHE_SZ, sizeof(struct rte_pktmbuf_pool_private),
      vr_dpdk_pktmbuf_pool_init, NULL, vr_dpdk_pktmbuf_init, NULL, rte_socket_id(), 0);
  ...
  vr_dpdk.frag_direct_mempool = rte_mempool_create(
      "frag_direct_mempool", frag_direct_mempool_sz, VR_DPDK_FRAG_DIRECT_MBUF_SZ,
      VR_DPDK_FRAG_DIRECT_MEMPOOL_CACHE_SZ, sizeof(struct rte_pktmbuf_pool_private),
  ...
  vr_dpdk.frag_indirect_mempool =
      rte_mempool_create("frag_indirect_mempool", frag_indirect_mempool_sz,
                         VR_DPDK_FRAG_INDIRECT_MBUF_SZ, VR_DPDK_FRAG_INDIRECT_MEMPOOL_CACHE_SZ, 0,
                         NULL, NULL, rte_pktmbuf_init, NULL, rte_socket_id(), 0);
  ...
  for (i = 0; i < VR_DPDK_MAX_VM_MEMPOOLS; i++) {
    ...
    vr_dpdk.free_mempools[i] = rte_mempool_create(
        mempool_name, vm_mempool_sz, VR_DPDK_MBUF_HDR_SZ + vr_packet_sz,
        VR_DPDK_VM_MEMPOOL_CACHE_SZ, sizeof(struct rte_pktmbuf_pool_private),
        vr_dpdk_pktmbuf_pool_init, NULL, vr_dpdk_pktmbuf_init, NULL, rte_socket_id(), 0);
 ...
 */
static cpu_set_t dpdk_core_mask_get(long system_cpus_count) {
  cpu_set_t cs;
  ...
  if (core_mask_count == system_cpus_count) {
    RTE_LOG(NOTICE, VROUTER,
            "Use taskset(1) to set the core mask."
            " Falling back do the default core mask 0x%" PRIx64 "\n",
            (uint64_t)(VR_DPDK_DEF_LCORE_MASK));
    return def_cpu_core_mask;
  }
 ...
 */
static char *dpdk_shared_io_core_mask_stringify(cpu_set_t core_set_mask) {
  int cpu_id = 0;
  ...
  while (cpu_id < CPU_SETSIZE) {
    if (CPU_ISSET(cpu_id, &core_set_mask)) {
      ...
      iop += snprintf(iop, sizeof(io_cpus_string) - (iop - io_cpus_string), "%d", cpu_id);
      ...
      nb_fwd_cores++;
      if (nb_fwd_cores >= VR_DPDK_FWD_LCORES_PER_IO || cpu_id + 1 >= CPU_SETSIZE) {
        if (io_lcore_id > VR_DPDK_LAST_IO_LCORE_ID) {
          RTE_LOG(WARNING, VROUTER, "Warning: IO lcores limit exceeded (%d > %d)\n", io_lcore_id,
                  VR_DPDK_LAST_IO_LCORE_ID);
          break;
        }

        p += snprintf(p, sizeof(core_mask_string) - (p - core_mask_string), "%d@(%s),", io_lcore_id,
                      io_cpus_string);
 ...
 */
static char *dpdk_fwd_core_mask_stringify(cpu_set_t core_set_mask) {
  int cpu_id = 0;
  ...
  while (cpu_id < CPU_SETSIZE) {
    if (CPU_ISSET(cpu_id, &core_set_mask)) {
      ...
      if (nb_fwd_cores == 0 && io_lcore_id == VR_DPDK_LAST_IO_LCORE_ID + 1) {
        RTE_LOG(WARNING, VROUTER, "Warning: IO lcores limit exceeded (%d > %d)\n", io_lcore_id,
                VR_DPDK_LAST_IO_LCORE_ID);
        io_lcore_id++;
      }
      if (VR_DPDK_USE_IO_LCORES && !VR_DPDK_SHARED_IO_LCORES && nb_fwd_cores == 0 &&
          io_lcore_id <= VR_DPDK_LAST_IO_LCORE_ID) {
        /* first dedicated CPU is an IO lcore */
        p += snprintf(p, sizeof(core_mask_string) - (p - core_mask_string), "%d@%d", io_lcore_id,
                      cpu_id);
        if (p - core_mask_string >= sizeof(core_mask_string)) {
          RTE_LOG(ERR, VROUTER, "Error stringifying IO core mask: buffer overflow\n");
          return NULL;
        ...
        /* forwarding lcore */
        p += snprintf(p, sizeof(core_mask_string) - (p - core_mask_string), "%d@%d", fwd_lcore_id,
                      cpu_id);
        if (p - core_mask_string >= sizeof(core_mask_string)) {
 ...
 */
static int dpdk_argv_remove(char *arg) {
  int i;
  int len = strlen(arg);
 ...
 * Returns 0 on success, < 0 otherwise.
 */
static int dpdk_argv_append(char *arg, char *val) {
  int i;
 ...
 */
static int dpdk_argv_update(void) {
  ...
  if (system_cpus_count == -1) {
    system_cpus_count = __builtin_popcountll((unsigned long long)VR_DPDK_DEF_LCORE_MASK);
  }
  ...
  if (VR_DPDK_USE_IO_LCORES) {
    if (VR_DPDK_SHARED_IO_LCORES) {
      vr_dpdk.nb_io_lcores =
          (vr_dpdk.nb_fwd_lcores + VR_DPDK_FWD_LCORES_PER_IO - 1) / VR_DPDK_FWD_LCORES_PER_IO;
      if (vr_dpdk.nb_io_lcores > VR_DPDK_MAX_IO_LORES)
        vr_dpdk.nb_io_lcores = VR_DPDK_MAX_IO_LORES;
    } else {
      vr_dpdk.nb_io_lcores =
          (vr_dpdk.nb_fwd_lcores + VR_DPDK_FWD_LCORES_PER_IO) / (VR_DPDK_FWD_LCORES_PER_IO + 1);
      if (vr_dpdk.nb_io_lcores > VR_DPDK_MAX_IO_LORES)
        vr_dpdk.nb_io_lcores = VR_DPDK_MAX_IO_LORES;
  ...
  if (vr_dpdk.nb_fwd_lcores > VR_MAX_CPUS - VR_DPDK_FWD_LCORE_ID) {
    RTE_LOG(ERR, VROUTER,
            "Error configuring lcores: number of forwarding cores exceeds maximum of %u\n",
  ...
  if (snprintf(lcores_string, sizeof(lcores_string), "(0-%d)@(0-%ld),%s(%d-%d)@(0-%ld),%s",
               VR_DPDK_IO_LCORE_ID - 1, system_cpus_count - 1, io_core_mask_str,
               VR_DPDK_PACKET_LCORE_ID, VR_DPDK_FWD_LCORE_ID - 1, system_cpus_count - 1,
               fwd_core_mask_str) >= sizeof(lcores_string)) {
    return -1;
  }
...
static void dpdk_check_sriov_vf(void) {
  ...
  VR_DPDK_RTE_ETH_FOREACH_DEV(i) {
    ...
    if (soff > 0 &&
        strncmp(dev_info.driver_name + soff, VR_DPDK_VF_PMD_SFX, sizeof(VR_DPDK_VF_PMD_SFX)) == 0) {
      ...
      if (dev_info.max_tx_queues < vr_dpdk.nb_fwd_lcores
        ...
        RTE_LOG(INFO, VROUTER, "%s: Lcore %d: SR-IOV virtual function IO for eth device %d (%s)\n",
...
static int dpdk_init(void) {
  ...
  ret = vr_dpdk_table_mem_init(VR_MEM_FLOW_TABLE_OBJECT, vr_flow_entries, VR_FLOW_TABLE_SIZE,
                               vr_oflow_entries, VR_OFLOW_TABLE_SIZE);
  if (ret < 0) {
    RTE_LOG(ERR, VROUTER, "Error initializing flow table: %s (%d)\n", rte_strerror(-ret), -ret);
    return ret;
  }

  ret = vr_dpdk_table_mem_init(VR_MEM_BRIDGE_TABLE_OBJECT, vr_bridge_entries, VR_BRIDGE_TABLE_SIZE,
                               vr_bridge_oentries, VR_BRIDGE_OFLOW_TABLE_SIZE);
  if (ret < 0) {
  ...
  RTE_LOG(INFO, VROUTER, "Using %d service lcores\n",
          rte_lcore_count() - vr_dpdk.nb_fwd_lcores - vr_dpdk.nb_io_lcores);
...
static void dpdk_stop_flag_set(void) {
...
static void dpdk_signal_handler_stop(int signum) {
  RTE_LOG(INFO, VROUTER, "Got signal %d on lcore %u, stopping...\n", signum, rte_lcore_id());

  dpdk_stop_flag_set();
}
static void dpdk_signal_handler_ignore(int signum) {
  RTE_LOG(INFO, VROUTER, "Got signal %d on lcore %u, ignoring...\n", signum, rte_lcore_id());
}

/* Setup signal handlers */
static int dpdk_signals_init(void) {
...
static struct option long_options[] = {
    ...
    [HELP_OPT_INDEX] = {HELP_OPT, no_argument, NULL, 0},
    [VERSION_OPT_INDEX] = {VERSION_OPT, no_argument, NULL, 0},
    [MEMPOOL_SIZE_OPT_INDEX] = {MEMPOOL_SIZE_OPT, required_argument, NULL, 0},
    [DPDK_TXD_SIZE_OPT_INDEX] = {DPDK_TXD_SIZE_OPT, required_argument, NULL, 0},
    [DPDK_RXD_SIZE_OPT_INDEX] = {DPDK_RXD_SIZE_OPT, required_argument, NULL, 0},
    [PACKET_SIZE_OPT_INDEX] = {PACKET_SIZE_OPT, required_argument, NULL, 0},
    [VLAN_TCI_OPT_INDEX] = {VLAN_TCI_OPT, required_argument, NULL, 0},
    [VLAN_NAME_OPT_INDEX] = {VLAN_NAME_OPT, required_argument, NULL, 0},
    [LOG_FILE_OPT_INDEX] = {LOG_FILE_OPT, required_argument, NULL, 0},
    [VTEST_VLAN_OPT_INDEX] = {VTEST_VLAN_OPT, no_argument, NULL, 0},
    [VDEV_OPT_INDEX] = {VDEV_OPT, required_argument, NULL, 0},
    [NO_GRO_OPT_INDEX] = {NO_GRO_OPT, no_argument, &no_gro_set, 1},
    [NO_GSO_OPT_INDEX] = {NO_GSO_OPT, no_argument, &no_gso_set, 1},
    [NO_RX_MRG_BUF_INDEX] = {NO_RX_MRG_BUF_OPT, no_argument, &no_rx_mrgbuf, 1},
    [BRIDGE_ENTRIES_OPT_INDEX] = {BRIDGE_ENTRIES_OPT, required_argument, NULL, 0},
    [BRIDGE_OENTRIES_OPT_INDEX] = {BRIDGE_OENTRIES_OPT, required_argument, NULL, 0},
    ...
    [MPLS_LABELS_OPT_INDEX] = {MPLS_LABELS_OPT, required_argument, NULL, 0},
    [NEXTHOPS_OPT_INDEX] = {NEXTHOPS_OPT, required_argument, NULL, 0},
    [VRFS_OPT_INDEX] = {VRFS_OPT, required_argument, NULL, 0},
    [SOCKET_DIR_OPT_INDEX] = {SOCKET_DIR_OPT, required_argument, NULL, 0},
    [NETLINK_PORT_OPT_INDEX] = {NETLINK_PORT_OPT, required_argument, NULL, 0},
    [SOCKET_MEM_OPT_INDEX] = {SOCKET_MEM_OPT, required_argument, NULL, 0},
    [OFFLOADS_OPT_INDEX] = {OFFLOADS_OPT, no_argument, NULL, 0},
    [PKT_DROP_LOG_BUFFER_SIZE_OPT_INDEX] = {PKT_DROP_LOG_BUFFER_SIZE_OPT, required_argument, NULL,
                                            0},
    [MEMORY_ALLOC_CHECKS_OPT_INDEX] = {MEMORY_ALLOC_CHECKS_OPT, no_argument, NULL, 0},
    [MAX_OPT_INDEX] = {NULL, 0, NULL, 0},
};

static void Usage(void) {
  printf("Usage: contrail-vrouter-dpdk [options]\n"
...
static void parse_long_opts(int opt_flow_index, char *optarg) {
  errno = 0;
...
static ssize_t timestamp_log_write(void *c, const char *buf, size_t size) {
  ssize_t ret;
  struct timeval tv;
  struct tm *tm;
...
int main(int argc, char *argv[]) {
  ...
  vr_dpdk.vlan_tag = VLAN_ID_INVALID;
  strncpy(vr_dpdk.vlan_name, VR_DPDK_VLAN_FWD_DEF_NAME, sizeof(vr_dpdk.vlan_name) - 1);

  rte_openlog_stream(stdout);

  while ((opt = getopt_long(argc, argv, "", long_options, &option_index)) >= 0) {
    switch (opt) {
    case 0:
