...
struct rte_eth_conf ethdev_conf = {
    ...
    .rxmode =
        {
            ...
            .offloads =
                DEV_RX_OFFLOAD_CHECKSUM | DEV_RX_OFFLOAD_JUMBO_FRAME | DEV_RX_OFFLOAD_CRC_STRIP,
...
#if (RTE_VERSION >= RTE_VERSION_NUM(18, 05, 0, 0))
            .offloads =
                DEV_TX_OFFLOAD_UDP_CKSUM | DEV_TX_OFFLOAD_TCP_CKSUM | DEV_TX_OFFLOAD_IPV4_CKSUM,
#endif
    ...
    .intr_conf =
        {
            .lsc = 1, /* Enable Link status interrupts */
        },
};
...
#if (RTE_VERSION >= RTE_VERSION_NUM(18, 05, 0, 0))
    .offloads = DEV_RX_OFFLOAD_CHECKSUM | DEV_RX_OFFLOAD_JUMBO_FRAME | DEV_RX_OFFLOAD_CRC_STRIP,
#endif
};
...
#if VR_DPDK_USE_HW_FILTERING
/* Add hardware filter */
int vr_dpdk_ethdev_filter_add(struct vr_interface *vif, uint16_t queue_id, unsigned dst_ip,
                              unsigned mpls_label) {
  ...
  if (queue_id >= 0xFF) {
    RTE_LOG(ERR, VROUTER,
            "    error adding perfect filter for eth device %" PRIu8 ": queue ID %" PRIu16
            " is out of range\n",
            port_id, queue_id);
    return -EINVAL;
  }
...
static void dpdk_ethdev_rx_queue_release(unsigned lcore_id,
...
/* Init eth RX queue */
struct vr_dpdk_queue *vr_dpdk_ethdev_rx_queue_init(unsigned lcore_id, struct vr_interface *vif,
  ...
  if (rx_queue->q_queue_h == NULL) {
    RTE_LOG(ERR, VROUTER, "    error creating eth device %" PRIu8 " RX queue %" PRIu16 "\n",
            port_id, rx_queue_id);
    return NULL;
  }
...
static void dpdk_ethdev_tx_queue_release(unsigned lcore_id, unsigned queue_index,
                                         struct vr_interface *vif) {
  int i;
  ...
  tx_queue_params = &lcore->lcore_tx_queue_params[vif->vif_idx][queue_index];
...
/* Init eth TX queue */
struct vr_dpdk_queue *vr_dpdk_ethdev_tx_queue_init(unsigned lcore_id, struct vr_interface *vif,
                                                   unsigned queue_or_lcore_id) {
  uint8_t port_id;
  ...
  if (lcore->lcore_hw_queue_to_dpdk_index[vif->vif_idx]) {
    dpdk_queue_index = lcore->lcore_hw_queue_to_dpdk_index[vif->vif_idx][tx_queue_id];
  } else {
    dpdk_queue_index = 0;
  }
  ...
  if (tx_queue->q_queue_h == NULL) {
    RTE_LOG(ERR, VROUTER, "    error creating eth device %" PRIu8 " TX queue %" PRIu16 "\n",
            port_id, tx_queue_id);
    return NULL;
  }
 ...
 */
static void vr_max_tx_queues_adjust(struct vr_dpdk_ethdev *ethdev, uint16_t *nb_tx_q) {
...
/* Update device info */
static void dpdk_ethdev_info_update(struct vr_dpdk_ethdev *ethdev) {
  struct rte_eth_dev_info dev_info;

  rte_eth_dev_info_get(ethdev->ethdev_port_id, &dev_info);

  ethdev->ethdev_nb_rx_queues = RTE_MIN(dev_info.max_rx_queues, VR_DPDK_MAX_NB_RX_QUEUES);
  ...
  ethdev->ethdev_nb_rss_queues = RTE_MIN(
      RTE_MIN(ethdev->ethdev_nb_rx_queues, vr_dpdk.nb_fwd_lcores), VR_DPDK_MAX_NB_RSS_QUEUES);
  ...
  RTE_LOG_DP(DEBUG, VROUTER,
             "dev_info: driver_name=%s if_index=%u"
             " max_rx_queues=%" PRIu16 " max_tx_queues=%" PRIu16 " max_vfs=%" PRIu16
             " max_vmdq_pools=%" PRIu16
#if (RTE_VERSION >= RTE_VERSION_NUM(17, 11, 0, 0))
             ...
             dev_info.max_tx_queues, dev_info.max_vfs, dev_info.max_vmdq_pools,
...
static int dpdk_ethdev_queues_setup(struct vr_dpdk_ethdev *ethdev) {
  int ret, i;
  uint8_t port_id = ethdev->ethdev_port_id;
  ...
  RTE_LOG_DP(DEBUG, VROUTER, "%s: nb_rx_queues=%u nb_tx_queues=%u\n", __func__,
             (unsigned)ethdev->ethdev_nb_rx_queues, (unsigned)ethdev->ethdev_nb_tx_queues);
  ...
  for (i = 0; i < VR_DPDK_MAX_NB_RX_QUEUES; i++) {
    if (i < ethdev->ethdev_nb_rss_queues) {
      ...
      if (vr_dpdk.nb_free_mempools == 0) {
        RTE_LOG(ERR, VROUTER, "    error assigning mempool to eth device %" PRIu8 " RX queue %d\n",
                port_id, i);
        return -ENOMEM;
      }
    ...
    ret = rte_eth_rx_queue_setup(port_id, i, vr_rxd_sz, SOCKET_ID_ANY, &rx_queue_conf, mempool);
    if (ret < 0) {
      /* return mempool to the list */
      if (mempool != vr_dpdk.rss_mempool)
        vr_dpdk.nb_free_mempools++;
      RTE_LOG(ERR, VROUTER,
              "    error setting up eth device %" PRIu8 " RX queue %d"
              ": %s (%d)\n",
  ...
  for (i = 0; i < ethdev->ethdev_nb_tx_queues; i++) {
    ret = rte_eth_tx_queue_setup(port_id, i, vr_txd_sz, SOCKET_ID_ANY, &tx_queue_conf);
    if (ret < 0) {
      RTE_LOG(ERR, VROUTER,
              "    error setting up eth device %" PRIu8 " TX queue %d"
              ": %s (%d)\n",
              port_id, i, rte_strerror(-ret), -ret);
      return ret;
    }
...
static void dpdk_ethdev_reta_show(uint8_t port_id, uint16_t reta_size) {
...
int vr_dpdk_ethdev_rss_init(struct vr_dpdk_ethdev *ethdev) {
  ...
  ret = rte_eth_dev_rss_reta_update(port_id, reta_entries, ethdev->ethdev_reta_size);
...
static void dpdk_ethdev_mempools_free(struct vr_dpdk_ethdev *ethdev) {
  int i;

  for (i = ethdev->ethdev_nb_rss_queues; i < ethdev->ethdev_nb_rx_queues; i++) {
    if (ethdev->ethdev_mempools[i] != NULL && ethdev->ethdev_mempools[i] != vr_dpdk.rss_mempool) {
      vr_dpdk.free_mempools[vr_dpdk.nb_free_mempools++] = ethdev->ethdev_mempools[i];
...
int vr_dpdk_ethdev_filtering_init(struct vr_interface *vif, struct vr_dpdk_ethdev *ethdev) {
  ...
  if (ret == 0) {
    ...
    RTE_LOG(INFO, VROUTER, "    enable hardware filtering for ethdev %" PRIu8 "\n", port_id);
  ...
  if (ret < 0) {
    RTE_LOG(ERR, VROUTER, "    error setting ethdev %" PRIu8 " Flow Director masks: %s (%d)\n",
...
static void dpdk_ethdev_bond_info_update(struct vr_dpdk_ethdev *ethdev) {
  ...
  if (rte_eth_bond_mode_get(port_id) == -1) {
    ...
    ethdev->ethdev_nb_slaves =
        rte_eth_bond_slaves_get(port_id, ethdev->ethdev_slaves, VR_DPDK_BOND_MAX_SLAVES);

    memset(&mac_addr, 0, sizeof(bond_mac));
    rte_eth_macaddr_get(port_id, &bond_mac);
    RTE_LOG(INFO, VROUTER, "    bond eth device %" PRIu8 " configured MAC " MAC_FORMAT "\n",
    ...
    for (i = 0; i < ethdev->ethdev_nb_slaves; i++) {
      ...
      RTE_LOG(INFO, VROUTER,
              "    bond member eth device %" PRIu8 " PCI " PCI_PRI_FMT " MAC " MAC_FORMAT "\n",
              slave_port_id, pci_addr->domain, pci_addr->bus, pci_addr->devid, pci_addr->function,
      ...
      if ((ret = rte_eth_dev_set_mc_addr_list(slave_port_id, &lacp_mac, 1)) != 0)
        RTE_LOG(INFO, VROUTER,
                "    bond member eth device %" PRIu8 ": unable to add multicast addresses %d\n",
                slave_port_id, ret);
      ...
      if ((ret = rte_eth_dev_mac_addr_add(slave_port_id, &bond_mac, 0)) != 0)
        RTE_LOG(INFO, VROUTER,
                "    bond member eth device %" PRIu8 ": unable to add MAC addresses %d\n",
                slave_port_id, ret);
      else {
 ...
 */
static void vr_dpdk_nl_send_bond_intf_state(struct vr_dpdk_bond_member_info *info, unsigned type,
                                            uint8_t vif_idx) {
  int nl_fd;
  char buf[1024];
  struct nlmsghdr *nlh;
  ...
  if (info == NULL) {
    RTE_LOG(ERR, VROUTER, "Bond member info is NULL\n");
    return;
  }
  ...
  sprintf(if_name_buf, "%s %s %x", info->intf_name, info->intf_drv_name, vif_idx);

  len = NLA_HDRLEN + NLA_ALIGN(strlen(if_name_buf) + 1);
  ...
  if (n != len) {
    ...
    RTE_LOG(INFO, VROUTER, "Notifed Link status update to agent for \
                interface name %s\n",
            info->intf_name);
...
static int vr_dpdk_bond_send_port_info(uint16_t port_id, uint8_t vif_idx) {
  int ret;
  char *str[] = {"UP", "DOWN"};
  ...
  RTE_LOG(INFO, VROUTER, "Port ID: %d Link Status: %s intf_name:%s \
            drv_name:%s \n\n",
          port_id, (link.link_status ? str[0] : str[1]), member_info.intf_name,
          member_info.intf_drv_name);

  dev_flags = rte_eth_devices[port_id].data->dev_flags;
  bond_type = (dev_flags & RTE_ETH_DEV_BONDED_SLAVE) ? VR_DPDK_BOND_SLAVE : VR_DPDK_BOND_MASTER;
...
static int vr_dpdk_bond_intf_callback(uint16_t port_id, enum rte_eth_event_type type, void *param,
...
static void vr_dpdk_bond_intf_cb_register(struct vr_dpdk_ethdev *ethdev) {
  ...
  ret = rte_eth_dev_callback_register(port_id, RTE_ETH_EVENT_INTR_LSC, vr_dpdk_bond_intf_callback,
                                      NULL);
  ...
  if (ret)
    ...
    /* During system initialization, notify agent about the master
  ...
  for (i = 0; i < ethdev->ethdev_nb_slaves; i++) {
    ...
    rte_eth_dev_callback_register(port_id, RTE_ETH_EVENT_INTR_LSC, vr_dpdk_bond_intf_callback,
                                  NULL);
    if (ret)
      RTE_LOG(ERR, VROUTER, "Failed to setup callback for event \
                    RTE_ETH_EVENT_INTR_LSC for portid: %d\n",
...
int vr_dpdk_ethdev_init(struct vr_dpdk_ethdev *ethdev, struct rte_eth_conf *dev_conf) {
  uint8_t port_id;
  int ret;
  struct vr_interface *vif;
  ...
  ret = rte_eth_dev_configure(port_id, ethdev->ethdev_nb_rx_queues, ethdev->ethdev_nb_tx_queues,
                              dev_conf);
  if (ret < 0) {
...
int vr_dpdk_ethdev_release(struct vr_dpdk_ethdev *ethdev) {
 ...
 */
static inline int dpdk_mbuf_rss_hash(struct rte_mbuf *mbuf, struct vr_ip *ipv4_hdr,
  ...
  if (likely(ipv4_hdr != NULL)) {
     ...
     */
    ip_addr_ptr = (uint64_t *)((uintptr_t)ipv4_hdr + offsetof(struct vr_ip, ip_saddr));
     ...
     */
    for (i = 0; i < 4; i++) {
      ip_addr_ptr = (uint64_t *)((uintptr_t)ipv6_hdr + offsetof(struct ipv6_hdr, src_addr) + 8 * i);
      hash = rte_hash_crc_8byte(*ip_addr_ptr, hash);
    }
 ...
 */
static int dpdk_mbuf_parse_and_hash_packets(struct rte_mbuf *mbuf) {
  ...
  unsigned short gre_udp_encap = 0, gre_hdr_len = VR_GRE_BASIC_HDR_LEN, eth_proto, udp_port;
  ...
  if (likely(eth_proto == rte_cpu_to_be_16(VR_ETH_PROTO_IP))) {
    ...
    if (ipv4_hdr->ip_proto == VR_IP_PROTO_GRE) {
      ...
      if (likely(gre_hdr->gre_proto == VR_GRE_PROTO_MPLS_NO)) {
        ...
        if (unlikely(gre_hdr->gre_flags & (~(VR_GRE_FLAG_CSUM | VR_GRE_FLAG_KEY))))
          return 0;

        if (gre_hdr->gre_flags & VR_GRE_FLAG_CSUM) {
          gre_hdr_len += (VR_GRE_CKSUM_HDR_LEN - VR_GRE_BASIC_HDR_LEN);
        }
        if (gre_hdr->gre_flags & VR_GRE_FLAG_KEY) {
          gre_hdr_len += (VR_GRE_KEY_HDR_LEN - VR_GRE_BASIC_HDR_LEN);
        }
    ...
    helper_ret = vr_inner_pkt_parse(rte_pktmbuf_mtod(mbuf, unsigned char *), vr_mpls_tunnel_type,
                                    &encap_type, NULL, &pull_len, mbuf->buf_len, &ipv4_inner_hdr,
                                    &ipv6_inner_hdr, gre_udp_encap, ipv4_hdr->ip_proto);
    ...
    helper_ret = vr_ip_transport_parse(ipv4_inner_hdr, ipv6_inner_hdr, NULL, mbuf->buf_len,
...
uint64_t vr_dpdk_ethdev_rx_emulate(struct vr_interface *vif,
  ...
  uint64_t mask_to_distribute = 0, mask_to_distribute_ret = 0, mask_to_drop = 0;
