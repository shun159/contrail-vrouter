...
struct dpdk_gso_state {
  void (*update)(struct rte_mbuf *, struct dpdk_gso_state *);
  void (*internal)(struct rte_mbuf *, struct dpdk_gso_state *);
  union {
    struct ipv4_hdr *ip;
    struct ipv6_hdr *ip6;
  };
 ...
 */
inline uint16_t dpdk_ipv6_udptcp_cksum(struct rte_mbuf *m, const struct ipv6_hdr *ipv6_hdr,
                                       uint8_t *l4_hdr) {
  uint32_t cksum = 0;
  uint32_t l4_len;
 ...
 *   The complemented checksum to set in the IP packet.
 */
inline uint16_t dpdk_ipv4_udptcp_cksum(struct rte_mbuf *m, const struct ipv4_hdr *ipv4_hdr,
                                       uint8_t *l4_hdr) {
  uint32_t cksum = 0;
  uint32_t l4_len;
  uint32_t data_len = 0, rem_len = 0;
  ...
  l4_len = rte_be_to_cpu_16(ipv4_hdr->total_length) - ((ipv4_hdr->version_ihl & 0xf) << 2);
...
static int32_t dpdk_split_chained_mbuf(struct rte_mbuf *pkt_in, struct rte_mbuf **pkts_out,
  ...
  while (curr_pkt) {
    ...
    if (likely(nb_segs > 0)) {
      ...
      if (unlikely(pkt_addr == NULL)) {
        /* pkts_out[0] will be freed in the caller
         * when -1 is returned
         */
        for (j = 1; j < i; j++)
 ...
 */
static inline void dpdk_gso_ipv6_tcp_update(struct rte_mbuf *m, struct dpdk_gso_state *state) {
 ...
 */
static inline void dpdk_gso_ipv4_tcp_update(struct rte_mbuf *m, struct dpdk_gso_state *state) {
 ...
 */
static inline void dpdk_gso_ipv6_tcp_internal(struct rte_mbuf *m, struct dpdk_gso_state *state) {
  state->ip6->payload_len = rte_cpu_to_be_16(m->pkt_len - state->mac_hlen - state->ip_hlen);
  ...
  if (state->tx_cksum_offload) {
    ...
    state->tcp->cksum = 0;
    state->tcp->cksum = rte_ipv6_phdr_cksum((struct ipv6_hdr *)state->ip6, m->ol_flags);
    m->ol_flags |= PKT_TX_IPV6;
    m->ol_flags |= PKT_TX_TCP_CKSUM;
  } else {
    /* TCP Checksum */
    state->tcp->cksum = 0;
    state->tcp->cksum = rte_ipv6_udptcp_cksum((struct ipv6_hdr *)state->ip6, (uint8_t *)state->tcp);
  }
 ...
 */
static inline void dpdk_gso_ipv4_tcp_internal(struct rte_mbuf *m, struct dpdk_gso_state *state) {
  /* Update IP header */
  ...
  if (state->tx_cksum_offload) {
    ...
    state->tcp->cksum = 0;
    state->tcp->cksum = rte_ipv4_phdr_cksum((struct ipv4_hdr *)state->ip, m->ol_flags);
    m->ol_flags |= PKT_TX_IP_CKSUM | PKT_TX_IPV4;
    m->ol_flags |= PKT_TX_TCP_CKSUM;
  } else {
    /* TCP Checksum */
    state->tcp->cksum = 0;
    state->tcp->cksum = rte_ipv4_udptcp_cksum((struct ipv4_hdr *)state->ip, (uint8_t *)state->tcp);
    /* IP Checksum */
    state->ip->hdr_checksum = 0;
 ...
 */
static int dpdk_gso_init_state(struct dpdk_gso_state *state, struct rte_mbuf *m, int mac_hlen,
                               int isipv6, int hw_cksum) {
  uint32_t ip_hlen;
 ...
 */
static int dpdk_gso_segment_ip_tcp(struct rte_mbuf *pkt_in, struct dpdk_gso_state *state,
                                   struct rte_mbuf **mbufs_out, const unsigned short nb_pkts_out,
                                   uint16_t mss_size) {
  int i = 0, nsegs = 0;

  /* Check that pkts_out is big enough to hold all segments */
  if (unlikely(mss_size * nb_pkts_out < (uint16_t)(pkt_in->pkt_len - state->hlen)))
    return -EINVAL;

  nsegs = dpdk_split_chained_mbuf(pkt_in, mbufs_out, nb_pkts_out, state->hlen, mss_size);
...
static void dpdk_adjust_outer_header(struct rte_mbuf *m, uint16_t outer_header_len) {
...
int dpdk_segment_packet(struct vr_packet *pkt, struct rte_mbuf *mbuf_in, struct rte_mbuf **mbuf_out,
  ...
  if (dpdk_gso_init_state(&state, mbuf_in, outer_header_len, (pkt->vp_type == VP_TYPE_IP6OIP),
                          do_outer_ip_csum) < 0)
    return -1;

  number_of_packets = dpdk_gso_segment_ip_tcp(mbuf_in, &state, mbuf_out, out_num, mss_size);
