...
static void *dpdk_page_alloc(unsigned int size) { return rte_malloc(0, size, PAGE_SIZE); }

static void dpdk_page_free(void *address, unsigned int size) { rte_free(address); }

static int dpdk_printf(const char *format, ...) {
  va_list args;
...
static void *dpdk_malloc(unsigned int size, unsigned int object) {
...
static void *dpdk_zalloc(unsigned int size, unsigned int object) {
  struct vr_malloc_md *md;
  void *mem;
...
static void dpdk_free(void *mem, unsigned int object) {
  if (mem) {
    if (vr_memory_alloc_checks) {
...
static uint64_t dpdk_vtop(void *address) {
  /* TODO: not used */
...
static struct vr_packet *dpdk_palloc(unsigned int size) {
  struct rte_mbuf *m;
...
static struct vr_packet *dpdk_palloc_head(struct vr_packet *pkt, unsigned int size) {
  /* TODO: not implemented */
  RTE_LOG(ERR, VROUTER, "%s: not implemented\n", __func__);
  return NULL;
}

static struct vr_packet *dpdk_pexpand_head(struct vr_packet *pkt, unsigned int hspace) {
  /* TODO: not implemented */
  return pkt;
}

static void dpdk_pfree(struct vr_packet *pkt, unsigned short reason) {
  if (pkt) {
...
static void dpdk_preset(struct vr_packet *pkt) {
  struct rte_mbuf *m;
...
static inline void dpdk_pktmbuf_data_copy(struct rte_mbuf *dst, struct rte_mbuf *src) {
 ...
 */
inline struct rte_mbuf *vr_dpdk_pktmbuf_copy(struct rte_mbuf *md, struct rte_mempool *mp) {
...
inline struct rte_mbuf *vr_dpdk_pktmbuf_copy_mon(struct rte_mbuf *md, struct rte_mempool *mp) {
  struct rte_mbuf *mc;
  char *append_ptr;
  uint32_t append_len;
...
/* VRouter callback */
static struct vr_packet *dpdk_pclone(struct vr_packet *pkt) {
  /*
 ...
 * destination buffer.
 */
static int dpdk_pktmbuf_copy_bits(const struct rte_mbuf *mbuf, int offset, void *to, int len) {
...
/* VRouter callback */
static int dpdk_pcopy(unsigned char *dst, struct vr_packet *p_src, unsigned int offset,
                      unsigned int len) {
  int ret;
  struct rte_mbuf *src;
...
static unsigned short dpdk_pfrag_len(struct vr_packet *pkt) {
  struct rte_mbuf *m;
...
static unsigned short dpdk_phead_len(struct vr_packet *pkt) {
  struct rte_mbuf *m;
...
static void dpdk_pset_data(struct vr_packet *pkt, unsigned short offset) {
  struct rte_mbuf *m;
...
static unsigned int dpdk_get_cpu(void) {
...
/* DPDK timer callback */
static void dpdk_timer(struct rte_timer *tim, void *arg) {
  struct vr_timer *vtimer = (struct vr_timer *)arg;

  vtimer->vt_timer(vtimer->vt_vr_arg);
}

static int dpdk_create_timer(struct vr_timer *vtimer) {
  ...
  if (rte_timer_reset(timer, ticks, PERIODICAL, VR_DPDK_TIMER_LCORE_ID, dpdk_timer, vtimer) == -1) {
...
static void dpdk_delete_timer(struct vr_timer *vtimer) {
...
static void dpdk_get_time(uint64_t *sec, uint64_t *usec) {
  struct timespec ts;
...
static void dpdk_get_mono_time(uint64_t *sec, uint64_t *nsec) {
  struct timespec ts;
...
static void dpdk_htable_work_cb(struct vrouter *router __attribute__((unused)), void *arg) {
...
static int dpdk_schedule_work(unsigned int cpu, void (*fn)(void *), void *arg) {
...
static void dpdk_delay_op(void) {
  synchronize_rcu();
...
static void dpdk_rcu_cb(struct rcu_head *rh) {
  int i;
  ...
  if ((cb_data->rcd_user_cb == vr_flow_defer_cb) && cb_data->rcd_user_data) {
      ...
      for (i = 0; i < VR_MAX_FLOW_QUEUE_ENTRIES; i++) {
        ...
        if (pnode->pl_packet) {
          RTE_LOG_DP(DEBUG, VROUTER,
                     "%s: lcore %u passing RCU callback "
                     "to lcore %u\n",
                     __func__, rte_lcore_id(), VR_DPDK_PACKET_LCORE_ID);
          vr_dpdk_lcore_cmd_post(VR_DPDK_PACKET_LCORE_ID, VR_DPDK_LCORE_RCU_CMD, (uintptr_t)rh);
          return;
        }
      }
...
static void dpdk_defer(struct vrouter *router, vr_defer_cb user_cb, void *data) {
  struct vr_dpdk_rcu_cb_data *cb_data;
...
static void *dpdk_get_defer_data(unsigned int len) {
  struct vr_dpdk_rcu_cb_data *cb_data;
...
static void dpdk_put_defer_data(void *data) {
  struct vr_dpdk_rcu_cb_data *cb_data;
...
static void *dpdk_network_header(struct vr_packet *pkt) {
...
static void *dpdk_inner_network_header(struct vr_packet *pkt) {
  /* TODO: not used? */
...
static void *dpdk_data_at_offset(struct vr_packet *pkt, unsigned short off) {
  if (off < pkt->vp_end)
 ...
 */
static void *dpdk_pheader_pointer(struct vr_packet *pkt, unsigned short hdr_len, void *buf) {
  struct rte_mbuf *m;
  int offset;
...
/* VRouter callback */
static int dpdk_pcow(struct vr_packet **pktp, unsigned short head_room) {
 ...
 */
static uint16_t dpdk_get_udp_src_port(struct vr_packet *pkt, struct vr_forwarding_md *fmd,
                                      unsigned short vrf) {
  struct rte_mbuf *mbuf = vr_dpdk_pkt_to_mbuf(pkt);
 ...
 */
void dpdk_adjust_tcp_mss(struct tcphdr *tcph, unsigned short overlay_len, unsigned char iph_len) {
  ...
  while (opt_off < (tcph->doff * 4)) {
    ...
    case TCPOPT_MAXSEG:
      ...
      port_id = (((struct vr_dpdk_ethdev *)(router->vr_eth_if->vif_os))->ethdev_port_id);
 ...
 */
static int dpdk_pkt_from_vm_tcp_mss_adj(struct vr_packet *pkt, unsigned short overlay_len) {
...
static unsigned int dpdk_pgso_size(struct vr_packet *pkt) {
  struct rte_mbuf *m = vr_dpdk_pkt_to_mbuf(pkt);

  return m->tso_segsz;
}

static void dpdk_add_mpls(struct vrouter *router, unsigned mpls_label) {
  int ret, i;
  ...
  for (i = 0; i < router->vr_max_interfaces; i++) {
    ...
    if (eth_vif && (eth_vif->vif_type == VIF_TYPE_PHYSICAL) &&
      ...
      ret = vr_dpdk_lcore_mpls_schedule(eth_vif, ntohs(eth_vif->vif_ip), mpls_label);
      if (ret != 0)
...
static void dpdk_del_mpls(struct vrouter *router, unsigned mpls_label) {
  /* TODO: not implemented */
}

static int dpdk_pkt_may_pull(struct vr_packet *pkt, unsigned int len) {
  struct rte_mbuf *mbuf = vr_dpdk_pkt_to_mbuf(pkt);
...
static void dpdk_set_log_level(unsigned int log_level) {
  unsigned int level;
...
static unsigned int dpdk_get_log_level(void) {
#if (RTE_VERSION >= RTE_VERSION_NUM(17, 11, 0, 0))
...
static void dpdk_set_log_type(unsigned int log_type, int enable) {
  unsigned int type;
...
static unsigned int dpdk_log_type_to_vr_type(unsigned int type) {
  switch (type) {
...
#if (RTE_VERSION >= RTE_VERSION_NUM(17, 11, 0, 0))
static unsigned int *dpdk_get_enabled_log_types(int *size) {
  int num = rte_logs.dynamic_types_len;

  unsigned int *enabled_array = vr_malloc(sizeof(int) * num, VR_LOG_TYPES_OBJECT);
  int i;
...
static void dpdk_soft_reset(struct vrouter *router) {
  unsigned lcore_id;
  rcu_barrier();
...
static int dpdk_is_frag_limit_exceeded(void) {
  struct vrouter *router = vrouter_get(0);
...
static void dpdk_register_nic(struct vr_interface *vif __attribute__((unused)),
 ...
 */
static int vr_dpdk_set_fd_limit(void) {
  struct rlimit rl;
  int ret, old_cur;

  ret = getrlimit(RLIMIT_NOFILE, &rl);
  if (ret != 0) {
    RTE_LOG(ERR, VROUTER, "Could not get resource limits, error %d\n", errno);
    return -1;
  }
  ...
  if (ret) {
    RTE_LOG(ERR, VROUTER, "Could not set fd limit to %d (max %d), error %d\n", (int)rl.rlim_cur,
            (int)rl.rlim_max, errno);
    return -1;
  }

  RTE_LOG(INFO, VROUTER, "set fd limit to %d (prev %d, max %d)\n", (int)rl.rlim_cur, old_cur,
          (int)rl.rlim_max);
...
int vr_dpdk_host_init(void) {
   ...
   */
  vr_num_cpus = 0;
  RTE_LCORE_FOREACH(lcore_id) { vr_num_cpus = RTE_MAX(vr_num_cpus, lcore_id); }
  vr_num_cpus++;
...
int vr_dpdk_retry_connect(int sockfd, const struct sockaddr *addr, socklen_t alen) {
  int nsec;
...
static inline uint32_t dpdk_strhash(const char *k, uint32_t initval) {
  uint32_t a, b, c;
...
int vr_dpdk_ulog(uint32_t level, uint32_t logtype, uint32_t *last_hash, const char *format, ...) {
  va_list ap;
  int ret = 0;
  uint32_t hash;
