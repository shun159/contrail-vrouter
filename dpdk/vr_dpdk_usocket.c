...
extern void dpdk_burst_rx(unsigned int, struct rte_mbuf *[], struct vr_interface *, const char *,
                          unsigned int);
 ...
 */
static void usock_set_error(struct vr_usocket *usockp, int error) {
 ...
 */
static void usock_deinit_poll(struct vr_usocket *usockp) {
  if (!usockp)
    return;
 ...
 */
static int usock_init_poll(struct vr_usocket *usockp) {
  unsigned int i;
  unsigned int proto;
 ...
 */
static int usock_bind_usockets(struct vr_usocket *parent, struct vr_usocket *child) {
  unsigned int i;
  int ret;
  ...
  RTE_LOG_DP(DEBUG, USOCK, "%s[%lx]: parent FD %d child FD %d\n", __func__, pthread_self(),
             parent->usock_fd, child->usock_fd);
...
static int usock_clone(struct vr_usocket *parent, int cfd) {
...
static void usock_unbind(struct vr_usocket *child) {
...
static void usock_close(struct vr_usocket *usockp) {
  int i;
  struct vr_usocket *parent;
...
static int __usock_write(struct vr_usocket *usockp) {
  int ret;
  unsigned int len;
  unsigned char *buf;
...
retry_write:
  ...
  if (usockp->usock_owner != pthread_self()) {
    if (usockp->usock_owner)
      RTE_LOG(WARNING, USOCK,
              "WARNING: thread %lx (lcore %u) is trying to write %u bytes"
...
static void usock_netlink_write_responses(struct vr_usocket *usockp) {
  int ret;
...
static int usock_mbuf_write(struct vr_usocket *usockp, struct rte_mbuf *mbuf) {
...
static void vr_dpdk_packet_receive(struct vr_usocket *usockp) {
...
static void vr_dpdk_packet_ring_drain(struct vr_usocket *usockp) {
  int i;
  unsigned nb_pkts;
  ...
  do {
    ...
    nb_pkts = rte_ring_sc_dequeue_burst(vr_dpdk.packet_ring, (void **)&mbuf_arr,
                                        VR_DPDK_RX_BURST_SZ, NULL);
#else
    nb_pkts =
        rte_ring_sc_dequeue_burst(vr_dpdk.packet_ring, (void **)&mbuf_arr, VR_DPDK_RX_BURST_SZ);
#endif
    for (i = 0; i < nb_pkts; i++) {
      if (usock_mbuf_write(usockp->usock_parent, mbuf_arr[i]) >= 0)
        ...
        RTE_LOG_DP(DEBUG, USOCK, "%s: Error writing mbuf to packet socket: %s (%d)\n", __func__,
...
static int usock_read_done(struct vr_usocket *usockp) {
  ...
  case NETLINK:
    dpdk_netlink_receive(usockp, usockp->usock_rx_buf, usockp->usock_read_len);
    break;
...
static int usock_read_init(struct vr_usocket *usockp) {
  ...
  case PACKET:
    ...
    usockp->usock_buf_len = usockp->usock_mbuf->buf_len - rte_pktmbuf_headroom(usockp->usock_mbuf);
...
static int __usock_read(struct vr_usocket *usockp) {
  int ret;
...
static struct vr_usocket *usock_alloc(unsigned short proto, unsigned short type) {
    ...
    if (ret == 0) {
      ...
      if (ret == 0) {
        if (getsocksndbuff >= setsocksndbuff) {
          RTE_LOG(INFO, USOCK,
                  "%s[%lx]: setting socket FD %d send buff size.\n"
                  "Buffer size set to %d (requested %d)\n",
                  __func__, pthread_self(), sock_fd, getsocksndbuff, setsocksndbuff);
        } else { /* set other than requested */
          RTE_LOG(ERR, USOCK,
                  "%s[%lx]: setting socket FD %d send buff size failed.\n"
                  "Buffer size set to %d (requested %d)\n",
                  __func__, pthread_self(), sock_fd, getsocksndbuff, setsocksndbuff);
        }
      ...
      RTE_LOG(ERR, USOCK, "%s[%lx]: setting socket FD %d send buff size %d failed (%d)\n", __func__,
              pthread_self(), sock_fd, setsocksndbuff, errno);
    }
  }
  ...
  if (proto == PACKET) {
    ...
    if (!usockp->usock_mbuf_pool) {
      usockp->usock_mbuf_pool = rte_mempool_create(
          "packet_mbuf_pool", pkt0_mempool_sz, PKT0_MBUF_PACKET_SIZE, PKT0_MBUF_POOL_CACHE_SZ,
          sizeof(struct rte_pktmbuf_pool_private), vr_dpdk_pktmbuf_pool_init, NULL,
          vr_dpdk_pktmbuf_init, NULL, rte_socket_id(), 0);
      if (!usockp->usock_mbuf_pool)
        goto error_exit;
    }

    usockp->usock_iovec = vr_zalloc(sizeof(struct iovec) * PKT0_MAX_IOV_LEN, VR_USOCK_IOVEC_OBJECT);
    if (!usockp->usock_iovec)
      goto error_exit;
 ...
 */
static bool valid_usock(int proto, int type) {
  RTE_LOG_DP(DEBUG, USOCK, "%s[%lx]: proto %u type %u\n", __func__, pthread_self(), proto, type);
...
void vr_usocket_detach_vif(void *usockp) {
  RTE_LOG_DP(DEBUG, USOCK, "%s[%lx]: FD %d\n", __func__, pthread_self(),
...
int vr_usocket_message_write(struct vr_usocket *usockp, struct vr_message *message) {
  int ret;
  unsigned int len;
  unsigned char *buf;
...
static int vr_usocket_read(struct vr_usocket *usockp) {
  int ret;
...
static int vr_usocket_connect(struct vr_usocket *usockp) {
  ...
  strncat(sun.sun_path, "/" VR_PACKET_AGENT_UNIX_NAME,
...
#endif
  return vr_dpdk_retry_connect(usockp->usock_fd, (struct sockaddr *)&sun, sizeof(sun));
}

static int vr_usocket_accept(struct vr_usocket *usockp) {
  int ret;
...
static int vr_usocket_bind(struct vr_usocket *usockp) {
  int error = 0;
  ...
  case UNIX:
    ...
    memset(sun.sun_path, 0, sizeof(sun.sun_path));
    strncpy(sun.sun_path, vr_socket_dir, sizeof(sun.sun_path) - 1);
    strncat(sun.sun_path, "/" VR_NETLINK_UNIX_NAME,
            sizeof(sun.sun_path) - strlen(sun.sun_path) - 1);
 ...
 */
void *vr_usocket(int proto, int type) {
  RTE_LOG_DP(DEBUG, USOCK, "%s[%lx]: proto %u type %u\n", __func__, pthread_self(), proto, type);
...
static int usock_write(struct vr_usocket *usockp) {
  int ret;
