...
#define uvhm_client_name(vru_cl)                                                                   \
  (vru_cl->vruc_path + strlen(vr_socket_dir) + sizeof(VR_UVH_VIF_PFX) - 1)
 ...
 */
static void vr_uvh_nl_send_intf_state(int state, int intf_index, char *intf_name) {
  int nl_fd;
  char buf[1024];
  struct nlmsghdr *nlh;
 ...
 */
static int uvhm_client_mmap(vr_uvh_client_t *vru_cl) {
  int i;
  int ret;
  ...
  if (vum_msg->nregions > VHOST_MEMORY_MAX_NREGIONS) {
    vr_uvhost_log("Client %s: error mapping guest memory: too many regions"
                  "(%" PRIu32 " > %d)\n",
                  uvhm_client_name(vru_cl), vum_msg->nregions, VHOST_MEMORY_MAX_NREGIONS);
    return -1;
  }
  for (i = 0; i < vum_msg->nregions; i++) {
    vr_uvhost_log("    %d: FD %d addr 0x%" PRIx64 " size 0x%" PRIx64 " off 0x%" PRIx64 "\n", i,
                  vru_cl->vruc_fds_sent[i], vum_msg->regions[i].guest_phys_addr,
                  vum_msg->regions[i].memory_size, vum_msg->regions[i].mmap_offset);
    ...
    if (vru_cl->vruc_fds_sent[i]) {
      ...
      size = vum_msg->regions[i].mmap_offset + vum_msg->regions[i].memory_size;
      region->vrucmr_mmap_addr =
          (uint64_t)mmap(0, size, PROT_READ | PROT_WRITE, MAP_SHARED, vru_cl->vruc_fds_sent[i], 0);

      if (region->vrucmr_mmap_addr == ((uint64_t)MAP_FAILED)) {
        vr_uvhost_log("Client %s: error mmaping FD %d size 0x%" PRIx64 ": %s (%d)\n",
                      uvhm_client_name(vru_cl), vru_cl->vruc_fds_sent[i], size, rte_strerror(errno),
                      errno);
        /*
      ...
      ret = vr_dpdk_virtio_uvh_get_blk_size(vru_cl->vruc_fds_sent[i], &region->vrucmr_blksize);
      if (ret) {
        vr_uvhost_log("Client %s: error getting block size for FD %d\n", uvhm_client_name(vru_cl),
                      vru_cl->vruc_fds_sent[i]);
        return -1;
      }
      region->vrucmr_mmap_addr_aligned =
          (void *)(uintptr_t)RTE_ALIGN_FLOOR(region->vrucmr_mmap_addr, region->vrucmr_blksize);
      region->vrucmr_size_aligned = RTE_ALIGN_CEIL(size, region->vrucmr_blksize);

      /*
       * Prevent guest memory from being dumped in vrouter-dpdk core.
       */
      if (madvise(region->vrucmr_mmap_addr_aligned, region->vrucmr_size_aligned, MADV_DONTDUMP)) {
        vr_uvhost_log("Client %s: error in madvise at addr 0x%" PRIx64 ", size 0x%" PRIx64
                      "for FD %d: %s (%d)\n",
                      uvhm_client_name(vru_cl), region->vrucmr_mmap_addr, size,
                      vru_cl->vruc_fds_sent[i], rte_strerror(errno), errno);
        /*
 ...
 */
static void uvhm_client_munmap(vr_uvh_client_t *vru_cl) {
  int i, ret;
  vr_uvh_client_mem_region_t *region;
  ...
  for (i = 0; i < vru_cl->vruc_num_mem_regions; i++) {
    ...
    if (region->vrucmr_mmap_addr_aligned) {
      vr_uvhost_log("    %d: unmapping addr 0x%" PRIx64 " size 0x%" PRIx64 "\n", i,
                    region->vrucmr_phys_addr, region->vrucmr_size);

      ret = munmap(region->vrucmr_mmap_addr_aligned, region->vrucmr_size_aligned);
      if (ret) {
        vr_uvhost_log("Client %s: error unmapping memory region %d: %s (%d)\n",
...
static int vr_uvmh_get_features(vr_uvh_client_t *vru_cl) {
  ...
  vru_cl->vruc_msg.u64 = (1ULL << VIRTIO_NET_F_CTRL_VQ) | (1ULL << VIRTIO_NET_F_CSUM) |
                         (1ULL << VIRTIO_NET_F_GUEST_CSUM) | (1ULL << VIRTIO_NET_F_MQ) |
                         (1ULL << VHOST_USER_F_PROTOCOL_FEATURES) | (1ULL << VHOST_F_LOG_ALL);

  if (dpdk_check_rx_mrgbuf_disable() == 0)
    vru_cl->vruc_msg.u64 |= (1ULL << VIRTIO_NET_F_MRG_RXBUF);

  if (vr_perfs)
    vru_cl->vruc_msg.u64 |= (1ULL << VIRTIO_NET_F_GUEST_TSO4) | (1ULL << VIRTIO_NET_F_HOST_TSO4) |
                            (1ULL << VIRTIO_NET_F_GUEST_TSO6) | (1ULL << VIRTIO_NET_F_HOST_TSO6);
...
static int vr_uvmh_set_features(vr_uvh_client_t *vru_cl) {
  ...
  if ((dpdk_check_rx_mrgbuf_disable() == 0) && !(vru_cl->vruc_flags & VRUC_FLAG_SET_FEATURE_DONE))
    if (!vr_dpdk_load_persist_feature(uvhm_client_name(vru_cl), &stored_features)) {
  ...
  if (dpdk_check_rx_mrgbuf_disable() == 0)
    vr_dpdk_store_persist_feature(uvhm_client_name(vru_cl), vru_cl->vruc_msg.u64);
...
static int vr_uvhm_set_vring_num(vr_uvh_client_t *vru_cl) {
  ...
  if (vr_dpdk_set_ring_num_desc(vru_cl->vruc_idx, vring_idx, vum_msg->state.num)) {
 ...
 */
static void *vr_uvhm_map_addr(vr_uvh_client_t *vru_cl, uint64_t addr) {
  int i;
  ...
  for (i = 0; i < vru_cl->vruc_num_mem_regions; i++) {
    ...
    if ((vmr_addr <= addr) && (addr < (vmr_addr + vmr_size))) {
      ret_addr = vru_cl->vruc_mem_regions[i].vrucmr_mmap_addr + (addr - vmr_addr);
 ...
 */
static int uvhm_check_vring_ready(vr_uvh_client_t *vru_cl, unsigned int vring_idx) {
...
static int vr_uvhm_set_vring_addr(vr_uvh_client_t *vru_cl) {
  ...
  vr_uvhost_log("    SET VRING ADDR: vring %u flags 0x%x desc 0x%llx"
                ...
                vring_idx, vaddr->flags, vaddr->desc_user_addr, vaddr->used_user_addr,
                vaddr->avail_user_addr);
  ...
  vrucv_desc = (struct vring_desc *)vr_uvhm_map_addr(vru_cl, vaddr->desc_user_addr);
  vrucv_avail = (struct vring_avail *)vr_uvhm_map_addr(vru_cl, vaddr->avail_user_addr);
  vrucv_used = (struct vring_used *)vr_uvhm_map_addr(vru_cl, vaddr->used_user_addr);

  if (!vrucv_desc || !vrucv_avail || !vrucv_used)
    return -1;

  if (vr_dpdk_set_vring_addr(vru_cl->vruc_idx, vring_idx, vrucv_desc, vrucv_avail, vrucv_used)) {
...
static int vr_uvhm_set_vring_base(vr_uvh_client_t *vru_cl) {
  ...
  if (vr_dpdk_virtio_set_vring_base(vru_cl->vruc_idx, vring_idx, vum_msg->state.num)) {
...
static int vr_uvhm_get_vring_base(vr_uvh_client_t *vru_cl) {
  ...
  if (vr_dpdk_virtio_get_vring_base(vru_cl->vruc_idx, vring_idx, &vum_msg->state.num)) {
...
static int vr_uvhm_set_vring_call(vr_uvh_client_t *vru_cl) {
  ...
  if (!(vring_idx & VHOST_USER_VRING_NOFD_MASK)) {
    ...
    if (vr_dpdk_set_ring_callfd(vru_cl->vruc_idx, vring_idx, vru_cl->vruc_fds_sent[0])) {
...
static int vr_uvhm_set_vring_enable(vr_uvh_client_t *vru_cl) {
  ...
  if ((vring_idx / 2) >= vr_dpdk.nb_fwd_lcores) {
    RTE_LOG(ERR, UVHOST, "%s: Can not %s %s queue %d (only %d queues)\n", __func__,
            enable ? "enable" : "disable", (vring_idx & 1) ? "RX" : "TX", vring_idx / 2,
  ...
  if (vring_idx & 1) {
    /* RX queues */
    vr_dpdk_virtio_rx_queue_enable_disable(vru_cl->vruc_idx, vru_cl->vruc_vif_gen, queue_num,
                                           enable);
  } else {
    /* TX queues */
    vr_dpdk_virtio_tx_queue_enable_disable(vru_cl->vruc_idx, vru_cl->vruc_vif_gen, queue_num,
                                           enable);
  }
 ...
 * Returns 0 on success, -1 otherwise.
 */
static int vr_uvh_cl_call_handler(vr_uvh_client_t *vru_cl) {
  VhostUserMsg *msg = &vru_cl->vruc_msg;

  if ((msg->request <= VHOST_USER_NONE) || (msg->request >= VHOST_USER_MAX)) {
    return -1;
  }
 ...
 * Returns 0 on success, -1 otherwise.
 */
static int vr_uvh_cl_send_reply(int fd, vr_uvh_client_t *vru_cl) {
  int ret;
  VhostUserMsg *msg = &vru_cl->vruc_msg;
  ...
  case VHOST_USER_SET_LOG_BASE:
    ...
    if (vru_cl->vruc_owner != pthread_self()) {
      if (vru_cl->vruc_owner)
        RTE_LOG(WARNING, UVHOST,
                "WARNING: thread %lx is trying to write"
                " to uvhost client FD %d owned by thread %lx\n",
                pthread_self(), fd, vru_cl->vruc_owner);
      vru_cl->vruc_owner = pthread_self();
    }
    ret = send(fd, (void *)msg, VHOST_USER_HSIZE + msg->size, MSG_DONTWAIT);
 ...
 */
static int vr_uvh_cl_msg_handler(int fd, void *arg) {
  ...
  if (vru_cl->vruc_msg_bytes_read == 0) {
    ...
    if (ret < 0) {
      ...
      if (cmsg && (cmsg->cmsg_len > 0) && (cmsg->cmsg_level == SOL_SOCKET) &&
        ...
        vru_cl->vruc_num_fds_sent = (cmsg->cmsg_len - CMSG_LEN(0)) / sizeof(int);
        if (vru_cl->vruc_num_fds_sent > VHOST_MEMORY_MAX_NREGIONS) {
          vr_uvhost_log("Client %s: error handling FDs: too many FDs (%d > %d)\n",
                        uvhm_client_name(vru_cl), vru_cl->vruc_num_fds_sent,
                        VHOST_MEMORY_MAX_NREGIONS);
          vru_cl->vruc_num_fds_sent = VHOST_MEMORY_MAX_NREGIONS;
        }

        memcpy(vru_cl->vruc_fds_sent, CMSG_DATA(cmsg), vru_cl->vruc_num_fds_sent * sizeof(int));
      }
    ...
    read_len = vru_cl->vruc_msg.size - (vru_cl->vruc_msg_bytes_read - VHOST_USER_HSIZE);
  }
  ...
  if (read_len) {
    if (vru_cl->vruc_owner != pthread_self()) {
      if (vru_cl->vruc_owner)
        RTE_LOG(WARNING, UVHOST,
                "WARNING: thread %lx is trying to read"
                " uvhost client FD %d owned by thread %lx\n",
                pthread_self(), fd, vru_cl->vruc_owner);
      vru_cl->vruc_owner = pthread_self();
    }
    ret = read(fd, (((char *)&vru_cl->vruc_msg) + vru_cl->vruc_msg_bytes_read), read_len);
#ifdef VR_DPDK_RX_PKT_DUMP
    if (ret > 0) {
      RTE_LOG_DP(DEBUG, UVHOST, "%s[%lx]: FD %d read %d bytes\n", __func__, pthread_self(), fd,
                 ret);
      rte_hexdump(stdout, "uvhost full message dump:", (((char *)&vru_cl->vruc_msg)),
    ...
    if (ret < 0) {
      ...
      vr_uvhost_log("Client %s: error reading message: %s (%d)\n", uvhm_client_name(vru_cl),
                    strerror(errno), errno);
      ret = -1;
      goto cleanup;
    } else if (ret == 0) {
    ...
    if (vru_cl->vruc_msg_bytes_read < (vru_cl->vruc_msg.size + VHOST_USER_HSIZE)) {
      ret = 0;
      goto cleanup;
    }
  }
 ...
 * Returns 0 on success, -1 otherwise.
 */
static int vr_uvh_cl_listen_handler(int fd, void *arg) {
  int s = 0, err;
 ...
 *
 */
static int vr_uvh_cl_timer_handler(int fd, void *arg) {
  vr_uvh_client_t *vru_cl = (vr_uvh_client_t *)arg;
  struct sockaddr_un sun;
  int ret = 0;
  ...
  if (ret == -1) {
    RTE_LOG_DP(DEBUG, UVHOST,
               "Error connecting uvhost socket FD %d to %s:"
               " %s (%d)\n",
     ...
     * add to msg handler
     */
    ret = vr_uvhost_add_fd(vru_cl->vruc_fd, UVH_FD_READ, vru_cl, vr_uvh_cl_msg_handler);
    if (ret == -1) {
...
static int vr_uvh_cl_timer_setup(vr_uvh_client_t *vru_cl) {
  ...
  if (vru_cl->vruc_timer_fd == -1) {
    ...
    if (ret == -1) {
      ...
      if (vr_uvhost_add_fd(vru_cl->vruc_timer_fd, UVH_FD_READ, vru_cl, vr_uvh_cl_timer_handler)) {
        ret = -1;
 ...
 * Returns 0 on success, -1 otherwise.
 */
int vr_uvh_nl_vif_del_handler(vrnu_vif_del_t *msg) {
 ...
 * Returns 0 on success, -1 otherwise.
 */
static int vr_uvh_nl_vif_add_handler(vrnu_vif_add_t *msg) {
  ...
  strncat(sun.sun_path, "/" VR_UVH_VIF_PFX, sizeof(sun.sun_path) - strlen(sun.sun_path) - 1);
  strncat(sun.sun_path, msg->vrnu_vif_name, sizeof(sun.sun_path) - strlen(sun.sun_path) - 1);
  ...
  if (msg->vrnu_vif_vhostuser_mode == VRNU_VIF_MODE_CLIENT) {
     ...
     * able to connect.
     */
    umask_mode = umask(~(S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH));

    ret = bind(s, (struct sockaddr *)&sun, sizeof(sun));
    if (ret == -1) {
      vr_uvhost_log("    error binding vif %u FD %d to %s: %s (%d)\n", msg->vrnu_vif_idx, s,
                    sun.sun_path, rte_strerror(errno), errno);
      goto error;
    }
 ...
 */
static int vr_uvh_nl_msg_handler(int fd, void *arg) {
  vrnu_msg_t msg;
  int ret;
  ...
  if (ret != sizeof(msg)) {
    vr_uvhost_log("Received msg of length %d, expected %zu in vhost server", ret, sizeof(msg));
    return 0;
  }
