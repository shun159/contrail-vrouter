...
static int dpdk_virtio_from_vm_rx(void *port, struct rte_mbuf **pkts, uint32_t max_pkts);
static int dpdk_virtio_to_vm_tx(void *port, struct rte_mbuf *pkt);
static int dpdk_virtio_to_vm_flush(void *port);
static int dpdk_virtio_writer_stats_read(void *port, struct rte_port_out_stats *stats, int clear);
static int dpdk_virtio_reader_stats_read(void *port, struct rte_port_in_stats *stats, int clear);
...
static void *dpdk_virtio_writer_create(void *params, int socket_id) {
  struct dpdk_virtio_writer_params *conf = (struct dpdk_virtio_writer_params *)params;
  ...
  /* Memory allocation */
  port = rte_zmalloc_socket("PORT", sizeof(*port), RTE_CACHE_LINE_SIZE, socket_id);
  if (port == NULL) {
...
static int dpdk_virtio_writer_free(void *port) {
  vr_dpdk_virtioq_t *tx_virtioq;
...
struct rte_port_out_ops vr_dpdk_virtio_writer_ops = {.f_create = dpdk_virtio_writer_create,
...
static void *dpdk_virtio_reader_create(void *params, int socket_id) {
  struct dpdk_virtio_reader_params *conf = (struct dpdk_virtio_reader_params *)params;
...
static int dpdk_virtio_reader_free(void *port) {
  vr_dpdk_virtioq_t *rx_virtioq;
...
struct rte_port_in_ops vr_dpdk_virtio_reader_ops = {.f_create = dpdk_virtio_reader_create,
 ...
 * interface.
 */
uint16_t vr_dpdk_virtio_nrxqs(struct vr_interface *vif) { return vr_dpdk.nb_fwd_lcores; }

/*
 * vr_dpdk_virtio_ntxqs - returns the number of transmit queues for a virtio
 * interface.
 */
uint16_t vr_dpdk_virtio_ntxqs(struct vr_interface *vif) { return vr_dpdk.nb_fwd_lcores; }
 ...
 * Returns nothing.
 */
static void dpdk_virtio_rx_queue_release(unsigned lcore_id,
                                         unsigned queue_index __attribute__((unused)),
                                         struct vr_interface *vif) {
  struct vr_dpdk_lcore *lcore = vr_dpdk.lcores[lcore_id];
  struct vr_dpdk_queue *rx_queue = &lcore->lcore_rx_queues[vif->vif_idx];
  struct vr_dpdk_queue_params *rx_queue_params = &lcore->lcore_rx_queue_params[vif->vif_idx];
  /* free the queue */
 ...
 */
struct vr_dpdk_queue *vr_dpdk_virtio_rx_queue_init(unsigned int lcore_id, struct vr_interface *vif,
  ...
  struct vr_dpdk_queue_params *rx_queue_params = &lcore->lcore_rx_queue_params[vif_idx];

  /* Check input parameters */
  if (queue_id >= vr_dpdk_virtio_nrxqs(vif)) {
    RTE_LOG(ERR, VROUTER, "    error creating virtio device %s RX queue %" PRIu16 "\n",
 ...
 * Returns nothing.
 */
static void dpdk_virtio_tx_queue_release(unsigned lcore_id, unsigned queue_index,
                                         struct vr_interface *vif) {
  struct vr_dpdk_lcore *lcore = vr_dpdk.lcores[lcore_id];
  struct vr_dpdk_queue *tx_queue = &lcore->lcore_tx_queues[vif->vif_idx][queue_index];
 ...
 */
struct vr_dpdk_queue *vr_dpdk_virtio_tx_queue_init(unsigned int lcore_id, struct vr_interface *vif,
                                                   unsigned int queue_or_lcore_id) {
  uint16_t queue_id = queue_or_lcore_id;
  struct vr_dpdk_lcore *lcore = vr_dpdk.lcores[lcore_id];
  const unsigned int socket_id = rte_lcore_to_socket_id(lcore_id);
  ...
  struct vr_dpdk_queue_params *tx_queue_params = &lcore->lcore_tx_queue_params[vif_idx][0];
  ...
  if (tx_queue->q_queue_h == NULL) {
    RTE_LOG(ERR, VROUTER, "    error creating virtio device %s TX queue %" PRIu16 "\n",
            vif->vif_name, queue_id);
    return NULL;
  }
 ...
 */
void vr_dpdk_virtio_tx_queue_enable_disable(unsigned int vif_id, unsigned int vif_gen,
                                            unsigned int queue_id, bool enable) {
  ...
  for (lcore_id = starting_lcore, qid = 0; lcore_id < vr_dpdk.nb_fwd_lcores + VR_DPDK_FWD_LCORE_ID;
       ++lcore_id) {
     ...
     * curently.
     */
    if (vif_lcore_tx_queue[vif_id][lcore_id - VR_DPDK_PACKET_LCORE_ID] != qid) {
      vif_lcore_tx_queue[vif_id][lcore_id - VR_DPDK_PACKET_LCORE_ID] = qid;
      ...
      vr_dpdk_lcore_cmd_post(lcore_id, VR_DPDK_LCORE_TX_QUEUE_SET_CMD, (uint64_t)arg);
    }
 ...
 */
void vr_dpdk_virtio_tx_queue_set(void *arg) {
 ...
 */
void vr_dpdk_virtio_rx_queue_enable_disable(unsigned int vif_id, unsigned int vif_gen,
                                            unsigned int queue_id, bool enable) {
  ...
  vr_dpdk_lcore_cmd_post(VR_DPDK_NETLINK_LCORE_ID, VR_DPDK_LCORE_RX_QUEUE_SET_CMD, (uint64_t)arg);
}

/*
 * Called only on netlink lcore.
 */
void vr_dpdk_virtio_rx_queue_set(void *arg) {
  ...
  if (p->enable) {
    ...
    rx_queue = &lcore->lcore_rx_queues[p->vif_id];
    if (rx_queue->enabled) {
      rx_rm_arg = rte_malloc("lcore_rx_queue_rm_cmd", sizeof(*rx_rm_arg), 0);
      rx_rm_arg->vif_id = vif->vif_idx;
      rx_rm_arg->clear_f_rx = false;
      rx_rm_arg->free_arg = true;
      vr_dpdk_lcore_cmd_post(lcore_id, VR_DPDK_LCORE_RX_RM_CMD, (uint64_t)rx_rm_arg);
    }
  }
 ...
 */
static char *vr_dpdk_guest_phys_to_host_virt(vr_uvh_client_t *vru_cl, uint64_t paddr) {
  int i;
  ...
  for (i = 0; i < vru_cl->vruc_num_mem_regions; i++) {
    ...
    if ((paddr >= reg->vrucmr_phys_addr) && (paddr <= (reg->vrucmr_phys_addr + reg->vrucmr_size))) {
      return ((char *)reg->vrucmr_mmap_addr) + (paddr - reg->vrucmr_phys_addr);
    }
  }
...
#define DPDK_VIRTIO_READER_STATS_PKTS_IN_ADD(port, val) port->stats.n_pkts_in += val
#define DPDK_VIRTIO_READER_STATS_PKTS_DROP_ADD(port, val) port->stats.n_pkts_drop += val

#else

/* keep compiler happy, for unused variables */
#define DPDK_VIRTIO_READER_STATS_PKTS_IN_ADD(port, val) (void)(val)
#define DPDK_VIRTIO_READER_STATS_PKTS_DROP_ADD(port, val) (void)(val)

#endif

static inline uint32_t dpdk_virtio_get_ip_tcp_hdr_len(char *pkt_addr, uint32_t pkt_len) {
 ...
 *   -1: failure
 */
static int dpdk_virtio_create_mss_sized_mbuf_chain(struct rte_mbuf *mbuf, uint32_t mss,
 ...
 */
static int dpdk_virtio_create_chained_mbuf(struct rte_mbuf *mbuf, char *pkt_addr,
                                           uint32_t pkt_len) {
  char *tail_addr, *append_addr = pkt_addr;
...
static int dpdk_virtio_from_vm_rx(void *port, struct rte_mbuf **pkts, uint32_t max_pkts) {
  ...
  for (i = 0; i < avail_pkts; i++) {
    ...
    if (unlikely(desc->len < vq->vdv_hlen || desc->addr == 0 || pkt_addr == NULL)) {
      goto free_mbuf;
    }
    mbuf->tso_segsz = 0;
    ...
    /* Skip virtio_net_hdr  */
    if (likely(desc->flags & VRING_DESC_F_NEXT && pkt_len == vq->vdv_hlen)) {
    ...
    if (mbuf->tso_segsz == 0) {
      ...
      if (unlikely(dpdk_virtio_create_mss_sized_mbuf_chain(mbuf, mbuf->tso_segsz, pkt_addr, pkt_len,
                                                           header_len) < 0)) {
        goto free_mbuf;
      }
    }
    ...
    while (unlikely(desc->flags & VRING_DESC_F_NEXT)) {
      ...
      if (mbuf->tso_segsz == 0) {
        ...
        if (unlikely(dpdk_virtio_create_mss_sized_mbuf_chain(mbuf, mbuf->tso_segsz, pkt_addr,
                                                             pkt_len, header_len) < 0)) {
          goto free_mbuf;
        }
      }
    }
  ...
  free_mbuf:
    DPDK_UDEBUG(VROUTER, &vq->vdv_hash,
                "%s: queue %p DROP desc->addr %p "
...
#define DPDK_VIRTIO_WRITER_STATS_PKTS_IN_ADD(port, val) port->stats.n_pkts_in += val
#define DPDK_VIRTIO_WRITER_STATS_PKTS_DROP_ADD(port, val) port->stats.n_pkts_drop += val

#else

/* keep compiler happy, for unused variables */
#define DPDK_VIRTIO_WRITER_STATS_PKTS_IN_ADD(port, val) (void)(val)
#define DPDK_VIRTIO_WRITER_STATS_PKTS_DROP_ADD(port, val) (void)(val)

#endif

static inline int32_t __attribute__((always_inline))
dpdk_virtio_dev_to_vm_tx_burst_simple(struct dpdk_virtio_writer *p, vr_dpdk_virtioq_t *vq,
  ...
  for (head_idx = 0; head_idx < count; head_idx++)
    head[head_idx] = vq->vdv_avail->ring[(res_cur_idx + head_idx) & (vq->vdv_size - 1)];

  virtio_hdr_len =
      (mrg_hdr) ? sizeof(struct virtio_net_hdr_mrg_rxbuf) : sizeof(struct virtio_net_hdr);
  ...
  while (res_cur_idx != res_end_idx) {
     ...
     */
    if (likely(desc->flags & VRING_DESC_F_NEXT) && !mrg_hdr &&
    ...
    len_to_cpy = RTE_MIN(data_len, hdr ? desc->len - virtio_hdr_len : desc->len);
    while (total_copied < pkt_len) {
      /* Copy mbuf data to buffer */
      rte_memcpy((void *)(uintptr_t)(buff_addr + vb_offset),
                 rte_pktmbuf_mtod_offset(buff, const void *, offset), len_to_cpy);
  ...
  next_descr:
    ...
    vq->vdv_used->ring[res_cur_idx & (vq->vdv_size - 1)].id = head[packet_success];

    /* Drop the packet if it is uncompleted */
    if (unlikely(uncompleted_pkt == 1))
      vq->vdv_used->ring[res_cur_idx & (vq->vdv_size - 1)].len = virtio_hdr_len;
    else
      vq->vdv_used->ring[res_cur_idx & (vq->vdv_size - 1)].len = pkt_len + virtio_hdr_len;
    ...
    */
    if (buff_hdr_addr) {
      rte_memcpy((void *)(uintptr_t)buff_hdr_addr, (const void *)&virtio_hdr, virtio_hdr_len);
    }
  ...
  *(volatile uint16_t *)&vq->vdv_used->idx += count;
  ...
  RTE_LOG_DP(DEBUG, VROUTER, "%s: vif %d vq %p last_used_idx %d used->idx %d\n", __func__,
             vq->vdv_vif_idx, vq, vq->vdv_last_used_idx, vq->vdv_used->idx);
...
dpdk_virtio_dev_to_vm_tx_burst(struct dpdk_virtio_writer *p, vr_dpdk_virtioq_t *vq,
  ...
  do {
    ...
    success = rte_atomic16_cmpset(&vq->vdv_last_used_idx_res, res_base_idx, res_end_idx);
  } while (unlikely(success == 0));

  return dpdk_virtio_dev_to_vm_tx_burst_simple(p, vq, res_base_idx, res_end_idx, pkts, count,
                                               !VIRTIO_HDR_MRG_RXBUF);
}
...
copy_from_mbuf_to_vring(vr_dpdk_virtioq_t *vq, vr_uvh_client_t *vru_cl, uint16_t res_base_idx,
                        ...
                        struct virtio_net_hdr_mrg_rxbuf *virtio_hdr, struct rte_mbuf *pkt) {
  uint32_t vec_idx = 0;
   ...
   */
  vb_addr = (uintptr_t)vr_dpdk_guest_phys_to_host_virt(vru_cl, buf_vec[vec_idx].buf_addr);
  vb_hdr_addr = vb_addr;
  ...
  rte_memcpy((void *)(uintptr_t)vb_hdr_addr, (const void *)virtio_hdr, vq->vdv_hlen);
  ...
  if (vb_avail == 0) {
    ...
    if ((vq->vdv_desc[desc_idx].flags & VRING_DESC_F_NEXT) == 0) {
      /* Update vdv_used ring with vdv_desc information */
      vq->vdv_used->ring[cur_idx & (vq->vdv_size - 1)].id = buf_vec[vec_idx].desc_idx;
      vq->vdv_used->ring[cur_idx & (vq->vdv_size - 1)].len = entry_len;
  ...
  while (cpy_len > 0) {
    ...
    rte_memcpy((void *)(uintptr_t)(vb_addr + vb_offset),
               rte_pktmbuf_mtod_offset(pkt, const void *, seg_offset), cpy_len);
    ...
    if (seg_avail != 0) {
       ...
       */
      if ((vq->vdv_desc[buf_vec[vec_idx].desc_idx].flags & VRING_DESC_F_NEXT) == 0) {
        /* Update vdv_used ring with vdv_desc information */
        vq->vdv_used->ring[cur_idx & (vq->vdv_size - 1)].id = buf_vec[vec_idx].desc_idx;
        vq->vdv_used->ring[cur_idx & (vq->vdv_size - 1)].len = entry_len;
        entry_len = 0;
      ...
      if (pkt != NULL) {
        ...
        if (vb_avail == 0) {
          ...
          if ((vq->vdv_desc[desc_idx].flags & VRING_DESC_F_NEXT) == 0) {
            uint16_t wrapped_idx = cur_idx & (vq->vdv_size - 1);
            /*
             * Update vdv_used ring with the
             * descriptor information
             */
            vq->vdv_used->ring[wrapped_idx].id = desc_idx;
            vq->vdv_used->ring[wrapped_idx].len = entry_len;
            entry_success++;
            entry_len = 0;
            cur_idx++;
          }
...
update_secure_len(vr_dpdk_virtioq_t *vq, uint32_t id, uint32_t *secure_len,
                  struct vq_buf_vector *buf_vec, uint32_t *vec_idx) {
...
static inline uint32_t __attribute__((always_inline))
dpdk_virtio_dev_to_vm_tx_burst_mergeable(struct dpdk_virtio_writer *p, vr_dpdk_virtioq_t *vq,
                                         struct rte_mbuf **pkts, uint32_t count) {
  ...
  do {
    ...
    success = rte_atomic16_cmpset(&vq->vdv_last_used_idx_res, res_base_idx, res_cur_idx);
  } while (unlikely(success == 0));

  if (pkt_idx) {
    simple_count = dpdk_virtio_dev_to_vm_tx_burst_simple(p, vq, res_base_idx, res_cur_idx, pkts,
                                                         pkt_idx, VIRTIO_HDR_MRG_RXBUF);
  ...
  for (pkt_idx = start_idx; pkt_idx < count; pkt_idx++) {
    ...
    entry_success = copy_from_mbuf_to_vring(vq, vru_cl, res_base_idx, res_cur_idx, buf_vec,
                                            &virtio_hdr, pkts[pkt_idx]);
...
static inline void dpdk_virtio_send_burst(struct dpdk_virtio_writer *p) {
  uint32_t nb_tx = 0;
  int i;
  ...
  if (likely(p->tx_buf_count)) {
    ...
    if (likely(p->tx_virtioq->vdv_send_func != NULL)) {
      nb_tx = p->tx_virtioq->vdv_send_func(p, p->tx_virtioq, p->tx_buf, p->tx_buf_count);
    }
...
static int dpdk_virtio_to_vm_flush(void *port) {
  ...
  if (lcore) {
     ...
     */
    if ((lcore->lcore_fwd_loops - p->last_pkt_tx_flush) < VR_DPDK_TX_FLUSH_LOOPS) {
      if ((lcore->lcore_fwd_loops - p->last_pkt_tx) < VR_DPDK_TX_IDLE_LOOPS) {
        return 0;
      }
    }
  }
...
int vr_dpdk_set_vring_addr(unsigned int vif_idx, unsigned int vring_idx,
                           struct vring_desc *vrucv_desc, struct vring_avail *vrucv_avail,
 ...
 * Returns 0 on success, -1 otherwise.
 */
int vr_dpdk_set_ring_num_desc(unsigned int vif_idx, unsigned int vring_idx, unsigned int num_desc) {
  vr_dpdk_virtioq_t *vq;
 ...
 */
int vr_dpdk_set_ring_callfd(unsigned int vif_idx, unsigned int vring_idx, int callfd) {
  vr_dpdk_virtioq_t *vq;
...
void vr_dpdk_virtio_xstats_update(struct vr_interface_stats *stats, struct vr_dpdk_queue *queue) {
