...
#define DPDK_KNIDEV_READER_STATS_PKTS_IN_ADD(port, val) port->stats.n_pkts_in += val
#define DPDK_KNIDEV_READER_STATS_PKTS_DROP_ADD(port, val) port->stats.n_pkts_drop += val
...
static void *dpdk_knidev_reader_create(void *params, int socket_id) {
  struct dpdk_knidev_reader_params *conf = (struct dpdk_knidev_reader_params *)params;
...
static int dpdk_knidev_reader_rx(void *port, struct rte_mbuf **pkts, uint32_t n_pkts) {
  struct dpdk_knidev_reader *p = (struct dpdk_knidev_reader *)port;
  uint32_t nb_rx;
...
static int dpdk_knidev_reader_free(void *port) {
  if (port == NULL) {
...
static int dpdk_knidev_reader_stats_read(void *port, struct rte_port_in_stats *stats, int clear) {
  struct dpdk_knidev_reader *p = (struct dpdk_knidev_reader *)port;
...
#define DPDK_KNIDEV_WRITER_STATS_PKTS_IN_ADD(port, val) port->stats.n_pkts_in += val
#define DPDK_KNIDEV_WRITER_STATS_PKTS_DROP_ADD(port, val) port->stats.n_pkts_drop += val
...
static void *dpdk_knidev_writer_create(void *params, int socket_id) {
  struct dpdk_knidev_writer_params *conf = (struct dpdk_knidev_writer_params *)params;
  struct dpdk_knidev_writer *port;

  /* Check input parameters */
  if ((conf == NULL) || (conf->tx_burst_sz == 0) || (conf->tx_burst_sz > VR_DPDK_TX_BURST_SZ) ||
...
static inline void send_burst(struct dpdk_knidev_writer *p) {
  uint32_t nb_tx;
...
/*
 ...
 memory region
 */
static void mempool_info_cb(struct rte_mempool *mp, void *opaque, struct rte_mempool_memhdr *memhdr,
                            unsigned index) {
...
static int addr_out_range(struct rte_mempool *mp, uintptr_t addr) {
  ...
  if ((addr < info[vr_elt_va_start]) || (addr > info[vr_elt_va_end])) {
    return true;
  }
  return false;
}
...
static int dpdk_knidev_writer_tx(void *port, struct rte_mbuf *pkt) {
...
static int dpdk_knidev_writer_flush(void *port) {
  struct dpdk_knidev_writer *p = (struct dpdk_knidev_writer *)port;
...
static int dpdk_knidev_writer_free(void *port) {
  if (port == NULL) {
    RTE_LOG(ERR, PORT, "%s: port is NULL\n", __func__);
    return -EINVAL;
  }
...
static int dpdk_knidev_writer_stats_read(void *port, struct rte_port_out_stats *stats, int clear) {
  struct dpdk_knidev_writer *p = (struct dpdk_knidev_writer *)port;
...
static void dpdk_kni_rx_queue_release(unsigned lcore_id,
...
struct vr_dpdk_queue *vr_dpdk_kni_rx_queue_init(unsigned lcore_id, struct vr_interface *vif,
  ...
  if (vif->vif_type == VIF_TYPE_HOST) {
    port_id = (((struct vr_dpdk_ethdev *)(vif->vif_bridge->vif_os))->ethdev_port_id);
  }
  ...
  if (rx_queue->q_queue_h == NULL) {
    RTE_LOG(ERR, VROUTER,
            "    error creating KNI device %s RX queue"
            " at eth device %" PRIu8 "\n",
...
static void dpdk_kni_tx_queue_release(unsigned lcore_id, unsigned queue_index,
                                      struct vr_interface *vif) {
  struct vr_dpdk_lcore *lcore = vr_dpdk.lcores[lcore_id];
...
struct vr_dpdk_queue *vr_dpdk_kni_tx_queue_init(unsigned lcore_id, struct vr_interface *vif,
  ...
  if (vif->vif_type == VIF_TYPE_HOST) {
    ...
    if (ethdev == NULL) {
      RTE_LOG(ERR, VROUTER,
              "    error creating KNI device %s TX queue:"
...
#endif
{
  ...
  if (ethdev && vr_dpdk_ethdev_bond_port_match(port_id, ethdev)) {
    ...
    for (i = 0; i < ethdev->ethdev_nb_slaves; i++) {
      ...
      RTE_LOG(INFO, VROUTER, "    changing bond member eth device %" PRIu8 " MTU to %u\n",
    ...
    if (ret < 0) {
       ...
       * set to support jumbo frames, so continue further here to set vif_mtu.
       */
      RTE_LOG(DEBUG, VROUTER, "Error changing eth device %" PRIu8 " MTU: %s (%d)\n", port_id,
...
#endif
{
  ...
  if (ret < 0) {
    RTE_LOG(ERR, VROUTER,
            "Configuring eth device %" PRIu8 " UP "
            "failed (%d)\n",
            port_id, ret);
  }
...
int vr_dpdk_knidev_init(uint8_t port_id, struct vr_interface *vif) {
  ...
  if (vr_dpdk.kni_state == 0) {
    ...
    if (access("/dev/kni", R_OK | W_OK)) {
      ...
      RTE_LOG(INFO, VROUTER, "    initializing KNI with %d maximum interfaces\n",
  ...
  if (!rte_eth_dev_is_valid_port(port_id)) {
    RTE_LOG(ERR, VROUTER, "    error initializing KNI device %s: invalid eth device %" PRIu8 "\n",
            vif->vif_name, port_id);
    return -EINVAL;
  }
  ...
  if (!pci_dev) {
    RTE_LOG(ERR, VROUTER,
            "    error initializing KNI device %s: eth device"
  ...
  for (i = 0; i < RTE_MAX_LCORE; i++) {
    if (lcore_config[i].detected && rte_conf->lcore_role[VR_DPDK_FWD_LCORE_ID + i] == ROLE_OFF) {
