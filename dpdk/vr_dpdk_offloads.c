...
int dpdk_offload_flow_destroy(struct vr_offload_flow *oflow) {
  ...
  RTE_LOG(DEBUG, VROUTER,
          ...
          "\t\tflow tunnel tag = %u\n"
          "\t\tflow nexthop ID = %u\n",
          oflow->fe_index, oflow->pvif->vif_idx, oflow->ip, oflow->tunnel_type, oflow->tunnel_tag,
          oflow->nh->nh_id);
...
int dpdk_offload_flow_create(struct vr_offload_flow *oflow) {
  ...
  case VP_TYPE_IP:
    ...
    rss_action.types &=
        ~(ETH_RSS_IPV6 | ETH_RSS_FRAG_IPV6 | ETH_RSS_NONFRAG_IPV6_OTHER | ETH_RSS_IPV6_EX);
    RTE_LOG(DEBUG, VROUTER, "\t\tflow L3 tuples: ipv4 src = %u dst = %u\n",
            ntohl(oflow->fe->fe_key.flow4_sip), ntohl(oflow->fe->fe_key.flow4_dip));
    break;
  case VP_TYPE_IP6:
    ...
    memcpy(&iipv6_spec.hdr.src_addr, oflow->fe->fe_key.flow6_sip, VR_IP6_ADDRESS_LEN);
    memcpy(&iipv6_spec.hdr.dst_addr, oflow->fe->fe_key.flow6_dip, VR_IP6_ADDRESS_LEN);
    iipv6_spec.hdr.proto = oflow->fe->fe_key.flow6_proto;
    rss_action.types &= ~(ETH_RSS_IPV4 | ETH_RSS_FRAG_IPV4 | ETH_RSS_NONFRAG_IPV4_OTHER);
    RTE_LOG(DEBUG, VROUTER,
            "\t\tflow L3 tuples: ipv6 src ="
            " %02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x"
            " dst = %02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x\n",
            iipv6_spec.hdr.src_addr[0], iipv6_spec.hdr.src_addr[1], iipv6_spec.hdr.src_addr[2],
            iipv6_spec.hdr.src_addr[3], iipv6_spec.hdr.src_addr[4], iipv6_spec.hdr.src_addr[5],
            iipv6_spec.hdr.src_addr[6], iipv6_spec.hdr.src_addr[7], iipv6_spec.hdr.src_addr[8],
            iipv6_spec.hdr.src_addr[9], iipv6_spec.hdr.src_addr[10], iipv6_spec.hdr.src_addr[11],
            iipv6_spec.hdr.src_addr[12], iipv6_spec.hdr.src_addr[13], iipv6_spec.hdr.src_addr[14],
            iipv6_spec.hdr.src_addr[15], iipv6_spec.hdr.dst_addr[0], iipv6_spec.hdr.dst_addr[1],
            iipv6_spec.hdr.dst_addr[2], iipv6_spec.hdr.dst_addr[3], iipv6_spec.hdr.dst_addr[4],
            iipv6_spec.hdr.dst_addr[5], iipv6_spec.hdr.dst_addr[6], iipv6_spec.hdr.dst_addr[7],
            iipv6_spec.hdr.dst_addr[8], iipv6_spec.hdr.dst_addr[9], iipv6_spec.hdr.dst_addr[10],
            iipv6_spec.hdr.dst_addr[11], iipv6_spec.hdr.dst_addr[12], iipv6_spec.hdr.dst_addr[13],
            iipv6_spec.hdr.dst_addr[14], iipv6_spec.hdr.dst_addr[15]);
    break;
  default:
    RTE_LOG(ERR, VROUTER,
            "Failed to create flow - flow %u has unsupported L3"
            " protocol %u\n",
  ...
  default:
    RTE_LOG(ERR, VROUTER,
            "Failed to create flow - flow %u has unsupported L4"
            " protocol %u\n",
  ...
  RTE_LOG(DEBUG, VROUTER, "flow L4 tuples: %s src = %u dst = %u\n",
          ...
          ntohs(oflow->fe->fe_key.flow_sport), ntohs(oflow->fe->fe_key.flow_dport));
  ...
  case NH_FLAG_TUNNEL_UDP_MPLS:
    ...
    if (oflow->tunnel_type == NH_FLAG_TUNNEL_GRE) {
      ...
      rss_action.types =
          ETH_RSS_IPV4 | ETH_RSS_FRAG_IPV4 | ETH_RSS_NONFRAG_IPV4_OTHER | ETH_RSS_NONFRAG_IPV4_UDP;
    }
    break;
  ...
  default:
    RTE_LOG(ERR, VROUTER,
            "Failed to create flow - flow %u has unsupported tunnel"
            " protocol %u\n",
  ...
  /* Create dpdk flow */
  oflow->flow_handle = rte_flow_create(ethdev->ethdev_port_id, &attr, pattern, actions, &error);
  if (unlikely(oflow->flow_handle == NULL)) {
    RTE_LOG(ERR, VROUTER, "Failed to create flow - %s (%d)\n",
            error.message ? error.message : "no error message", error.type);
    return -rte_errno;
  }
  return 0;
#else
  RTE_SET_USED(oflow);
  RTE_LOG(DEBUG, VROUTER,
          "Cannot create a dpdk offload flow, the version must be"
          " at least 18.05\n");
  return -ENOTSUP;
#endif
}
