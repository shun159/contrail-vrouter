...
static void dpdk_vif_queue_free(struct vr_interface *vif) {
  ...
  do {
    ...
    if (lcore_p->lcore_tx_queues[vif->vif_idx]) {
      vr_free(lcore_p->lcore_tx_queues[vif->vif_idx], VR_INTERFACE_QUEUE_OBJECT);
      lcore_p->lcore_tx_queues[vif->vif_idx] = NULL;
    }

    if (lcore_p->lcore_tx_queue_params[vif->vif_idx]) {
      vr_free(lcore_p->lcore_tx_queue_params[vif->vif_idx], VR_INTERFACE_QUEUE_OBJECT);
      lcore_p->lcore_tx_queue_params[vif->vif_idx] = NULL;
    }

    if (lcore_p->lcore_hw_queue_to_dpdk_index[vif->vif_idx]) {
      vr_free(lcore_p->lcore_hw_queue_to_dpdk_index[vif->vif_idx], VR_INTERFACE_QUEUE_OBJECT);
...
static int dpdk_vif_queue_setup(struct vr_interface *vif) {
  ...
  if (vif->vif_num_hw_queues) {
    ...
    if (!q_data) {
      vif->vif_queue_host_data = vr_malloc(sizeof(*q_data), VR_INTERFACE_QUEUE_OBJECT);
      if (!vif->vif_queue_host_data) {
        goto unwind;
      }

      q_data = (struct vif_queue_dpdk_data *)vif->vif_queue_host_data;
      memset(q_data->vqdd_queue_to_lcore, -1, VR_DPDK_MAX_NB_TX_QUEUES * sizeof(int16_t));
    }
  } else {
  ...
  do {
    if (lcore >= VR_DPDK_PACKET_LCORE_ID) {
      ...
      lcore_p->lcore_tx_queues[vif->vif_idx] = vr_zalloc(
          num_tx_queues_per_lcore * sizeof(struct vr_dpdk_queue), VR_INTERFACE_QUEUE_OBJECT);
      if (!lcore_p->lcore_tx_queues) {
        goto unwind;
      }

      lcore_p->lcore_tx_queue_params[vif->vif_idx] = vr_zalloc(
          num_tx_queues_per_lcore * sizeof(struct vr_dpdk_queue_params), VR_INTERFACE_QUEUE_OBJECT);
      if (!lcore_p->lcore_tx_queue_params) {
        goto unwind;
      }
      lcore_p->num_tx_queues_per_lcore[vif->vif_idx] = num_tx_queues_per_lcore;

      if (vif_max_queue > 0) {
        lcore_p->lcore_hw_queue_to_dpdk_index[vif->vif_idx] =
            (int16_t *)vr_zalloc(sizeof(int16_t) * (vif_max_queue + 1), VR_INTERFACE_QUEUE_OBJECT);
        if (!lcore_p->lcore_hw_queue_to_dpdk_index[vif->vif_idx]) {
          goto unwind;
        }
      }
 ...
 */
static int dpdk_virtual_if_add(struct vr_interface *vif) {
  int ret;
  uint16_t nrxqs, ntxqs;

  RTE_LOG(INFO, VROUTER, "Adding vif %u (gen. %u) virtual device %s\n", vif->vif_idx, vif->vif_gen,
          vif->vif_name);
  ...
  ret = vr_dpdk_lcore_if_schedule(vif, vr_dpdk_lcore_least_used_get(), nrxqs,
                                  &vr_dpdk_virtio_rx_queue_init, ntxqs,
                                  &vr_dpdk_virtio_tx_queue_init);
  if (ret) {
    return ret;
  }
   ...
   */
  return vr_netlink_uvhost_vif_add(vif->vif_name, vif->vif_idx, vif->vif_gen, nrxqs, ntxqs,
                                   vif->vif_vhostuser_mode);
}

/* Creating mock physical/vhost/agent device used for simulating
 * interfaces through vtest */
static int dpdk_mock_vif_add(struct vr_interface *vif) {
  int ret = 0;
 ...
 * Returns 0 on success, < 0 otherwise.
 */
static int dpdk_virtual_vlan_if_add(struct vr_interface *vif) {

  RTE_LOG(INFO, VROUTER, "Adding vif %u (gen. %u) virtual VLAN(o/i) %u/%u device %s\n",
          vif->vif_idx, vif->vif_gen, vif->vif_ovlan_id, vif->vif_vlan_id, vif->vif_name);
  RTE_LOG(INFO, VROUTER, "    parent vif %u device %s\n", vif->vif_parent->vif_idx,
          vif->vif_parent->vif_name);
 ...
 */
static int dpdk_virtual_if_del(struct vr_interface *vif) {
  int ret;
 ...
 * Returns 0 on success, < 0 otherwise.
 */
static int dpdk_virtual_vlan_if_del(struct vr_interface *vif) {
  RTE_LOG(INFO, VROUTER, "Deleting vif %u virtual VLAN(o/i) %u/%u device\n", vif->vif_idx,
          vif->vif_ovlan_id, vif->vif_vlan_id);

  return 0;
}

static inline void dpdk_dbdf_to_pci(unsigned int dbdf, struct rte_pci_addr *address) {
...
static inline unsigned dpdk_pci_to_dbdf(struct rte_pci_addr *address) {
  return address->domain << 16 | address->bus << 8 | address->devid | address->function;
}

/* mirrors the function used in bonding */
static inline uint8_t dpdk_find_port_id_by_pci_addr(const struct rte_pci_addr *addr) {
  uint8_t i;
  ...
  VR_DPDK_RTE_ETH_FOREACH_DEV(i) {
    ...
    if (rte_eth_devices[i].device != NULL) {
      ...
      RTE_LOG_DP(DEBUG, VROUTER, "count %d eth_pci_addr %x %x %x %x \n", i, eth_pci_addr->bus,
                 eth_pci_addr->devid, eth_pci_addr->domain, eth_pci_addr->function);
      RTE_LOG_DP(DEBUG, VROUTER, "count %d addr %x %x %x %x \n", i, addr->bus, addr->devid,
                 addr->domain, addr->function);
      if (addr->bus == eth_pci_addr->bus && addr->devid == eth_pci_addr->devid &&
          addr->domain == eth_pci_addr->domain && addr->function == eth_pci_addr->function) {
        return i;
      }
    } else {
#if (RTE_VERSION >= RTE_VERSION_NUM(17, 11, 0, 0))
...
static inline void dpdk_find_pci_addr_by_port(struct rte_pci_addr *addr, uint8_t port_id) {
#if (RTE_VERSION >= RTE_VERSION_NUM(17, 2, 0, 0))
  rte_memcpy(addr, &(RTE_DEV_TO_PCI(rte_eth_devices[port_id].device)->addr),
...
static inline void dpdk_set_addr_vlan_filter_strip(uint32_t port_id, struct vr_interface *vif) {
  uint32_t i, ret;
#if (RTE_VERSION >= RTE_VERSION_NUM(17, 11, 0, 0))
  ...
  port_id_ptr = (ethdev->ethdev_nb_slaves == -1) ? &ethdev->ethdev_port_id : ethdev->ethdev_slaves;
  ...
  do {
    ...
    if ((ethdev->ethdev_nb_slaves != -1) && vr_dpdk.vf_lcore_id) {
      ret = rte_eth_dev_default_mac_addr_set(*port_id_ptr, (struct ether_addr *)vif->vif_mac);
      if (ret == 0) {
        RTE_LOG(INFO, VROUTER, "Bond slave port %d now uses vif MAC " MAC_FORMAT "\n", *port_id_ptr,
                MAC_VALUE(vif->vif_mac));
      } else {
        RTE_LOG(ERR, VROUTER,
                "Error setting vif MAC to bond slave port %d: "
                "%s (%d)\n",
 ...
 */
static int vr_ethdev_inner_cksum_capable(struct vr_dpdk_ethdev *ethdev) {
  ...
  do {
    ...
    if (dev_info.driver_name) {
      if ((strncmp(dev_info.driver_name, "net_ixgbe", strlen("net_ixgbe")) != 0) &&
          (strncmp(dev_info.driver_name, "net_i40e", strlen("net_i40e")) != 0)) {
        return 0;
      }
    } else {
      return 0;
...
void dpdk_vif_attach_ethdev(struct vr_interface *vif, struct vr_dpdk_ethdev *ethdev) {
  ...
  if (memcmp(vif->vif_mac, mac_addr.addr_bytes, ETHER_ADDR_LEN) == 0) {
    ...
    if (memcmp(vif->vif_mac, mac_addr.addr_bytes, ETHER_ADDR_LEN) != 0) {
      ...
      if (ret == 0) {
        RTE_LOG(INFO, VROUTER, "    eth dev %s now use vif MAC " MAC_FORMAT "\n", vif->vif_name,
                MAC_VALUE(vif->vif_mac));
      } else {
        RTE_LOG(ERR, VROUTER,
                "    error setting vif MAC to eth dev %s: "
                "%s (%d)\n",
 ...
 * Returns 0 on success, < 0 otherwise.
 */
int dpdk_vlan_forwarding_if_add(void) {
  int ret;

  RTE_LOG(INFO, VROUTER, "Adding VLAN forwarding interface %s\n", vr_dpdk.vlan_name);

  /* Allocate vlan vif. */
  vr_dpdk.vlan_vif = vr_zalloc(sizeof(struct vr_interface), VR_INTERFACE_OBJECT);
  if (!vr_dpdk.vlan_vif) {
    RTE_LOG(ERR, VROUTER, "Error allocating interface object\n");
    return -ENOMEM;
  }

  vr_dpdk.vlan_vif->vif_stats =
  ...
  if (ret != 0) {
    RTE_LOG(ERR, VROUTER, "Error initializing device for VLAN forwarding interface: %s (%d)\n",
   ...
   */
  vr_dpdk.vlan_ring = vr_dpdk_ring_allocate(VR_DPDK_FWD_LCORE_ID, vr_dpdk.vlan_name,
                                            VR_DPDK_TX_RING_SZ, RING_F_SC_DEQ);
...
static void vr_af_ethdev_conf_update(struct rte_eth_conf *dev_conf) { dev_conf->intr_conf.lsc = 0; }
 ...
 */
static int dpdk_af_packet_if_add(struct vr_interface *vif) {
  int ret;
  ...
  RTE_LOG(INFO, VROUTER, "Adding vif %u (gen. %u) af_packet device %s\n", vif->vif_idx,
          vif->vif_gen, vif->vif_name);

  ret = snprintf(name, sizeof(name), "eth_af_packet_%d", vif->vif_idx);
  if (ret >= sizeof(name)) {
    RTE_LOG(ERR, VROUTER, "    error creating name for af_packet device %s\n", name);
    return ret;
  }
  ...
  ret = snprintf(params, sizeof(params),
                  ...
                  */
                 "iface=%s,framesz=%d,blocksz=%d", vif->vif_name, frame_size, frame_size);
  if (ret >= sizeof(params)) {
    RTE_LOG(ERR, VROUTER, "    error creating config for af_packet device %s\n", name);
    return ret;
  }
 ...
 */
static void vr_ethdev_conf_update(struct rte_eth_conf *dev_conf) {
  int i;
  struct rte_eth_dev_info dev_info;
  ...
  VR_DPDK_RTE_ETH_FOREACH_DEV(i) {
    ...
    if (dev_info.driver_name) {
      if ((strncmp(dev_info.driver_name, "net_bnxt", strlen("net_bnxt") + 1) == 0) ||
          (strncmp(dev_info.driver_name, "net_enic", strlen("net_enic") + 1) == 0)) {
...
static int dpdk_fabric_if_add(struct vr_interface *vif) {
  int ret;
  ...
  if (vif->vif_flags & VIF_FLAG_PMD) {
    if (vif->vif_os_idx >= ports_num) {
      RTE_LOG(ERR, VROUTER,
              "Error adding vif %u eth device %s: invalid PMD %u"
              " (must be less than %u)\n",
              vif->vif_idx, vif->vif_name, vif->vif_os_idx, ports_num);
      return -ENOENT;
    }
    ...
    */
    ...
    RTE_LOG(INFO, VROUTER,
            "Adding vif %u (gen. %u) eth device %" PRIu8 " (PMD) MAC " MAC_FORMAT
            " (vif MAC " MAC_FORMAT ")\n",
            vif->vif_idx, vif->vif_gen, port_id, MAC_VALUE(mac_addr.addr_bytes),
            MAC_VALUE(vif->vif_mac));
  } else {
    ...
    if (port_id == VR_DPDK_INVALID_PORT_ID) {
      RTE_LOG(ERR, VROUTER,
              "Error adding vif %u eth device %s:"
              " no port ID found for PCI " PCI_PRI_FMT "\n",
              vif->vif_idx, vif->vif_name, pci_address.domain, pci_address.bus, pci_address.devid,
    ...
    rte_eth_macaddr_get(port_id, &mac_addr);
    RTE_LOG(INFO, VROUTER,
            "Adding vif %u (gen. %u) eth device %" PRIu8 " PCI " PCI_PRI_FMT " MAC " MAC_FORMAT
            " (vif MAC " MAC_FORMAT ")\n",
            vif->vif_idx, vif->vif_gen, port_id, pci_address.domain, pci_address.bus,
            pci_address.devid, pci_address.function, MAC_VALUE(mac_addr.addr_bytes),
            MAC_VALUE(vif->vif_mac));
  }
...
static int dpdk_fabric_af_packet_if_del(struct vr_interface *vif) {
  uint8_t port_id;
  struct vr_dpdk_ethdev *ethdev;
#if (RTE_VERSION < RTE_VERSION_NUM(17, 11, 0, 0))
...
/* Add vhost interface */
static int dpdk_vhost_if_add(struct vr_interface *vif) {
  uint8_t port_id;
  int ret;
  struct ether_addr mac_addr;
  struct vr_dpdk_ethdev *ethdev;
  ...
  RTE_LOG(INFO, VROUTER,
          "Adding vif %u (gen. %u) device %s at eth device %" PRIu8 " MAC " MAC_FORMAT
          " (vif MAC " MAC_FORMAT ")\n",
  ...
  if (ethdev->ethdev_nb_slaves > 0) {
    ...
    memset(&mac_addr, 0, sizeof(mac_addr));
    rte_eth_macaddr_get(port_id, &mac_addr);
    RTE_LOG(INFO, VROUTER, "    using bond slave eth device %" PRIu8 " MAC " MAC_FORMAT "\n",
  ...
  switch (ret) {
  case 0:
    ret = vr_dpdk_lcore_if_schedule(vif, vr_dpdk_lcore_least_used_get(), 1,
                                    &vr_dpdk_kni_rx_queue_init, 1, &vr_dpdk_kni_tx_queue_init);
    break;
  case -ENOTSUP:
    /* Try TAP instead. */
    ...
    ret = vr_dpdk_lcore_if_schedule(vif, vr_dpdk_lcore_least_used_get(), 1,
                                    &vr_dpdk_tapdev_rx_queue_init, nb_txqs,
                                    &vr_dpdk_tapdev_tx_queue_init);
    break;
  default:
    /* Other KNI error. */
    return ret;
...
static int dpdk_vhost_if_del(struct vr_interface *vif) {
...
static void dpdk_monitoring_start(struct vr_interface *monitored_vif,
...
static void dpdk_monitoring_stop(struct vr_interface *monitored_vif,
                                 struct vr_interface *monitoring_vif) {
...
static int dpdk_monitoring_if_add(struct vr_interface *vif) {
  int ret;
  ...
  RTE_LOG(INFO, VROUTER,
          "Adding monitoring vif %u (gen. %u) device %s"
          " to monitor vif %u\n",
  ...
  case 0:
    ...
    ret = vr_dpdk_lcore_if_schedule(vif, vr_dpdk_lcore_least_used_get(), 0, NULL, 1,
                                    &vr_dpdk_kni_tx_queue_init);
    break;
  case -ENOTSUP:
    /* Try TAP instead. */
    ret = vr_dpdk_tapdev_init(vif);
    ...
    /* Schedule the TAP interface with 1 RX queue and unlimited TX queues. */
    /* Write-only interface. */
    ret = vr_dpdk_lcore_if_schedule(vif, vr_dpdk_lcore_least_used_get(), 0, NULL, nb_txqs,
                                    &vr_dpdk_tapdev_tx_queue_init);
    break;
  default:
    /* Other KNI error. */
...
static int dpdk_monitoring_if_del(struct vr_interface *vif) {
  unsigned short monitored_vif_id = vif->vif_os_idx;
  struct vr_interface *monitored_vif;

  RTE_LOG(INFO, VROUTER,
          "Deleting monitoring vif %u device"
          " to monitor vif %u\n",
          vif->vif_idx, monitored_vif_id);

  /* check if vif exist */
  monitored_vif = __vrouter_get_interface(vrouter_get(vif->vif_rid), monitored_vif_id);
  if (!monitored_vif) {
    RTE_LOG(ERR, VROUTER,
            "    error getting vif to monitor:"
            " vif %u does not exist\n",
...
/* Add agent interface */
static int dpdk_agent_if_add(struct vr_interface *vif) {
  int ret;
  uint16_t ports_num;
...
static int dpdk_agent_if_del(struct vr_interface *vif) {
...
/* vRouter callback */
static int dpdk_if_add(struct vr_interface *vif) {
  ...
  RTE_LOG(ERR, VROUTER, "Error adding vif %d (%s): unsupported interface type %d transport %d\n",
          vif->vif_idx, vif->vif_name, vif->vif_type, vif->vif_transport);

  return -EFAULT;
}

static int dpdk_if_del(struct vr_interface *vif) {
  if (vr_dpdk_is_stop_flag_set())
    return -EINPROGRESS;
  ...
  RTE_LOG(ERR, VROUTER, "Error deleting vif %d: unsupported interface type %d transport %d\n",
...
/* vRouter callback */
static int dpdk_if_del_tap(struct vr_interface *vif) {
  /* TODO: we untap interfaces at if_del */
  return 0;
}

/* vRouter callback */
static int dpdk_if_add_tap(struct vr_interface *vif) {
  /* TODO: we tap interfaces at if_add */
  return 0;
}

static int dpdk_pkt_is_gso(struct rte_mbuf *m) {
  /*
...
static inline void dpdk_hw_checksum_at_offset(struct vr_packet *pkt, unsigned offset) {
...
static inline void dpdk_ipv4_sw_iphdr_checksum_at_offset(struct vr_packet *pkt, unsigned offset) {
  struct vr_ip *iph;
...
static inline void dpdk_sw_checksum_at_offset(struct vr_packet *pkt, unsigned offset) {
  struct vr_ip *iph = NULL;
  struct vr_ip6 *ip6h = NULL;
  unsigned char iph_len = 0, iph_proto = 0;
  struct vr_udp *udph;
...
static inline void dpdk_ipv4_outer_tunnel_hw_checksum(struct vr_packet *pkt) {
  struct rte_mbuf *m = vr_dpdk_pkt_to_mbuf(pkt);
  unsigned offset = pkt->vp_data + dpdk_get_ether_header_len(pkt_data_at_offset(pkt, pkt->vp_data));
...
static inline void dpdk_ipv4_outer_tunnel_sw_checksum(struct vr_packet *pkt) {
  unsigned offset = pkt->vp_data + dpdk_get_ether_header_len(pkt_data_at_offset(pkt, pkt->vp_data));
  struct vr_ip *iph = (struct vr_ip *)pkt_data_at_offset(pkt, offset);
...
static inline void dpdk_hw_checksum(struct vr_packet *pkt) {
  /* if a tunnel */
  if (vr_pkt_type_is_overlay(pkt->vp_type)) {
    /* calculate outer checksum in soft */
    dpdk_ipv4_sw_iphdr_checksum_at_offset(
        pkt, pkt->vp_data + dpdk_get_ether_header_len(pkt_data_at_offset(pkt, pkt->vp_data)));
    ...
    dpdk_hw_checksum_at_offset(
        pkt, pkt->vp_data + dpdk_get_ether_header_len(pkt_data_at_offset(pkt, pkt->vp_data)));
  }
}

static inline void dpdk_sw_checksum(struct vr_packet *pkt, bool will_fragment) {
  /* if a tunnel */
  if (vr_pkt_type_is_overlay(pkt->vp_type)) {
 ...
 */
static int dpdk_fragment_packet(struct vr_packet *pkt, struct rte_mbuf *mbuf_in,
  ...
  outer_header_len = pkt_get_inner_network_header_off(pkt) - pkt_head_space(pkt);
  ...
  number_of_packets = rte_ipv4_fragment_packet(mbuf_in, mbuf_out, out_num, max_frag_size,
  ...
  for (i = 0; i < number_of_packets; ++i) {
    ...
    if (outer_ip->ip_proto == VR_IP_PROTO_UDP) {
      ...
      struct vr_udp *udp = (struct vr_udp *)((char *)outer_ip + header_len);
      udp->udp_length = rte_cpu_to_be_16(rte_be_to_cpu_16(outer_ip->ip_len) - header_len);
    }
...
/* TX packet callback */
static int dpdk_if_tx(struct vr_interface *vif, struct vr_packet *pkt) {
  uint8_t queue_index;
  int ret, i;
  ...
  if (lcore->lcore_hw_queue_to_dpdk_index[vif->vif_idx]) {
    dpdk_queue_index = lcore->lcore_hw_queue_to_dpdk_index[vif->vif_idx][queue_index];
  } else {
    dpdk_queue_index = 0;
  }
   ...
   * an interface. */
  will_fragment = (vr_pkt_type_is_overlay(pkt->vp_type) && vif->vif_mtu < rte_pktmbuf_pkt_len(m));
  ...
  if (!will_segment) {
    if (unlikely(pkt->vp_flags & VP_FLAG_CSUM_PARTIAL)) {
      ...
      if (likely((vif->vif_flags & VIF_FLAG_TX_CSUM_OFFLOAD) && !will_fragment))
        ...
        if (unlikely(will_fragment && (vif->vif_flags & VIF_FLAG_TX_CSUM_OFFLOAD)))
   ...
   *
   */
  if (unlikely(vr_dpdk.vlan_tag != VLAN_ID_INVALID && vif_is_fabric(vif)) || vr_dpdk.vtest_vlan) {
  ...
  if (unlikely(will_segment)) {
    num_of_segs = dpdk_segment_packet(pkt, m, mbufs_segs_out, VR_DPDK_FRAG_MAX_IP_SEGS,
                                      m->tso_segsz, (vif->vif_flags & VIF_FLAG_TX_CSUM_OFFLOAD));
    if (num_of_segs < 0) {
      RTE_LOG_DP(DEBUG, VROUTER,
                 "%s: error %d during GSO of an "
                 "IP packet for interface %s on lcore %u\n",
                 __func__, num_of_segs, vif->vif_name, lcore_id);
      vr_dpdk_pfree(m, pkt->vp_if, VP_DROP_INTERFACE_DROP);
      return -1;
    }
  } else if (unlikely(will_fragment)) {
    num_of_frags =
        dpdk_fragment_packet(pkt, m, mbufs_frags_out, VR_DPDK_FRAG_MAX_IP_FRAGS, vif->vif_mtu,
                             !(vif->vif_flags & VIF_FLAG_TX_CSUM_OFFLOAD), lcore_id);
    if (num_of_frags < 0) {
      RTE_LOG_DP(DEBUG, VROUTER,
                 "%s: error %d during fragmentation of an "
                 "IP packet for interface %s on lcore %u\n",
                 __func__, num_of_frags, vif->vif_name, lcore_id);
      vr_dpdk_pfree(m, pkt->vp_if, VP_DROP_INTERFACE_DROP);
      return -1;
    }
  }
  ...
  if (unlikely(num_of_frags > 1)) {
    ...
    if (likely(tx_queue->txq_ops.f_tx_bulk != NULL)) {
      ...
      RTE_LOG_DP(DEBUG, VROUTER,
                 "%s: error TXing to interface %s: no queue "
                 "for lcore %u\n",
...
static int dpdk_if_rx(struct vr_interface *vif, struct vr_packet *pkt) {
   ...
   */
  if (unlikely((vr_dpdk.kni_state <= 0) && vif_is_virtual(pkt->vp_if) && vif_is_vhost(vif)))
...
static int dpdk_if_get_vlan_info(struct vr_interface *vif,
...
static int dpdk_if_get_bond_info(struct vr_interface *vif,
...
static int dpdk_if_get_settings(struct vr_interface *vif, struct vr_interface_settings *settings) {
  uint8_t port_id;
  struct rte_eth_link link;
  ...
  if (link.link_speed != 0) {
    ...
    settings->vis_duplex = link.link_duplex == ETH_LINK_FULL_DUPLEX ? 1 : 0;
  } else {
    /* default values */
...
static unsigned int dpdk_if_get_mtu(struct vr_interface *vif) {
  uint8_t port_id;
  uint16_t mtu;
  unsigned l3_mtu;
...
static void dpdk_if_unlock(void) { vr_dpdk_if_unlock(); }

static void dpdk_if_lock(void) { vr_dpdk_if_lock(); }

static unsigned short dpdk_if_get_encap(struct vr_interface *vif) { return VIF_ENCAP_TYPE_ETHER; }

/* Update port statistics */
static void dpdk_port_stats_update(struct vr_interface *vif, unsigned lcore_id) {
  unsigned int i;
  ...
  if (queue->q_vif == vif) {
    ...
    if (queue->rxq_ops.f_stats != NULL) {
      if (queue->rxq_ops.f_stats(queue->q_queue_h, &rx_stats, 0) == 0) {
  ...
  for (i = 0; i < lcore->num_tx_queues_per_lcore[vif->vif_idx]; i++) {
    ...
    if (queue && (queue->q_vif == vif)) {
      ...
      if (queue->txq_ops.f_stats != NULL) {
        if (queue->txq_ops.f_stats(queue->q_queue_h, &tx_stats, 0) == 0) {
 ...
 */
static void vr_dpdk_eth_xstats_get(uint32_t port_id, struct rte_eth_stats *eth_stats) {
  /*
  ...
  do {
    ...
    if (nb_xstats > 0) {
      eth_xstats = rte_malloc("xstats", sizeof(*eth_xstats) * nb_xstats, 0);
      if (eth_xstats != NULL) {
        if (rte_eth_xstats_get(*port_id_ptr, eth_xstats, nb_xstats) == nb_xstats) {
          /* look for XEC counter */
          for (i = 0; i < nb_xstats; i++) {
            if (strncmp(eth_xstats[i].name, "l3_l4_xsum_error", sizeof(eth_xstats[i].name)) == 0) {
    ...
    if (nb_xstats > 0) {
      ...
      if (xstats_names != NULL) {
        if (nb_xstats != rte_eth_xstats_get_names(*port_id_ptr, xstats_names, nb_xstats)) {
      ...
      eth_xstats = rte_malloc("xstats", sizeof(struct rte_eth_xstat) * nb_xstats, 0);
      if (eth_xstats != NULL) {
        if (rte_eth_xstats_get(*port_id_ptr, eth_xstats, nb_xstats) == nb_xstats) {
          /* look for XEC counter */
          for (i = 0; i < nb_xstats; i++) {
            if (strncmp(xstats_names[i].name, "l3_l4_xsum_error", sizeof(xstats_names[i].name)) ==
                0) {
              eth_stats->ierrors -= eth_xstats[i].value;
              break;
            }
...
static void dpdk_dev_stats_update(struct vr_interface *vif, unsigned lcore_id) {
  uint8_t port_id;
  ...
  struct vif_queue_dpdk_data *q_data = (struct vif_queue_dpdk_data *)vif->vif_queue_host_data;
  ...
  for (i = 0; i < num_queues; i++) {
    ...
    if (lcore->lcore_tx_queues[vif->vif_idx]) {
      if (lcore->lcore_hw_queue_to_dpdk_index[vif->vif_idx]) {
        dpdk_queue_index = lcore->lcore_hw_queue_to_dpdk_index[vif->vif_idx][queue_id];
      } else {
        dpdk_queue_index = 0;
      }
...
static void dpdk_if_stats_update(struct vr_interface *vif, unsigned core) {
  int i;
...
void vr_host_vif_init(struct vrouter *router) { return; }

struct vr_host_interface_ops *vr_host_interface_init(void) {
  return &dpdk_interface_ops;
}

void vr_host_interface_exit(void) { return; }
