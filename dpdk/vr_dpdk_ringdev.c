...
struct rte_ring *vr_dpdk_ring_allocate(unsigned host_lcore_id, char *ring_name,
  ...
  ring = (struct rte_ring *)rte_malloc_socket(ring_name, ring_size, RTE_CACHE_LINE_SIZE,
                                              rte_lcore_to_socket_id(host_lcore_id));
  if (ring == NULL)
    return NULL;
...
void dpdk_ring_to_push_add(unsigned lcore_id, struct rte_ring *tx_ring,
  ...
  RTE_LOG(INFO, VROUTER, "    lcore %u now has %" PRIu16 " ring(s) to push\n", lcore_id,
          lcore->lcore_nb_rings_to_push);
 ...
 * The function is called by the NetLink lcore only.
 */
static void dpdk_ring_tx_queue_release(unsigned lcore_id, unsigned queue_index,
  ...
  /* remove the ring from the list of rings to push */
  dpdk_ring_to_push_remove(tx_queue_params->qp_ring.host_lcore_id, tx_queue_params->qp_ring.ring_p);
...
struct vr_dpdk_queue *vr_dpdk_ring_tx_queue_init(unsigned lcore_id, struct vr_interface *vif,
  ...
  ret = snprintf(ring_name, sizeof(ring_name), "vr_dpdk_ring_%u_%u_%u", host_lcore_id, vif_idx,
                 lcore_id);
...
error:
  RTE_LOG(ERR, VROUTER, "    error initializing ring TX queue for device %" PRIu8 "\n", port_id);
  return NULL;
}
