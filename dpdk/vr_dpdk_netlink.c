...
static void dpdk_nl_process_response(void *usockp, struct nlmsghdr *nlh) {
  __u32 seq;
...
unsigned int dpdk_nl_message_len(struct vr_message *message) {
  return message->vr_message_len + HDR_LEN;
}

struct nlmsghdr *dpdk_nl_message_hdr(struct vr_message *message) {
  return (struct nlmsghdr *)(message->vr_message_buf - HDR_LEN);
}

static void dpdk_nl_trans_free(char *buf) {
  buf -= HDR_LEN;
...
static char *dpdk_nl_trans_alloc(unsigned int size) {
  char *buf;
...
int vr_netlink_uvhost_vif_del(unsigned int vif_idx) {
   ...
   */
  if (send(vr_nl_uvh_sock, (void *)&msg, sizeof(msg), 0) != sizeof(msg)) {
    RTE_LOG(ERR, VROUTER,
            "    error deleting vif %u from user space vhost:"
            " %s (%d)\n",
 ...
 * Returns 0 on success, -1 otherwise.
 */
int vr_netlink_uvhost_vif_add(char *vif_name, unsigned int vif_idx, unsigned int vif_gen,
  ...
  strncpy(msg.vrnum_vif_add.vrnu_vif_name, vif_name, sizeof(msg.vrnum_vif_add.vrnu_vif_name) - 1);
  ...
  if (send(vr_nl_uvh_sock, (void *)&msg, sizeof(msg), 0) != sizeof(msg)) {
    RTE_LOG(ERR, VROUTER,
            "    error adding vif %u to user space vhost:"
            " %s (%d)\n",
            vif_idx, rte_strerror(errno), errno);
    return -1;
  }
 ...
 */
static int vr_nl_uvhost_connect(void) {
  int s = 0, ret = -1, err;
  ...
  strncat(nl_sun.sun_path, "/" VR_NL_UVH_SOCK_NAME,
          sizeof(nl_sun.sun_path) - strlen(nl_sun.sun_path) - 1);
  ...
  strncat(uvh_sun.sun_path, "/" VR_UVH_NL_SOCK_NAME,
          sizeof(uvh_sun.sun_path) - strlen(uvh_sun.sun_path) - 1);
...
int vr_dpdk_netlink_init(void) {
  ...
  if (!vr_dpdk.netlink_sock) {
    RTE_LOG(ERR, VROUTER,
            "    error creating NetLink server socket:"
            " %s (%d)\n",
            rte_strerror(errno), errno);
    goto error;
  }
  ...
  if (vr_usocket_bind_usockets(vr_dpdk.netlink_sock, event_sock)) {
