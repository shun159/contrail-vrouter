...
int vr_dpdk_gro_init(unsigned lcore_id, struct vr_dpdk_lcore *lcore) {
  ...
  if (lcore->gro.gro_tbl_v4_handle == NULL) {
    RTE_LOG(ERR, VROUTER,
            "Warning! %s: lcore:%d, Unable to allocate memory "
            "for IPv4 GRO table\n",
            __func__, lcore_id);
    return -ENOMEM;
  }
...
static uint16_t dpdk_gro_csum_tcph(struct vr_tcp *tcph) {
  uint32_t ch;
  uint16_t *p, l;
...
static uint16_t dpdk_gro_rx_csum_fixup(struct gro_entry *entry, uint8_t is_ipv6, void *nw_hdr,
...
static int dpdk_gro_flush(struct gro_ctrl *gro, void *key, struct gro_entry *entry) {
  int ret;
  ...
  if (entry->mbuf_cnt > 1) {
    ...
    m->pkt_len = entry->p_len + pkt_get_network_header_off(pkt) - pkt_head_space(pkt);
    ...
    while (entry->ulp_csum > 0xffff)
      entry->ulp_csum = (entry->ulp_csum >> 16) + (entry->ulp_csum & 0xffff);
  ...
  if (unlikely((vif->vif_idx != entry->dst_vif_idx) || (vif->vif_gen != entry->dst_vif_gen))) {
    drop_reason = VP_DROP_INVALID_IF;
    goto drop;
  }
...
void dpdk_gro_free_all_flows(struct vr_dpdk_lcore *lcore) {
  ...
  if (gro->gro_tbl_v4_handle != NULL) {
    while (rte_hash_iterate(gro->gro_tbl_v4_handle, (const void **)&next_key, (void **)&entry,
                            &iter) >= 0) {
  ...
  if (gro->gro_tbl_v6_handle != NULL) {
    while (rte_hash_iterate(gro->gro_tbl_v6_handle, (const void **)&next_key, (void **)&entry,
                            &iter) >= 0) {
      vr_dpdk_pfree(entry->mbuf_head, NULL, VP_DROP_DISCARD);
      rte_free(entry);
    }
...
int dpdk_gro_process(struct vr_packet *pkt, struct vr_interface *vif, bool l2_pkt) {
   ...
   */
  if ((vif->vif_flags & (VIF_FLAG_GRO_NEEDED | VIF_FLAG_MRG_RXBUF)) !=
  ...
  if (unlikely((lcore->gro.gro_tbl_v4_handle == NULL) || (lcore->gro.gro_tbl_v6_handle == NULL))) {
    return 0;
  }
  ...
  if ((nh == NULL) || (nh->nh_dev == NULL) || (!vif_is_virtual(nh->nh_dev))) {
    ret = GRO_NORMAL;
    goto func_exit;
  }
  ...
  if (ret >= 0) {
    ...
    if (unlikely(rte_be_to_cpu_32(tcph->tcp_seq) != entry->next_seq || (tcp_data_len == 0))) {
    ...
    if (l != 0) {
       ...
       */
      if (unlikely(entry->tsval > tsval || rte_be_to_cpu_32(*(ts_ptr + 2)) == 0)) {
        dpdk_gro_flush(&lcore->gro, key, entry);
        ret = GRO_CANNOT;
        goto func_exit;
      }
  ...
  create:
    ...
    if (!is_ipv6) {
      ret = (rte_hash_add_key_data(lcore->gro.gro_tbl_v4_handle, &flow4, entry) < 0) ? GRO_ERROR
                                                                                     : GRO_MERGED;
    } else {
      ret = (rte_hash_add_key_data(lcore->gro.gro_tbl_v6_handle, &flow6, entry) < 0) ? GRO_ERROR
                                                                                     : GRO_MERGED;
    }
