...
static inline int vr_offload_flow_set(struct vr_flow_entry *fe, unsigned int fe_index,
 ...
 */
static inline int vr_offload_flow_meta_data_set(unsigned int fe_index, unsigned int meta_data_len,
                                                void *meta_data, unsigned short mir_vrf) {
  struct vr_offload_ops *offload;
  int ret = 0;
  ...
  if (offload && offload->voo_flow_meta_data_set)
    ret = offload->voo_flow_meta_data_set(fe_index, meta_data_len, meta_data, mir_vrf);
  vr_rcu_read_unlock();
...
static inline int vr_offload_nexthop_get(struct vr_nexthop *nh, vr_nexthop_req *resp) {
  struct vr_offload_ops *offload;
  int ret = 0;
...
static inline int vr_offload_mirror_add(struct vr_mirror_entry *mirror, unsigned int index) {
  struct vr_offload_ops *offload;
  int ret = 0;
