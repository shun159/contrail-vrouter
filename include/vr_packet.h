...
#define PKT_SRC_ANY_REPL_TREE                                                                      \
  (PKT_SRC_TOR_REPL_TREE | PKT_SRC_SRC_REPL_TREE | PKT_SRC_EDGE_REPL_TREE)
...
struct vr_packet {
  ...
  unsigned char vp_priority : 4, vp_notused : 4;
};
...
extern struct vr_packet *pkt_copy(struct vr_packet *, unsigned short, unsigned short);
...
__attribute__packed__open__ struct vr_eth {
...
__attribute__packed__open__ struct vr_vlan_hdr {
  unsigned short vlan_tag;
  unsigned short vlan_proto;
} __attribute__packed__close__;

__attribute__packed__open__ struct vr_pbb_itag {
  uint8_t pbbi_pcp : 3, pbbi_dei : 1, pbbi_uca : 1, pbbi_res : 3;
  uint32_t pbbi_isid : 24;
} __attribute__packed__close__;

__attribute__packed__open__ struct vr_eth_hbs_md {
...
static inline int8_t vr_vlan_get_tos(uint8_t *eth_data) {
...
__attribute__packed__open__ struct vr_arp {
...
static inline bool vr_grat_arp(struct vr_arp *sarp) {
  /*
...
__attribute__packed__open__ struct vr_ip {
#if defined(__KERNEL__) && defined(__linux__)
#if defined(__LITTLE_ENDIAN_BITFIELD)
  unsigned char ip_hl : 4, ip_version : 4;
#elif defined(__BIG_ENDIAN_BITFIELD)
  unsigned char ip_version : 4, ip_hl : 4;
#endif
...
static inline void vr_incremental_diff(unsigned int oldval, unsigned int newval,
                                       unsigned int *diff) {
  unsigned int tmp;
...
static inline void vr_ip_incremental_csum(struct vr_ip *ip, unsigned int diff) {
  unsigned int csum;
...
static inline uint8_t vr_inet_get_tos(struct vr_ip *iph) { return (iph->ip_tos & 0xFC) >> 2; }

static inline void vr_inet_set_tos(struct vr_ip *iph, uint8_t tos) {
  unsigned int diff = 0;
...
__attribute__packed__open__ struct vr_neighbor_option {
  uint8_t vno_type;
  uint8_t vno_length;
  uint8_t vno_value[0];
} __attribute__packed__close__;

__attribute__packed__open__ struct vr_ip6_frag {
...
__attribute__packed__open__ struct vr_ip6_pseudo {
...
__attribute__packed__open__ struct vr_ip6 {
#ifdef __KERNEL__
#if defined(__LITTLE_ENDIAN_BITFIELD)
  uint8_t ip6_priority_h : 4, ip6_version : 4;
  uint8_t ip6_flow_h : 4, ip6_priority_l : 4;
#elif defined(__BIG_ENDIAN_BITFIELD)
  uint8_t ip6_version : 4, ip6_priority_h : 4;
  uint8_t ip6_priority_l : 4, ip6_flow_h : 4;
#endif
#else
#if (__BYTE_ORDER == __LITTLE_ENDIAN)
  uint8_t ip6_priority_h : 4, ip6_version : 4;
...
static inline uint8_t vr_inet6_get_tos(struct vr_ip6 *ip6h) {
  return ((((ip6h->ip6_priority_h << 4) | (ip6h->ip6_priority_l)) >> 2) & 0x3F);
}

static inline void vr_inet6_set_tos(struct vr_ip6 *ip6h, uint8_t tos) {
...
static inline void vr_inet6_generate_ip6(uint8_t *ip6, uint32_t ip) {
  memset(ip6, 0, VR_IP4_MAPPED_IP6_ZERO_BYTES);
  memset(ip6 + VR_IP4_MAPPED_IP6_ZERO_BYTES, 1, VR_IP4_MAPPED_IP6_ONE_BYTES);
  memcpy(ip6 + VR_IP4_MAPPED_IP6_ZERO_BYTES + VR_IP4_MAPPED_IP6_ONE_BYTES, &ip, sizeof(ip));
...
int vr_ip_transport_parse(struct vr_ip *iph, struct vr_ip6 *ip6h, void **thp,
                          unsigned int frag_size,
                          void(do_tcp_mss_adj)(struct tcphdr *, unsigned short, unsigned char),
                          unsigned int *hlenp, unsigned short *th_csump,
                          unsigned int *tcph_pull_lenp, unsigned int *pull_lenp);
...
#define IS_BMCAST_IP(ip) (((ntohl(ip) & MCAST_IP_MASK) == MCAST_IP) || (ip == 0xFFFFFFFF))

static inline void vr_mcast_mac_from_isid(uint32_t isid, uint8_t *mac) {
  if (!mac)
    return;
...
#define VR_IP_ADDR_SIZE(type) ((type == VP_TYPE_IP6) ? VR_IP6_ADDRESS_LEN : VR_IP_ADDRESS_LEN)

static inline unsigned char vr_eth_proto_to_pkt_type(unsigned short eth_proto) {
...
static inline bool vr_ip_is_ip4(struct vr_ip *iph) {
  if ((iph->ip_version & 0xf) == 0x4)
    return true;
  return false;
}

static inline bool vr_ip_is_ip6(struct vr_ip *iph) {
...
static inline bool vr_ip_dont_fragment_set(struct vr_packet *pkt) {
  struct vr_ip *ip;
...
static inline bool vr_ip_fragment_tail(struct vr_ip *iph) {
...
static inline bool vr_pkt_is_ip(struct vr_packet *pkt) {
  if (pkt->vp_type == VP_TYPE_IPOIP || pkt->vp_type == VP_TYPE_IP || pkt->vp_type == VP_TYPE_IP6OIP)
    return true;

  return false;
}

static inline bool vr_pkt_type_is_overlay(unsigned short type) {
...
static inline bool vr_pkt_needs_csum_gso_update(struct vr_packet *pkt) {
...
static inline bool vr_pkt_is_diag(struct vr_packet *pkt) {
  if (pkt->vp_flags & VP_FLAG_DIAG)
    return true;
  return false;
}

static inline void vr_pkt_set_diag(struct vr_packet *pkt) {
  pkt->vp_flags |= VP_FLAG_DIAG;
  return;
}

static inline bool vr_pkt_is_gro(struct vr_packet *pkt) {
  if (pkt->vp_flags & VP_FLAG_GRO)
    return true;
  return false;
}

static inline void vr_pkt_set_gro(struct vr_packet *pkt) {
  pkt->vp_flags |= VP_FLAG_GRO;
  return;
}

static inline void vr_pkt_unset_gro(struct vr_packet *pkt) {
  pkt->vp_flags &= ~VP_FLAG_GRO;
  return;
}

static inline bool vr_ip_fragment_head(struct vr_ip *iph) {
  unsigned short frag = ntohs(iph->ip_frag_off);
  bool more = (frag & VR_IP_MF) ? true : false;
  unsigned short offset = frag & VR_IP_FRAG_OFFSET_MASK;
...
static inline bool vr_ip_fragment(struct vr_ip *iph) {
  unsigned short frag = ntohs(iph->ip_frag_off);
  bool more = (frag & VR_IP_MF) ? true : false;
  unsigned short offset = frag & VR_IP_FRAG_OFFSET_MASK;
...
static inline bool vr_ip_transport_header_valid(struct vr_ip *iph) {
  unsigned short frag = ntohs(iph->ip_frag_off);
  unsigned short offset = frag & VR_IP_FRAG_OFFSET_MASK;
...
__attribute__packed__open__ struct vr_tcp {
  ...
  union {
    ...
    struct {
      uint16_t tcp_res1 : 4, tcp_doff : 4, tcp_flag_fin : 1, tcp_flag_syn : 1, tcp_flag_rst : 1,
          tcp_flag_psh : 1, tcp_flag_ack : 1, tcp_flag_urg : 1, tcp_flag_ece : 1, tcp_flag_cwr : 1;
    };
  };
...
__attribute__packed__open__ struct vr_udp {
...
__attribute__packed__open__ struct vr_sctp {
...
bool vr_adjust_tcp_mss(struct vr_tcp *tcph, uint16_t len_overhead, uint16_t *old_mss,
                       uint16_t *new_mss);

bool __vr_adjust_tcp_mss(struct vr_tcp *tcph, uint16_t len_overhead, uint16_t eth_mtu,
                         uint16_t *old_mss, uint16_t *new_mss);
...
__attribute__packed__open__ struct vr_icmp {
  uint8_t icmp_type;
  uint8_t icmp_code;
  uint16_t icmp_csum;
...
static inline bool vr_icmp_echo(struct vr_icmp *icmph) {
  uint8_t type = icmph->icmp_type;

  if ((type == VR_ICMP_TYPE_ECHO) || (type == VR_ICMP_TYPE_ECHO_REPLY))
    return true;

  return false;
}

static inline bool vr_v6_prefix_is_ll(uint8_t *prefix) {
...
static inline bool vr_v6_prefix_null(uint8_t *prefix) {
...
static inline bool vr_icmp_error(struct vr_icmp *icmph) {
  uint8_t type = icmph->icmp_type;

  if ((type == VR_ICMP_TYPE_DEST_UNREACH) || (type == VR_ICMP_TYPE_TIME_EXCEEDED))
    return true;

  return false;
}

static inline bool vr_ip6_transport_header_valid(struct vr_ip6 *ip6) {
...
static inline bool vr_ip6_fragment(struct vr_ip6 *ip6) {
  if (ip6->ip6_nxt == VR_IP6_PROTO_FRAG)
    return true;
...
static inline bool vr_ip6_fragment_head(struct vr_ip6 *ip6) {
  struct vr_ip6_frag *frag;
  unsigned short offset;
  bool more;
...
static inline bool vr_icmp6_error(struct vr_icmp *icmp6) {
...
__attribute__packed__open__ struct vr_gre {
  unsigned short gre_flags;
  unsigned short gre_proto;
} __attribute__packed__close__;

__attribute__packed__open__ struct vr_gre_key {
...
__attribute__packed__open__ struct vr_vxlan {
 ...
 */
#define VR_L3_MCAST_PKT_HEAD_SPACE                                                                 \
  (sizeof(struct vr_eth) + sizeof(struct vr_vlan_hdr) + sizeof(struct vr_ip) +                     \
   sizeof(struct vr_udp) + 4)

/*
 * The complete VXlan header contains IP and UDP header
 */
#define VR_VXLAN_HDR_LEN (sizeof(struct vr_vxlan) + sizeof(struct vr_ip) + sizeof(struct vr_udp))
 ...
 */
#define VR_L2_MCAST_PKT_HEAD_SPACE                                                                 \
  (VR_L3_MCAST_PKT_HEAD_SPACE + VR_VXLAN_HDR_LEN + VR_L2_CTRL_DATA_LEN)
...
enum { VR_LABEL_TYPE_UNKNOWN, VR_LABEL_TYPE_MPLS, VR_LABEL_TYPE_VXLAN_ID };
...
static inline void vr_init_forwarding_md(struct vr_forwarding_md *fmd) {
  fmd->fmd_fe = NULL;
...
static inline bool vr_fmd_label_is_vxlan_id(struct vr_forwarding_md *fmd) {
  if (fmd->fmd_flags & FMD_FLAG_LABEL_IS_VXLAN_ID)
    return true;
  return false;
}

static inline void vr_fmd_update_etree(struct vr_forwarding_md *fmd, bool valid) {
  if (valid)
...
static inline void vr_fmd_update_etree_root(struct vr_forwarding_md *fmd, bool root) {
  if (root)
...
static inline bool vr_fmd_etree_is_enabled(struct vr_forwarding_md *fmd) {
...
static inline bool vr_fmd_etree_is_root(struct vr_forwarding_md *fmd) {
...
static inline void vr_fmd_update_label_type(struct vr_forwarding_md *fmd, unsigned int type) {
...
static inline void vr_fmd_set_label(struct vr_forwarding_md *fmd, unsigned int label,
                                    unsigned int type) {
  fmd->fmd_label = label;
...
static inline void vr_fmd_update_l2_control_data(struct vr_forwarding_md *fmd, bool enable) {
  if (enable)
...
static inline bool vr_fmd_l2_control_data_is_enabled(struct vr_forwarding_md *fmd) {
...
static inline uint16_t vr_fmd_get_mirror_vlan(struct vr_forwarding_md *fmd) {
...
static inline mirror_type_t vr_fmd_get_mirror_type(struct vr_forwarding_md *fmd) {
  if (fmd->fmd_mirror_data != FMD_MIRROR_INVALID_DATA) {
    if (fmd->fmd_mirror_data & 0x8000)
...
static inline uint16_t vr_fmd_get_mirror_if_id(struct vr_forwarding_md *fmd) {
...
static inline void vr_fmd_put_mirror_vlan(struct vr_forwarding_md *fmd, uint16_t vlan) {
  fmd->fmd_mirror_data = vlan & 0x0FFF;

  return;
}

static inline void vr_fmd_put_mirror_type(struct vr_forwarding_md *fmd, mirror_type_t type) {
...
static inline void vr_fmd_put_mirror_if_id(struct vr_forwarding_md *fmd, uint16_t if_id) {
 ...
 */
#define VR_AGENT_MIN_PACKET_LEN                                                                    \
  ((2 * sizeof(unsigned short)) + sizeof(struct vr_ip6) + sizeof(struct vr_icmp) +                 \
   sizeof(struct vr_ip6) + VR_VXLAN_HDR_LEN + VR_L2_CTRL_DATA_LEN + (2 * VR_ETHER_HLEN) +          \
   sizeof(struct agent_hdr))

static inline bool pkt_is_gso(struct vr_packet *pkt) {
  if (vr_pgso_size && vr_pgso_size(pkt))
    return true;
  return false;
}

static inline unsigned char *pkt_data_at_offset(struct vr_packet *pkt, unsigned short off) {
  if (off < pkt->vp_end)
...
static inline unsigned char *pkt_data(struct vr_packet *pkt) { return pkt->vp_head + pkt->vp_data; }

static inline void pkt_set_data(struct vr_packet *pkt, unsigned short off) {
  if (pkt->vp_data == off)
    return;
...
static inline void pkt_set_inner_network_header(struct vr_packet *pkt, unsigned short off) {
  pkt->vp_inner_network_h = off;
  return;
}

static inline unsigned short pkt_get_inner_network_header_off(struct vr_packet *pkt) {
  return pkt->vp_inner_network_h;
}

static inline unsigned char *pkt_inner_network_header(struct vr_packet *pkt) {
...
static inline void pkt_set_network_header(struct vr_packet *pkt, unsigned short off) {
  pkt->vp_network_h = off;
  return;
}

static inline unsigned short pkt_get_network_header_off(struct vr_packet *pkt) {
  return pkt->vp_network_h;
}

static inline unsigned char *pkt_network_header(struct vr_packet *pkt) {
...
static inline unsigned char *pkt_pull(struct vr_packet *pkt, unsigned int len) {
...
static inline unsigned char *pkt_pull_tail(struct vr_packet *pkt, unsigned int len) {
...
static inline unsigned char *pkt_push(struct vr_packet *pkt, unsigned int len) {
  if (len > pkt->vp_data)
    return NULL;
...
static inline unsigned short pkt_head_len(struct vr_packet *pkt) { return pkt->vp_len; }

static inline unsigned int pkt_len(struct vr_packet *pkt) {
  return pkt_head_len(pkt) + vr_pfrag_len(pkt);
}

static inline unsigned char *pkt_reserve_head_space(struct vr_packet *pkt, unsigned short len) {
...
static inline unsigned short pkt_head_space(struct vr_packet *pkt) { return pkt->vp_data; }

static inline void pkt_init_fragment(struct vr_packet *dst, struct vr_packet *src) {
  dst->vp_if = src->vp_if;
  dst->vp_nh = src->vp_nh;
...
static inline void pkt_drop_stats(struct vr_interface *vif, unsigned short reason, int cpu) {
  uint8_t *data;
  ...
  if (vif->vif_pcpu_drop_stats) {
    data = vr_btable_get(vif->vif_pcpu_drop_stats, ((cpu * VP_DROP_MAX) + reason));
    if (data) {
      count = ++(*data);
      if (count < 255)
        return;
 ...
 * and logs in corresponding core */
static inline void vr_pkt_drop_log_func(unsigned short drop_reason, struct vr_packet *pkt,
                                        struct vr_flow *flow, map_t file, unsigned int line) {
  int cpu = vr_get_cpu();
  ...
  if (vr_pkt_droplog_buf_en == 1) {
    ...
    if (pkt != NULL) {
      ...
      if (pkt->vp_type == VP_TYPE_IP) {
        ...
        PKT_LOG_FILL(vr_pkt_drop_log_buffer[cpu][buf_idx].src.ipv4.s_addr, ip->ip_saddr)
        PKT_LOG_FILL(vr_pkt_drop_log_buffer[cpu][buf_idx].dst.ipv4.s_addr, ip->ip_daddr)
      ...
      if (vr_pkt_droplog_min_sysctl_en != 1) {
        ...
        if (pkt->vp_if != NULL) {
          PKT_LOG_FILL(vr_pkt_drop_log_buffer[cpu][buf_idx].vif_idx, pkt->vp_if->vif_idx)
        }
        if (pkt->vp_nh != NULL) {
        ...
        if (pkt->vp_len < 100)
          memcpy(vr_pkt_drop_log_buffer[cpu][buf_idx].pkt_header, pkt_network_header(pkt),
                 pkt->vp_len);
        else
          memcpy(vr_pkt_drop_log_buffer[cpu][buf_idx].pkt_header, pkt_network_header(pkt), 100);
      }
    }
