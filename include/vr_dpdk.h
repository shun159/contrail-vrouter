...
#define VR_DPDK_LOGTYPE_DISABLE                                                                    \
  (RTE_LOGTYPE_USER5 | RTE_LOGTYPE_USER6 | RTE_LOGTYPE_USER7 | RTE_LOGTYPE_USER8)
...
#define VR_DPDK_MPLS_OFFSET ((VR_ETHER_HLEN + sizeof(struct vr_ip) + sizeof(struct vr_udp)) / 2)
...
#define VR_DPDK_MBUF_HDR_SZ                                                                        \
  (sizeof(struct rte_mbuf) + sizeof(struct vr_packet) + RTE_PKTMBUF_HEADROOM)
 ...
 * header. */
#define VR_DPDK_FRAG_DIRECT_MBUF_SZ (sizeof(struct rte_mbuf) + 2 * RTE_PKTMBUF_HEADROOM)
...
#define VR_DPDK_RTE_ETH_FOREACH_DEV(p) RTE_ETH_FOREACH_DEV(p)
#else
#define VR_DPDK_RTE_ETH_FOREACH_DEV(p) for (p = 0; p < rte_eth_dev_count(); p++)
#endif
...
typedef struct vr_dpdk_queue *(*vr_dpdk_queue_init_op)(unsigned lcore_id, struct vr_interface *vif,
                                                       unsigned queue_or_lcore_id);
/* Release queue operation */
typedef void (*vr_dpdk_queue_release_op)(unsigned lcore_id, unsigned queue_index,
 ...
 */
static inline struct rte_mbuf *vr_dpdk_pkt_to_mbuf(struct vr_packet *pkt) {
  return (struct rte_mbuf *)((uintptr_t)pkt - sizeof(struct rte_mbuf));
}
static inline struct vr_packet *vr_dpdk_mbuf_to_pkt(struct rte_mbuf *mbuf) {
 ...
 */
static inline void vr_dpdk_mbuf_reset(struct vr_packet *pkt) {
...
int vr_dpdk_table_mem_init(unsigned int, unsigned int, unsigned long, unsigned int, unsigned long);
...
#define DPDK_UDEBUG(t, h, ...)                                                                     \
  (void)(((RTE_LOG_DEBUG <= RTE_LOG_LEVEL) && (RTE_LOG_DEBUG <= rte_logs.level) &&                 \
          (RTE_LOGTYPE_##t & rte_logs.type))                                                       \
             ? vr_dpdk_ulog(RTE_LOG_DEBUG, RTE_LOGTYPE_##t, h, #t ": " __VA_ARGS__)                \
             : 0)
#else
...
struct rte_mbuf *vr_dpdk_pktmbuf_copy(struct rte_mbuf *md, struct rte_mempool *mp);
struct rte_mbuf *vr_dpdk_pktmbuf_copy_mon(struct rte_mbuf *md, struct rte_mempool *mp);

/*
 * vr_dpdk_interface.c
 */
/* Lock interface operations */
static inline int vr_dpdk_if_lock() { return pthread_mutex_lock(&vr_dpdk.if_lock); }
/* Unlock interface operations */
static inline int vr_dpdk_if_unlock() { return pthread_mutex_unlock(&vr_dpdk.if_lock); }
static inline uint16_t dpdk_get_ether_header_len(const void *data) {
...
/* Init TAP RX queue. */
struct vr_dpdk_queue *vr_dpdk_tapdev_rx_queue_init(unsigned lcore_id, struct vr_interface *vif,
                                                   unsigned queue_id);
/* Init TAP TX queue. */
struct vr_dpdk_queue *vr_dpdk_tapdev_tx_queue_init(unsigned lcore_id, struct vr_interface *vif,
                                                   unsigned queue_id);
/* RX/TX to/from all the TAP devices. */
uint64_t vr_dpdk_tapdev_rxtx(void);
/* RX a burst of packets from the TAP device. */
unsigned vr_dpdk_tapdev_rx_burst(struct vr_dpdk_tapdev *, struct rte_mbuf **, unsigned num,
                                 int datapath);
/* Dequeue a burst of packets from the TAP device. */
unsigned vr_dpdk_tapdev_dequeue_burst(struct vr_dpdk_tapdev *, struct rte_mbuf **, unsigned num);
/* TX a burst of packets to the TAP device. */
unsigned vr_dpdk_tapdev_tx_burst(struct vr_dpdk_tapdev *, struct rte_mbuf **, unsigned num,
                                 int datapath);
/* Enqueue a burst of packets to the TAP device. */
unsigned vr_dpdk_tapdev_enqueue_burst(struct vr_dpdk_tapdev *, struct rte_mbuf **, unsigned num);
...
/* Init KNI RX queue */
struct vr_dpdk_queue *vr_dpdk_kni_rx_queue_init(unsigned lcore_id, struct vr_interface *vif,
                                                unsigned host_lcore_id);
/* Init KNI TX queue */
struct vr_dpdk_queue *vr_dpdk_kni_tx_queue_init(unsigned lcore_id, struct vr_interface *vif,
                                                unsigned host_lcore_id);
...
/* Flush TX queues */
static inline void vr_dpdk_lcore_flush(struct vr_dpdk_lcore *lcore) {
...
struct rte_ring *vr_dpdk_ring_allocate(unsigned host_lcore_id, char *ring_name,
                                       unsigned vr_dpdk_tx_ring_sz, unsigned flags);
/* Init ring RX queue */
struct vr_dpdk_queue *vr_dpdk_ring_rx_queue_init(unsigned lcore_id, struct vr_interface *vif,
                                                 unsigned host_lcore_id);
/* Init ring TX queue */
struct vr_dpdk_queue *vr_dpdk_ring_tx_queue_init(unsigned lcore_id, struct vr_interface *vif,
                                                 unsigned int queue_id, unsigned host_lcore_id);
void dpdk_ring_to_push_add(unsigned lcore_id, struct rte_ring *tx_ring,
                           struct vr_dpdk_queue *tx_queue);

void dpdk_ring_to_push_remove(unsigned lcore_id, struct rte_ring *tx_ring);
...
int dpdk_fragment_assembler_enqueue(struct vrouter *router, struct vr_packet *pkt,
...
int dpdk_segment_packet(struct vr_packet *pkt, struct rte_mbuf *mbuf_in, struct rte_mbuf **mbuf_out,
                        const unsigned short out_num, const unsigned short mss_size,
                        bool do_outer_ip_csum);
uint16_t dpdk_ipv4_udptcp_cksum(struct rte_mbuf *m, const struct ipv4_hdr *ipv4_hdr,
                                uint8_t *l4_hdr);
uint16_t dpdk_ipv6_udptcp_cksum(struct rte_mbuf *m, const struct ipv6_hdr *ipv6_hdr,
                                uint8_t *l4_hdr);
