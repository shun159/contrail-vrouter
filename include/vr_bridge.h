...
#define IS_MAC_ZERO(dst)                                                                           \
  ((((uint16_t *)dst)[0] == 0) && (((uint16_t *)dst)[1] == 0) && (((uint16_t *)dst)[2] == 0))

#define IS_MAC_BCAST(dst)                                                                          \
  ((((uint16_t *)dst)[0] == 0xffff) && (((uint16_t *)dst)[1] == 0xffff) &&                         \
...
__attribute__packed__open__ struct vr_bridge_entry_key {
  unsigned char be_mac[VR_ETHER_ALEN];
  unsigned short be_vrf_id;
} __attribute__packed__close__;

__attribute__packed__open__ struct vr_dummy_bridge_entry {
...
__attribute__packed__open__ struct vr_bridge_entry {
  vr_hentry_t be_hentry;
  struct vr_bridge_entry_key be_key;
  struct vr_nexthop *be_nh;
  uint64_t be_packets;
...
#define VR_BRIDGE_TABLE_SIZE (vr_bridge_entries * sizeof(struct vr_bridge_entry))
#define VR_BRIDGE_OFLOW_TABLE_SIZE (vr_bridge_oentries * sizeof(struct vr_bridge_entry))
...
mac_learn_t vr_bridge_learn(struct vrouter *, struct vr_packet *, struct vr_eth *,
                            struct vr_forwarding_md *);
