...
#define VR_QOS_MAP_ENTRY_SIZE (VR_DSCP_QOS_ENTRIES + VR_MPLS_QOS_ENTRIES + VR_DOTONEP_QOS_ENTRIES)
 ...
 * the flow structure
 */
__attribute__packed__open__ struct vr_forwarding_class_qos {
  uint8_t vfcq_dscp;
  uint8_t vfcq_mpls_qos : 3, vfcq_dotonep_qos : 3, vfcq_untrusted : 1, vfcq_valid : 1;
  uint8_t vfcq_queue_id;
} __attribute__packed__close__;

__attribute__packed__open__ struct vr_forwarding_class {
  uint8_t vfc_id;
...
struct vr_forwarding_class_qos *vr_qos_get_forwarding_class(struct vrouter *, struct vr_packet *,
                                                            struct vr_forwarding_md *);
