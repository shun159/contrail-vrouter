...
#define vif_is_virtual(vif)                                                                        \
  ((vif->vif_type == VIF_TYPE_VIRTUAL) || (vif->vif_type == VIF_TYPE_VIRTUAL_VLAN))
#define vif_is_fabric(vif) (vif->vif_type == VIF_TYPE_PHYSICAL)
#define vif_is_vlan(vif) ((vif->vif_type == VIF_TYPE_VIRTUAL_VLAN))

#define vif_is_tap(vif) ((vif->vif_type == VIF_TYPE_VIRTUAL) || (vif->vif_type == VIF_TYPE_AGENT))

#define vif_is_vhost(vif)                                                                          \
  ((vif->vif_type == VIF_TYPE_HOST) || (vif->vif_type == VIF_TYPE_XEN_LL_HOST) ||                  \
...
/* DPDK definitions */
#define vif_is_vm(vif)                                                                             \
  ((vif->vif_type == VIF_TYPE_VIRTUAL) && (vif->vif_transport == VIF_TRANSPORT_PMD))
#define vif_is_namespace(vif)                                                                      \
  ((vif->vif_type == VIF_TYPE_VIRTUAL) && (vif->vif_transport == VIF_TRANSPORT_ETH))
#define vif_is_agent(vif)                                                                          \
  ((vif->vif_type == VIF_TYPE_AGENT) && (vif->vif_transport == VIF_TRANSPORT_SOCKET))
...
#define VIF_VR_CAP_MASK                                                                            \
  (VIF_FLAG_TX_CSUM_OFFLOAD | VIF_FLAG_VLAN_OFFLOAD | VIF_FLAG_GRO_NEEDED | VIF_FLAG_MRG_RXBUF)
...
struct vr_interface_driver {
  ...
  int (*drv_add_sub_interface)(struct vr_interface *, struct vr_interface *);
  int (*drv_delete_sub_interface)(struct vr_interface *, struct vr_interface *);
};
...
struct vr_interface {
  ...
  void *vif_os;
  int (*vif_tx)(struct vr_interface *, struct vr_packet *, struct vr_forwarding_md *);
  /*
  ...
  int (*vif_set_rewrite)(struct vr_interface *, struct vr_packet **, struct vr_forwarding_md *,
  ...
  mac_response_t (*vif_mac_request)(struct vr_interface *, struct vr_packet *,
...
struct vr_host_interface_ops {
  ...
  int (*hif_get_settings)(struct vr_interface *, struct vr_interface_settings *);
  ...
  int (*hif_get_bond_info)(struct vr_interface *vif, struct vr_interface_bond_info *bond_info);
  int (*hif_get_vlan_info)(struct vr_interface *vif, struct vr_interface_vlan_info *vlan_info);
};
...
extern int vif_xconnect(struct vr_interface *, struct vr_packet *, struct vr_forwarding_md *);
...
extern int vif_vrf_table_set(struct vr_interface *, unsigned int, int, unsigned int);
