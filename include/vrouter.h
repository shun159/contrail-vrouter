...
#define CONTAINER_OF(member, struct_type, pointer)                                                 \
  ((struct_type *)((uintptr_t)pointer - (uintptr_t) & (((struct_type *)0)->member)))
...
struct host_os {
  ...
  int (*hos_pcopy)(unsigned char *, struct vr_packet *, unsigned int, unsigned int);
  ...
  void *(*hos_pheader_pointer)(struct vr_packet *, unsigned short, void *);
  int (*hos_pull_inner_headers)(struct vr_packet *, unsigned short, unsigned short *,
                                int (*is_label_l2)(unsigned int, unsigned int, unsigned short *));
  int (*hos_pcow)(struct vr_packet **, unsigned short);
  uint16_t (*hos_get_udp_src_port)(struct vr_packet *, struct vr_forwarding_md *, unsigned short);
  int (*hos_pkt_from_vm_tcp_mss_adj)(struct vr_packet *, uint16_t);
  int (*hos_pull_inner_headers_fast)(struct vr_packet *, unsigned char,
                                     int (*is_label_l2)(unsigned int, unsigned int,
                                                        unsigned short *),
                                     int *, int *);
  ...
  int (*hos_enqueue_to_assembler)(struct vrouter *, struct vr_packet *, struct vr_forwarding_md *);
...
__attribute__packed__open__ struct vr_malloc_md {
  char vmm_magic[3];
  uint8_t vmm_state;
  unsigned int vmm_object;
} __attribute__packed__close__;

static inline void vr_malloc_md_set(void *mem, unsigned int object) {
...
static inline void vr_malloc_md_check(void *mem, unsigned int object) {
  struct vr_malloc_md *vmm;
