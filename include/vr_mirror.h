...
#define VR_MIRROR_PKT_HEAD_SPACE                                                                   \
  (sizeof(struct vr_udp) + sizeof(struct vr_ip6) + VR_MPLS_HDR_LEN + sizeof(struct vr_udp) +       \
   sizeof(struct vr_ip) + VR_ETHER_HLEN)
...
extern int vr_mirror(struct vrouter *, uint8_t, struct vr_packet *, struct vr_forwarding_md *,
                     mirror_type_t);
...
extern struct vr_mirror_meta_entry *vr_mirror_meta_entry_set(struct vrouter *, unsigned int,
                                                             unsigned int, unsigned short, void *,
                                                             unsigned int, unsigned short);
extern void vr_mirror_meta_entry_del(struct vrouter *, struct vr_mirror_meta_entry *);
