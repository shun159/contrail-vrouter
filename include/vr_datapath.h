...
__attribute__packed__open__ struct vr_gro {
  uint16_t vg_vif_id;
  uint16_t pad;
  uint32_t vg_nh_id;
} __attribute__packed__close__;

static inline bool well_known_mac(unsigned char *dmac) {
  unsigned char vr_well_known_mac_infix[] = {0x80, 0xc2};
  if (!memcmp(&dmac[VR_ETHER_PROTO_MAC_OFF], vr_well_known_mac_infix, VR_ETHER_PROTO_MAC_LEN))
    if (!*dmac || (*dmac == 0x1))
      return true;
...
unsigned int vr_virtual_input(unsigned short, struct vr_interface *, struct vr_packet *,
                              struct vr_forwarding_md *, unsigned short);
unsigned int vr_fabric_input(struct vr_interface *, struct vr_packet *, struct vr_forwarding_md *,
                             unsigned short);
...
int vr_ip_input(struct vrouter *, struct vr_packet *, struct vr_forwarding_md *);
int vr_neighbor_input(struct vr_packet *, struct vr_forwarding_md *, unsigned char *);
int vr_ip6_input(struct vrouter *, struct vr_packet *, struct vr_forwarding_md *);
...
extern int vr_forward(struct vrouter *, struct vr_packet *, struct vr_forwarding_md *);
unsigned int vr_bridge_input(struct vrouter *, struct vr_packet *, struct vr_forwarding_md *);
...
mac_response_t vm_arp_request(struct vr_interface *, struct vr_packet *, struct vr_forwarding_md *,
                              unsigned char *);
