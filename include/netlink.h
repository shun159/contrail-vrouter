...
#define NLMSG_NEXT(nlhp, len)                                                                      \
  ((len) -= NLMSG_ALIGN((nlhp)->nlmsg_len),                                                        \
   (struct nlmsghdr *)(((char *)(nlhp)) + NLMSG_ALIGN((nlhp)->nlmsg_len)))

#define NLMSG_OK(nlh, len)                                                                         \
  ((len) >= (int)sizeof(struct nlmsghdr) && (nlhp)->nlmsg_len >= sizeof(struct nlmsghdr) &&        \
   (nlhp)->nlmsg_len <= (len))
...
#define NLMSG_MIN_TYPE                                                                             \
  0x10 /* Linux reserves below values for control                                                  \
        * messages */
