...
#if (!(defined(RHEL_MAJOR) && defined(RHEL_MINOR) && (RHEL_MAJOR == 6) && (RHEL_MINOR >= 5)))
...
static inline __u32 skb_get_hash(struct sk_buff *skb) { return skb_get_rxhash(skb); }
#endif
#endif
...
static netdev_features_t harmonize_features(struct sk_buff *skb, __be16 protocol,
                                            netdev_features_t features) {
  if (skb->ip_summed != CHECKSUM_NONE && !can_checksum_protocol(features, protocol)) {
...
static inline netdev_features_t netif_skb_features(struct sk_buff *skb) {
  ...
  if (protocol != htons(ETH_P_8021Q)) {
    ...
    features &=
        NETIF_F_SG | NETIF_F_HIGHDMA | NETIF_F_FRAGLIST | NETIF_F_GEN_CSUM | NETIF_F_HW_VLAN_TX;
    return harmonize_features(skb, protocol, features);
  }
}
#endif
