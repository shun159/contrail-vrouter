...
#define NETLINK_RESPONSE_HEADER_LEN (NLMSG_HDRLEN + GENL_HDRLEN + NLA_HDRLEN)
#define NETLINK_BUFFER(skb_data) ((char *)skb_data + NETLINK_RESPONSE_HEADER_LEN)
#define NETLINK_SKB(buf) *(struct sk_buff **)((char *)buf - NETLINK_RESPONSE_HEADER_LEN)

static char *netlink_trans_alloc(unsigned int size) {
  struct sk_buff *skb;
  int len;
...
static struct sk_buff *netlink_skb(char *buf) { return NETLINK_SKB(buf); }

static void netlink_trans_free(char *buf) {
  struct sk_buff *skb;
...
static int netlink_trans_request(struct sk_buff *in_skb, struct genl_info *info) {
  char *buf;
  int ret;
  unsigned int len;
  uint32_t multi_flag;
  ...
  while ((response = vr_message_dequeue_response())) {
    if (!response->vr_message_broadcast) {
      ...
      len = NLMSG_ALIGN(len);
      rep = __nlmsg_put(skb, netlink_id, nlh->nlmsg_seq, nlh->nlmsg_type, len, multi_flag);
      ...
      if (nla_put(skb, NL_ATTR_VR_MESSAGE_PROTOCOL, response->vr_message_len,
                  response->vr_message_buf) < 0) {
        nlmsg_free(skb);
        goto next;
      }
...
#else
  return genl_register_family_with_ops_groups(&vrouter_genl_family, vrouter_genl_ops,
                                              vrouter_genl_groups);
#endif
}
