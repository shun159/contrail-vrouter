...
void lh_fragment_sync_assemble(struct vr_fragment_queue_element *vfqe) {
  uint32_t hash, index;
...
static void vr_linux_fragment_assembler(struct work_struct *work) {
...
int lh_enqueue_to_assembler(struct vrouter *router, struct vr_packet *pkt,
  ...
  ret = vr_fragment_enqueue(router, &vr_lfq_pcpu_queues[cpu].vrlfq_queue, pkt, fmd);
  if (!ret)
...
static void vr_linux_assembler_table_scan(void *arg) {
...
static void vr_linux_assembler_table_exit(void) {
...
static int vr_linux_assembler_table_init(void) {
  unsigned int i, size;
...
static void vr_linux_fragment_queue_exit(void) {
  int i;
...
static int vr_linux_fragment_queue_init(void) {
  unsigned int i, size;
  ...
  for (i = 0; i < vr_num_cpus; i++) {
    INIT_WORK(&vr_lfq_pcpu_queues[i].vrlfq_work, vr_linux_fragment_assembler);
  }
