...
extern struct vr_packet *linux_get_packet(struct sk_buff *, struct vr_interface *);

extern int lh_enqueue_to_assembler(struct vrouter *, struct vr_packet *, struct vr_forwarding_md *);
...
static int lh_printk(const char *format, ...) {
  int printed;
  va_list args;
...
static void *lh_malloc(unsigned int size, unsigned int object) {
...
static void *lh_zalloc(unsigned int size, unsigned int object) {
  struct vr_malloc_md *md;
  void *mem;
...
static void lh_free(void *mem, unsigned int object) {
  if (mem) {
    if (vr_memory_alloc_checks) {
...
static void *lh_page_alloc(unsigned int size) {
  unsigned int order;
...
static void lh_page_free(void *address, unsigned int size) {
  unsigned int order;
...
uint64_t lh_vtop(void *address) { return (uint64_t)(virt_to_phys(address)); }

struct vr_packet *lh_palloc(unsigned int size) {
  struct sk_buff *skb;
...
static struct vr_packet *lh_pexpand_head(struct vr_packet *pkt, unsigned int hspace) {
  struct sk_buff *skb;
...
static struct vr_packet *lh_palloc_head(struct vr_packet *pkt, unsigned int size) {
...
static struct vr_packet *lh_pclone(struct vr_packet *pkt) {
...
static void lh_preset(struct vr_packet *pkt) {
  struct sk_buff *skb;
...
static void lh_pset_data(struct vr_packet *pkt, unsigned short offset) {
...
static unsigned int lh_pgso_size(struct vr_packet *pkt) {
  struct sk_buff *skb = vp_os_packet(pkt);

  return skb_shinfo(skb)->gso_size;
}

static void lh_pfree(struct vr_packet *pkt, unsigned short reason) {
  struct sk_buff *skb;
...
static int lh_pcopy(unsigned char *dst, struct vr_packet *p_src, unsigned int offset,
                    unsigned int len) {
  int ret;
  struct sk_buff *src;
...
static unsigned short lh_pfrag_len(struct vr_packet *pkt) {
  struct sk_buff *skb;
...
static unsigned short lh_phead_len(struct vr_packet *pkt) {
  struct sk_buff *skb;
...
static void lh_get_time(uint64_t *sec, uint64_t *usec) {
...
void lh_pfree_skb(struct sk_buff *skb, struct vr_interface *vif, unsigned short reason) {
...
static void lh_get_mono_time(uint64_t *sec, uint64_t *nsec) {
  struct timespec t;
...
static void lh_work(struct work_struct *work) {
...
static int lh_schedule_work(unsigned int cpu, void (*fn)(void *), void *arg) {
...
static void lh_delay_op(void) {
  synchronize_net();
  return;
}

static void *lh_inner_network_header(struct vr_packet *pkt) {
  struct sk_buff *skb;
  struct sk_buff *frag;
 ...
 */
static void *lh_pheader_pointer(struct vr_packet *pkt, unsigned short hdr_len, void *buf) {
  int offset;
  struct sk_buff *skb = vp_os_packet(pkt);
...
static void rcu_cb(struct rcu_head *rh) {
...
static void lh_defer(struct vrouter *router, vr_defer_cb user_cb, void *data) {
...
static void *lh_get_defer_data(unsigned int len) {
  struct rcu_cb_data *cb_data;
...
static void lh_put_defer_data(void *data) {
  struct rcu_cb_data *cb_data;
...
static int lh_pcow(struct vr_packet **pktp, unsigned short head_room) {
 ...
 */
static __u16 lh_get_udp_src_port(struct vr_packet *pkt, struct vr_forwarding_md *fmd,
                                 unsigned short vrf) {
  struct sk_buff *skb = vp_os_packet(pkt);
  ...
  if (pkt->vp_type == VP_TYPE_IP || pkt->vp_type == VP_TYPE_IP6) {
    ...
    if ((pkt_get_network_header_off(pkt) + hdr_len) > pkt->vp_tail) {
    ...
    if (pkt->vp_type == VP_TYPE_IP6) {
      ...
      if ((ip6h->ip6_nxt == VR_IP_PROTO_TCP) || (ip6h->ip6_nxt == VR_IP_PROTO_UDP)) {
        /* Pull in L4 ports */
        pull_len += 4;
        if ((pull_len > 0) && !pskb_may_pull(skb, (unsigned int)pull_len)) {
          goto error;
        }
        ip6h = (struct vr_ip6 *)(skb->head + pkt_get_network_header_off(pkt));
        l4_hdr = (__u16 *)(((char *)ip6h) + sizeof(struct vr_ip6));
        sport = *l4_hdr;
        dport = *(l4_hdr + 1);
      }
    } else if (vr_ip_transport_header_valid(iph)) {
      if ((iph->ip_proto == VR_IP_PROTO_TCP) || (iph->ip_proto == VR_IP_PROTO_UDP)) {
 ...
 */
static void lh_adjust_tcp_mss(struct tcphdr *tcph, struct sk_buff *skb, unsigned short overlay_len,
                              unsigned short hlen) {
  uint16_t old_mss, new_mss;

  if (vr_adjust_tcp_mss((struct vr_tcp *)tcph, overlay_len + hlen, &old_mss, &new_mss)) {
    inet_proto_csum_replace2(&tcph->check, skb, htons(old_mss), htons(new_mss), 0);
  }
}
 ...
 */
static int lh_pkt_from_vm_tcp_mss_adj(struct vr_packet *pkt, unsigned short overlay_len) {
  struct sk_buff *skb = vp_os_packet(pkt);
 ...
 */
static int lh_csum_verify_fast(struct vr_ip *iph, void *transport_hdr, unsigned char proto,
                               unsigned int size) {
  __wsum csum;
  struct ipv6hdr *ip6;

  if (vr_ip_is_ip4(iph)) {
    csum = csum_tcpudp_nofold(iph->ip_saddr, iph->ip_daddr, size, proto, 0);
  } else if (vr_ip_is_ip6(iph)) {
    ip6 = (struct ipv6hdr *)iph;
    csum = ~csum_unfold(csum_ipv6_magic(&ip6->saddr, &ip6->daddr, size, proto, 0));
  } else {
    return -1;
  }
 ...
 */
static int lh_csum_verify(struct sk_buff *skb, struct vr_ip *iph) {
  uint8_t proto;
  uint32_t size;
  struct ipv6hdr *ip6;
  ...
  if (vr_ip_is_ip4(iph)) {
    ...
    skb->csum = csum_tcpudp_nofold(iph->ip_saddr, iph->ip_daddr, size, proto, 0);
  } else if (vr_ip_is_ip6(iph)) {
    ip6 = (struct ipv6hdr *)iph;
    proto = ip6->nexthdr;
    size = ntohs(ip6->payload_len);
    skb->csum = ~csum_unfold(csum_ipv6_magic(&ip6->saddr, &ip6->daddr, size, proto, 0));
  } else {
    return -1;
  }
 ...
 */
static int lh_csum_verify_udp(struct sk_buff *skb, struct vr_ip *iph) {
  uint32_t size;

  size = ntohs(iph->ip_len) - (iph->ip_hl * 4);

  if (skb->ip_summed == CHECKSUM_COMPLETE) {
    if (!csum_tcpudp_magic(iph->ip_saddr, iph->ip_daddr, size, IPPROTO_UDP, skb->csum)) {
 ...
 * function on 2.6.32.
 */
static void *vr_kmap_atomic(struct page *page) {
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 32))
#if defined(RHEL_MAJOR) && defined(RHEL_MINOR) && (RHEL_MAJOR == 6) && (RHEL_MINOR >= 4)
 ...
 * function on 2.6.32.
 */
static void vr_kunmap_atomic(void *va) {
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 32))
#if defined(RHEL_MAJOR) && defined(RHEL_MINOR) && (RHEL_MAJOR == 6) && (RHEL_MINOR >= 4)
 ...
 */
static int lh_pull_inner_headers_fast_udp(struct vr_packet *pkt,
  ...
  helper_ret = vr_inner_pkt_parse(va, tunnel_type_cb, encap_type, &pkt_type, &pull_len, frag_size,
                                  &iph, &ip6h, udph->udp_dport, VR_IP_PROTO_UDP);
  ...
  parse_ret = vr_ip_transport_parse(iph, ip6h, &th, frag_size, NULL, &hlen, &th_csum, &th_pull_len,
                                    &pull_len);
  ...
  if (udph->udp_csum != 0) {
    ...
    if (!fragment) {
      if (((proto == VR_IP_PROTO_UDP) && th_csum == VR_DIAG_CSUM) || (proto == VR_IP_PROTO_TCP)) {
        ...
        if (skb_shinfo(skb)->nr_frags == 1) {
          ...
          skb_pull_len = (pkt_data(pkt) - skb->data) + pkt_headlen + th_pull_len;
  ...
  if ((*encap_type == PKT_ENCAP_VXLAN) || (pkt_type != PKT_MPLS_TUNNEL_L3)) {
 ...
 */
static int lh_pull_inner_headers_fast_gre(struct vr_packet *pkt,
                                          int (*tunnel_type_cb)(unsigned int, unsigned int,
                                                                unsigned short *),
                                          int *ret, int *encap_type) {
  bool fragment = false;
  ...
  helper_ret = vr_inner_pkt_parse(va, tunnel_type_cb, encap_type, &pkt_type, &pull_len, frag_size,
                                  &iph, &ip6h, gre_proto, VR_IP_PROTO_GRE);
  if (helper_ret == PKT_RET_SLOW_PATH) {
    goto slow_path;
  } else if (helper_ret == PKT_RET_UNHANDLED) {
    goto unhandled;
  ...
  if (!skb_csum_unnecessary(skb) && iph) {
    ...
    if (!fragment) {
      if ((th_csum == VR_DIAG_CSUM && (proto == VR_IP_PROTO_UDP)) || (proto == VR_IP_PROTO_TCP)) {
        lh_handle_checksum_complete_skb(skb);
        if (skb_shinfo(skb)->nr_frags == 1) {
          if (lh_csum_verify_fast(iph, th, proto, transport_size)) {
            if (th_csum == VR_DIAG_CSUM) {
  ...
  if (pkt_type != PKT_MPLS_TUNNEL_L3 && skb->ip_summed == CHECKSUM_PARTIAL)
    skb->ip_summed = CHECKSUM_UNNECESSARY;
...
static int lh_pkt_may_pull(struct vr_packet *pkt, unsigned int len) {
  struct sk_buff *skb = vp_os_packet(pkt);
 ...
 * multicast
 */
static int lh_pull_inner_headers_fast(struct vr_packet *pkt, unsigned char proto,
                                      int (*tunnel_type_cb)(unsigned int, unsigned int,
                                                            unsigned short *),
                                      int *ret, int *encap_type) {
  if (proto == VR_IP_PROTO_GRE) {
    return lh_pull_inner_headers_fast_gre(pkt, tunnel_type_cb, ret, encap_type);
  } else if (proto == VR_IP_PROTO_UDP) {
    return lh_pull_inner_headers_fast_udp(pkt, tunnel_type_cb, ret, encap_type);
  }
 ...
 */
static int
lh_pull_inner_headers(struct vr_packet *pkt, unsigned short ip_proto, unsigned short *reason,
  ...
  bool thdr_valid = false, mpls_pkt = true, outer_cksum_validate, fragment = false;
  ...
  *reason = VP_DROP_PULL;
  ...
  if (mpls_pkt) {
    ...
    if (!(label & VR_MPLS_LABEL_STACK_BIT_MASK) &&
      ...
      label = ntohl(*(uint32_t *)(skb->head + pkt->vp_data + hdr_len + VR_MPLS_HDR_LEN));
    ...
    if (ret == PKT_MPLS_TUNNEL_L3) {
      ...
      pull_len = pull_len - VR_L2_CTRL_DATA_LEN + sizeof(struct vr_ip);
      if (!pskb_may_pull(skb, pull_len))
        goto error;
  ...
  if (iph || ip6h) {
    ...
    if (thdr_valid) {
      ...
      } else if ((l4_proto == VR_IP_PROTO_ICMP) || (l4_proto == VR_IP_PROTO_ICMP6)) {
      ...
      if (l4_proto == VR_IP_PROTO_TCP) {
        ...
        if (vr_icmp_error(icmph)) {
          ...
          if (skb->len >= pull_len) {
            if (pskb_may_pull(skb, pull_len)) {
              ...
              icmp_pl_iph = (struct vr_ip *)(icmph + 1);
              if (icmp_pl_ip_proto == VR_IP_PROTO_TCP) {
                th_csum = ((struct vr_tcp *)((unsigned char *)icmp_pl_iph + icmp_pl_iph->ip_hl * 4))
                              ->tcp_csum;
              } else if (icmp_pl_ip_proto == VR_IP_PROTO_UDP) {
                th_csum = ((struct vr_udp *)((unsigned char *)icmp_pl_iph + icmp_pl_iph->ip_hl * 4))
                              ->udp_csum;
              } else if (icmp_pl_ip_proto == VR_IP_PROTO_ICMP) {
                th_csum =
                    ((struct vr_icmp *)((unsigned char *)icmp_pl_iph + icmp_pl_iph->ip_hl * 4))
                        ->icmp_csum;
              }
            }
          }
        ...
        th_csum = ((struct udphdr *)((unsigned char *)iph + hlen))->check;
        ...
        if (icmph->icmp_type == VR_ICMP6_TYPE_NEIGH_SOL) {
          ...
          if (skb->len >= (pull_len + sizeof(struct vr_neighbor_option) + VR_ETHER_ALEN)) {
            pull_len += sizeof(struct vr_neighbor_option) + VR_ETHER_ALEN;
          }
          if (!pskb_may_pull(skb, pull_len))
            goto error;
          ...
          if (skb->len >= pull_len) {
            if (pskb_may_pull(skb, pull_len)) {
              ...
              if (icmp_pl_ip6h->ip6_nxt == VR_IP6_PROTO_FRAG) {
                thdr = (unsigned char *)icmp_pl_ip6h + sizeof(struct vr_ip6) +
    ...
    if (!skb_csum_unnecessary(skb)) {
      outer_iph = (struct vr_ip *)pkt_network_header(pkt);
      if (outer_iph && (outer_iph->ip_proto == VR_IP_PROTO_UDP) && udph_cksum) {
        skb_pull_len = pkt_data(pkt) - skb->data;
  ...
  if ((!mpls_pkt || ret != PKT_MPLS_TUNNEL_L3) && skb->ip_summed == CHECKSUM_PARTIAL)
    skb->ip_summed = CHECKSUM_UNNECESSARY;
...
static void *lh_data_at_offset(struct vr_packet *pkt, unsigned short off) {
  struct sk_buff *skb;
  struct sk_buff *frag;
  struct vr_packet *frag_pkt;
...
static void *lh_network_header(struct vr_packet *pkt) {
  struct sk_buff *skb;
  struct sk_buff *frag;
  struct vr_packet *frag_pkt;
  unsigned short off;
...
static void linux_timer(linux_timer_callback_param_t arg) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
...
static void lh_delete_timer(struct vr_timer *vtimer) {
  struct timer_list *timer = &vtimer->timer;
...
static int lh_restart_timer(struct vr_timer *vtimer) {
  struct timer_list *timer = &vtimer->timer;
...
static int lh_create_timer(struct vr_timer *vtimer) {
...
static void lh_set_log_level(unsigned int log_level) {
  /* TODO: Implement similarly to the DPDK vRouter */
}

static void lh_set_log_type(unsigned int log_type, int enable) {
  /* TODO: Implement similarly to the DPDK vRouter */
}

static unsigned int lh_get_log_level(void) {
  /* TODO: Implement similarly to the DPDK vRouter */

  return 0;
}

static unsigned int *lh_get_enabled_log_types(int *size) {
  /* TODO: Implement similarly to the DPDK vRouter */
...
static void lh_soft_reset(struct vrouter *router) {
...
static void lh_register_nic(struct vr_interface *vif __attribute__((unused)),
 ...
 */
static int lh_huge_page_config(uint64_t *hpages, int n_hpages, int *hpage_size, int n_hpage_sizes) {
  int i;
...
static void *lh_huge_mem_get(int size) { return vr_huge_mem_get(size); }
...
static void vr_message_exit(void) {
  vr_genetlink_exit();
  vr_sandesh_exit();
...
static int vr_message_init(void) {
  int ret;
...
static void vr_sysctl_exit(void) {
  if (vr_sysctl_header) {
...
static void vr_sysctl_init(void) {
...
static void vrouter_linux_exit(void) {
  vr_sysctl_exit();
  vr_message_exit();
  vr_assembler_exit();
  vr_mem_exit();
...
static int __init vrouter_linux_init(void) {
  int ret;
...
MODULE_PARM_DESC(vr_flow_entries, "Number of entries in the flow table. Default is "__stringify(
                                      VR_DEF_FLOW_ENTRIES));
...
MODULE_PARM_DESC(vr_bridge_entries, "Number of entries in the bridge table. Default is "__stringify(
                                        VR_DEF_BRIDGE_ENTRIES));
...
MODULE_PARM_DESC(vif_bridge_entries,
                 "Number of entries in the per interface bridge table. Default is "__stringify(
                     VIF_BRIDGE_ENTRIES));
module_param(vif_bridge_oentries, uint, S_IRUGO);
MODULE_PARM_DESC(vif_bridge_oentries,
                 "Number of overflow entries in the per interface bridge table.");
...
MODULE_PARM_DESC(vr_pkt_droplog_buf_en,
                 "Enable/Disable vrouter packet drop log support at load time");

module_param(vr_pkt_droplog_sysctl_en, uint, S_IRUGO);
MODULE_PARM_DESC(vr_pkt_droplog_sysctl_en,
                 "Sysctl implementation for Enable/Disable vrouter packet drop log support");

module_param(vr_pkt_droplog_min_sysctl_en, uint, S_IRUGO);
MODULE_PARM_DESC(
    vr_pkt_droplog_min_sysctl_en,
    "Sysctl implementation for Enable/Disable minimum vrouter packet drop log support");

module_param(vr_mpls_labels, uint, S_IRUGO);
MODULE_PARM_DESC(vr_mpls_labels,
                 "Number of entries in the MPLS table. Default is "__stringify(VR_DEF_LABELS));
module_param(vr_nexthops, uint, S_IRUGO);
MODULE_PARM_DESC(vr_nexthops,
                 "Number of entries in the nexhop table. Default is "__stringify(VR_DEF_NEXTHOPS));
...
MODULE_PARM_DESC(
    vr_flow_hold_limit,
    "Maximum number of entries in the flow table that can be in the HOLD state. Default is 8192");
module_param(vr_interfaces, uint, S_IRUGO);
MODULE_PARM_DESC(vr_interfaces, "Number of entries in the interface table. Default is "__stringify(
                                    VR_MAX_INTERFACES));
