...
static int linux_xmit_segments(struct vr_interface *, struct sk_buff *, unsigned short);
 ...
 * allows it.
 */
void vr_skb_set_rxhash(struct sk_buff *skb, __u32 val) {
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 32))
#if defined(RHEL_MAJOR) && defined(RHEL_MINOR) && (RHEL_MAJOR == 6) && (RHEL_MINOR >= 4)
  skb->rxhash = val;
#endif
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(3, 15, 0))
#if defined(RHEL_MAJOR) && defined(RHEL_MINOR) && (RHEL_MAJOR == 7) && (RHEL_MINOR >= 2)
  skb->hash = val;
#else
  skb->rxhash = val;
#endif
 ...
 * allows it.
 */
__u32 vr_skb_get_rxhash(struct sk_buff *skb) {
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 32))
#if defined(RHEL_MAJOR) && defined(RHEL_MINOR) && (RHEL_MAJOR == 6) && (RHEL_MINOR >= 4)
  return skb->rxhash;
#elif
...
static inline struct sk_buff *linux_skb_vlan_insert(struct vr_interface *vif, struct sk_buff *skb,
...
static int linux_if_rx(struct vr_interface *vif, struct vr_packet *pkt) {
  int rc;
  ...
  if ((pkt->vp_flags & VP_FLAG_FROM_DP) && (pkt->vp_flags & VP_FLAG_CSUM_PARTIAL)) {
    ...
    if (cksum_off)
      *(unsigned short *)(pkt_data_at_offset(pkt, transport_off + cksum_off)) = 0;
  }
...
static long linux_inet_fragment(struct vr_interface *vif, struct sk_buff *skb,
                                unsigned short type) {
...
static int linux_xmit(struct vr_interface *vif, struct sk_buff *skb, unsigned short type) {
  if (vif->vif_type == VIF_TYPE_VIRTUAL && skb->ip_summed == CHECKSUM_NONE)
    skb->ip_summed = CHECKSUM_UNNECESSARY;

  if ((type == VP_TYPE_IPOIP) && (skb->len > skb->dev->mtu + skb->dev->hard_header_len))
    return linux_inet_fragment(vif, skb, type);

  return dev_queue_xmit(skb);
}

static int linux_xmit_segment(struct vr_interface *vif, struct sk_buff *seg, unsigned short type,
                              int diag) {
  int err = -ENOMEM;
  ...
  if (seg->len > seg->dev->mtu + seg->dev->hard_header_len || !vr_pkt_type_is_overlay(type)) {
  ...
  if (iph->ip_proto == VR_IP_PROTO_UDP) {
    if (!pskb_may_pull(seg, ethlen + iphlen + sizeof(struct udphdr))) {
      reason = VP_DROP_PULL;
      goto exit_xmit;
    }
    ...
    if (vr_udp_coff && !diag) {
      ...
      if (!skb_partial_csum_set(seg, skb_transport_offset(seg), offsetof(struct udphdr, check))) {
        reason = VP_DROP_MISC;
        goto exit_xmit;
      }
      ...
      iph->ip_csum = ip_fast_csum(iph, iph->ip_hl);
      udph->check =
          ~csum_tcpudp_magic(iph->ip_saddr, iph->ip_daddr, htons(udph->len), IPPROTO_UDP, 0);
...
static int linux_xmit_segments(struct vr_interface *vif, struct sk_buff *segs,
                               unsigned short type) {
  int err;
 ...
 */
static void linux_gso_xmit(struct vr_interface *vif, struct sk_buff *skb, unsigned short type) {
  netdev_features_t features;
  struct sk_buff *segs;
  ...
  if (seg_size > ndev->mtu + ndev->hard_header_len) {
    skb_shinfo(skb)->gso_size -= (seg_size - ndev->mtu - ndev->hard_header_len);
 ...
 * packet.
 */
static void linux_get_rxq(struct sk_buff *skb, u16 *rxq, unsigned int curr_cpu,
...
#else
  cpumask_andnot(&noht_cpumask, node_cpumask, topology_sibling_cpumask(curr_cpu));
#endif
 ...
 *
 */
void linux_enqueue_pkt_for_gro(struct sk_buff *skb, struct vr_interface *vif, bool l2_pkt) {
...
static int linux_if_tx(struct vr_interface *vif, struct vr_packet *pkt) {
  struct net_device *dev = (struct net_device *)vif->vif_os;
  struct sk_buff *skb = vp_os_packet(pkt);
  ...
  if (!vif_is_fabric(vif) || (vr_priority_tagging || is_vlan_dev(dev))) {
  ...
  } else if (vr_pkt_type_is_overlay(pkt->vp_type) || vr_pkt_needs_csum_gso_update(pkt)) {
    network_off = pkt_get_inner_network_header_off(pkt);
      ...
      if ((pkt->vp_flags & VP_FLAG_GSO) && skb_is_gso(skb)) {
        ...
        if (proto == VR_IP_PROTO_UDP) {
          ...
          if (!(sinfo->gso_type & SKB_GSO_UDP)) {
            sinfo->gso_type &= ~(SKB_GSO_TCPV4 | SKB_GSO_TCP_ECN | SKB_GSO_TCPV6 | SKB_GSO_FCOE);
  ...
  linux_xmit_segment(vif, skb, pkt->vp_type, (pkt->vp_flags & VP_FLAG_DIAG));

  return 0;
}

inline struct vr_packet *linux_get_packet(struct sk_buff *skb, struct vr_interface *vif) {
...
static int linux_pull_outer_headers(struct sk_buff *skb) {
  ...
  if (thdr && pull && (iph || ip6h)) {
    ...
    if (ip_proto == VR_IP_PROTO_ICMP) {
      if (vr_icmp_error((struct vr_icmp *)((unsigned char *)iph + ip_hdr_len))) {
        ...
        if (vr_ip_proto_pull((struct vr_ip *)iph)) {
          offset += (iph->ihl * 4) - sizeof(struct iphdr) + sizeof(struct vr_icmp);
      ...
      if (icmph->icmp_type == VR_ICMP6_TYPE_NEIGH_SOL) {
        ...
        if (skb->len >= (offset + sizeof(struct vr_neighbor_option) + VR_ETHER_ALEN)) {
          offset += VR_ETHER_ALEN + sizeof(struct vr_neighbor_option);
        }
        ...
        if (!pskb_may_pull(skb, offset))
          goto pull_fail;
        ip6h = (struct ipv6hdr *)(skb->data + offset - sizeof(struct ipv6hdr));
        if (ip6h->nexthdr == VR_IP6_PROTO_FRAG) {
          offset += sizeof(struct vr_ip6_frag) + sizeof(struct vr_icmp);
          if (!pskb_may_pull(skb, offset))
            goto pull_fail;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 39))
rx_handler_result_t linux_rx_handler(struct sk_buff **pskb) {
  int ret;
  ...
  if (skb->dev == pkt_rps_dev) {
    ...
    vif = __vrouter_get_interface(router, ((vr_rps_t *)skb->cb)->vif_idx);
  ...
  if (dev->type == ARPHRD_ETHER) {
    ...
    if (skb->vlan_tci & VLAN_CFI_MASK) {
      if (!(skb = linux_skb_vlan_insert(vif, skb, skb->vlan_tci & 0xEFFF)))
        return RX_HANDLER_CONSUMED;
    ...
    if (skb_headroom(skb) < ETH_HLEN) {
      ret = pskb_expand_head(
          skb, ETH_HLEN - skb_headroom(skb) + ETH_HLEN + sizeof(struct agent_hdr), 0, GFP_ATOMIC);
      if (ret)
        goto error;
    }
  }
  ...
  if (vif->vif_type == VIF_TYPE_PHYSICAL) {
    if ((!(vif->vif_flags & VIF_FLAG_PROMISCOUS)) && (skb->pkt_type == PACKET_OTHERHOST)) {
 ...
 */
static void vr_do_rps_outer(struct sk_buff *skb, struct vr_interface *vif) {
  unsigned int curr_cpu;
  u16 rxq;
 ...
 */
static struct vr_interface *vr_get_vif_ptr(struct net_device *dev) {
  struct vr_interface *vif;
 ...
 * structure depending on whether vrouter uses the bridge or OVS hook.
 */
void vr_set_vif_ptr(struct net_device *dev, void *vif) {
  if (vr_use_linux_br) {
 ...
 */
static struct net_device *vr_post_rps_outer_get_phys_dev(struct sk_buff *skb) {
 ...
 */
static struct sk_buff *vr_interface_common_hook(struct sk_buff *skb) {
  unsigned short vlan_id = VLAN_ID_INVALID;
  struct vr_interface *vif;
  struct vr_packet *pkt;
  struct vlan_hdr *vhdr;
 ...
 */
static struct sk_buff *vr_interface_bridge_hook(struct net_bridge_port *port, struct sk_buff *skb) {
 ...
 */
static struct sk_buff *vr_interface_ovs_hook(struct sk_buff *skb) {
  return vr_interface_common_hook(skb);
}
...
static int linux_if_del_tap(struct vr_interface *vif) {
...
static int linux_if_del_tap(struct vr_interface *vif) {
  ...
  if (vr_get_vif_ptr(dev) == (void *)vif) {
    if ((vif->vif_type == VIF_TYPE_PHYSICAL) && (vif->vif_flags & VIF_FLAG_VHOST_PHYS)) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 39))
static int linux_if_add_tap(struct vr_interface *vif) {
  struct net_device *dev;
...
static int linux_if_get_settings(struct vr_interface *vif, struct vr_interface_settings *settings) {
  struct net_device *dev = (struct net_device *)vif->vif_os;
  int ret = -EINVAL;
  ...
  if (netif_running(dev)) {
     ...
     (defined(RHEL_MAJOR) && RHEL_MAJOR >= 7 && RHEL_MINOR >= 4 &&                                 \
...
static unsigned int linux_if_get_mtu(struct vr_interface *vif) {
  struct net_device *dev = (struct net_device *)vif->vif_os;
 ...
 */
static int linux_if_tx_csum_offload(struct net_device *dev) {
  ...
  if (dev->dev.parent) {
    ...
    if (driver_name &&
        ((!strncmp(driver_name, "ixgbe", 6)) || (!strncmp(driver_name, "i40e", 5)))) {
      return 1;
    }
  }
#endif
...
static int linux_if_del(struct vr_interface *vif) {
...
static int linux_if_add(struct vr_interface *vif) {
  struct net_device *dev;
...
static void linux_if_unlock(void) {
  rtnl_unlock();
  return;
}

static void linux_if_lock(void) {
  rtnl_lock();
  return;
}
 ...
 */
static void linux_pkt_dev_free_helper(struct net_device **dev) {
  if (*dev == NULL) {
    return;
  }
 ...
 */
static void linux_pkt_dev_free(void) {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 39))
  if (pkt_gro_dev) {
 ...
 */
static void pkt_gro_dev_setup(struct net_device *dev) {
  /*
...
static void pkt_l2_gro_dev_setup(struct net_device *dev) {
 ...
 */
static void pkt_rps_dev_setup(struct net_device *dev) {
  /*
   * Initializing the interfaces with basic parameters to setup address
   * families.
   */
 ...
 */
static struct net_device *linux_pkt_dev_init(char *name, void (*setup)(struct net_device *),
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 39))
    if ((err = netdev_rx_handler_register(pdev, handler, NULL))) {
      vr_module_error(err, __FUNCTION__, __LINE__, 0);
...
#ifdef CONFIG_RPS
int lh_rps_process(struct vr_packet *pkt) {
  u16 rxq;
  ...
  if (vr_perfr2) {
    if (vr_perfq2) {
      ...
      linux_get_rxq(skb, &rxq, vr_get_cpu(), (vr_perfr1 || vr_perfr3) ? prev_cpu + 1 : 0);
    }
...
int lh_gro_process(struct vr_packet *pkt, struct vr_interface *vif, bool l2_pkt) {
  int handled = 1;
  ...
  if (unlikely(skb_headroom(skb) < (ETH_HLEN - sizeof(unsigned short)))) {
    struct sk_buff *nskb = skb_realloc_headroom(skb, LL_RESERVED_SPACE(skb->dev));
    if (!nskb) {
      vif_drop_pkt(vif, pkt, false);
      if (net_ratelimit())
        printk(KERN_WARNING "Insufficient memory: %s %d\n", __FUNCTION__, __LINE__);
      return handled;
    }
...
static rx_handler_result_t pkt_gro_dev_rx_handler(struct sk_buff **pskb) {
 ...
 */
static struct vr_interface *vif_from_napi(struct napi_struct *napi) {
  int offset;
  struct vr_interface *vif;
 ...
 */
static struct vr_interface *vif_from_l2_napi(struct napi_struct *napi) {
  int offset;
  struct vr_interface *vif;
 ...
 * GRO.
 */
static int vr_napi_poll(struct napi_struct *napi, int budget) {
  struct sk_buff *skb;
  struct vr_interface *vif;
  int quota = 0;
...
static int linux_if_notifier(struct notifier_block *__unused, unsigned long event, void *arg) {
...
void vr_host_vif_init(struct vrouter *router) {
  if (pkt_gro_dev)
    vr_gro_vif_add(router, pkt_gro_dev->ifindex, pkt_gro_dev->name, router->vr_max_interfaces - 1);

  if (pkt_l2_gro_dev)
    vr_gro_vif_add(router, pkt_l2_gro_dev->ifindex, pkt_l2_gro_dev->name,
                   router->vr_max_interfaces - 2);
  return;
}
...
static int linux_pkt_dev_alloc(void) {
  if (pkt_gro_dev == NULL) {
    pkt_gro_dev = linux_pkt_dev_init("pkt1", &pkt_gro_dev_setup, &pkt_gro_dev_rx_handler);
    if (pkt_gro_dev == NULL) {
      vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, 0);
      return -ENOMEM;
    }
  ...
  if (pkt_l2_gro_dev == NULL) {
    pkt_l2_gro_dev = linux_pkt_dev_init("pkt3", &pkt_l2_gro_dev_setup, &pkt_gro_dev_rx_handler);
    if (pkt_l2_gro_dev == NULL) {
      vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, 0);
      return -ENOMEM;
    }
  ...
  if (pkt_rps_dev == NULL) {
    pkt_rps_dev = linux_pkt_dev_init("pkt2", &pkt_rps_dev_setup, &pkt_rps_dev_rx_handler);
    if (pkt_rps_dev == NULL) {
      vr_module_error(-ENOMEM, __FUNCTION__, __LINE__, 0);
      return -ENOMEM;
    }
