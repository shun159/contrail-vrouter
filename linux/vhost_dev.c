...
static void vhost_ethtool_get_info(struct net_device *netdev, struct ethtool_drvinfo *info) {
...
static void vhost_dev_destructor(struct net_device *dev) {
  free_netdev(dev);

  return;
}

static int vhost_dev_open(struct net_device *dev) {
  netif_start_queue(dev);

  return 0;
}

static int vhost_dev_stop(struct net_device *dev) {
  netif_stop_queue(dev);

  return 0;
}

static int vhost_dev_set_mac_address(struct net_device *dev, void *addr) {
...
void vhost_del_tap_phys(struct net_device *pdev) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 39))
 ...
 */
void vhost_tap_phys(struct net_device *vdev, struct net_device *pdev) {
...
void vhost_if_add(struct vr_interface *vif) {
  ...
  if (vif->vif_type == VIF_TYPE_HOST) {
    ...
    if (vif->vif_bridge) {
       ...
       */
      if ((vp->vp_phys_dev) &&
          (vp->vp_phys_dev != ((struct net_device *)vif->vif_bridge->vif_os))) {
        vhost_del_tap_phys(vp->vp_phys_dev);
      }

      vp->vp_phys_dev = (struct net_device *)vif->vif_bridge->vif_os;
      strncpy(vp->vp_phys_name, vp->vp_phys_dev->name, sizeof(vp->vp_phys_name) - 1);
      ...
      if (i < VHOST_MAX_INTERFACES) {
        ...
        vr_printf("%s not added to vhost database. ", vp->vp_dev->name);
...
void vhost_attach_phys(struct net_device *dev) {
  int i;
  struct vhost_priv *vp;
...
static struct vhost_priv *vhost_get_priv_for_phys(struct net_device *dev) {
  int i;
  struct vhost_priv *vp;
...
struct net_device *vhost_get_vhost_for_phys(struct net_device *dev) {
  struct vhost_priv *vp;
 ...
 */
void vhost_detach_phys(struct net_device *dev) {
  struct vhost_priv *vp;
...
netdev_tx_t vhost_dev_xmit(struct sk_buff *skb, struct net_device *dev) {
  struct vhost_priv *vp;
...
void vhost_setup(struct net_device *dev) {
  struct vhost_priv *vp = netdev_priv(dev);

  /* follow the standard steps */
  random_ether_addr(dev->dev_addr);
  ether_setup(dev);
#if ((defined(RHEL_MAJOR) && defined(RHEL_MINOR) && (RHEL_MAJOR == 7) && (RHEL_MINOR >= 5)))
#ifdef ETH_MAX_MTU
...
static void vhost_dellink(struct net_device *dev)
#else
static void vhost_dellink(struct net_device *dev, struct list_head *head)
#endif
{
  struct vhost_priv *vp;
...
static int vhost_validate(struct nlattr *tb[], struct nlattr *data[],
...
static void vhost_netlink_exit(void) {
  if (vhost_drv_inited) {
...
static int vhost_netlink_init(void) {
  int ret;
...
int vhost_init(void) {
  if (!vhost_priv_db) {
    vhost_priv_db = kzalloc(sizeof(struct vhost_priv *) * VHOST_MAX_INTERFACES, GFP_KERNEL);
    if (!vhost_priv_db)
      return -ENOMEM;
  }
