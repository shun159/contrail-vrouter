...
static int uvrouter_agent_rx(void *arg) {
  int ret;
...
static int uvrouter_interface_init(void) {
  int i = 0;
  struct vr_hinterface *hif = NULL;
  struct vr_hinterface *agent_hif = NULL, *eth_hif = NULL;

  agent_hif = vr_hinterface_create(HIF_AGENT_INTERFACE_INDEX, HIF_TYPE_UDP, VIF_TYPE_AGENT);
  if (!agent_hif)
    return -1;

  eth_hif = vr_hinterface_create(HIF_PHYSICAL_INTERFACE_INDEX, HIF_TYPE_UDP, VIF_TYPE_PHYSICAL);
  if (!eth_hif)
    goto cleanup;

  for (i = 0; i < HIF_NUM_VIRTUAL_INTERFACES; i++) {
    hif =
        vr_hinterface_create(HIF_VIRTUAL_INTERFACE_INDEX_START + i, HIF_TYPE_UDP, VIF_TYPE_VIRTUAL);
    if (!hif)
      goto cleanup;
  }
...
static int uvrouter_agent_sock_init(void) {
  int ret;
  struct sockaddr_in sock_addr;
  ...
  ret = bind(uvr_agent_fd, (const struct sockaddr *)&sock_addr, sizeof(sock_addr));
  if (ret < 0)
    goto cleanup;
