...
int fake_response_cb(void *ptr1, unsigned int i, void *ptr2) { return 1; }

void drop_stats_memory_test(void **state) {
  vr_drop_stats_req req = {.h_op = SANDESH_OP_GET, .vds_rid = 0};
...
static void teardown(void **state) { free(*state); }
