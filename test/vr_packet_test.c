...
static void test_TearDown(void **state) { free(*state); }
...
static void test_vr_adjust_tcp_mss_not_changed(void **state) {
  ...
  assert_false(__vr_adjust_tcp_mss(tcph, len_overhead, eth_mtu, &old_mss, &new_mss));
...
static void test_vr_adjust_tcp_mss_changed(void **state) {
  ...
  assert_true(__vr_adjust_tcp_mss(tcph, len_overhead, eth_mtu, &old_mss, &new_mss));
