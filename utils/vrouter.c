...
static int write_options[] = {SET_LOG_LEVEL_INDEX, LOG_ENABLE_INDEX, LOG_DISABLE_INDEX, -1};

static int read_options[] = {INFO_OPT_INDEX, GET_LOG_LEVEL_INDEX, GET_ENABLED_LOGS_INDEX, -1};
...
static bool vrouter_read_op(void) {
  int i;
...
static bool vrouter_write_op(void) {
  int i;
...
static unsigned int log_type_name_to_id(const char *name) {
  int i;
...
static char *log_type_id_to_name(unsigned int id) {
  int i;
...
static unsigned int log_level_name_to_id(const char *name) {
  int i;
...
static char *log_level_id_to_name(unsigned int id) {
  int i;
...
void print_field(char *start, char *end) {
...
void print_build_info(char *buildinfo) {
  char *start, *end;
...
static void print_vrouter_parameters(vrouter_ops *req) {
  printf("Startup parameters\n"
         ...
         req->vo_interfaces, req->vo_vrfs, req->vo_nexthops, req->vo_mpls_labels,
         req->vo_bridge_entries, req->vo_oflow_bridge_entries, req->vo_flow_entries,
         req->vo_oflow_entries, req->vo_mirror_entries, req->vo_memory_alloc_checks,
         req->vo_vif_bridge_entries, req->vo_vif_oflow_bridge_entries, req->vo_pkt_droplog_bufsz,
  ...
  printf("Runtime parameters\n"
         ...
         req->vo_perfr, req->vo_perfs, req->vo_from_vm_mss_adj, req->vo_to_vm_mss_adj,
         req->vo_perfr1, req->vo_perfr2, req->vo_perfr3, req->vo_perfp, req->vo_perfq1,
         req->vo_perfq2, req->vo_perfq3, req->vo_udp_coff, req->vo_flow_hold_limit, req->vo_mudp,
         req->vo_flow_used_entries, req->vo_flow_used_oentries, req->vo_bridge_used_entries,
         req->vo_bridge_used_oentries, req->vo_burst_tokens, req->vo_burst_interval,
         req->vo_burst_step, req->vo_priority_tagging, req->vo_packet_dump, req->vo_pkt_droplog_en,
         req->vo_pkt_droplog_min_en);

  return;
}

static void print_log_level(vrouter_ops *req) {
...
static void print_enabled_log_types(vrouter_ops *req) {
  int i;
...
static void _vrouter_ops_process(void *s_req) {
...
static int vr_vrouter_op(struct nl_client *cl) {
  ...
  case SANDESH_OP_ADD:
    if (opt[SET_LOG_LEVEL_INDEX] || opt[LOG_ENABLE_INDEX]) {
      ret = vr_send_vrouter_set_logging(cl, 0, log_level, log_types_to_enable.types,
                                        log_types_to_enable.size, log_types_to_disable.types,
                                        log_types_to_disable.size);
    } else {
      ret = vr_send_vrouter_set_runtime_opts(
          cl, 0, perfr, perfs, from_vm_mss_adj, to_vm_mss_adj, perfr1, perfr2, perfr3, perfp,
          perfq1, perfq2, perfq3, udp_coff, flow_hold_limit, mudp, burst_tokens, burst_interval,
          burst_step, priority_tagging, packet_dump);
    }
    break;
...
static struct option long_options[] = {
    [INFO_OPT_INDEX] = {"info", no_argument, &opt[INFO_OPT_INDEX], 1},
    [HELP_OPT_INDEX] = {"help", no_argument, &opt[HELP_OPT_INDEX], 1},
    [GET_LOG_LEVEL_INDEX] = {"get-log-level", no_argument, &opt[GET_LOG_LEVEL_INDEX], 1},
    [SET_LOG_LEVEL_INDEX] = {"set-log-level", required_argument, &opt[SET_LOG_LEVEL_INDEX], 1},
    [LOG_ENABLE_INDEX] = {"enable-log-type", required_argument, &opt[LOG_ENABLE_INDEX], 1},
    [LOG_DISABLE_INDEX] = {"disable-log-type", required_argument, &opt[LOG_DISABLE_INDEX], 1},
    [GET_ENABLED_LOGS_INDEX] = {"get-enabled-log-types", no_argument, &opt[GET_ENABLED_LOGS_INDEX],
                                1},
    [SET_PERFR_INDEX] = {"perfr", required_argument, &opt[SET_PERFR_INDEX], 1},
    [SET_PERFS_INDEX] = {"perfs", required_argument, &opt[SET_PERFS_INDEX], 1},
    [SET_FROM_VM_MSS_ADJ_INDEX] = {"from_vm_mss_adj", required_argument,
                                   &opt[SET_FROM_VM_MSS_ADJ_INDEX], 1},
    [SET_TO_VM_MSS_ADJ_INDEX] = {"to_vm_mss_adj", required_argument, &opt[SET_TO_VM_MSS_ADJ_INDEX],
                                 1},
    [SET_PERFR1_INDEX] = {"perfr1", required_argument, &opt[SET_PERFR1_INDEX], 1},
    [SET_PERFR2_INDEX] = {"perfr2", required_argument, &opt[SET_PERFR2_INDEX], 1},
    [SET_PERFR3_INDEX] = {"perfr3", required_argument, &opt[SET_PERFR3_INDEX], 1},
    [SET_PERFP_INDEX] = {"perfp", required_argument, &opt[SET_PERFP_INDEX], 1},
    [SET_PERFQ1_INDEX] = {"perfq1", required_argument, &opt[SET_PERFQ1_INDEX], 1},
    [SET_PERFQ2_INDEX] = {"perfq2", required_argument, &opt[SET_PERFQ2_INDEX], 1},
    [SET_PERFQ3_INDEX] = {"perfq3", required_argument, &opt[SET_PERFQ3_INDEX], 1},
    [SET_UDP_COFF_INDEX] = {"udp_coff", required_argument, &opt[SET_UDP_COFF_INDEX], 1},
    [SET_FLOW_HOLD_LIMIT_INDEX] = {"flow_hold_limit", required_argument,
                                   &opt[SET_FLOW_HOLD_LIMIT_INDEX], 1},
    [SET_MUDP_INDEX] = {"mudp", required_argument, &opt[SET_MUDP_INDEX], 1},
    [SET_BURST_TOKENS_INDEX] = {"burst_tokens", required_argument, &opt[SET_BURST_TOKENS_INDEX], 1},
    [SET_BURST_INTERVAL_INDEX] = {"burst_interval", required_argument,
                                  &opt[SET_BURST_INTERVAL_INDEX], 1},
    [SET_BURST_STEP_INDEX] = {"burst_step", required_argument, &opt[SET_BURST_STEP_INDEX], 1},
    [SET_PRIORITY_TAGGING_INDEX] = {"set-priority-tagging", required_argument,
                                    &opt[SET_PRIORITY_TAGGING_INDEX], 1},
    [SET_PACKET_DUMP_INDEX] = {"packet-dump", required_argument, &opt[SET_PACKET_DUMP_INDEX], 1},
...
static bool log_types_add(log_types_t *types, const char *name) {
  int i;
...
static void log_types_print(log_types_t *types) {
  int i;
...
static void assert_platform_for_option(int required_platform, int opt_index) {
...
static void parse_long_opts(int opt_index, char *opt_arg) {
  bool ret;
