...
static void response_process(void *sresp) {
...
static void flow_response_process(void *sresp) {
  if (array_index == -1)
    return;
...
static void flow_table_data_process(void *sreq) {
...
static void interface_req_process(void *arg) {
...
static void nexthop_req_process(void *arg) {
...
static void route_req_process(void *arg) {
...
static void drop_stats_req_process(void *arg) {
...
static void flow_fill_nl_callbacks() {
...
struct vr_flow_entry *flow_get(unsigned long flow_index) {
...
static vr_drop_stats_req *flow_get_dropstats(void) {
  int ret;
...
static vr_nexthop_req *flow_get_nexthop(int id) {
  int ret;
...
static vr_nexthop_req *flow_get_mirror_nh(int id) {
  int ret;
...
static vr_interface_req *flow_get_vif(int vif_index) {
  int ret;
...
static vr_route_req *flow_get_route(unsigned int family, unsigned int vrf, uint8_t *prefix) {
  int ret;
...
const char *flow_get_drop_reason(uint8_t drop_code) {
  switch (drop_code) {
...
static void flow_dump_legend(void) {
...
static bool flow_match_dest(struct vr_flow_entry *fe, uint8_t *addr, int32_t port) {
  if (!addr && (port < 0))
    return false;
...
static bool flow_match_source(struct vr_flow_entry *fe, uint8_t *addr, int32_t port) {
  if (!addr && (port < 0))
    return false;
...
static void flow_print_spaces(void) {
  unsigned int i;
...
static void flow_print_field_name(const char *field) {
...
static void flow_print_nh_header(vr_nexthop_req *nh) {
...
static void flow_print_vif(vr_interface_req *vif, char *vif_name, bool ingress) {
  if (vif) {
    flow_print_spaces();
    if (ingress)
      printf("Ingress ");
    ...
    printf("Interface Statistics(Out, In, Errors): %" PRIu64 ", %" PRIu64 ", %" PRIu64 "\n",
           vif->vifr_opackets, vif->vifr_ipackets, vif->vifr_ierrors + vif->vifr_oerrors);
  }

  return;
}

static void flow_dump_tunnel(vr_nexthop_req *nh, vr_interface_req *vif, char *vif_name,
                             bool ingress) {
  if (nh) {
...
static void flow_dump_composite(vr_nexthop_req *nh, vr_interface_req *vif, char *vif_name,
                                bool ingress) {
  unsigned int i;
...
static void flow_dump_encap(vr_nexthop_req *nh, vr_interface_req *vif, char *vif_name,
                            bool ingress) {
  flow_print_nh_header(nh);
  flow_print_vif(vif, vif_name, ingress);
...
static void flow_dump_nexthop(vr_nexthop_req *src, vr_interface_req *vif, char *vif_name,
                              bool ingress) {
  if (src) {
    switch (src->nhr_type) {
    case NH_ENCAP:
...
static void flow_dump_source(vr_nexthop_req *src) {
...
static void flow_dump_mirror(vr_nexthop_req *req) {
  if (req->nhr_type == NH_TUNNEL) {
    ...
    if (req->nhr_family == AF_INET) {
      printf("%s ",
             inet_ntop(req->nhr_family, &req->nhr_tun_dip, addr_string, sizeof(addr_string)));
    } else if (req->nhr_family == AF_INET6) {
      printf("%s ",
             inet_ntop(req->nhr_family, req->nhr_tun_dip6, addr_string, sizeof(addr_string)));
    }
    ...
    if (req->nhr_flags & NH_FLAG_TUNNEL_SIP_COPY) {
      ...
      if (req->nhr_family == AF_INET) {
        printf("%s ",
               inet_ntop(req->nhr_family, &req->nhr_tun_sip, addr_string, sizeof(addr_string)));
      } else if (req->nhr_family == AF_INET6) {
        printf("%s ",
               inet_ntop(req->nhr_family, req->nhr_tun_sip6, addr_string, sizeof(addr_string)));
      }
    }
  } else {
...
static uint64_t flow_sum_drops_stats(vr_drop_stats_req *req) {
  uint64_t sum = 0;
...
static void flow_dump_entry(struct vr_flow_entry *fe) {
  unsigned int j;
  ...
  if ((fe->fe_flags & VR_RFLOW_VALID) && (fe->fe_rflow >= 0)) {
    ...
    if (rfe) {
      if ((rfe->fe_type == VP_TYPE_IP) || (rfe->fe_type == VP_TYPE_IP6)) {
        ...
        inet_ntop(VR_FLOW_FAMILY(rfe->fe_type), &rfe->fe_key.flow_ip[VR_IP_ADDR_SIZE(rfe->fe_type)],
  ...
  if ((fe->fe_type == VP_TYPE_IP) || (fe->fe_type == VP_TYPE_IP6)) {
    ...
    inet_ntop(VR_FLOW_FAMILY(fe->fe_type), &fe->fe_key.flow_ip[VR_IP_ADDR_SIZE(fe->fe_type)],
...
static void flow_get_routes(struct vr_flow_entry *fe) {
  unsigned int vrf;
...
static void flow_get_source(struct vr_flow_entry *fe) {
  int i;
  struct vr_flow_entry *rfe;
 ...
 * the get operation
 */
static void flow_get_cleanup(void) {
  unsigned int i;
...
static void flow_get_entry(struct vr_flow_entry *fe) {
  /*
...
static inline int print_new_line_if_required(int printed, int max_print) {
...
static void flow_dump_table(struct flow_table *ft) {
  ...
  printf("Entries: Created %" PRIu64 " Added %" PRIu64 " Deleted %" PRIu64 " Changed %" PRIu64
         "Processed %" PRIu64 " Used Overflow entries %u\n",
         ft->ft_created, ft->ft_added, ft->ft_deleted, ft->ft_changed, ft->ft_processed,
         ft->ft_oflow_entries);
...
static void flow_stats(void) {
  ...
  while (1) {
    ...
    if (rate != 0 || total_rate != 0) {
      if (rate < -1000) {
        avg_teardown_rate =
            avg_teardown_rate * teardown_time - (active_entries - prev_active_entries) * 1000;
      ...
      if (rate > 1000) {
        avg_setup_rate =
            avg_setup_rate * setup_time + (active_entries - prev_active_entries) * 1000;
        setup_time += diff_ms;
    ...
    printf("    Hold   Entries  --- Total = %7d, new = %7d \n", hold_entries,
           (hold_entries - prev_hold_entries));
  ...
  while (1) {
    ...
    if (hold_rate || processed_rate || added_rate || deleted_rate) {
      printf("%s.%03d:  Entries = %8d "
             ...
             fmt, (int)now.tv_usec / 1000, (int)ft->ft_total_entries, flow_op_rate, processed_count,
             added_count, deleted_count, hold_count, ft->ft_burst_free_tokens, ft->ft_hold_entries);
      fflush(stdout);
    }
...
static int get_flow_table_map_counts(vr_flow_table_data *table, struct flow_table *ft) {
...
static int flow_table_map(vr_flow_table_data *table) {
...
static int flow_make_flow_req(void *req, char *flow_str) {
  int ret;
...
static int flow_table_setup(void) {
  int ret = 0;
...
static int fill_flow_req(vr_flow_req *req, unsigned long flow_index, char action) {
  struct vr_flow_entry *fe;
  ...
  if (fe->fe_type == VP_TYPE_IP) {
    ...
    memcpy(&req->fr_flow_sip_l, fe->fe_key.flow_ip, 2 * VR_IP_ADDR_SIZE(fe->fe_type));
  }
...
static void flow_do_op(unsigned long flow_index, char action) {
...
static void flow_process_response() {
  int ret;
...
static int flow_make_flow_req_perf(vr_flow_req *req) {
  ...
  error = 0;
  ret = sandesh_encode(req, "vr_flow_req", vr_find_sandesh_info, (nl_get_buf_ptr(cl) + attr_len),
...
static void Usage(void) {
  ...
  printf("                       --match \"10.204.217.10:56910,169.254.0.3:22 & vrf 0 & proto "
         "tcp\"\n");
...
static void validate_options(void) {
  if (!flow_index && !list && !rate && !stats && !match_set && !perf && !flush)
    Usage();
...
static int flow_set_family(unsigned int family, char *addr, const char *port) {
 ...
 * [a:a::a:a]:p OR a:a::a:a
 */
static int flow_set_tuple(char *ip_port) {
...
static int flow_set_ip(char *match_string) {
  int ret = 0;
...
static int flow_set_vrf(char *string) {
  if (!strlen(string))
    return -EINVAL;
...
static int flow_set_proto(char *string) {
  if (!strlen(string))
    return -EINVAL;
...
static int flow_set_match(char *match_string) {
  int ret = 0;
  unsigned int length = strlen(match_string), token_length;
  char *token, *string = match_string;
...
static void parse_long_opts(int opt_flow_index, char *opt_arg) {
  errno = 0;
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "d:f:g:i:e:p:u:b:lrsF", long_options, &option_index)) >=
         0) {
    switch (opt) {
    case 'f':
    case 'g':
