...
static void vrf_req_process(void *s_req) {
...
op_retry:
  switch (vrf_op) {
  case SANDESH_OP_ADD:
    ret = vr_send_vrf_add(cl, 0, vrf_index, hbsl_idx, hbsr_idx,
                          VRF_FLAG_VALID | VRF_FLAG_HBF_L_VALID | VRF_FLAG_HBF_R_VALID);
    break;
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "bcdgl:r:v:s:", long_options, &option_index)) >= 0) {
    switch (opt) {
    case 'c':
      if (vrf_op >= 0)
        Usage();
  ...
  if ((vrf_op == SANDESH_OP_DUMP) || (vrf_op == SANDESH_OP_GET)) {
    printf("VRF Table\n\n");
