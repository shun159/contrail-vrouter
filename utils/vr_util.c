...
char *vr_extract_token(char *string, char token_separator) {
  int ret;
  unsigned int length;
...
bool vr_valid_ipv6_address(const char *addr) {
...
bool vr_valid_ipv4_address(const char *addr) {
  unsigned int i = 0, j = 0, sep_count = 0;
...
bool vr_valid_mac_address(const char *mac) {
...
char *vr_proto_string(unsigned short proto) {
  switch (proto) {
...
/* send and receive */
int vr_recvmsg_generic(struct nl_client *cl, bool dump, bool msg_wait) {
  int ret = 0;
  bool pending = true;
...
int vr_recvmsg(struct nl_client *cl, bool dump) { return vr_recvmsg_generic(cl, dump, false); }

int vr_recvmsg_waitall(struct nl_client *cl, bool dump) {
  return vr_recvmsg_generic(cl, dump, true);
}

int vr_sendmsg(struct nl_client *cl, void *request, char *request_string) {
  ...
  ret = sandesh_encode(request, request_string, vr_find_sandesh_info,
                       (nl_get_buf_ptr(cl) + attr_len), (nl_get_buf_len(cl) - attr_len), &error);
  if (ret <= 0)
    return ret;
...
struct nl_client *vr_get_nl_client(int proto) {
  int ret;
...
int vr_send_get_bridge_table_data(struct nl_client *cl) {
  int ret;
...
int vr_send_set_dcb_state(struct nl_client *cl, uint8_t *ifname, uint8_t state) {
  int ret;
...
int vr_send_get_dcb_state(struct nl_client *cl, uint8_t *ifname) {
  int ret;
...
int vr_send_set_dcbx(struct nl_client *cl, uint8_t *ifname, uint8_t dcbx) {
  int ret;
...
int vr_send_get_dcbx(struct nl_client *cl, uint8_t *ifname) {
  int ret;
...
int vr_send_get_priority_config(struct nl_client *cl, uint8_t *ifname, struct priority *p) {
  int ret;
...
int vr_send_set_priority_config(struct nl_client *cl, uint8_t *ifname, struct priority *p) {
  int ret;
...
int vr_send_set_dcb_all(struct nl_client *cl, uint8_t *ifname) {
  int ret;
...
int vr_send_get_ieee_ets(struct nl_client *cl, uint8_t *ifname, struct priority *p) {
  int ret;
...
int vr_send_set_ieee_ets(struct nl_client *cl, uint8_t *ifname, struct priority *p) {
  int ret;
...
void vr_print_drop_stats(vr_drop_stats_req *stats, int core) {
...
void vr_print_pkt_drop_log_data(vr_pkt_drop_log_req *pkt_log, int i) {
  ...
  char vr_pkt_droplog_rsn[][50] = {DROP_RSN_MAP(string)};
...
void vr_print_pkt_drop_log(vr_pkt_drop_log_req *pkt_log) {
...
int vr_response_common_process(vr_response *resp, bool *dump_pending) {
  int ret = 0;
...
/* dropstats start */
uint64_t vr_sum_drop_stats(vr_drop_stats_req *req) {
  uint64_t sum = 0;
...
void vr_drop_stats_req_destroy(vr_drop_stats_req *req) {
  if (!req)
    return;
...
vr_drop_stats_req *vr_drop_stats_req_get_copy(vr_drop_stats_req *src) {
...
int vr_send_drop_stats_get(struct nl_client *cl, unsigned int router_id, short core) {
...
int vr_drop_stats_reset(struct nl_client *cl) {
  vr_drop_stats_req req;
...
int vr_pkt_drop_log_request(struct nl_client *cl, unsigned int router_id, unsigned int core,
                            int log_idx) {
  int ret = 0;
...
/* Interface start */
void vr_interface_req_destroy(vr_interface_req *req) {
  if (!req)
    return;
...
vr_interface_req *vr_interface_req_get_copy(vr_interface_req *src) {
  ...
  *dst = *src;
  ...
  if (src->vifr_queue_ierrors_to_lcore_size && src->vifr_queue_ierrors_to_lcore) {
    ...
    memcpy(dst->vifr_queue_ierrors_to_lcore, src->vifr_queue_ierrors_to_lcore,
  ...
  if (src->vifr_fat_flow_protocol_port_size && src->vifr_fat_flow_protocol_port) {
    ...
    memcpy(dst->vifr_fat_flow_protocol_port, src->vifr_fat_flow_protocol_port,
  ...
  if (src->vifr_fat_flow_exclude_ip_list_size && src->vifr_fat_flow_exclude_ip_list) {
    dst->vifr_fat_flow_exclude_ip_list =
        malloc(src->vifr_fat_flow_exclude_ip_list_size * sizeof(uint64_t));
  ...
  if (src->vifr_fat_flow_exclude_ip6_u_list_size && src->vifr_fat_flow_exclude_ip6_u_list) {
    dst->vifr_fat_flow_exclude_ip6_u_list =
        malloc(src->vifr_fat_flow_exclude_ip6_u_list_size * sizeof(uint64_t));
  ...
  if (src->vifr_fat_flow_exclude_ip6_l_list_size && src->vifr_fat_flow_exclude_ip6_l_list) {
    dst->vifr_fat_flow_exclude_ip6_l_list =
        malloc(src->vifr_fat_flow_exclude_ip6_l_list_size * sizeof(uint64_t));
...
int vr_send_interface_dump(struct nl_client *cl, unsigned int router_id, int marker, int core) {
...
int vr_send_interface_get(struct nl_client *cl, unsigned int router_id, int vif_index, int os_index,
                          int core, int get_drops) {
  vr_interface_req req;
...
int vr_send_interface_delete(struct nl_client *cl, unsigned int router_id, char *vif_name,
                             int vif_index) {
  vr_interface_req req;
...
int vr_send_interface_add(struct nl_client *cl, int router_id, char *vif_name, int os_index,
                          int vif_index, int vif_xconnect_index, int vif_type, unsigned int vrf,
                          unsigned int flags, int8_t *vif_mac, int8_t vif_transport,
                          const char *guid) {
  int platform;
  vr_interface_req req;
...
int vr_send_mem_stats_get(struct nl_client *cl, unsigned int router_id) {
...
/* mirror start */
void vr_mirror_req_destroy(vr_mirror_req *req) {
  if (!req)
    return;
...
vr_mirror_req *vr_mirror_req_get_copy(vr_mirror_req *req) {
  vr_mirror_req *dst;
...
int vr_send_mirror_dump(struct nl_client *cl, unsigned int router_id, int marker) {
  vr_mirror_req req;
...
int vr_send_mirror_get(struct nl_client *cl, unsigned int router_id, unsigned int mirror_index) {
  vr_mirror_req req;
...
int vr_send_mirror_delete(struct nl_client *cl, unsigned int router_id, unsigned int mirror_index) {
  vr_mirror_req req;
...
int vr_send_mirror_add(struct nl_client *cl, unsigned int router_id, unsigned int mirror_index,
                       int mirror_nh_index, unsigned int mirror_flags, int vni_id) {
  vr_mirror_req req;
...
int vr_send_vrf_dump(struct nl_client *cl, unsigned int router_id, int marker) {
  vr_vrf_req req;
...
int vr_send_vrf_get(struct nl_client *cl, unsigned int router_id, unsigned int vrf_index) {
  vr_vrf_req req;
...
int vr_send_vrf_delete(struct nl_client *cl, unsigned int router_id, unsigned int vrf_index) {
  vr_vrf_req req;
...
int vr_send_vrf_add(struct nl_client *cl, unsigned int router_id, unsigned int vrf_index,
                    int hbfl_idx, int hbfr_idx, unsigned int vrf_flags) {
  vr_vrf_req req;
...
int vr_send_mpls_delete(struct nl_client *cl, unsigned int router_id, unsigned int label) {
  vr_mpls_req req;
...
int vr_send_mpls_dump(struct nl_client *cl, unsigned int router_id, int marker) {
  vr_mpls_req req;
...
int vr_send_mpls_get(struct nl_client *cl, unsigned int router_id, unsigned int label) {
  vr_mpls_req req;
...
int vr_send_mpls_add(struct nl_client *cl, unsigned int router_id, unsigned int label,
...
char *vr_nexthop_type_string(vr_nexthop_req *nh) {
  switch (nh->nhr_type) {
  case NH_DEAD:
    return "DEAD";
    break;
...
bool vr_nexthop_req_has_vif(vr_nexthop_req *req) {
  switch (req->nhr_type) {
  case NH_ENCAP:
  case NH_TUNNEL:
  case NH_RCV:
...
void vr_nexthop_req_destroy(vr_nexthop_req *req) {
  if (!req)
    return;
...
vr_nexthop_req *vr_nexthop_req_get_copy(vr_nexthop_req *src) {
  vr_nexthop_req *dst;
...
int vr_send_nexthop_delete(struct nl_client *cl, unsigned int router_id, unsigned int nh_index) {
  vr_nexthop_req req;
...
int vr_send_nexthop_dump(struct nl_client *cl, unsigned int router_id, int marker) {
  vr_nexthop_req req;
...
int vr_send_nexthop_get(struct nl_client *cl, unsigned int router_id, unsigned int nh_index) {
  vr_nexthop_req req;
...
int vr_send_pbb_tunnel_add(struct nl_client *cl, unsigned int router_id, int nh_index,
                           unsigned int flags, int vrf_index, int8_t *bmac,
...
int vr_send_nexthop_composite_add(struct nl_client *cl, unsigned int router_id, int nh_index,
                                  unsigned int flags, int vrf_index, unsigned int num_components,
...
int vr_send_nexthop_encap_tunnel_add(struct nl_client *cl, unsigned int router_id,
                                     unsigned int type, int nh_index, unsigned int flags,
                                     int vrf_index, int vif_index, int8_t *smac, int8_t *dmac,
                                     struct in_addr sip, struct in_addr dip, int sport, int dport,
                                     int8_t *l3_vxlan_mac) {
  vr_nexthop_req req;
...
int vr_send_nexthop_add(struct nl_client *cl, unsigned int router_id, unsigned int type,
                        int nh_index, unsigned int flags, int vrf_index, int vif_index) {
  vr_nexthop_req req;
...
void vr_route_req_destroy(vr_route_req *req) {
  if (!req)
    return;
...
void address_mask(uint8_t *addr, uint8_t plen, unsigned int family) {
  int i;
...
vr_route_req *vr_route_req_get_copy(vr_route_req *src) {
  vr_route_req *dst;
...
int vr_send_route_dump(struct nl_client *cl, unsigned int router_id, unsigned int vrf,
...
static int vr_send_route_common(struct nl_client *cl, unsigned int op, unsigned int router_id,
                                unsigned int vrf, unsigned int family, uint8_t *prefix,
                                unsigned int prefix_len, unsigned int nh_index, int label,
                                uint8_t *mac, uint32_t replace_len, unsigned int flags) {
  vr_route_req req;
...
int vr_send_route_get(struct nl_client *cl, unsigned int router_id, unsigned int vrf,
                      unsigned int family, uint8_t *prefix, unsigned int prefix_len, uint8_t *mac) {
  return vr_send_route_common(cl, SANDESH_OP_GET, router_id, vrf, family, prefix, prefix_len, 0, 0,
                              mac, 0, 0);
}

int vr_send_route_delete(struct nl_client *cl, unsigned int router_id, unsigned int vrf,
                         unsigned int family, uint8_t *prefix, unsigned int prefix_len,
                         unsigned int nh_index, int label, uint8_t *mac, uint32_t replace_len,
                         unsigned int flags) {
  return vr_send_route_common(cl, SANDESH_OP_DEL, router_id, vrf, family, prefix, prefix_len,
                              nh_index, label, mac, replace_len, flags);
}

int vr_send_route_add(struct nl_client *cl, unsigned int router_id, unsigned int vrf,
                      unsigned int family, uint8_t *prefix, unsigned int prefix_len,
                      unsigned int nh_index, int label, uint8_t *mac, uint32_t replace_len,
                      unsigned int flags) {
  return vr_send_route_common(cl, SANDESH_OP_ADD, router_id, vrf, family, prefix, prefix_len,
                              nh_index, label, mac, replace_len, flags);
}

/* vrf assign start */
int vr_send_vrf_assign_dump(struct nl_client *cl, unsigned int router_id, unsigned int vif_index,
                            int marker) {
...
int vr_send_vrf_assign_set(struct nl_client *cl, unsigned int router_id, unsigned int vif_index,
                           unsigned int vlan_id, unsigned int vrf_id) {
...
int vr_send_vrf_stats_dump(struct nl_client *cl, unsigned int router_id, int marker) {
...
int vr_send_vrf_stats_get(struct nl_client *cl, unsigned int router_id, unsigned int vrf) {
  vr_vrf_stats_req req;
...
int vr_send_vrouter_get(struct nl_client *cl, unsigned int router_id) {
  vrouter_ops req;
...
int vr_send_vrouter_set_logging(struct nl_client *cl, unsigned int router_id,
                                unsigned int log_level, unsigned int *e_log_types,
                                unsigned int e_size, unsigned int *d_log_types,
                                unsigned int d_size) {
  vrouter_ops req;
...
int vr_send_vrouter_set_runtime_opts(struct nl_client *cl, unsigned int router_id, int perfr,
                                     int perfs, int from_vm_mss_adj, int to_vm_mss_adj, int perfr1,
                                     int perfr2, int perfr3, int perfp, int perfq1, int perfq2,
                                     int perfq3, int udp_coff, int flow_hold_limit, int mudp,
                                     int btokens, int binterval, int bstep,
...
int vr_send_vxlan_delete(struct nl_client *cl, unsigned int router_id, unsigned int vnid) {
  vr_vxlan_req req;
...
int vr_send_vxlan_dump(struct nl_client *cl, unsigned int router_id, int marker) {
  vr_vxlan_req req;
...
int vr_send_vxlan_get(struct nl_client *cl, unsigned int router_id, unsigned int vnid) {
  vr_vxlan_req req;
...
int vr_send_vxlan_add(struct nl_client *cl, unsigned int router_id, unsigned int vnid,
                      unsigned int nh_index) {
  vr_vxlan_req req;
...
int vr_send_fc_map_get(struct nl_client *cl, unsigned int router_id, uint8_t fc_map_id) {
  vr_fc_map_req req;
...
int vr_send_fc_map_dump(struct nl_client *cl, unsigned int router_id, int marker) {
  vr_fc_map_req req;
...
int vr_send_fc_map_delete(struct nl_client *cl, unsigned int router_id, uint8_t fc_id) {
  vr_fc_map_req req;
  int16_t id = fc_id;
...
int vr_send_fc_map_add(struct nl_client *cl, unsigned int router_id, int16_t *fc_id,
                       uint8_t fc_id_size, uint8_t *dscp, uint8_t *mpls_qos, uint8_t *dotonep,
                       uint8_t *queue) {
  vr_fc_map_req req;
...
int vr_send_qos_map_get(struct nl_client *cl, unsigned int router_id, unsigned int qos_map_id) {
  vr_qos_map_req req;
...
int vr_send_qos_map_dump(struct nl_client *cl, unsigned int router_id, int marker) {
  vr_qos_map_req req;
...
int vr_send_qos_map_delete(struct nl_client *cl, unsigned int router_id, unsigned int qos_map_id) {
  vr_qos_map_req req;
...
int vr_send_qos_map_add(struct nl_client *cl, unsigned int router_id, unsigned int qos_id,
                        uint8_t *dscp, uint8_t num_dscp, uint8_t *dscp_fc_id, uint8_t *mpls_qos,
                        uint8_t num_mpls_qos, uint8_t *mpls_qos_fc_id, uint8_t *dotonep,
                        uint8_t num_dotonep, uint8_t *dotonep_fc_id) {
  vr_qos_map_req req;
