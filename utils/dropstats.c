  ...
  if (stats->vdl_pkt_droplog_sysctl_en == 1) {
    ...
    if (stats->vdl_pkt_droplog_en == 1) {
      ...
      if (stats->vdl_pkt_droplog_max_bufsz > VR_PKT_DROPLOG_MAX_ALLOW_BUFSZ) {
         ...
         * the console */
        if (stats->vdl_log_idx <
            (stats->vdl_pkt_droplog_max_bufsz - VR_PKT_DROPLOG_MAX_ALLOW_BUFSZ)) {
          /* Request packet drop buffer for next iteration by
           * incrementing with MAX_ALLOWED_BUFFER  */
          stats_index = stats->vdl_log_idx + VR_PKT_DROPLOG_MAX_ALLOW_BUFSZ;
        ...
        else if (stats->vdl_pkt_droplog_max_bufsz % VR_PKT_DROPLOG_MAX_ALLOW_BUFSZ != 0) {
...
static void pkt_drop_log_nlutils_callbacks() {
...
static void dropstats_fill_nl_callbacks() {
  nl_cb.vr_drop_stats_req_process = drop_stats_req_process;
}

static int vr_get_drop_stats(struct nl_client *cl) {
  int ret;
...
static int vr_clear_drop_stats(struct nl_client *cl) {
...
int main(int argc, char *argv[]) {
  ...
  while (((opt = getopt_long(argc, argv, "h:c:o:l:s:", long_options, &option_index)) >= 0)) {
    switch (opt) {
    case 'c':
      core_set = 1;
