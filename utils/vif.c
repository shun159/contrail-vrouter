...
static char *vr_get_if_type_string(int t) {
  switch (t) {
  case VIF_TYPE_HOST:
    return "Host";
  case VIF_TYPE_AGENT:
...
static unsigned int vr_get_if_type(char *type_str) {
  if (!strncmp(type_str, VHOST_TYPE_STRING, strlen(VHOST_TYPE_STRING)))
    return VIF_TYPE_HOST;
  else if (!strncmp(type_str, AGENT_TYPE_STRING, strlen(AGENT_TYPE_STRING)))
    return VIF_TYPE_AGENT;
  else if (!strncmp(type_str, PHYSICAL_TYPE_STRING, strlen(PHYSICAL_TYPE_STRING)))
    return VIF_TYPE_PHYSICAL;
  else if (!strncmp(type_str, VIRTUAL_VLAN_TYPE_STRING, strlen(VIRTUAL_VLAN_TYPE_STRING)))
    return VIF_TYPE_VIRTUAL_VLAN;
  else if (!strncmp(type_str, VIRTUAL_TYPE_STRING, strlen(VIRTUAL_TYPE_STRING)))
    return VIF_TYPE_VIRTUAL;
  else if (!strncmp(type_str, XEN_LL_TYPE_STRING, strlen(XEN_LL_TYPE_STRING)))
    return VIF_TYPE_XEN_LL_HOST;
  else if (!strncmp(type_str, GATEWAY_TYPE_STRING, strlen(GATEWAY_TYPE_STRING)))
    return VIF_TYPE_GATEWAY;
  else if (!strncmp(type_str, STATS_TYPE_STRING, strlen(STATS_TYPE_STRING)))
    return VIF_TYPE_STATS;
  else if (!strncmp(type_str, MONITORING_TYPE_STRING, strlen(MONITORING_TYPE_STRING)))
...
static unsigned int vr_get_if_transport(char *transport_str) {
  if (!strncmp(transport_str, ETH_TRANSPORT_STRING, strlen(ETH_TRANSPORT_STRING)))
    return VIF_TRANSPORT_ETH;
  else if (!strncmp(transport_str, PMD_TRASPORT_STRING, strlen(PMD_TRASPORT_STRING)))
    return VIF_TRANSPORT_PMD;
  else if (!strncmp(transport_str, VIRTUAL_TRANSPORT_STRING, strlen(VIRTUAL_TRANSPORT_STRING)))
    return VIF_TRANSPORT_VIRTUAL;
  else if (!strncmp(transport_str, SOCKET_TRANSPORT_STRING, strlen(SOCKET_TRANSPORT_STRING)))
...
static char *vr_if_flags(int flags) {
...
static void vr_interface_print_header(void) {
  unsigned int i, array_size;
  ...
  for (i = 0; i < array_size; i++) {
    ...
    printf("%s=%s", flag_metadata[i].vuf_flag_symbol, flag_metadata[i].vuf_flag_string);
  }
...
static void drop_stats_req_process(void *s_req) {
...
static void vrf_assign_req_process(void *s) {
...
static int vr_interface_print_head_space(void) {
  int i;
...
char *vr_if_transport_string(vr_interface_req *req) {
...
static void vr_interface_core_print(void) {
  if (core != (unsigned)-1) {
    printf("Core %u ", core);
  }
}

static void vr_interface_nombufs_print(uint64_t nombufs) {
  if (nombufs)
    printf(" no mbufs:%" PRId64, nombufs);
  printf("\n");
}

static void vr_interface_pbem_counters_print(const char *title, bool print_always, uint64_t packets,
                                             uint64_t bytes, uint64_t errors, uint64_t nombufs) {
  if (print_always || packets || bytes || errors) {
    vr_interface_print_head_space();
    vr_interface_core_print();
    printf("%s packets:%" PRId64 "  bytes:%" PRId64 " errors:%" PRId64, title, packets, bytes,
           errors);
    vr_interface_nombufs_print(nombufs);
  }
}

static void vr_interface_pesm_counters_print(const char *title, bool print_always, uint64_t packets,
                                             uint64_t errors, uint64_t syscalls, uint64_t nombufs) {
  if (print_always || packets || errors) {
    vr_interface_print_head_space();
    vr_interface_core_print();
    printf("%s packets:%" PRId64 " errors:%" PRId64, title, packets, errors);
    if (syscalls)
...
static void vr_interface_pe_counters_print(const char *title, bool print_always, uint64_t packets,
                                           uint64_t errors) {
  if (print_always || packets || errors) {
    vr_interface_print_head_space();
    vr_interface_core_print();
...
static void vr_interface_e_per_lcore_counters_print(const char *title, bool print_always,
...
static void vr_interface_fabric_info(vr_interface_req *req) {
  ...
  printf("Fabric Interface: %s  Status: %s  Driver: %s\n", req->vifr_fab_name,
         fabric_link[(req->vifr_intf_status & 0x01)], req->vifr_fab_drv_name);
  ...
  for (i = 0; i < req->vifr_num_bond_slave; i++) {
    ...
    printf("Slave Interface(%d): %s  Status: %s  Driver: %s\n", i, p_name,
           fabric_link[(req->vifr_intf_status >> (i + 1)) & 0x01], p_drv_name);

    p_name = strchr(p_name, '\0');
    p_name++;
    p_drv_name = strchr(p_drv_name, '\0');
    p_drv_name++;
  }
}

/* Display VLAN ID & VLAN fwd interface only on DPDK platforms */
static void vr_interface_vlan_info(vr_interface_req *req) {
  if (req->vifr_type != VIF_TYPE_PHYSICAL)
    return;
...
static void list_get_print(vr_interface_req *req) {
  char ip6_addr[INET6_ADDRSTRLEN], ip_addr[INET_ADDRSTRLEN], name[50] = {0}, ip6_ip[16];
  ...
  if (req->vifr_flags & VIF_FLAG_PMD) {
    ...
    case VIF_TYPE_PHYSICAL:
      if (req->vifr_flags & VIF_FLAG_MOCK_DEVICE)
        ...
        printf("PCI: "
               "%.4" PRIx16 ":%.2" PRIx8 ":%.2" PRIx8 ".%" PRIx8,
               (uint16_t)(req->vifr_os_idx >> 16), (uint8_t)(req->vifr_os_idx >> 8) & 0xFF,
               (uint8_t)(req->vifr_os_idx >> 3) & 0x1F, (uint8_t)(req->vifr_os_idx & 0x7));
      break;

    case VIF_TYPE_MONITORING:
      printf("Monitoring: %s for vif%d/%d", req->vifr_name, req->vifr_rid, req->vifr_os_idx);
      break;

    default:
      if (req->vifr_name)
        printf("%s: %s", vr_if_transport_string(req), req->vifr_name);
      if (req->vifr_flags & VIF_FLAG_MOCK_DEVICE)
        printf(" Mock");
      break;
    }
  ...
  if ((req->vifr_type == VIF_TYPE_PHYSICAL) && (!(req->vifr_flags & VIF_FLAG_MOCK_DEVICE))) {
    if (req->vifr_speed >= 0) {
    ...
    if (req->vifr_src_mac_size && req->vifr_src_mac) {
      for (i = 0; i < (req->vifr_src_mac_size / VR_ETHER_ALEN); i = i + 1) {
        printf(", " MAC_FORMAT, MAC_VALUE((uint8_t *)(req->vifr_src_mac + (i * VR_ETHER_ALEN))));
  ...
  printf("Type:%s HWaddr:" MAC_FORMAT " IPaddr:%s\n", vr_get_if_type_string(req->vifr_type),
         ...
         inet_ntop(AF_INET, &req->vifr_ip, ip_addr, INET_ADDRSTRLEN));
  ...
  vr_interface_print_head_space();
  printf("Vrf:%d Mcast Vrf:%d Flags:%s QOS:%d Ref:%d", req->vifr_vrf, req->vifr_mcast_vrf,
         req->vifr_flags ? vr_if_flags(req->vifr_flags) : "NULL", req->vifr_qos_map_index,
         req->vifr_ref_cnt);
  ...
  if (platform == DPDK_PLATFORM || platform == VTEST_PLATFORM) {
    vr_interface_pbem_counters_print("RX device", print_zero, req->vifr_dev_ipackets,
                                     req->vifr_dev_ibytes, req->vifr_dev_ierrors,
                                     req->vifr_dev_inombufs);
    vr_interface_pesm_counters_print("RX port  ", print_zero, req->vifr_port_ipackets,
                                     req->vifr_port_ierrors, req->vifr_port_isyscalls,
                                     req->vifr_port_inombufs);
    vr_interface_pe_counters_print("RX queue ", print_zero, req->vifr_queue_ipackets,
  ...
  vr_interface_pbem_counters_print("RX", true, req->vifr_ipackets, req->vifr_ibytes,
                                   req->vifr_ierrors, 0);
  vr_interface_pbem_counters_print("TX", true, req->vifr_opackets, req->vifr_obytes,
                                   req->vifr_oerrors, 0);
  if (req->vifr_isid || req->vifr_pbb_mac_size) {
    vr_interface_print_head_space();
    printf("ISID: %d Bmac: " MAC_FORMAT "\n", req->vifr_isid,
           MAC_VALUE((uint8_t *)req->vifr_pbb_mac));
  }
  vr_interface_print_head_space();
  printf("Drops:%" PRIu64 "\n", req->vifr_dpackets);
  ...
  if (platform == DPDK_PLATFORM || platform == VTEST_PLATFORM) {
    vr_interface_pe_counters_print("TX queue ", print_zero, req->vifr_queue_opackets,
                                   req->vifr_queue_oerrors);
    vr_interface_pesm_counters_print("TX port  ", print_zero, req->vifr_port_opackets,
                                     req->vifr_port_oerrors, req->vifr_port_osyscalls, 0);
    vr_interface_pbem_counters_print("TX device", print_zero, req->vifr_dev_opackets,
                                     req->vifr_dev_obytes, req->vifr_dev_oerrors, 0);
  }
  ...
  if (req->vifr_fat_flow_protocol_port_size) {
    ...
    for (i = 0; i < req->vifr_fat_flow_protocol_port_size; i++) {
      ...
      case VR_AGGREGATE_SRC_IPV4:
        ...
        printf(" AggrSrc %s/%d %d", inet_ntop(AF_INET, ip, ip_addr, INET_ADDRSTRLEN),
               req->vifr_fat_flow_src_prefix_mask[i], req->vifr_fat_flow_src_aggregate_plen[i]);
        break;
      case VR_AGGREGATE_DST_IPV4:
        ip = (uint32_t *)&req->vifr_fat_flow_dst_prefix_l[i];
        printf(" AggrDst %s/%d %d", inet_ntop(AF_INET, ip, ip_addr, INET_ADDRSTRLEN),
               req->vifr_fat_flow_dst_prefix_mask[i], req->vifr_fat_flow_dst_aggregate_plen[i]);
        break;
      ...
      case VR_AGGREGATE_SRC_IPV6:
        ...
        *(tmp + 1) = req->vifr_fat_flow_src_prefix_l[i];
        printf(" AggrSrc %s/%d %d", inet_ntop(AF_INET6, ip6_ip, ip6_addr, INET6_ADDRSTRLEN),
               req->vifr_fat_flow_src_prefix_mask[i], req->vifr_fat_flow_src_aggregate_plen[i]);
        break;
      case VR_AGGREGATE_DST_IPV6:
        ...
        *(tmp + 1) = req->vifr_fat_flow_dst_prefix_l[i];
        printf(" AggrDst %s/%d %d", inet_ntop(AF_INET6, ip6_ip, ip6_addr, INET6_ADDRSTRLEN),
               req->vifr_fat_flow_dst_prefix_mask[i], req->vifr_fat_flow_dst_aggregate_plen[i]);
        break;
...
static void list_rate_print(vr_interface_req *req) {
  ...
  printed = printf("%s: %s", vr_get_if_type_string(req->vifr_type), req->vifr_name);
  for (; printed < 30; printed++)
    printf(" ");
  printed = printf("vif%d/%d", req->vifr_rid, req->vifr_idx);
...
static void rate_process(vr_interface_req *req, vr_interface_req *prev_req) {
  ...
  if (first_rate_iter) {
    ...
    *prev_req = *req;
    ...
    memcpy(prev_req->vifr_queue_ierrors_to_lcore, req->vifr_queue_ierrors_to_lcore,
  ...
  memcpy(rate_req_temp.vifr_queue_ierrors_to_lcore, req->vifr_queue_ierrors_to_lcore,
         req->vifr_queue_ierrors_to_lcore_size * sizeof(uint64_t));
  ...
  *prev_req = rate_req_temp;
  ...
  memcpy(prev_req->vifr_queue_ierrors_to_lcore, rate_req_temp.vifr_queue_ierrors_to_lcore,
 ...
 */
static void interface_req_process(void *s) {
...
static void response_process(void *s) {
  vr_response_common_process((vr_response *)s, &dump_pending);
  return;
}

static void vif_fill_nl_callbacks() {
...
static int vr_intf_op(struct nl_client *cl, unsigned int op) {
  int ret = -EINVAL, vrf;
  bool dump = false;
  ...
  if ((op == SANDESH_OP_DUMP && !(rate_set)) || ((op == SANDESH_OP_GET) && !(add_set))) {
...
op_retry:
  ...
  case SANDESH_OP_ADD:
    ...
    ret = vr_send_interface_add(cl, 0, if_name, if_kindex, vr_ifindex, if_xconnect_kindex,
                                vr_if_type, vrf, vr_ifflags, vr_ifmac, vr_transport, NULL);
    break;
  ...
  case SANDESH_OP_GET:
    ...
    if (!vr_vrf_assign_dump) {
      ret = vr_send_interface_get(cl, 0, vr_ifindex, if_kindex, core + 1, drop_set);
    } else {
      dump = true;
...
static unsigned long safer_strtoul(const char *nptr, char **endptr, int base) {
  if (nptr == NULL) {
    errno = EINVAL;
    return 0;
  } else {
...
static void parse_long_opts(int option_index, char *opt_arg) {
  errno = 0;
...
static void rate_stats_diff(vr_interface_req *req, vr_interface_req *prev_req) {
  struct timeval now;
  int64_t diff_ms = 0;
  unsigned int i = 0;
...
static void rate_stats(struct nl_client *cl, unsigned int vr_op) {
  struct tm *tm;
  time_t secs;
  char fmt[80] = {0};
  int ret = 0;
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "ba:c:d:g:klm:t:T:v:p:C:DPi:s:", long_options,
                            &option_index)) >= 0) {
    switch (opt) {
    case 'a':
      add_set = 1;
