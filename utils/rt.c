...
static void dump_legend(int family) {
  ...
  for (i = 0; i < array_size; i++) {
    printf("%s=%s", rt_flags[i].vuf_flag_symbol, rt_flags[i].vuf_flag_string);
    if (i != (array_size - 1))
      printf(", ");
  }
...
static int family_string_to_id(char *fname) {
...
static void vr_bridge_print_route(uint8_t *mac, unsigned int index, unsigned int flags,
                                  unsigned int label, unsigned int nh_id, uint64_t packets) {
  int ret, i;
  char flag_string[32];
...
static void vr_print_route_json(vr_route_req *rt) {
  ...
  if ((rt->rtr_family == AF_INET) || (rt->rtr_family == AF_INET6)) {
...
static void route_req_process(void *s_req) {
  int ret = 0, i;
  char addr[INET6_ADDRSTRLEN];
  char flags[32];
  ...
  if ((rt->rtr_family == AF_INET) || (rt->rtr_family == AF_INET6)) {
    ...
    if (rt->rtr_mac_size) {
      printf("%s(%d)", ether_ntoa((struct ether_addr *)(rt->rtr_mac)), rt->rtr_index);
    } else {
      printf("-");
    }
    ...
    vr_bridge_print_route(rt->rtr_mac, rt->rtr_index, rt->rtr_label_flags, rt->rtr_label,
                          rt->rtr_nh_id, 0);
  }
...
static void rt_fill_nl_callbacks() {
...
static void vr_print_rtable_header(unsigned int family, unsigned int vrf) {
  char addr[INET6_ADDRSTRLEN];
...
static void vr_bridge_table_dump(unsigned int vrf) {
  unsigned int i;
  struct vr_bridge_entry *be;

  for (i = 0; i < vr_bridge_table.bt_num_entries; i++) {
    be = &vr_bridge_table.bt_entries[i];
    if (!(be->be_flags & VR_BE_VALID_FLAG) || (be->be_key.be_vrf_id != vrf))
      continue;
    vr_bridge_print_route(be->be_key.be_mac, i, be->be_flags, be->be_label, be->be_nh_id,
                          be->be_packets);
  }
...
static int vr_route_op(struct nl_client *cl) {
  int ret;
  bool dump = false;
...
op_retry:
  switch (cmd_op) {
  case SANDESH_OP_ADD:
    ret = vr_send_route_add(cl, 0, cmd_vrf_id, cmd_family_id, cmd_prefix, cmd_plen, cmd_nh_id,
                            cmd_label, dst_mac, cmd_replace_plen, flags | cust_flags);
    break;
  ...
  case SANDESH_OP_DUMP:
    if (cmd_family_id == AF_BRIDGE) {
      ...
      dump = true;
      ret = vr_send_route_dump(cl, 0, cmd_vrf_id, cmd_family_id, rt_marker);
    }

    break;

  case SANDESH_OP_DEL:
    ret = vr_send_route_delete(cl, 0, cmd_vrf_id, cmd_family_id, cmd_prefix, cmd_plen, cmd_nh_id,
                               cmd_label, cmd_dst_mac, cmd_replace_plen, flags);
    break;

  case SANDESH_OP_GET:
    ret = vr_send_route_get(cl, 0, cmd_vrf_id, cmd_family_id, cmd_prefix, cmd_plen, cmd_dst_mac);
    break;
...
static int bridge_table_map(vr_bridge_table_data *table) {
  const char *mmap_error_msg;

  vr_bridge_table.bt_size = table->btable_size;
  vr_bridge_table.bt_num_entries = table->btable_size / sizeof(struct vr_bridge_entry);

  mmap_error_msg =
      vr_table_map(table->btable_dev, VR_MEM_BRIDGE_TABLE_OBJECT, table->btable_file_path,
                   table->btable_size, &vr_bridge_table.bt_entries);
...
static int vr_bridge_table_setup(struct nl_client *cl) {
  int ret;
  ...
  case SANDESH_OP_GET:
    ...
    case AF_INET6:
      ...
      printf("Match %s/%u in vRouter inet%c table %u/%u/unicast\n\n",
             inet_ntop(cmd_family_id, &cmd_prefix, addr, sizeof(addr)), cmd_plen,
             (cmd_family_id == AF_INET) ? '4' : '6', 0, cmd_vrf_id);
      break;
    ...
    case AF_BRIDGE:
      ...
      printf("Match %s in vRouter bridge table %u/%u/unicast\n\n",
             ether_ntoa((const struct ether_addr *)cmd_dst_mac), 0, cmd_vrf_id);
      break;
...
static void parse_long_opts(int opt_flow_index, char *opt_arg) {
  ...
  case FAMILY_OPT_INDEX:
    ...
    if (cmd_family_id != AF_INET && cmd_family_id != AF_BRIDGE && cmd_family_id != AF_INET6)
      Usage();
    break;
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "TcdbmPn:p:l:v:t:s:e:f:u:r:Fx:", long_options,
                            &option_index)) >= 0) {
    switch (opt) {
    case 'c':
      if (cmd_op >= 0) {
