...
static void vxlan_req_process(void *s_req) {
  vr_vxlan_req *req = (vr_vxlan_req *)s_req;

  printf("%7d    %d\n", req->vxlanr_vnid, req->vxlanr_nhid);
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "bcdgn:l:s:", long_options, &option_index)) >= 0) {
    switch (opt) {
    case 'c':
      if (vxlan_op >= 0)
        Usage();
  ...
  if ((vxlan_op == SANDESH_OP_DUMP) || (vxlan_op == SANDESH_OP_GET)) {
    printf("VXLAN Table\n\n");
    printf(" VNID    NextHop\n");
    printf("----------------\n");
  }
