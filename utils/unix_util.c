...
const char *vr_table_map(int major, unsigned int table, const char *table_path, size_t size,
                         void **mem) {
  enum { ERROR_LEN = 1024 };
...
const char *vr_table_unlink(const char *path) {
  enum { ERROR_LEN = 1024 };
  static char error_msg[ERROR_LEN];

  if (unlink(path) != 0) {
    if (errno != ENOENT) {
      snprintf(error_msg, ERROR_LEN, "Error deleting %s. Error <%d>: %s", path, errno,
               strerror(errno));
      return error_msg;
    }
  }
...
int nl_socket(struct nl_client *cl, int domain, int type, int protocol) {
  if (cl->cl_sock >= 0)
    return -EEXIST;
...
int nl_connect(struct nl_client *cl, uint32_t ip, uint16_t port) {
...
int nl_client_datagram_recvmsg(struct nl_client *cl, bool msg_wait) {
  int ret;
  struct msghdr msg;
  struct iovec iov;
...
int nl_client_stream_recvmsg(struct nl_client *cl, bool msg_wait) {
  int ret;
  struct msghdr msg;
  struct iovec iov;
...
int nl_sendmsg(struct nl_client *cl) {
  struct msghdr msg;
  struct iovec iov;
...
void nl_free_os_specific(struct nl_client *cl) {
  if (cl->cl_sock >= 0) {
    close(cl->cl_sock);
    cl->cl_sock = -1;
  }
...
void nl_reset_cl_sock(struct nl_client *cl) { cl->cl_sock = -1; }

struct nl_response *nl_parse_reply_os_specific(struct nl_client *cl) {
  struct nlmsghdr *nlh = (struct nlmsghdr *)(cl->cl_buf + cl->cl_buf_offset - NLMSG_HDRLEN);
...
#if defined(__linux__)
int nl_build_attr_linkinfo(struct nl_client *cl, struct vn_if *ifp) {
  char *link_info_buf;
  int len;
  struct nlattr *nla = (struct nlattr *)((char *)cl->cl_buf + cl->cl_buf_offset);

  len = NLA_HDRLEN + NLA_HDRLEN + NLA_ALIGN(strlen(ifp->if_kind) + 1);
...
int nl_build_attr_ifname(struct nl_client *cl, struct vn_if *ifp) {
  char *if_name_buf;
  int len;
  struct nlattr *nla = (struct nlattr *)((char *)cl->cl_buf + cl->cl_buf_offset);
...
int nl_build_mac_address(struct nl_client *cl, struct vn_if *ifp) {
  int len;
  char *mac_buf;
  struct nlattr *nla = (struct nlattr *)((char *)cl->cl_buf + cl->cl_buf_offset);
...
int nl_build_ifinfo(struct nl_client *cl, struct vn_if *ifp) {
  struct ifinfomsg *ifi_msg = (struct ifinfomsg *)(cl->cl_buf + cl->cl_buf_offset);

  if (cl->cl_buf_offset + NLMSG_ALIGN(sizeof(*ifi_msg)) > cl->cl_buf_len)
    return -ENOMEM;
...
int nl_build_if_create_msg(struct nl_client *cl, struct vn_if *ifp, uint8_t ack) {
  int ret;
  uint32_t flags;
...
static int nl_build_attr_dcb_set_all(struct nl_client *cl) {
  uint8_t *dst;
...
static int nl_build_attr_dcb_state(struct nl_client *cl, uint8_t state) {
  uint8_t *dst;
...
static int nl_build_attr_dcb_ifname(struct nl_client *cl, uint8_t *ifname) {
  uint8_t *dst;
  struct nlattr *attr = (struct nlattr *)nl_get_buf_ptr(cl);
...
static int nl_build_attr_set_dcbx(struct nl_client *cl, uint8_t dcbx) {
  uint8_t *dst;
  struct nlattr *nla = (struct nlattr *)nl_get_buf_ptr(cl);
...
static int nl_build_attr_dcb_get_pgtx(struct nl_client *cl) {
  unsigned int len = 0;
...
static int nl_build_attr_dcb_pgtx(struct nl_client *cl, struct priority *p) {
  uint8_t i, j, map, *dst;
...
static int nl_build_dcb_msg(struct nl_client *cl, uint8_t cmd) {
...
static bool nl_dcb_set_cmd(uint8_t cmd) {
  switch (cmd) {
  case DCB_CMD_SSTATE:
...
static int nl_build_dcb_nl_msg(struct nl_client *cl, uint8_t *ifname, uint8_t cmd, uint8_t ack) {
  int ret, msgtype;
  uint32_t flags;
...
static int nl_parse_dcb_tc_attr(struct nlattr *nla, void *buf, uint8_t parent_attr_type) {
  int ret;
  uint8_t byte, j;
...
static int nl_parse_dcb_pg_attr(struct nlattr *nla, void *buf) {
  int ret;
  uint8_t byte;
  unsigned int processed = 0, nla_len;
...
static int nl_parse_dcb_ieee_attr_ets(struct nlattr *nla, void *buf) {
  uint8_t i;
...
static int nl_parse_dcb_ieee_attr(struct nlattr *nla, void *buf) {
  int ret;
...
static int nla_parse_dcb_ieee_response(struct nlattr *nla, void *buf) {
  int ret;
  unsigned int processed = 0, nla_len;
  struct nlattr *nla_p;
...
static int nla_parse_dcb_pg_response(struct nlattr *nla, void *buf) {
  int ret;
  unsigned int processed = 0, nla_len;
  struct nlattr *nla_p;
...
static int nl_parse_dcb_response(uint8_t *dcbm, uint8_t cmd, uint8_t attr, void *buf) {
  int ret;
  struct nlattr *nla;
  ...
  if ((nla->nla_type == DCB_ATTR_PG_CFG) && (cmd == DCB_CMD_PGTX_GCFG)) {
...
int nl_build_set_dcb_state_msg(struct nl_client *cl, uint8_t *ifname, uint8_t state) {
  int ret;
...
int nl_build_get_dcb_state_msg(struct nl_client *cl, uint8_t *ifname) {
  int ret;
...
int nl_build_set_priority_config_msg(struct nl_client *cl, uint8_t *ifname, struct priority *p) {
  int ret;
...
int nl_build_get_priority_config_msg(struct nl_client *cl, uint8_t *ifname) {
  int ret;
...
int nl_build_set_dcb_all(struct nl_client *cl, uint8_t *ifname) {
  int ret;
...
int nl_build_set_dcbx(struct nl_client *cl, uint8_t *ifname, uint8_t dcbx) {
  int ret;
...
int nl_build_get_dcbx(struct nl_client *cl, uint8_t *ifname) {
  int ret;
...
int nl_dcb_parse_reply(struct nl_client *cl, uint8_t cmd, void *resp_buf) {
  int ret = 0;
  ...
  if (resp) {
    ...
    ret = nl_parse_dcb_response((uint8_t *)resp->nl_data, cmd, attr, resp_buf);
  }
...
static int nl_build_attr_ieee_ets(struct nl_client *cl, struct priority *p) {
  unsigned int i;
...
int nl_build_get_ieee_ets(struct nl_client *cl, uint8_t *ifname, struct priority *p) {
  int ret;
...
int nl_build_set_ieee_ets(struct nl_client *cl, uint8_t *ifname, struct priority *p) {
  int ret;
...
int nl_dcb_sendmsg(struct nl_client *cl, uint8_t cmd, void *resp_buf) {
  int ret;
...
int vrouter_obtain_family_id(struct nl_client *cl) {
  int ret;
  struct nl_response *resp;
  ...
  resp = nl_parse_reply(cl);
  if (!resp || resp->nl_type != NL_MSG_TYPE_GEN_CTRL || resp->nl_op != CTRL_CMD_NEWFAMILY)
    return -EINVAL;
