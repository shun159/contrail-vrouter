...
uint8_t *udp_get_buf_ptr(struct udp_client *cl) {
  return (uint8_t *)(cl->cl_buf + cl->cl_buf_offset);
}

uint32_t udp_get_buf_len(struct udp_client *cl) { return cl->cl_buf_len - cl->cl_buf_offset; }

struct udp_response *udp_parse_reply(struct udp_client *cl) {
...
void udp_free(struct udp_client *cl) {
  if (cl->cl_sock >= 0) {
    close(cl->cl_sock);
    cl->cl_sock = -1;
  }
...
void udp_update_len(struct udp_client *cl, unsigned int len) {
  cl->cl_buf_offset += len;
  return;
}

int udp_socket(struct udp_client *cl, uint16_t port) {
...
int udp_recvmsg(struct udp_client *cl) {
  int ret;
  struct sockaddr_in sa;
  struct msghdr msg;
  struct iovec iov;
...
int udp_sendmsg(struct udp_client *cl) {
  struct msghdr msg;
  struct iovec iov;
...
struct udp_client *udp_register_client(void) {
...
void udp_free_client(struct udp_client *cl) {
  udp_free(cl);
  free(cl);
...
int uvr_nametotype(const char *name) {
  unsigned int len;
...
int uvr_nametoindex(const char *name) {
  unsigned int len;
  unsigned int ifnum;
