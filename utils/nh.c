...
static int vr_nh_op(struct nl_client *cl, int command, int type, uint32_t nh_id, uint32_t if_id,
                    uint32_t vrf_id, int8_t *dst, int8_t *src, struct in_addr sip,
                    struct in_addr dip, uint32_t flags);

char *nh_type(uint32_t type) {
  switch (type) {
  case NH_DEAD:
    return "Dead";
...
char *nh_ecmp_config_hash_str(uint8_t hash, char *ptr) {
  int i;
...
char *nh_flags(uint32_t flags, uint8_t type, char *ptr) {
  int i;
  uint32_t mask;
...
static void nh_print_newline_header(void) {
  printf("\n%14c", ' ');
  return;
}

static void nexthop_req_process(void *s_req) {
  ...
  printf("Id:%-9d  Type:%-13s  Fmly:%8s  Rid:%d  Ref_cnt:%-10d Vrf:%d", req->nhr_id,
         nh_type(req->nhr_type), fam, req->nhr_rid, req->nhr_ref_cnt, req->nhr_vrf);
  nh_print_newline_header();
  printf("Flags:%s", nh_flags(req->nhr_flags, req->nhr_type, flags_mem));
  ...
  if (req->nhr_type == NH_RCV) {
    ...
    nh_print_newline_header();
    printf("EncapFmly:%04x Oif:%d Len:%d", req->nhr_encap_family, req->nhr_encap_oif_id,
           req->nhr_encap_size);
    nh_print_newline_header();
    printf("Encap Data: ");
    ...
    if (!(req->nhr_flags & NH_FLAG_TUNNEL_PBB)) {
      if (req->nhr_family == AF_INET) {
        ...
        printf("Sip: %s",
               inet_ntop(AF_INET6, (struct in6_addr *)req->nhr_tun_sip6, in6_dst, sizeof(in6_dst)));
        printf(" Dip: %s",
               inet_ntop(AF_INET6, (struct in6_addr *)req->nhr_tun_dip6, in6_dst, sizeof(in6_dst)));
      }
    }

    if (req->nhr_flags & NH_FLAG_TUNNEL_UDP) {
      nh_print_newline_header();
      printf("Sport:%d Dport:%d\n", ntohs(req->nhr_tun_sport), ntohs(req->nhr_tun_dport));
    }

    if (req->nhr_flags & NH_FLAG_L3_VXLAN)
      printf(" L3_Vxlan_Mac: " MAC_FORMAT, MAC_VALUE((uint8_t *)req->nhr_rw_dst_mac));

    if (req->nhr_flags & NH_FLAG_TUNNEL_PBB) {
      i = -1;
      if (req->nhr_label_list_size)
        i = req->nhr_label_list[0];
      printf("Bmac:" MAC_FORMAT " Label:%d", MAC_VALUE((uint8_t *)req->nhr_pbb_mac), i);
    }
    if (req->nhr_flags & NH_FLAG_TUNNEL_MPLS_O_MPLS) {
      printf(" Transport Label:%u", req->nhr_transport_label);
    }
    if (req->nhr_encap_crypt_oif_id != (int)-1 && req->nhr_encap_crypt_oif_id != 0) {
      nh_print_newline_header();
    ...
    if (req->nhr_nh_count && (req->nhr_nh_count - req->nhr_nh_list_size)) {
    ...
    if (req->nhr_type == NH_COMPOSITE) {
      for (i = 0; i < req->nhr_nh_list_size; i++) {
        ...
        if (req->nhr_nh_list[i] == -1)
          continue;
        vr_nh_op(cl, command, type, req->nhr_nh_list[i], if_id, vrf_id, dst_mac, src_mac, sip, dip,
                 flags);
      }
    }

    if ((req->nhr_flags & NH_FLAG_INDIRECT) && (req->nhr_nh_list_size)) {
      vr_nh_op(cl, command, type, req->nhr_nh_list[0], if_id, vrf_id, dst_mac, src_mac, sip, dip,
               flags);
    }
  }
}
...
static void nh_fill_nl_callbacks() {
...
op_retry:
  ...
  case SANDESH_OP_ADD:
    if (flags & NH_FLAG_TUNNEL_PBB) {
      ret = vr_send_pbb_tunnel_add(cl, 0, nh_id, flags, vrf_id, dst, comp_nh[0], lbl[0]);
    } else if ((type == NH_ENCAP) || (type == NH_TUNNEL)) {
      ret = vr_send_nexthop_encap_tunnel_add(cl, 0, type, nh_id, flags, vrf_id, if_id, src, dst,
                                             sip, dip, sport, dport, l3_vxlan_mac);
    } else if (type == NH_COMPOSITE) {
      ret = vr_send_nexthop_composite_add(cl, 0, nh_id, flags, vrf_id, comp_nh_ind, comp_nh, lbl,
                                          family);
    } else {
...
static void parse_long_opts(int ind, char *opt_arg) {
  int errno;
...
static void validate_options(void) {
  ...
  case SANDESH_OP_ADD:
    ...
    if (type == NH_RCV) {
      ...
      } else if (!opt_set(OIF_OPT_IND) || !opt_set(SMAC_OPT_IND) || !opt_set(DMAC_OPT_IND) ||
                 !opt_set(SIP_OPT_IND) || !opt_set(DIP_OPT_IND)) {
        cmd_usage();
      }
      ...
      if (!(flags & (NH_FLAG_TUNNEL_UDP_MPLS | NH_FLAG_TUNNEL_UDP | NH_FLAG_TUNNEL_VXLAN |
                     NH_FLAG_TUNNEL_PBB)))
...
int main(int argc, char *argv[]) {
  ...
  vr_nh_op(cl, command, type, nh_id, if_id, vrf_id, dst_mac, src_mac, sip, dip, flags);
