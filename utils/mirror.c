...
static void mirror_req_process(void *s_req) {
...
op_retry:
  switch (mirror_op) {
  case SANDESH_OP_ADD:
    ret = vr_send_mirror_add(cl, 0, mirror_index, mirror_nh, mirror_flags, vni_id);
    break;
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "bcdgn:m:s:", long_options, &option_index)) >= 0) {
    switch (opt) {
    case 'c':
      if (mirror_op >= 0)
        Usage();
  ...
  if ((mirror_op == SANDESH_OP_DUMP) || (mirror_op == SANDESH_OP_GET)) {
    printf("Mirror Table\n\n");
