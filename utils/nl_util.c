...
void vrouter_ops_process(void *s_req) {
...
void vr_flow_response_process(void *s_req) {
...
void vr_route_req_process(void *s_req) {
...
void vr_vxlan_req_process(void *s_req) {
...
void vr_qos_map_req_process(void *s_req) {
  if (nl_cb.vr_qos_map_req_process) {
    nl_cb.vr_qos_map_req_process(s_req);
  }
}

void vr_fc_map_req_process(void *s_req) {
...
void vr_hugepage_config_process(void *s_req) {
  if (nl_cb.vr_hugepage_config_process) {
    nl_cb.vr_hugepage_config_process(s_req);
  }
}

struct nl_response *nl_parse_gen_ctrl(struct nl_client *cl) {
  int len;
  struct nlattr *nla;
...
struct nl_response *nl_parse_gen(struct nl_client *cl) {
  char *buf = cl->cl_buf + cl->cl_buf_offset;
...
static int nl_build_sandesh_attr_without_attr_len(struct nl_client *cl) {
...
int nl_build_header(struct nl_client *cl, unsigned char **buf, uint32_t *buf_len) {
  int ret;
...
void nl_update_header(struct nl_client *cl, int data_len) {
  /* First update attribute header len for NLA_SANDESH_ATTR */
  struct nlattr *nla = (struct nlattr *)((char *)cl->cl_buf + cl->cl_buf_offset - NLA_HDRLEN);
...
int nl_family_name_attr_length(char *family) { return NLA_HDRLEN + NLA_ALIGN(strlen(family) + 1); }

int nl_build_family_name_attr(struct nl_client *cl, char *family) {
  char *buf;
  int len;
  struct nlattr *nla = (struct nlattr *)((char *)cl->cl_buf + cl->cl_buf_offset);
...
int nl_build_get_family_id(struct nl_client *cl, char *family) {
  int ret;
...
int nl_build_genlh(struct nl_client *cl, uint8_t cmd, uint8_t version) {
  struct genlmsghdr *genlh = (struct genlmsghdr *)((char *)cl->cl_buf + cl->cl_buf_offset);
...
struct nl_response *nl_set_resp_err(struct nl_client *cl, int error) {
...
void nl_update_nlh(struct nl_client *cl) {
  struct nlmsghdr *nlh = (struct nlmsghdr *)cl->cl_buf;
  nlh->nlmsg_len = cl->cl_msg_len = cl->cl_buf_offset;
}

int nl_get_attr_hdr_size() { return NLA_HDRLEN; }

uint8_t *nl_get_buf_ptr(struct nl_client *cl) {
  return (uint8_t *)(cl->cl_buf + cl->cl_buf_offset);
}

uint32_t nl_get_buf_len(struct nl_client *cl) { return cl->cl_buf_len - cl->cl_buf_offset; }

void nl_update_attr_len(struct nl_client *cl, int len) {
  struct nlattr *nla;
...
void nl_build_attr(struct nl_client *cl, int len, int attr) {
  struct nlattr *nla;
...
int nl_build_nlh(struct nl_client *cl, uint32_t type, uint32_t flags) {
  struct nlmsghdr *nlh = (struct nlmsghdr *)(cl->cl_buf);
...
void nl_free(struct nl_client *cl) {
...
int nl_recvmsg(struct nl_client *cl) { return cl->cl_recvmsg(cl, false); }

int nl_recvmsg_waitall(struct nl_client *cl) { return cl->cl_recvmsg(cl, true); }

void nl_set_buf(struct nl_client *cl, char *buf, unsigned int len) {
  if (cl->cl_buf)
    free(cl->cl_buf);
...
void nl_set_rcv_len(struct nl_client *cl, unsigned int rcv_len) {
  cl->cl_recv_len = rcv_len;
  cl->cl_buf_offset = 0;
  cl->cl_msg_len = 0;
}

void nl_set_genl_family_id(struct nl_client *cl, unsigned int family_id) {
  cl->cl_genl_family_id = family_id;
}

struct nl_client *nl_register_client(void) {
  struct nl_client *cl;
...
void nl_free_client(struct nl_client *cl) {
  nl_free(cl);
  free(cl);
}

int nl_init_generic_client_req(struct nl_client *cl, int family) {
...
struct nl_response *nl_parse_reply(struct nl_client *cl) {
