...
static void dump_priority(void) {
  unsigned int i;
...
static void fc_map_req_process(void *s) {
  unsigned int i;
...
static void qos_map_req_process(void *s) {
  unsigned int i;
  ...
  if (req->qmr_dscp_size) {
    ...
    for (i = 0; i < req->qmr_dscp_size; i++) {
      if (req->qmr_dscp_fc_id[i])
        printf("%8u        %8u\n", req->qmr_dscp[i], req->qmr_dscp_fc_id[i]);
    }
    printf("\n");
  }
  ...
  if (req->qmr_mpls_qos_size) {
    ...
    for (i = 0; i < req->qmr_mpls_qos_size; i++) {
      if (req->qmr_mpls_qos_fc_id[i])
        printf("%8u        %8u\n", req->qmr_mpls_qos[i], req->qmr_mpls_qos_fc_id[i]);
    }
    printf("\n");
  }
  ...
  if (req->qmr_dotonep_size) {
    ...
    for (i = 0; i < req->qmr_dotonep_size; i++) {
      if (req->qmr_dotonep_fc_id[i])
        printf("%8u        %8u\n", req->qmr_dotonep[i], req->qmr_dotonep_fc_id[i]);
    }
    printf("\n");
  }
...
static void qosmap_fill_nl_callbacks() {
...
op_retry:
  ...
  if (get_fc_set) {
    ...
    ret = vr_send_fc_map_add(cl, 0, (int16_t *)&qos_index, 1, &dscp, &mpls_qos, &dotonep, &queue);
  } else if (dump_fc_set) {
    dump = true;
    ...
    ret = vr_send_qos_map_add(cl, 0, qos_index, &dscp, num_dscp, &fc, &mpls_qos, num_mpls_qos, &fc,
...
static int interface_queue_get(void) {
  int ret;
...
static int interface_queue_set(void) {
  int ret, i;
...
static int extract_priority_to_tc_map(char *up2tc) {
  char *tok;
...
static int extract_strictness_map(char *strictness) {
  unsigned int i;
...
static int extract_priority_group_bandwidth(char *pgbw_string) {
  char *tok;
...
static int extract_priority_groups(char *pg_string) {
  char *tok;
...
static int get_dcbx_mode(char *mode) {
...
static void Usage(void) {
  ...
  printf("       --set-queue <ifname> --dcbx <cee | ieee> --pg 0,1,2.. --bw 10,20,.. --strict "
         "101..\n");
...
static void build_default_priority_to_tc_map(void) {
  unsigned int i;
  if (dcbx_mode == DCB_CAP_DCBX_VER_IEEE) {
    for (i = 0; i < NUM_TC; i++) {
...
static void validate_options(void) {
  ...
  unsigned int op_set = get_fc_set + get_qos_set + set_fc_set + set_qos_set + dump_fc_set +
                        dump_qos_set + delete_qos_set + get_queue_set;

  if (set_queue_set && tc_set != 1) {
    build_default_priority_to_tc_map();
  }

  unsigned int prio_opt_set =
      dcbx_set + set_queue_set + pg_set + pg_bw_set + strict_set + tc_set + default_tc;

  if ((prio_opt_set || get_queue_set) && (get_platform() != LINUX_PLATFORM)) {
  ...
  if (get_fc_set) {
    ...
    if (set_queue_set) {
      for (i = 0; i < NUM_PG; i++) {
        if ((priority_map.tc_strictness & (1 << i)) && (priority_map.prio_group_bw[i])) {
